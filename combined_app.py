...
def _prediction_audit_for_date(date_text: str) -> dict[str, Any]:
    ...
    is_past = date_text < datetime.now().strftime('%Y-%m-%d')
    ...
    def evaluate(item: dict[str, Any]) -> dict[str, Any] | None:
        ...
        stake = 1.0 if odd is not None else 0.0
        returned = float(odd) if won and odd is not None else 0.0
        return {
    ...
    odds_count = sum(1 for row in rows if row.get('odd') is not None)
    return {
        'date': date_text,
...
def _prediction_feed_for_date(date_text: str) -> dict[str, Any]:
    ...
    feed_rows = [row for row in rows if row.get('odd') is not None]
...
def _api_football_wsgi(environ: dict[str, Any], start_response: Callable[..., Any]) -> list[bytes]:
    ...
    try:
        ...
        if path == "/api-football/prediction-feed":
            date_text = params.get("date", "").strip() or datetime.now().strftime("%Y-%m-%d")
            if not __import__("re").fullmatch(r"\d{4}-\d{2}-\d{2}", date_text):
                return _json_response(start_response, {"ok": False, "error": "invalid date"}, 400)
            if API_FOOTBALL_DEMO or not API_FOOTBALL_KEY:
                return _json_response(start_response, {"date": date_text, "fixtures_total": 0, "finished_total": 0, "predicted_total": 0, "won_total": 0, "lost_total": 0, "hit_rate": 0, "stake_total": 0, "returned_total": 0, "profit_total": 0, "roi_percent": 0, "odds_total": 0, "response": [], "feed": [], "demo": True, "_quota": _quota_snapshot()})
            return _json_response(start_response, _prediction_feed_for_date(date_text))
        ...
        if path == "/api-football/prediction-audit":
            ...
            if not __import__("re").fullmatch(r"\d{4}-\d{2}-\d{2}", date_text):
                return _json_response(start_response, {"ok": False, "error": "invalid date"}, 400)
            if API_FOOTBALL_DEMO or not API_FOOTBALL_KEY:
                return _json_response(start_response, {"date": date_text, "fixtures_total": 0, "finished_total": 0, "predicted_total": 0, "won_total": 0, "lost_total": 0, "hit_rate": 0, "stake_total": 0, "returned_total": 0, "profit_total": 0, "roi_percent": 0, "odds_total": 0, "response": [], "demo": True, "_quota": _quota_snapshot()})
            return _json_response(start_response, _prediction_audit_for_date(date_text))
