...
def _row_fixture(row: Any, context: dict[str, str], default_date: dt.date | None) -> dict[str, Any] | None:
    ...
    team_logos: list[str] = []
    for link in body.select('a[href*="/football/team/"]'):
        img = link.select_one('img[src]')
        src = _absolute_url(img.get("src")) if img else ""
        if src and src not in team_logos:
    ...
    obj: dict[str, Any] = {
        ...
        "competition": {
            ...
            "url": context.get("competition_url") or "",
        },
...
def parse_upcoming_html(text: str, default_date: dt.date | None = None) -> list[dict[str, Any]]:
    ...
    for section in sections:
        context = {"league": "Без лиги", "country": "Без страны", "competition_url": "", "country_url": "", "league_logo": ""}
