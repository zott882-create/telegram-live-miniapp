...
const SAVED_MATCH_CARDS_KEY = "miniapp-v5-saved-match-cards";
...
function getSavedMatchCards() {
  try { return JSON.parse(localStorage.getItem(SAVED_MATCH_CARDS_KEY) || "{}") || {}; }
  catch { return {}; }
}
function saveSavedMatchCards(map) {
  const entries = Object.entries(map || {}).sort((a, b) => safeInt(b[1]?.saved_at, 0) - safeInt(a[1]?.saved_at, 0));
  localStorage.setItem(SAVED_MATCH_CARDS_KEY, JSON.stringify(Object.fromEntries(entries.slice(0, 300))));
}
function snapshotMatchCard(m, extra = {}) {
  ...
  return {
    ...
    home_logo: String(m.home_logo || ""),
    away_logo: String(m.away_logo || ""),
    score_home: safeInt(m.score_home, 0),
    score_away: safeInt(m.score_away, 0),
    score: String(m.score || `${safeInt(m.score_home, 0)}-${safeInt(m.score_away, 0)}`),
    minute: safeInt(m.minute, 0),
    minute_text: String(m.minute_text || ""),
    period: String(m.period || ""),
    country: String(m.country || ""),
    country_code: String(m.country_code || ""),
    country_logo: String(m.country_logo || ""),
    league: String(m.league || ""),
    league_logo: String(m.league_logo || ""),
    link: String(m.link || ""),
    stats: (m.stats && typeof m.stats === "object") ? m.stats : {},
    events: Array.isArray(m.events) ? m.events : (Array.isArray(extra.events) ? extra.events : []),
    saved_reason: String(extra.saved_reason || m.saved_reason || "saved"),
    saved_at: Date.now(),
  };
}
function rememberMatchCard(m, extra = {}) {
  const snap = snapshotMatchCard(m, extra);
  if (!snap?.id) return null;
  const map = getSavedMatchCards();
  const old = map[snap.id] || {};
  map[snap.id] = { ...old, ...snap, saved_reasons: [...new Set([...(old.saved_reasons || []), snap.saved_reason].filter(Boolean))] };
  saveSavedMatchCards(map);
  return map[snap.id];
}
function rememberManyMatchCards(items, reason = "notify") {
  if (!Array.isArray(items) || !items.length) return;
  const map = getSavedMatchCards();
  let changed = false;
  for (const item of items) {
    const snap = snapshotMatchCard(item, { saved_reason: reason, events: item?.events || [] });
    if (!snap?.id) continue;
    const old = map[snap.id] || {};
    map[snap.id] = { ...old, ...snap, saved_reasons: [...new Set([...(old.saved_reasons || []), reason].filter(Boolean))] };
    changed = true;
  }
  if (changed) saveSavedMatchCards(map);
}
function forgetSavedMatchReason(id, reason) {
  const k = String(id || "");
  if (!k) return;
  const map = getSavedMatchCards();
  const item = map[k];
  if (!item) return;
  item.saved_reasons = (item.saved_reasons || []).filter(x => x !== reason);
  if (!item.saved_reasons.length && !isFav(k) && !isGoalNotify(k)) delete map[k];
  else map[k] = item;
  saveSavedMatchCards(map);
}
function savedMatchById(id) {
  return getSavedMatchCards()[String(id || "")] || null;
}
function mergeLiveAndSaved(ids) {
  const result = [];
  const seen = new Set();
  const liveMap = new Map(flattenLive(state.live).map(m => [String(m.id), m]));
  const saved = getSavedMatchCards();
  for (const rawId of ids) {
    ...
    if (live) {
      result.push(live);
      rememberMatchCard(live, { saved_reason: isFav(id) ? "favorite" : "saved" });
      seen.add(id);
    } else if (snap) {
      result.push({ ...snap, is_saved_snapshot: true });
      seen.add(id);
    }
  }
  return result;
...
function imageProxyUrl(url) {
  ...
  if (!u) return "";
  if (u.startsWith("/api/image?") || u.startsWith("/api/flag?")) return u;
...
function flagUrl(code) {
  ...
  if (!cc) return "";
  return `/api/flag?code=${encodeURIComponent(cc)}`;
}

function fallbackFlagEl() {
  return "this.replaceWith(Object.assign(document.createElement('span'), {className:'flag-fallback', textContent:'🌐'}))";
}

function imgIcon(url, alt = "", fallback = "") {
  const u = imageProxyUrl(url);
  const fb = imageProxyUrl(fallback);
  if (!u && !fb) return `<span class="flag-fallback">🌐</span>`;
  const handler = fb
    ? `this.onerror=function(){${fallbackFlagEl()}};this.src='${esc(fb)}'`
    : fallbackFlagEl();
  return `<img src="${esc(u || fb)}" alt="${esc(alt)}" loading="lazy" referrerpolicy="no-referrer" onerror="${handler}" />`;
}

function flagSvg(code) {
  const cc = String(code || "").trim().toLowerCase();
  if (!cc) return `<span class="flag-fallback">🌐</span>`;
  return imgIcon(flagUrl(cc), cc.toUpperCase()) || `<span class="flag-fallback">🌐</span>`;
}
...
function findCurrentMatchById(id) {
  ...
  if (detailMatch && String(detailMatch.id) === k) return detailMatch;
  return savedMatchById(k);
}
...
async function toggleFavWithGoalPrompt(id) {
  ...
  if (nowFav) {
    ...
    if (m) rememberMatchCard(m, { saved_reason: "favorite", events: state.detail?.events || [] });
  } else {
    forgetSavedMatchReason(k, "favorite");
  }
...
function groupByLeague(matches) {
  ...
  for (const m of matches) {
    ...
    const key = c + "|||" + l;
    if (!map.has(key)) {
      map.set(key, { country: c, country_code: m.country_code || "", country_logo: m.country_logo || "", league: l, league_logo: m.league_logo || "", matches: [] });
    }
...
async function loadLive(force = false) {
  try {
    ...
    rememberManyMatchCards(flattenLive(state.live).filter(m => isFav(m.id) || isGoalNotify(m.id)), "favorite");
  } catch (err) {
...
async function loadDetail(id) {
  ...
  try {
    ...
    if (state.detail?.match?.id) {
      rememberMatchCard({ ...state.detail.match, stats: state.detail.stats || state.detail.match.stats || {}, events: state.detail.events || [] }, { saved_reason: isFav(id) ? "favorite" : "detail", events: state.detail.events || [] });
    }
  } catch (err) {
    const saved = savedMatchById(id);
    state.detail = saved ? { ok: true, archived: true, match: saved, stats: saved.stats || {}, avg: {}, events: saved.events || [] } : { ok: false, error: String(err) };
  }
  if (state.detail && state.detail.ok === false) {
    const saved = savedMatchById(id);
    if (saved) state.detail = { ok: true, archived: true, match: saved, stats: saved.stats || {}, avg: {}, events: saved.events || [] };
  }
  render();
}
...
async function loadNotifyMatches(rerender = true) {
  ...
  try {
    ...
    rememberManyMatchCards(state.notifyMatches, "notify");
  } catch (err) {
...
function checkNotifyMatches() {
  ...
  for (const m of flattenLive(state.live)) {
    ...
    seen.add(id);
    newAlerts.push({
      ...snapshotMatchCard(m, { saved_reason: "notify" }),
      id,
      ts: Date.now(),
  ...
  if (newAlerts.length) {
    saveSeenAlerts(seen);
    rememberManyMatchCards(newAlerts, "notify");
...
function leagueHeaderIcon(g) {
  // First try country/competition image discovered in IGScore API/F12.
  // If it is missing or broken, fall back to the country flag by country_code.
  const logo = String(g?.country_logo || g?.league_logo || "").trim();
  const fb = flagUrl(g?.country_code || "");
  if (logo) return imgIcon(logo, g?.country || g?.league || "flag", fb);
...
function renderMatchRow(m) {
  ...
  return `
    <div class="match-row ${m.is_saved_snapshot ? "saved-snapshot" : ""}" data-id="${esc(m.id)}">
      <div class="match-minute ${isHT ? "ht" : ""}">${esc(minuteText)}</div>
...
function renderFavorites() {
  const fav = getFavs();
  const matches = mergeLiveAndSaved([...fav]);
...
function renderFavEmpty() {
  return `
    <div class="fav-empty">
      ...
      <div class="fav-empty-text">Откройте любой матч в списке и нажмите ★. Карточка сохранится здесь даже если матч пропадёт из лайва.</div>
    </div>
  `;
}
...
function renderNotifyFoundPanel(items, serverCount) {
  ...
  return `
    ...
    <div class="page-subtitle">${state.notifyMatchesLoading ? "загрузка…" : `${total} матчей сохранено`} ${serverCount ? "· из базы" : ""}</div>
    ...
    ${total === 0 ? `
      <div class="fav-empty">
        ...
        <div class="fav-empty-text">Когда матч подойдёт под твой фильтр, он сохранится здесь. В Telegram придёт короткое сообщение с названиями команд.</div>
      </div>
    ` : `
      <div>
        ${items.map(renderAlert).join("")}
      </div>
    `}
  `;
}
...
function renderAlert(a) {
  ...
  const hasCard = a && a.home && a.away;
  if (hasCard) {
    const sh = safeInt(a.score_home, 0);
    const sa = safeInt(a.score_away, 0);
    const label = a.alert_kind === "goal" ? "⚽ Гол" : "🔔 Найден";
    return `
      <div class="notify-card">
        <div class="notify-card-top">
          <span>${leagueHeaderIcon({country_code: a.country_code, country_logo: a.country_logo, league_logo: a.league_logo, country: a.country, league: a.league})}</span>
          <span class="notify-card-league">${esc(a.country || "")} ${a.league ? "· " + esc(a.league) : ""}</span>
          <span class="notify-card-time">${label} · ${hh}:${mm}</span>
        </div>
        <div class="match-row notify-found-row saved-snapshot" data-id="${esc(a.id)}">
          <div class="match-minute">${esc(a.minute_text || (a.minute ? `${a.minute}'` : "LIVE"))}</div>
          <div class="match-teams">
            <div class="match-team-line">${teamLogoSmall(a, "home")}<div class="match-team">${esc(a.home)}</div></div>
            <div class="match-team-line">${teamLogoSmall(a, "away")}<div class="match-team">${esc(a.away)}</div></div>
          </div>
          <div class="match-score">
            <div class="match-score-row ${sh > 0 ? "scored" : ""}">${sh}</div>
            <div class="match-score-row ${sa > 0 ? "scored" : ""}">${sa}</div>
  ...
  const title = a.alert_title || (a.home && a.away ? `${a.home} — ${a.away}` : (a.title || "Матч нашёлся"));
  const subtitle = a.alert_subtitle || a.subtitle || `${a.minute_text || ""} · ${a.score || ""} · ${a.country || ""} ${a.league || ""}`;
  return `
    <div class="match-row notify-found-row" data-id="${esc(a.id)}">
      <div class="match-minute">🔔<small>${hh}:${mm}</small></div>
      <div class="match-teams">
        <div class="match-team">${esc(title)}</div>
        <div class="match-team" style="font-weight:500;color:var(--text-muted);font-size:11px;">${esc(subtitle)}</div>
      </div>
...
function renderDetail() {
  ...
  app.innerHTML = `
    <div class="detail-topbar">
      <button class="back-btn" onclick="goBack()">‹</button>
      <div class="detail-breadcrumb">${leagueHeaderIcon({country_code: m.country_code, country_logo: m.country_logo, league_logo: m.league_logo, country: m.country, league: m.league})} ${esc(m.country)} · ${esc(m.league)}</div>
      <button class="star-btn ${fav ? "active" : ""}" onclick="toggleFavWithGoalPrompt('${esc(m.id)}')">${fav ? "★" : "☆"}</button>
    </div>

    ${d.archived ? `<div class="saved-note">Карточка сохранена. Матч может уже не быть в лайве.</div>` : ""}
...
function renderMatchEvents(events, m) {
  const items = Array.isArray(events) ? events.filter(ev => ev && (ev.type === "goal" || ev.type === "yellow")) : [];
  const home = String(m?.home || "Хозяева");
  const away = String(m?.away || "Гости");
  const rows = items.map(ev => {
    const kind = ev.type === "goal" ? "goal" : "yellow";
    const icon = kind === "goal" ? "⚽" : "🟨";
    const label = kind === "goal" ? "Гол" : "Жёлтая карточка";
    const side = ev.side === "home" || ev.side === "away" ? ev.side : "";
    const team = ev.team || (side === "home" ? home : side === "away" ? away : "Команда");
    const player = ev.player ? `<div class="event-player">${esc(ev.player)}</div>` : "";
    return `
      <div class="event-row ${kind} ${side}">
        <div class="event-line"><span></span></div>
        <div class="event-minute">${esc(ev.minute_text || (ev.minute ? `${ev.minute}’` : "—"))}</div>
        <div class="event-icon">${icon}</div>
        <div class="event-body">
          <div class="event-title">${esc(label)}</div>
          <div class="event-team">${esc(team)}</div>
          ${player}
        </div>
      </div>
    `;
  ...
  return `
    <div class="event-section">
      <div class="stat-section-title">Ход матча</div>
      <div class="event-card">
        ${rows || `<div class="event-empty">Пока нет данных по голам и жёлтым карточкам.</div>`}
      </div>
    </div>
  `;
}
...
function renderAvgTotalPanel(fallbackAvg) {
  ...
  const countLine = (homeCount || awayCount)
    ? `<div class="avg-compare-counts"><span>${esc(m.home || "Хозяева")}: ${homeCount || 0} матч.</span><span>${esc(m.away || "Гости")}: ${awayCount || 0} матч.</span></div>`
    : "";
  return `
    <div class="avg-total-panel">
      <div class="stat-section-title">Средние за последние матчи</div>
      ${countLine}
