...
V5.9: saved favorite/notification cards + IGScore/FlagCDN country flags.
...
IMAGE_CACHE_SECONDS = int(os.environ.get("IMAGE_CACHE_SECONDS", "2592000"))
IMAGE_PROXY_MAX_BYTES = int(os.environ.get("IMAGE_PROXY_MAX_BYTES", "1500000"))
ALLOWED_IMAGE_HOSTS = {
    "www.igscore.net", "igscore.net", "api.igscore.net",
    "flagcdn.com", "www.flagcdn.com", "flagcdn.io", "flagsapi.com",
    "img.igscore.net", "cdn.igscore.net", "static.igscore.net",
}
...
RUS_COUNTRY_TO_CODE = {
    "англия": "GB", "шотландия": "GB", "уэльс": "GB",
    "испания": "ES", "италия": "IT", "германия": "DE", "франция": "FR",
    "нидерланды": "NL", "португалия": "PT", "турция": "TR",
    "бразилия": "BR", "аргентина": "AR", "сша": "US", "россия": "RU",
    "ангола": "AO", "камерун": "CM", "египет": "EG", "эфиопия": "ET",
    "мавритания": "MR", "марокко": "MA", "алжир": "DZ", "тунис": "TN",
    "нигерия": "NG", "гана": "GH", "кения": "KE", "юар": "ZA",
    "саудовская аравия": "SA", "катар": "QA", "оаэ": "AE", "иран": "IR",
    "ирак": "IQ", "израиль": "IL", "иордания": "JO", "япония": "JP",
    "китай": "CN", "индия": "IN", "австралия": "AU", "мексика": "MX",
    "чили": "CL", "колумбия": "CO", "перу": "PE", "уругвай": "UY",
}
...
# Extra country names used to infer country from IGScore league names like 'Mauritania League 2'.
EXTRA_COUNTRY_CODE_MAP = {
    "afghanistan": "AF",
    "albania": "AL",
    "algeria": "DZ",
    "america": "US",
...
for _country_name, _country_code in EXTRA_COUNTRY_CODE_MAP.items():
    COUNTRY_CODE_MAP.setdefault(_country_name, _country_code)

COUNTRY_NAME_ALIASES.update({
    'algerian': 'Algeria', 'angolan': 'Angola', 'cameroonian': 'Cameroon', 'egyptian': 'Egypt',
    'ethiopian': 'Ethiopia', 'ghanaian': 'Ghana', 'ivorian': 'Ivory Coast', 'kenyan': 'Kenya',
    'mauritanian': 'Mauritania', 'moroccan': 'Morocco', 'nigerian': 'Nigeria', 'rwandan': 'Rwanda',
    'senegalese': 'Senegal', 'south african': 'South Africa', 'tanzanian': 'Tanzania', 'tunisian': 'Tunisia',
    'ugandan': 'Uganda', 'zambian': 'Zambia', 'zimbabwean': 'Zimbabwe',
    'bolivian': 'Bolivia', 'chilean': 'Chile', 'colombian': 'Colombia', 'ecuadorian': 'Ecuador',
    'mexican': 'Mexico', 'peruvian': 'Peru', 'uruguayan': 'Uruguay', 'venezuelan': 'Venezuela',
    'emirati': 'United Arab Emirates', 'qatari': 'Qatar', 'saudi': 'Saudi Arabia', 'iraqi': 'Iraq',
    'iranian': 'Iran', 'israeli': 'Israel', 'jordanian': 'Jordan', 'kuwaiti': 'Kuwait', 'omani': 'Oman',
    'uzbek': 'Uzbekistan', 'vietnamese': 'Vietnam', 'welsh': 'Wales', 'scottish': 'Scotland',
})
...
def svg_response(handler: BaseHTTPRequestHandler, svg: str, status: int = 200, max_age: int = 604800) -> None:
    raw = svg.encode("utf-8")
    handler.send_response(status)
    handler.send_header("Content-Type", "image/svg+xml; charset=utf-8")
    handler.send_header("Cache-Control", f"public, max-age={int(max_age)}")
    handler.send_header("Content-Length", str(len(raw)))
    handler.end_headers()
    handler.wfile.write(raw)
...
def _first_logo_url(obj: Any, depth: int = 0) -> str:
    ...
    if isinstance(obj, dict):
        ...
        for key, value in obj.items():
            lk = str(key).lower()
            if any(token in lk for token in ("logo", "flag", "country", "image", "img", "icon", "badge", "crest", "emblem")):
...
def resolve_country_logo(match: dict[str, Any]) -> str:
    """Find the country/category image from the same IGScore objects visible in F12.

    IGScore may put country icons under category.flag/logo/icon/image or under
    competition.category. We keep this separate from league_logo so the frontend
    can show the real country image when it exists, and fall back to FlagCDN by
    country_code when it does not.
    """
    comp = match.get("competition") if isinstance(match.get("competition"), dict) else {}
    cat = match.get("category") or (comp.get("category") if isinstance(comp, dict) else {}) or {}
    candidates = []
    for obj in (cat if isinstance(cat, dict) else {}, comp, match):
        if not isinstance(obj, dict):
            continue
        candidates.extend([
            obj.get("countryLogo"), obj.get("country_logo"), obj.get("countryIcon"), obj.get("country_icon"),
            obj.get("flag"), obj.get("flagUrl"), obj.get("flag_url"),
            obj.get("icon"), obj.get("image"), obj.get("imageUrl"), obj.get("image_url"),
            obj.get("logo"), obj.get("logoUrl"), obj.get("logo_url"),
        ])
    for value in candidates:
        url = _normalize_logo_url(value)
        if url:
            return url
    return _first_logo_url(cat) if isinstance(cat, dict) else ""
...
def country_code(country: str) -> str:
    """Return ISO 3166-1 alpha-2 code for country/league text.

    IGScore sometimes sends a real country (Angola), sometimes an adjective or
    league text (Angolan Girabola League). This function accepts all of these so
    the frontend can always fall back to /api/flag?code=XX.
    """
    text = str(country or "").strip()
    if len(text) == 2 and text.isalpha():
        return text.upper()
    low = _country_lookup_key(text)
    if not low:
        return ""
    direct = RUS_COUNTRY_TO_CODE.get(low) or COUNTRY_CODE_MAP.get(low)
    if direct:
        return direct
    alias_country = COUNTRY_NAME_ALIASES.get(low)
    if alias_country:
        code = COUNTRY_CODE_MAP.get(_country_lookup_key(alias_country))
        if code:
            return code
    # Contains matching for strings like "Angolan Girabola League" or
    # "Egyptian Division 2". Check adjectives first, then country names.
    for alias, cname in sorted(COUNTRY_NAME_ALIASES.items(), key=lambda kv: len(kv[0]), reverse=True):
        a = _country_lookup_key(alias)
        if a and re.search(rf"(^|\s){re.escape(a)}($|\s)", low):
            code = COUNTRY_CODE_MAP.get(_country_lookup_key(cname))
            if code:
                return code
    for cname, code in sorted(COUNTRY_CODE_MAP.items(), key=lambda kv: len(kv[0]), reverse=True):
        name = _country_lookup_key(cname)
        if name and re.search(rf"(^|\s){re.escape(name)}($|\s)", low):
            return code
    return ""
...
def to_public_match(match: dict[str, Any], server_time: int | None = None, source: str = "igscore") -> dict[str, Any]:
    ...
    country_logo = resolve_country_logo(match)
...
def group_matches(matches: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for item in matches:
        ...
        c = countries.setdefault(ckey, {
            ...
            "country_logo": item.get("country_logo") or item.get("league_logo") or "",
            "match_count": 0,
            "leagues": {},
        })
        l = c["leagues"].setdefault(lkey, {"league": lkey, "league_logo": item.get("league_logo") or item.get("country_logo") or "", "country_logo": item.get("country_logo") or "", "match_count": 0, "matches": []})
    ...
    for c in countries.values():
        ...
        out.append({
            ...
            "country_logo": c.get("country_logo") or "",
...
def _init_live_cache_db() -> None:
    ...
    try:
        ...
        try:
            conn.execute("ALTER TABLE live_matches ADD COLUMN country_logo TEXT")
...
def _db_match_from_row(row: sqlite3.Row) -> dict[str, Any]:
    ...
    return {
        ...
        "country_logo": str(row["country_logo"] or "") if "country_logo" in row.keys() else "",
...
def collector_detail_for_match(match_id_value: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        return {"match": match, "stats": stats_nested, "events": events}
    except Exception:
        return None
    finally:
        try:
        ...
        for m in matches:
            conn.execute(
                ...
                INSERT INTO live_matches(
                    ...
                    country, country_code, country_logo, league, league_logo, link, source,
                ...
                (
                    ...
                    str(m.get("country") or "Без страны"), str(m.get("country_code") or ""), str(m.get("country_logo") or ""),
    ...
    if isinstance(obj, dict):
        for key, value in obj.items():
            ...
            if isinstance(value, list) and any(token in k for token in (
                "event", "incident", "timeline", "tlive", "score", "goal", "card", "yellow"
            )):
...
def _event_type_from_row(row: dict[str, Any], source_path: str = "") -> str:
    ...
    if any(x in text for x in ("yellow", "жёлт", "желт", "yellow card", "booking")) or "yellow" in path:
        return "yellow"
    if "red yellow" in text or "yellow red" in text:
        return "yellow"
...
def _normalise_event(row: Any, match: dict[str, Any], source_path: str = "") -> dict[str, Any] | None:
    ...
    if kind not in {"goal", "yellow"}:
        return None
    minute, minute_text = _minute_from_event(row)
    team, side = _team_from_event(row, match)
    label = "Гол" if kind == "goal" else "Жёлтая карточка"
    icon = "⚽" if kind == "goal" else "🟨"
...
def events_from_match_info(info: dict[str, Any], match: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if isinstance(result, dict):
        for key, value in result.items():
            k = str(key or "").lower()
            if any(token in k for token in ("goal", "yellow", "card")):
...
def match_events_cached(match_id_value: str, match: dict[str, Any] | None = None, stats: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    try:
        info = match_info(mid)
        events = events_from_match_info(info, match or {})
    except Exception:
        events = []
    with _cache_lock:
        _events_cache[mid] = {"saved_at": now, "events": events}
    return events
...
def detail_payload(match_id_value: str) -> dict[str, Any]:
    ...
    return {"ok": True, "match": match, "stats": stats, "avg": avg, "events": events}
...
def _notify_match_record(m: dict[str, Any], ts: int | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "country_logo": str(m.get("country_logo") or ""),
    ...
    try:
        ...
        for comp in comps[:max(1, min(int(limit), 300))]:
            ...
            rows.append({
                ...
                "country_logo": resolve_country_logo(fake_match),
...
def _proxy_cache_name(url: str, content_type: str = "") -> str:
    ...
    if ext not in {".png", ".jpg", ".jpeg", ".webp", ".gif", ".svg"}:
        if "svg" in content_type:
            ...
            ext = ".png"
    return hashlib.sha1(url.encode("utf-8", "replace")).hexdigest() + ext


def _flag_emoji_from_code(code: str) -> str:
...
def flag_image_response(handler: BaseHTTPRequestHandler, code: str) -> None:
    cc = str(code or "").strip().lower()
    if not re.fullmatch(r"[a-z]{2}", cc):
        return text_response(handler, "bad flag code", status=400)
    # Local SVG fallback: no /static/flags folder and no external request needed.
    # The browser/Telegram renders the country emoji inside the SVG.
    emoji = html.escape(_flag_emoji_from_code(cc))
    svg = f'''<svg xmlns="http://www.w3.org/2000/svg" width="24" height="18" viewBox="0 0 24 18">
  <rect width="24" height="18" rx="3" fill="#0d1814"/>
  <text x="12" y="13.5" text-anchor="middle" font-size="15" font-family="Apple Color Emoji, Segoe UI Emoji, Noto Color Emoji, sans-serif">{emoji}</text>
</svg>'''
    return svg_response(handler, svg, max_age=2592000)


def image_proxy_response(handler: BaseHTTPRequestHandler, url: str) -> None:
    url = str(url or "").strip()
    parsed = urllib.parse.urlparse(url)
    host = (parsed.hostname or "").lower()
    if parsed.scheme not in {"http", "https"} or host not in ALLOWED_IMAGE_HOSTS:
        return text_response(handler, "bad image url", status=400)

    IMAGE_CACHE_DIR.mkdir(parents=True, exist_ok=True)
    cache_name = _proxy_cache_name(url)
    cache_path = IMAGE_CACHE_DIR / cache_name
    meta_path = IMAGE_CACHE_DIR / (cache_name + ".json")
    content_type = "image/png"

    if cache_path.exists() and (time.time() - cache_path.stat().st_mtime) < IMAGE_CACHE_SECONDS:
        try:
            meta = json.loads(meta_path.read_text("utf-8")) if meta_path.exists() else {}
            content_type = str(meta.get("content_type") or mimetypes.guess_type(cache_path.name)[0] or content_type)
    ...
    try:
        with urllib.request.urlopen(req, timeout=10) as resp:
            content_type = str(resp.headers.get("Content-Type") or content_type).split(";", 1)[0].strip()
            data = resp.read(IMAGE_PROXY_MAX_BYTES + 1)
            if len(data) > IMAGE_PROXY_MAX_BYTES:
                return text_response(handler, "image too large", status=413)
    except Exception as exc:
        return text_response(handler, f"image fetch failed: {type(exc).__name__}", status=502)

    if not content_type.startswith("image/"):
        return text_response(handler, "not an image", status=415)

    try:
        cache_name = _proxy_cache_name(url, content_type)
        cache_path = IMAGE_CACHE_DIR / cache_name
        meta_path = IMAGE_CACHE_DIR / (cache_name + ".json")
        cache_path.write_bytes(data)
        meta_path.write_text(json.dumps({"url": url, "content_type": content_type}, ensure_ascii=False), "utf-8")
    except Exception:
        pass
...
class MiniAppHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        try:
            ...
            if path == "/api/image":
                params = urllib.parse.parse_qs(url.query)
                image_url = str((params.get("url") or [""])[0]).strip()
                return image_proxy_response(self, image_url)

            if path == "/api/flag":
                params = urllib.parse.parse_qs(url.query)
                code = str((params.get("code") or [""])[0]).strip()
                return flag_image_response(self, code)
