...
# v25: Railway can keep an old LIVE_FEED_MAX_MINUTE env value (for example 129)
# from earlier builds. Never let that make a completed football match linger in
# LIVE for 30-40 minutes. The hard cap affects only the public LIVE list; saved
# detail/prediction history remains available.
LIVE_FEED_HARD_MAX_MINUTE = max(95, int(os.environ.get("LIVE_FEED_HARD_MAX_MINUTE", "100")))
# Some IGScore competitions keep status=4 after FT. Once a match has reached the
# late second-half window, cap how long it may remain LIVE even if provider
# status/timestamps are stale. Exact match/info is still checked first.
LIVE_LATE_TIMEOUT_MINUTE = max(85, int(os.environ.get("LIVE_LATE_TIMEOUT_MINUTE", "90")))
LIVE_LATE_TIMEOUT_MINUTES = float(os.environ.get("LIVE_LATE_TIMEOUT_MINUTES", "12"))
LIVE_LATE_TIMEOUT_SECONDS = int(max(3.0, LIVE_LATE_TIMEOUT_MINUTES) * 60)
