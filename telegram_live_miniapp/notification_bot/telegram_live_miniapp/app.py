...
_DEFAULT_OWNER_IDS = {851766591} | _parse_int_set(os.environ.get("DEFAULT_OWNER_IDS", ""))
...
# Refresh only recently found unfinished cards. Once a final score is written,
# the saved card is permanent and no longer needs an upstream request.
NOTIFY_MATCH_FINAL_REFRESH_MAX = max(1, int(os.environ.get("NOTIFY_MATCH_FINAL_REFRESH_MAX", "40")))
NOTIFY_MATCH_FINAL_REFRESH_AGE_SECONDS = max(3600, int(os.environ.get("NOTIFY_MATCH_FINAL_REFRESH_AGE_SECONDS", "172800")))
...
# Reconcile the quick competition list with the exact match-info endpoint.
# IGScore can leave an already finished match in the live list with an old
# timer/score, while /v1/football/match/info already has the final state.
LIVE_INFO_SYNC_ENABLED = os.environ.get("LIVE_INFO_SYNC_ENABLED", "1").strip().lower() not in {"0", "false", "no", "off"}
LIVE_INFO_SYNC_MAX_MATCHES = max(0, int(os.environ.get("LIVE_INFO_SYNC_MAX_MATCHES", "80")))
LIVE_INFO_SYNC_WORKERS = max(1, min(4, int(os.environ.get("LIVE_INFO_SYNC_WORKERS", "3"))))
...
# v36: Railway may still contain old LIVE feed retention values from previous
# builds. Never allow those values to keep a vanished/finished match visible for
# tens of minutes. This cap affects only the public LIVE list; saved details,
# predictions and notification history keep their longer retention separately.
_CONFIG_LIVE_FEED_GRACE_SECONDS = int(os.environ.get("LIVE_FEED_GRACE_SECONDS", str(int(max(1.0, STALE_LIVE_HIDE_AFTER_MINUTES) * 60))))
LIVE_FEED_GRACE_SECONDS = min(180, max(30, _CONFIG_LIVE_FEED_GRACE_SECONDS))
...
_CONFIG_FINISHED_LIVE_FEED_GRACE_SECONDS = int(os.environ.get("FINISHED_LIVE_FEED_GRACE_SECONDS", str(LIVE_FEED_GRACE_SECONDS)))
FINISHED_LIVE_FEED_GRACE_SECONDS = min(180, max(30, _CONFIG_FINISHED_LIVE_FEED_GRACE_SECONDS))
...
LIVE_FEED_STUCK_MINUTE = min(94, max(88, int(os.environ.get("LIVE_FEED_STUCK_MINUTE", "90"))))
# v36: a timer frozen at 90-99' is treated as stale after at most four minutes.
# Old Railway env values are allowed to make this *shorter*, never longer.
_CONFIG_LIVE_FEED_STUCK_AFTER_MINUTES = float(os.environ.get("LIVE_FEED_STUCK_AFTER_MINUTES", "4"))
LIVE_FEED_STUCK_AFTER_MINUTES = min(4.0, max(1.0, _CONFIG_LIVE_FEED_STUCK_AFTER_MINUTES))
LIVE_FEED_STUCK_AFTER_SECONDS = int(LIVE_FEED_STUCK_AFTER_MINUTES * 60)
# v36: this is a real hard cap. An old env value such as 129 must not be able to
# raise it again. 100' still leaves room for normal stoppage time.
_CONFIG_LIVE_FEED_HARD_MAX_MINUTE = int(os.environ.get("LIVE_FEED_HARD_MAX_MINUTE", "100"))
LIVE_FEED_HARD_MAX_MINUTE = min(100, max(95, _CONFIG_LIVE_FEED_HARD_MAX_MINUTE))
# Some IGScore competitions keep status=4 after FT. Exact match/info is checked
# first; if the source remains stale, a late live row may stay no longer than ten
# minutes after entering the 90+ window, regardless of old Railway settings.
LIVE_LATE_TIMEOUT_MINUTE = min(92, max(88, int(os.environ.get("LIVE_LATE_TIMEOUT_MINUTE", "90"))))
_CONFIG_LIVE_LATE_TIMEOUT_MINUTES = float(os.environ.get("LIVE_LATE_TIMEOUT_MINUTES", "10"))
LIVE_LATE_TIMEOUT_MINUTES = min(10.0, max(3.0, _CONFIG_LIVE_LATE_TIMEOUT_MINUTES))
LIVE_LATE_TIMEOUT_SECONDS = int(LIVE_LATE_TIMEOUT_MINUTES * 60)
# Bypass the short match-info cache for late matches so FT is noticed on the
# very next collector pass instead of waiting on a stale cached detail.
LIVE_FINAL_INFO_FORCE_MINUTE = max(85, int(os.environ.get("LIVE_FINAL_INFO_FORCE_MINUTE", "90")))

def _effective_live_feed_max_minute() -> int:
    return max(1, min(STALE_LIVE_MINUTE_MAX, LIVE_FEED_MAX_MINUTE, LIVE_FEED_HARD_MAX_MINUTE))
...
def cleanup_runtime_caches(force: bool = False) -> int:
    ...
    with _cache_lock:
        ...
        removed += _cleanup_cache_dict(_lineup_cache, max(900.0, LINEUP_CACHE_SECONDS * 12), now_f)
...
def match_lineup(match_id: str) -> dict[str, Any]:
    """Fetch the official IGScore lineup payload discovered from MatchLive/Squad.

    Endpoint: POST /v1/football/match/lineup
    Payload keys intentionally match the browser request exactly.
    """
    return post_json(
        "/v1/football/match/lineup",
        {"matchId": str(match_id), **_base_payload()},
        timeout=15.0,
    )


def match_standings(match_id: str, table_type: int = 0) -> dict[str, Any]:
    """Fetch the IGScore competition standings for a concrete match.

    Endpoint discovered from MatchLive Network:
    POST /v1/football/match/standings
    type=0 is the overall table.
    """
    return post_json(
        "/v1/football/match/standings",
        {"matchId": str(match_id), "type": int(table_type), **_base_payload()},
        timeout=15.0,
    )
...
LINEUP_CACHE_SECONDS = float(os.environ.get("LINEUP_CACHE_SECONDS", "300"))
STANDINGS_CACHE_SECONDS = float(os.environ.get("STANDINGS_CACHE_SECONDS", "600"))
...
def is_stale_live_match(match: dict[str, Any], server_time: int | None = None) -> bool:
    ...
    if raw_minute >= _effective_live_feed_max_minute():
        return True
    first = _safe_int(match.get("firstHalfKickOffTime"), 0)
    second = _safe_int(match.get("secondHalfKickOffTime"), 0)
...
def minute_from_match(match: dict[str, Any], server_time: int | None = None) -> tuple[int, str, str]:
    ...
    # Never calculate a running minute from old kickoff timestamps after FT.
    if _is_finished_status(status) or _has_finished_live_text(match):
        return 90, "FT", "FT"
...
def fresh_match_info_public(match_id_value: str, force: bool = False) -> dict[str, Any] | None:
    ...
    try:
        ...
        public["finished"] = _is_finished_status(status) or _has_finished_live_text(result)
        if public["finished"]:
            public["minute"] = 90
            public["minute_text"] = "FT"
            public["period"] = "FT"
        with _cache_lock:
...
def reconcile_public_matches_with_info(matches: list[dict[str, Any]], force: bool = False) -> list[dict[str, Any]]:
    """Refresh score/timer/status from exact IGScore match-info.

    competition/list is fast but can briefly keep a finished match as LIVE with
    an old score. This bounded concurrent pass fixes the visible feed while the
    20-second match-info cache prevents duplicate upstream requests.
    """
    if not LIVE_INFO_SYNC_ENABLED or not matches or LIVE_INFO_SYNC_MAX_MATCHES <= 0:
        return matches
    targets = [m for m in matches[:LIVE_INFO_SYNC_MAX_MATCHES] if str(m.get("id") or "").strip()]
    if not targets:
        return matches

    def _load(item: dict[str, Any]) -> tuple[str, dict[str, Any] | None]:
        mid = str(item.get("id") or "")
        minute = _safe_int(item.get("raw_minute") or item.get("minute"), 0)
        force_exact = bool(force or minute >= LIVE_FINAL_INFO_FORCE_MINUTE)
        return mid, fresh_match_info_public(mid, force=force_exact)

    fresh_by_id: dict[str, dict[str, Any]] = {}
    workers = min(LIVE_INFO_SYNC_WORKERS, len(targets))
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as pool:
        futures = [pool.submit(_load, item) for item in targets]
        for fut in concurrent.futures.as_completed(futures):
            try:
                mid, fresh = fut.result()
            except Exception:
                continue
            if mid and isinstance(fresh, dict) and fresh:
                fresh_by_id[mid] = fresh

    out: list[dict[str, Any]] = []
    for item in matches:
        mid = str(item.get("id") or "")
        fresh = fresh_by_id.get(mid)
        merged = _merge_fresh_match(item, fresh) if fresh else dict(item)
        if fresh and fresh.get("finished"):
            merged["finished"] = True
            merged["_hidden_from_live"] = True
            merged["_hidden_reason"] = "finished_match_info"
        out.append(merged)
    return out
    ...
    try:
        ...
        conn.execute(
            ...
            CREATE TABLE IF NOT EXISTS live_matches (
                ...
                late_live_since INTEGER DEFAULT 0,
        ...
        if "late_live_since" not in existing_cols:
            conn.execute("ALTER TABLE live_matches ADD COLUMN late_live_since INTEGER DEFAULT 0")
        ...
        for start in range(0, len(clean_ids), 800):
            ...
            rows = conn.execute(
                ...
                SELECT match_id, score, minute,
                       ...
                       COALESCE(late_live_since, 0) AS late_live_since,
...
def _annotate_live_visibility(matches: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    feed_max_minute = _effective_live_feed_max_minute()
    stuck_minute = max(1, LIVE_FEED_STUCK_MINUTE)
    stuck_after = max(60, LIVE_FEED_STUCK_AFTER_SECONDS)
    terminal_hide_reasons = {
        "finished_match_info", "finished_text", "stale_timer",
        "minute_ge_feed_max", "stuck_late_live", "late_live_timeout",
    }
    for m in matches:
        reasons: list[str] = []
        ...
        if prev:
            # Once an exact/terminal rule has hidden a row, do not resurrect it
            # just because IGScore competition/list still reports status=4.
            prev_hidden_reason = str(prev.get("hidden_reason") or "").strip()
            prev_hidden_at = _safe_int(prev.get("hidden_from_live_at"), 0)
            if prev_hidden_at > 0 and prev_hidden_reason:
                for reason in prev_hidden_reason.split(","):
                    reason = reason.strip()
                    if reason in terminal_hide_reasons and reason not in reasons:
                        reasons.append(reason)

            late_since = _safe_int(prev.get("late_live_since"), 0)
            if (
                current_timer >= LIVE_LATE_TIMEOUT_MINUTE
                and status != 3
                and late_since > 0
                and now_i - late_since >= LIVE_LATE_TIMEOUT_SECONDS
                and "late_live_timeout" not in reasons
            ):
                reasons.append("late_live_timeout")
...
def collector_detail_for_match(match_id_value: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        try:
            ...
            stats_flat = {}
        # Detail page expects nested stats. Do not fabricate 0/0/50% when
        # the collector has no statistics snapshot yet.
        stats_nested = _nested_stats_from_flat(stats_flat)
...
def _upsert_live_matches(matches: list[dict[str, Any]]) -> None:
    ...
    try:
        ...
        if ids:
            ...
            for row in conn.execute(
                ...
                prev_meta[mid_prev] = {
                    ...
                    "late_live_since": _safe_int(row[7], 0),
                }
        ...
        for m in matches:
            ...
            if raw_minute >= LIVE_LATE_TIMEOUT_MINUTE and status != 3:
                prev_late_since = _safe_int((prev or {}).get("late_live_since"), 0)
                prev_raw = _safe_int((prev or {}).get("raw_minute"), 0)
                late_live_since = prev_late_since if prev_late_since > 0 and prev_raw >= LIVE_LATE_TIMEOUT_MINUTE else now
            else:
                late_live_since = 0
            ...
            conn.execute(
                ...
                INSERT INTO live_matches(
                    ...
                    status, raw_minute, minute_changed_at, late_live_since, hidden_from_live_at, hidden_reason,
                    last_seen_at, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(match_id) DO UPDATE SET
                    ...
                    late_live_since=excluded.late_live_since,
...
def collector_update_once() -> dict[str, Any]:
    ...
    try:
        ...
        for match in iter_competition_matches(response):
            ...
            matches.append(item)
        # Exact info fixes stale time/score and marks already finished rows.
        matches = reconcile_public_matches_with_info(matches, force=False)
        for item in matches:
            if item.get("finished"):
                item["_hidden_from_live"] = True
                item["_hidden_reason"] = str(item.get("_hidden_reason") or "finished_match_info")
        matches = _annotate_live_visibility(matches)
        matches.sort(key=league_sort_key)
        _upsert_live_matches(matches)

        visible_matches = [m for m in matches if not m.get("_hidden_from_live") and not m.get("finished")]
...
def fetch_live_igscore() -> tuple[list[dict[str, Any]], dict[str, dict[str, Any]]]:
    ...
    public = reconcile_public_matches_with_info(public, force=False)
    public = [m for m in public if not m.get("_hidden_from_live") and not m.get("finished")]
    ...
    with _live_fetch_lock:
        ...
        if mode in {"auto", "igscore"} and COLLECTOR_ENABLED and LIVE_FROM_DB_ONLY:
            try:
                ...
                if mode == "igscore":
                    payload = _load_last_good_live_payload("; ".join(errors)) or build_payload([], "collector_db", "; ".join(errors))
                    payload["collector"] = _collector_state_copy()
                    with _cache_lock:
                        _live_cache.update({"saved_at": now, "payload": payload, "raw": {}})
                    return payload
        ...
        if mode in {"auto", "igscore"}:
            try:
                ...
                if mode == "igscore":
                    payload = _load_last_good_live_payload("; ".join(errors)) or build_payload(DEMO_MATCHES, "demo", "; ".join(errors))
                    with _cache_lock:
                        _live_cache.update({"saved_at": now, "payload": payload, "raw": {}})
                    return payload
...
def _stat_number(value: Any) -> int | None:
    """Read a provider statistic without turning missing data into a fake zero."""
    if isinstance(value, dict):
        for key in ("value", "count", "score", "number", "val"):
            if key in value:
                return _stat_number(value.get(key))
        return None
...
def _stat_side_value(row: dict[str, Any], side: str) -> int | None:
    aliases = (
        side, f"{side}Value", f"{side}_value", f"{side}Score", f"{side}_score",
        f"{side}Count", f"{side}_count", "team1" if side == "home" else "team2",
        "local" if side == "home" else "visitor",
    )
    for key in aliases:
        if key in row:
            value = _stat_number(row.get(key))
    ...
    if isinstance(values, dict):
        for key in aliases:
            if key in values:
                value = _stat_number(values.get(key))
                if value is not None:
                    return value
    return None


def _stat_rows_from_response(resp: Any) -> list[dict[str, Any]]:
    """Find the actual statistics rows across old and new IGScore wrappers."""
    ...
    def walk(value: Any, depth: int = 0) -> None:
        ...
        if isinstance(value, dict):
            for key, child in value.items():
                ...
                if isinstance(child, list) and any(token in low for token in ("stat", "metric", "indicator")):
    ...
    for candidate in candidates:
        ...
        score = 0
        for row in rows[:30]:
            if any(key in row for key in ("type", "typeId", "statType", "name", "title", "key")):
                score += 1
            if _stat_side_value(row, "home") is not None or _stat_side_value(row, "away") is not None:
                score += 2
        if score > best_score:
            best_score = score
            best = rows
...
def _stat_key_from_row(row: dict[str, Any]) -> str:
    numeric_map = {
        2: "corners", 3: "yellow_cards", 4: "red_cards",
        20: "shots", 21: "on_target", 22: "off_target",
        23: "attacks", 24: "dangerous", 25: "possession",
    }
    for key in ("type", "typeId", "type_id", "statType", "stat_type", "id"):
        if key in row:
            raw = row.get(key)
            try:
            ...
            if number in numeric_map:
                return numeric_map[number]
    label = " ".join(str(row.get(key) or "") for key in ("name", "title", "key", "typeName", "statName", "label")).strip().lower()
    label = re.sub(r"[^a-zа-я0-9]+", " ", label)
    aliases = (
        (("ball possession", "possession", "владение"), "possession"),
        (("shots on target", "shots on goal", "on target", "в створ"), "on_target"),
        (("shots off target", "shots off goal", "off target", "мимо"), "off_target"),
        (("dangerous attacks", "danger attack", "опасн"), "dangerous"),
        (("total attacks", "attacks", "атаки"), "attacks"),
        (("corner kicks", "corners", "углов"), "corners"),
        (("yellow cards", "yellow card", "жёлт", "желт"), "yellow_cards"),
        (("red cards", "red card", "красн"), "red_cards"),
        (("total shots", "shots", "удары"), "shots"),
    )
...
def stat_pairs_from_response(resp: dict[str, Any]) -> dict[str, dict[str, int]]:
    """Parse live stats and return {} when the provider has not sent them yet.

    The old implementation always returned 50/50 and zeros, so an empty API
    response looked like real statistics and then got cached.
    """
    parsed: dict[str, dict[str, int]] = {}
    for row in _stat_rows_from_response(resp):
        key = _stat_key_from_row(row)
        if not key:
            continue
        home = _stat_side_value(row, "home")
        away = _stat_side_value(row, "away")
        if home is None and away is None:
            continue
        parsed[key] = {"home": int(home or 0), "away": int(away or 0)}

    if "shots" not in parsed and ("on_target" in parsed or "off_target" in parsed):
        on_target = parsed.get("on_target") or {"home": 0, "away": 0}
        off_target = parsed.get("off_target") or {"home": 0, "away": 0}
        parsed["shots"] = {
            "home": _safe_int(on_target.get("home"), 0) + _safe_int(off_target.get("home"), 0),
            "away": _safe_int(on_target.get("away"), 0) + _safe_int(off_target.get("away"), 0),
        }
    return parsed
...
def _nested_stats_from_flat(stats_flat: dict[str, Any]) -> dict[str, dict[str, int]]:
    if not isinstance(stats_flat, dict) or not stats_flat:
        return {}
    ...
    for key in ("shots", "on_target", "off_target", "attacks", "dangerous", "corners", "yellow_cards", "red_cards"):
        ...
        if home_key in stats_flat or away_key in stats_flat:
            nested[key] = {
                "home": _safe_int(stats_flat.get(home_key), 0),
                "away": _safe_int(stats_flat.get(away_key), 0),
            }
    if "possession_home" in stats_flat or "possession_away" in stats_flat:
        nested["possession"] = {
...
def _saved_match_stats_nested(match_id_value: str) -> dict[str, dict[str, int]]:
    mid = str(match_id_value or "").strip()
    if not mid:
        return {}
    try:
        _init_live_cache_db()
        conn = sqlite3.connect(str(LIVE_CACHE_DB), timeout=10)
        row = conn.execute("SELECT stats_json FROM match_stats WHERE match_id = ? LIMIT 1", (mid,)).fetchone()
        if not row or not row[0]:
            return {}
        return _nested_stats_from_flat(json.loads(row[0] or "{}") or {})
    except Exception:
        return {}
    finally:
        try:
...
def match_stats_cached(match_id_value: str, match_meta: dict[str, Any] | None = None, force: bool = False) -> dict[str, dict[str, int]]:
    """Fetch/cached IGScore stats without replacing good data with fake zeros.
    ...
    cached_stats = (cached or {}).get("stats") or {}
    if not force and cached and time.time() - float(cached.get("saved_at") or 0) < STAT_CACHE_SECONDS:
        return cached_stats
    ...
    # Provider sometimes returns an empty statistics array for a few polls.
    # Keep the last real snapshot instead of showing 0/0/50% in the card.
    fallback = cached_stats or _saved_match_stats_nested(mid)
    _stats_cache[mid] = {"saved_at": time.time(), "stats": fallback}
    return fallback
...
def enrich_public_matches_with_stats(public: list[dict[str, Any]]) -> None:
    ...
    def _load(item: dict[str, Any]) -> tuple[str, dict[str, int], bool]:
        mid = str(item.get("id") or "")
        try:
            nested = match_stats_cached(mid, item)
            if not isinstance(nested, dict) or not nested:
                return mid, {}, False
            return mid, flat_stats(nested), True
        except Exception:
            return mid, {}, False
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        for fut in concurrent.futures.as_completed(future_map):
            item = future_map[fut]
            try:
                _mid, stats, stats_available = fut.result()
            except Exception:
                stats, stats_available = {}, False
            item["stats_available"] = bool(stats_available)
...
def _recent_match_summary(match: dict[str, Any], team_id_value: str, result: str, gs: int, gc: int) -> dict[str, Any]:
    ...
    # v9.97: recent-match rows now receive the same cached/sanitized team crests
    # as the main live list. Unknown crests gracefully fall back to initials.
    home_logo = resolve_team_logo(match, "home", home_name)
    away_logo = resolve_team_logo(match, "away", away_name)
...
def _find_nested_value(obj: Any, keys: set[str], depth: int = 0) -> Any:
    if depth > 7:
        return None
    if isinstance(obj, dict):
        for key, value in obj.items():
            norm = re.sub(r"[^a-z0-9]", "", str(key).lower())
            if norm in keys and value not in (None, "", [], {}):
                return value
        for value in obj.values():
...
def _lineup_list(value: Any) -> list[dict[str, Any]]:
    """Return a clean player list from IGScore's {"0": subs, "1": starters} shape."""
    ...
    if isinstance(value, dict):
        ...
        for item in value.values():
            ...
            elif isinstance(item, dict) and any(k in item for k in ("name", "playerName", "shirtNumber")):
                out.append(item)
        return out
    return []


def _lineup_group(value: Any, key: str, first_value: int) -> list[dict[str, Any]]:
    if isinstance(value, dict):
        direct = value.get(key)
        if isinstance(direct, list):
            return [item for item in direct if isinstance(item, dict)]
    return [item for item in _lineup_list(value) if _safe_int(item.get("first"), -1) == first_value]


def _public_lineup_player(player: dict[str, Any]) -> dict[str, Any]:
    return {
        "name": str(player.get("name") or player.get("playerName") or "Игрок"),
        "logo": str(player.get("logo") or player.get("photo") or ""),
        "shirtNumber": player.get("shirtNumber") if player.get("shirtNumber") is not None else player.get("number"),
        "position": str(player.get("position") or player.get("pos") or ""),
        "rating": str(player.get("rating") or ""),
        "first": _safe_int(player.get("first"), 0),
        "captain": _safe_int(player.get("captain"), 0),
        "x": player.get("x"),
        "y": player.get("y"),
        "incidents": player.get("incidents"),
    }


def _public_lineup_team(
    team: Any,
    lineup: Any,
    formation: Any,
    fallback_name: str,
) -> dict[str, Any]:
    team_obj = team if isinstance(team, dict) else {}
    starters = [_public_lineup_player(x) for x in _lineup_group(lineup, "1", 1)]
    substitutes = [_public_lineup_player(x) for x in _lineup_group(lineup, "0", 0)]
    # Be tolerant if a league sends one flat list without first flags.
    if not starters and not substitutes:
        flat = [_public_lineup_player(x) for x in _lineup_list(lineup)]
        starters = flat[:11]
        substitutes = flat[11:]
    return {
        "team": {
            "id": str(team_obj.get("id") or ""),
            "name": str(team_obj.get("name") or fallback_name),
            "logo": str(team_obj.get("logo") or ""),
        },
        "coach": {
            "id": str(team_obj.get("coachId") or ""),
            "name": str(team_obj.get("coachName") or ""),
        },
...
def match_lineup_payload(match_id_value: str, force: bool = False) -> dict[str, Any]:
    mid = str(match_id_value or "").strip()
    if not mid:
        return {"ok": False, "error": "missing id"}
    now = time.time()
    with _cache_lock:
        cached = _lineup_cache.get(mid)
        if not force and cached and now - float(cached.get("saved_at") or 0) < LINEUP_CACHE_SECONDS:
            return dict(cached.get("payload") or {})
    try:
        raw = match_lineup(mid)
        result = raw.get("result") if isinstance(raw, dict) else None
        if not isinstance(result, dict):
            result = {}
        home = _public_lineup_team(
            result.get("homeTeam"),
            result.get("homeLineup"),
            result.get("homeFormation"),
            "Хозяева",
        )
        away = _public_lineup_team(
            result.get("awayTeam"),
            result.get("awayLineup"),
            result.get("awayFormation"),
            "Гости",
        )
        payload = {
            "ok": True,
            "tab": "lineups",
            "match_id": mid,
            "playGroundName": str(result.get("playGroundName") or ""),
            "temperature": str(result.get("temperature") or ""),
            "weather": result.get("weather"),
            "homeFormation": str(result.get("homeFormation") or ""),
            "awayFormation": str(result.get("awayFormation") or ""),
            "lineups": [home, away],
            "source": "igscore_match_lineup",
        }
    except Exception as exc:
        payload = {"ok": False, "tab": "lineups", "match_id": mid, "error": f"IGScore: {type(exc).__name__}: {exc}"}
    with _cache_lock:
        if payload.get("ok"):
            _lineup_cache[mid] = {"saved_at": now, "payload": payload}
    return payload


def _public_standing_row(row: Any) -> dict[str, Any]:
    item = row if isinstance(row, dict) else {}
    team = item.get("team") if isinstance(item.get("team"), dict) else {}
    return {
        "rank": item.get("position"),
        "position": item.get("position"),
        "teamId": str(item.get("teamId") or team.get("id") or ""),
        "team": {
            "id": str(team.get("id") or item.get("teamId") or ""),
            "name": str(team.get("name") or team.get("shortName") or "—"),
            "shortName": str(team.get("shortName") or team.get("name") or "—"),
            "logo": str(team.get("logo") or ""),
        },
        "played": item.get("total"),
        "matches": item.get("total"),
        "won": item.get("won"),
        "draw": item.get("draw"),
        "loss": item.get("loss"),
        "goalsFor": item.get("goals"),
        "goalsAgainst": item.get("goalsAgainst"),
        "goalDiff": item.get("goalDiff"),
        "points": item.get("points"),
        "deductPoints": item.get("deductPoints"),
        "promotionId": str(item.get("promotionId") or ""),
        "note": str(item.get("note") or ""),
        "home": {
            "played": item.get("homeTotal"),
            "won": item.get("homeWon"),
            "draw": item.get("homeDraw"),
            "loss": item.get("homeLoss"),
            "goalsFor": item.get("homeGoals"),
            "goalsAgainst": item.get("homeGoalsAgainst"),
            "goalDiff": item.get("homeGoalDiff"),
            "points": item.get("homePoints"),
        },
        "away": {
            "played": item.get("awayTotal"),
            "won": item.get("awayWon"),
            "draw": item.get("awayDraw"),
            "loss": item.get("awayLoss"),
            "goalsFor": item.get("awayGoals"),
            "goalsAgainst": item.get("awayGoalsAgainst"),
            "goalDiff": item.get("awayGoalDiff"),
            "points": item.get("awayPoints"),
        },
    }


def match_standings_payload(match_id_value: str, force: bool = False) -> dict[str, Any]:
    mid = str(match_id_value or "").strip()
    if not mid:
        return {"ok": False, "tab": "table", "error": "missing id"}
    now = time.time()
    with _cache_lock:
        cached = _standings_cache.get(mid)
        if not force and cached and now - float(cached.get("saved_at") or 0) < STANDINGS_CACHE_SECONDS:
            return dict(cached.get("payload") or {})
    try:
        raw = match_standings(mid, table_type=0)
        result = raw.get("result") if isinstance(raw, dict) else None
        if not isinstance(result, dict):
            result = {}
        ...
        for table in tables:
            table_obj = table if isinstance(table, dict) else {}
            rows_raw = table_obj.get("rows") if isinstance(table_obj.get("rows"), list) else []
            rows = [_public_standing_row(row) for row in rows_raw]
            flat_rows.extend(rows)
            public_tables.append({
                "id": table_obj.get("id"),
                "conference": str(table_obj.get("conference") or ""),
                "group": table_obj.get("group"),
                "stageId": str(table_obj.get("stageId") or ""),
                "rows": rows,
            })
        competition = result.get("competition") if isinstance(result.get("competition"), dict) else {}
        promotions_raw = result.get("promotions") if isinstance(result.get("promotions"), list) else []
        promotions = [{
            "id": str(x.get("id") or ""),
            "name": str(x.get("name") or ""),
            "color": str(x.get("color") or ""),
        } for x in promotions_raw if isinstance(x, dict)]
        payload = {
            "ok": True,
            "tab": "table",
            "match_id": mid,
            "competition": {
                ...
                "name": str(competition.get("name") or competition.get("shortName") or "Турнир"),
                "shortName": str(competition.get("shortName") or competition.get("name") or "Турнир"),
                "logo": str(competition.get("logo") or ""),
                "countryId": str(competition.get("countryId") or ""),
                "curSeasonId": str(competition.get("curSeasonId") or ""),
                "curStageId": str(competition.get("curStageId") or ""),
                "curRound": competition.get("curRound"),
                "roundCount": competition.get("roundCount"),
                "primaryColor": str(competition.get("primaryColor") or ""),
                "secondaryColor": str(competition.get("secondaryColor") or ""),
            },
            "promotions": promotions,
            "tables": public_tables,
            "table": flat_rows,
            "source": "igscore_match_standings",
        }
    except Exception as exc:
        payload = {"ok": False, "tab": "table", "match_id": mid, "error": f"IGScore: {type(exc).__name__}: {exc}"}
    with _cache_lock:
        if payload.get("ok"):
            _standings_cache[mid] = {"saved_at": now, "payload": payload}
    return payload


def match_extra_payload(match_id_value: str, tab: str, force: bool = False) -> dict[str, Any]:
    mid = str(match_id_value or "").strip()
    tab = str(tab or "").strip().lower()
    if not mid:
        return {"ok": False, "error": "missing id"}
    if tab == "matches":
        payload = avg_payload_for_match(mid)
        return {"ok": True, "tab": tab, "match_id": mid, "avg": payload.get("avg") or {}}
    if tab == "lineups":
        return match_lineup_payload(mid, force=force)
    if tab == "table":
        return match_standings_payload(mid, force=force)
    return {"ok": False, "error": "unsupported tab"}


def detail_payload(match_id_value: str, force_stats: bool = False) -> dict[str, Any]:
    live_payload = load_live_payload(force=False)
    ...
    if not match:
        ...
        if fresh_info:
            stats = {}
            try:
                stats = match_stats_cached(str(match_id_value), fresh_info, force=force_stats)
            except Exception:
                stats = {}
            return {
                "ok": True,
    ...
    if str(match.get("source")) == "collector_db":
        ...
        if collector_detail:
            collector_detail.setdefault("avg", {"home": _avg_empty(), "away": _avg_empty()})
            fresh_info = fresh_match_info_public(str(match.get("id")), force=False)
            if fresh_info and isinstance(collector_detail.get("match"), dict):
                collector_detail["match"] = _merge_fresh_match(collector_detail["match"], fresh_info)
                collector_detail["finished"] = bool(fresh_info.get("finished"))
            # The collector can be disabled or a DB snapshot can lag behind.
            # Opening a live card must still query the dedicated statistics endpoint.
            try:
                live_stats = match_stats_cached(str(match.get("id")), collector_detail.get("match") or match, force=force_stats)
                if live_stats:
                    collector_detail["stats"] = live_stats
            except Exception:
                pass
            return {"ok": True, **collector_detail}
    ...
    if str(match.get("source")) == "sqlite":
        ...
        if db_detail:
            ...
            if fresh_info and isinstance(db_detail.get("match"), dict):
                db_detail["match"] = _merge_fresh_match(db_detail["match"], fresh_info)
                db_detail["finished"] = bool(fresh_info.get("finished"))
            try:
                live_stats = match_stats_cached(str(match.get("id")), db_detail.get("match") or match, force=force_stats)
                if live_stats:
                    db_detail["stats"] = live_stats
            except Exception:
                pass
            return {"ok": True, **db_detail}
    ...
    if mid:
        try:
            stats = match_stats_cached(mid, match, force=force_stats)
        except Exception:
            stats = {}

    # Never substitute demo zeroes for a real live match. If the provider has
    # not published statistics yet, the frontend shows a waiting state and
    # the 30-second detail refresh will try again.
...
def _cleanup_notify_matches_unlocked(now: float | None = None) -> int:
    """Sanitize saved found matches without time-based expiry.

    v10.x requirement: matches in the "Найденные" tab must stay forever
    until the user removes them manually.  Therefore we no longer delete cards
    by age here.  We only drop obviously broken rows and keep the newest unique
    records per user, capped by NOTIFY_MATCHES_MAX_PER_USER.
    """
    removed = 0
...
def _notify_match_record(m: dict[str, Any], ts: int | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "finished": bool(m.get("finished") or str(m.get("minute_text") or "").strip().upper() in {"FT", "AET", "PEN"} or str(m.get("period") or "").strip().upper() in {"FT", "AET", "PEN"}),
...
def _notify_saved_match_finished(item: dict[str, Any]) -> bool:
    if not isinstance(item, dict):
        return False
    if bool(item.get("finished")):
        return True
    terminal = {"FT", "AET", "PEN", "FULL TIME", "FINISHED"}
    return (
        str(item.get("minute_text") or "").strip().upper() in terminal
        or str(item.get("period") or "").strip().upper() in terminal
        or str(item.get("status") or "").strip().upper() in terminal
    )


def _merge_notify_saved_match(saved: dict[str, Any], fresh: dict[str, Any]) -> tuple[dict[str, Any], bool]:
    """Merge current/final score fields while preserving the original alert."""
    out = dict(saved or {})
    before = json.dumps(out, ensure_ascii=False, sort_keys=True, separators=(",", ":"), default=str)
    for key in (
        "home", "away", "home_logo", "away_logo", "score_home", "score_away", "score",
        "minute", "minute_text", "period", "status", "status_id", "finished",
        "country", "country_code", "league", "league_logo", "stats",
    ):
        if key not in fresh:
            continue
        value = fresh.get(key)
        if value is None:
            continue
        if key in {"home_logo", "away_logo", "league_logo"} and not value:
            continue
        out[key] = value
    out["id"] = str(out.get("id") or out.get("match_id") or fresh.get("id") or "")
    sh = _safe_int(out.get("score_home"), 0)
    sa = _safe_int(out.get("score_away"), 0)
    out["score_home"] = sh
    out["score_away"] = sa
    out["score"] = str(out.get("score") or f"{sh}-{sa}")
    if bool(out.get("finished")):
        out["finished"] = True
        out["minute"] = max(90, _safe_int(out.get("minute"), 90))
        out["minute_text"] = "FT"
        out["period"] = "FT"
        out["final_updated_at"] = int(time.time())
    after = json.dumps(out, ensure_ascii=False, sort_keys=True, separators=(",", ":"), default=str)
    return out, before != after


def _refresh_notify_matches_snapshot(items: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], bool]:
    """Refresh found cards from the shared LIVE cache and exact match-info.

    The main LIVE list removes completed matches. A found card therefore used to
    retain its last live score forever. For a recently found card that vanished
    from LIVE we request exact match-info once, save the final result, and then
    never request it again because ``finished`` is persisted.
    """
    if not items:
        return items, False
    now_i = int(time.time())
    try:
        live_rows = flatten_payload(load_live_payload(force=False))
    except Exception:
        live_rows = []
    live_by_id = {str(row.get("id") or ""): row for row in live_rows if str(row.get("id") or "")}
    ...
    for raw in items:
        ...
        if not mid:
            continue
        current = live_by_id.get(mid)
        if current:
            item, changed = _merge_notify_saved_match(item, current)
            changed_any = changed_any or changed
        elif not _notify_saved_match_finished(item):
            found_at = _safe_int(item.get("found_at"), 0)
            recent = not found_at or now_i - found_at <= NOTIFY_MATCH_FINAL_REFRESH_AGE_SECONDS
            if recent and exact_used < NOTIFY_MATCH_FINAL_REFRESH_MAX:
                exact_used += 1
                fresh = fresh_match_info_public(mid, force=False)
                if fresh:
                    item, changed = _merge_notify_saved_match(item, fresh)
                    changed_any = changed_any or changed
        refreshed.append(item)
    return refreshed, changed_any
...
def handle_notify_matches(init_data: str) -> dict[str, Any]:
    ...
    if not chat_id:
        ...
        chat_id = "local"
    with _notify_lock:
        removed_invalid = _cleanup_notify_matches_unlocked()
        if removed_invalid:
            _save_notify_matches()
        items = [dict(x) for x in (_notify_matches.get(chat_id) or []) if isinstance(x, dict)]

    # Network/cache reads are deliberately outside _notify_lock so notification
    # scanning and Telegram delivery are not blocked while a final score loads.
    refreshed, changed = _refresh_notify_matches_snapshot(items)
    if changed:
        refreshed_by_id = {str(x.get("id") or x.get("match_id") or ""): x for x in refreshed}
        with _notify_lock:
            current_items = list(_notify_matches.get(chat_id) or [])
            merged_items: list[dict[str, Any]] = []
            for current in current_items:
                if not isinstance(current, dict):
                    continue
                mid = str(current.get("id") or current.get("match_id") or "")
                merged_items.append(dict(refreshed_by_id.get(mid) or current))
            _notify_matches[chat_id] = merged_items[:NOTIFY_MATCHES_MAX_PER_USER]
            _save_one_notify_matches(chat_id)
            items = [dict(x) for x in _notify_matches.get(chat_id) or []]
    else:
        items = refreshed
        ...
        try:
            ...
            with _notify_lock:
                ...
                for chat_id_str, sub in list(_notify_subs.items()):
                    ...
                    goal_wait = sub.get("goal_wait") if isinstance(sub.get("goal_wait"), dict) else {}
                    if goal_wait.get("enabled") or goal_wait.get("auto_goal_enabled"):
                        goal_wait["enabled"] = False
                        goal_wait["auto_goal_enabled"] = False
                        sub["goal_wait"] = goal_wait
                        changed = True
...
def _scan_notify_subscription_once(chat_id_str: str | int, include_goal_wait: bool = False) -> None:
            ...
            if cfg.get("enabled"):
                for m in matches:
                    ...
                    if ok or queued:
                        ...
                        changed = True
            # Спецрежим "Ждём гол" полностью отключён.
            include_goal_wait = False
...
def _scan_notify_subscription_async(chat_id_str: str | int, include_goal_wait: bool = False) -> None:
    try:
        threading.Thread(
...
def handle_goal_wait_subscribe(body: dict[str, Any]) -> dict[str, Any]:
    """Спецрежим "Ждём гол" удалён из бота."""
    init_data = str(body.get("init_data") or "")
    user = verify_init_data(init_data)
    if not user:
        if not BOT_TOKEN:
    ...
    with _notify_lock:
        sub = _notify_subs.setdefault(chat_id, {"chat_id": chat_id, "filter": {}, "seen": {}, "goal_alerts": {}})
        sub["goal_wait"] = {"enabled": False, "auto_goal_enabled": False, "updated_at": int(time.time())}
        sub.setdefault("goal_wait_seen", {})
        _drop_auto_goal_alerts(sub)
        sub["updated_at"] = int(time.time())
        _notify_subs[chat_id] = sub
        _save_one_notify_sub(chat_id)
    return {"ok": True, "chat_id": chat_id, "enabled": False, "removed": True}
    ...
    def do_GET(self) -> None:
        try:
            ...
            if path == "/api/match":
                ...
                force = str((params.get("force") or ["0"])[0]).lower() in {"1", "true", "yes"}
                try:
                    payload = detail_payload(mid, force_stats=force)
                    if payload.get("ok"):
            ...
            if path == "/api/match/extra":
                if not _rate_limit_ok(self, "match"):
                    return json_response(self, {"ok": False, "error": "rate limit"}, status=429)
                params = urllib.parse.parse_qs(url.query)
                mid = str((params.get("id") or [""])[0]).strip()
                tab = str((params.get("tab") or [""])[0]).strip()
                force = str((params.get("force") or ["0"])[0]).lower() in {"1", "true", "yes"}
                if not mid:
                    return json_response(self, {"ok": False, "error": "missing id"}, status=400)
                return json_response(self, match_extra_payload(mid, tab, force=force))
...
def main() -> None:
    ...
    print("=" * 72)
    print("Telegram Live Matches Mini App — v9.96 Telegram admin subscriptions")
    print(f"Local:    http://127.0.0.1:{PORT}")
    print(f"Host:     {HOST}:{PORT}")
    print(f"Mode:     {DATA_MODE}")
    print(f"Notify:   {'enabled (' + str(len(_notify_subs)) + ' subs)' if BOT_TOKEN else 'disabled (no BOT_TOKEN)'}")
    print(f"Storage:  notify={NOTIFY_STORAGE} live_cache=sqlite redis={'on' if REDIS_URL else 'off'}")
    print(f"Stability: redis_cache={'on' if REDIS_CACHE_ENABLED and REDIS_URL else 'off'} last_good={LAST_GOOD_LIVE_TTL_SECONDS}s circuit={'on' if IGSCORE_CIRCUIT_BREAKER_ENABLED else 'off'}")
