...
def match_extra_payload(match_id_value: str, tab: str, force: bool = False) -> dict[str, Any]:
    ...
    if tab not in {"lineups", "table"}:
        return {"ok": False, "error": "unsupported tab"}
    try:
        raw = match_info(mid)
    except Exception as exc:
        return {"ok": False, "error": f"IGScore: {type(exc).__name__}: {exc}"}
    if tab == "lineups":
        keys = {"lineup", "lineups", "homelineup", "awaylineup", "homeplayers", "awayplayers", "startinglineups", "players"}
        value = _find_nested_value(raw, keys)
        return {"ok": True, "tab": tab, "match_id": mid, "lineups": value or []}
    keys = {"standings", "standing", "leaguetable", "competitiontable", "table", "rankings", "ranking"}
    value = _find_nested_value(raw, keys)
    return {"ok": True, "tab": tab, "match_id": mid, "table": value or []}
