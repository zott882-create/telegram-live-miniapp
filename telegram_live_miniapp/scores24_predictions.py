...
MAX_TOTAL_DISCOVERY_PAGES = max(20, min(400, int(os.environ.get("SCORES24_MAX_TOTAL_DISCOVERY_PAGES", "240"))))
ATOMIC_FULL_SCAN = os.environ.get("SCORES24_ATOMIC_FULL_SCAN", "1").strip().lower() not in {"0", "false", "no", "off"}
REQUEST_GAP_SECONDS = max(0.10, min(2.0, float(os.environ.get("SCORES24_REQUEST_GAP_SECONDS", "0.45"))))
...
CATEGORY_REFRESH_SECONDS = max(300, int(os.environ.get("SCORES24_CATEGORY_REFRESH_SECONDS", "300")))
...
LEAGUE_REFRESH_SECONDS = max(300, int(os.environ.get("SCORES24_LEAGUE_REFRESH_SECONDS", "600")))
...
DISCOVERY_REFRESH_SECONDS = max(300, int(os.environ.get("SCORES24_DISCOVERY_REFRESH_SECONDS", "300")))
...
_LINK_ATTR_RE = re.compile(r'''(?:href|data-href|data-url|data-next-url)\s*=\s*[\"']([^\"']+)[\"']''', re.I)
_PAGINATION_MARKER_RE = re.compile(r'(?:show\s*more|load\s*more|показать\s*(?:ещ[её]|больше)|hasNextPage|nextPage|endCursor|pagination)', re.I)
_PAGINATION_QUERY_KEYS = {"page", "p", "offset", "skip", "start", "cursor", "after", "next", "pageToken"}
...
def _rows_from_index_page(page_html: str, source_page: str) -> list[dict[str, Any]]:
    ...
    for payload in _urql_payloads(page_html):
        walk(payload)
    # Scores24 alternates between URQL and React Query hydration. Some listing
    # pages expose only the first cards in URQL while the complete list is in
    # window.__REACT_QUERY_STATE__. Read both stores before deduplication.
    for query in _react_queries(page_html):
        state = query.get("state") if isinstance(query, dict) else {}
        wrapper = (state or {}).get("data") if isinstance(state, dict) else None
        if isinstance(wrapper, dict) and "data" in wrapper:
            walk(wrapper.get("data"))
        else:
            walk(wrapper)
...
_CATEGORY_PATHS = [
    # High-yield pages first. The today page normally exposes the complete
    # current list, so it is persisted before slower market/league enrichment.
...
def _listing_branch_key(url: str) -> str:
...
def _same_listing_page(candidate: str, source_url: str) -> bool:
    try:
        source = urllib.parse.urlsplit(source_url)
        target = urllib.parse.urlsplit(urllib.parse.urljoin(source_url, candidate))
    except Exception:
        return False
    if target.scheme not in {"http", "https"} or target.netloc.lower() != source.netloc.lower():
        return False
    if target.path.rstrip("/") != source.path.rstrip("/"):
        return False
    if "/soccer/m-" in target.path.lower():
        return False
    query = urllib.parse.parse_qs(target.query, keep_blank_values=True)
    return any(key in query for key in _PAGINATION_QUERY_KEYS)


def _query_url(source_url: str, **updates: Any) -> str:
...
def _pagination_urls_from_page(page_html: str, source_url: str) -> list[str]:
    """Extract server-visible pagination and conservative load-more URLs.

    Scores24 changes GraphQL root names regularly. The collector therefore
    inspects both rendered HTML and every embedded URQL object for common
    page/offset/cursor metadata. Only URLs that keep the same listing path are
    accepted, so match and league navigation links cannot expand the scan.
    """
    urls: list[str] = []
    normalized_html = (
        html_lib.unescape(str(page_html or ""))
        .replace("\\u0026", "&")
        .replace("\\/", "/")
    )
    ...
    def add(candidate: Any) -> None:
        ...
        if _same_listing_page(absolute, source_url):
            urls.append(absolute)

    for match in _LINK_ATTR_RE.finditer(normalized_html):
        add(match.group(1))
    # JSON/JS strings sometimes contain a pagination URL without an href.
    json_url_re = re.compile(
        r"[\"'](?:nextUrl|nextPageUrl|loadMoreUrl|url|href)[\"']\s*:\s*[\"']([^\"']+)[\"']",
        re.I,
    )
    for match in json_url_re.finditer(normalized_html):
        add(match.group(1))
    ...
    def walk(value: Any) -> None:
        ...
        for key in ("nexturl", "nextpageurl", "loadmoreurl", "nexthref"):
            if key in lower:
                add(lower.get(key))

        current = None
        for key in ("currentpage", "page", "pagenumber", "pageindex"):
            current = _int_value(lower.get(key))
            if current is not None:
                break
        total_pages = None
        for key in ("totalpages", "pagescount", "pagecount", "lastpage"):
            total_pages = _int_value(lower.get(key))
            if total_pages is not None:
                break
        next_page = _int_value(lower.get("nextpage"))
        has_next = bool(
            lower.get("hasnextpage")
            or lower.get("hasnext")
            or lower.get("more")
            or lower.get("canloadmore")
        )
        if next_page is not None:
            add(_query_url(source_url, page=next_page))
        elif current is not None and (
            (total_pages is not None and current < total_pages) or has_next
        ):
            source_page = urllib.parse.parse_qs(
                urllib.parse.urlsplit(source_url).query
            ).get("page", [])
            base_page = _int_value(source_page[-1]) if source_page else current
            add(_query_url(source_url, page=(base_page + 1 if base_page is not None else 2)))

        offset = _int_value(lower.get("offset"))
        limit = _int_value(lower.get("limit") or lower.get("pagesize") or lower.get("take"))
        total = _int_value(lower.get("total") or lower.get("totalcount") or lower.get("count"))
        if offset is not None and limit and (
            has_next or total is None or offset + limit < total
        ):
            add(_query_url(source_url, offset=offset + limit, limit=limit))

        page_info = lower.get("pageinfo")
        if isinstance(page_info, dict):
            end_cursor = page_info.get("endCursor") or page_info.get("end_cursor")
            page_has_next = bool(
                page_info.get("hasNextPage") or page_info.get("has_next_page")
            )
            if end_cursor and page_has_next:
                query = urllib.parse.parse_qs(urllib.parse.urlsplit(source_url).query)
                cursor_key = "after" if "after" in query else "cursor"
                add(_query_url(source_url, **{cursor_key: end_cursor}))
    ...
    # Some releases render only a generic Load more button. Probe the next page
    # via the document route; repeated or empty pages stop immediately.
    if not urls and _PAGINATION_MARKER_RE.search(normalized_html):
        query = urllib.parse.parse_qs(urllib.parse.urlsplit(source_url).query)
        current_page = _int_value((query.get("page") or [1])[-1]) or 1
        add(_query_url(source_url, page=current_page + 1))

    return list(dict.fromkeys(urls))[:MAX_PAGES_PER_SCAN]


def _scan_listing_roots(
    roots: list[str],
    *,
    force_first: bool,
    ttl: int,
    max_pages: int,
    paginate: Callable[[str], bool],
) -> tuple[list[dict[str, Any]], list[str], int, list[str], bool, bool]:
    """Breadth-first scan of listing roots without publishing partial data.

    Every first page is requested before page 2 of any listing. This prevents a
    large league from consuming the request budget and gives broad coverage in
    one scan. Rows stay in memory until the caller finishes the whole scan.
    """
    queue: list[tuple[str, int, bool]] = [
        (url, 0, bool(force_first and index == 0))
        for index, url in enumerate(list(dict.fromkeys(roots)))
        if url
    ]
    queued = {url for url, _, _ in queue}
    seen_urls: set[str] = set()
    branch_keys: dict[str, set[str]] = {}
    branch_fingerprints: dict[str, set[str]] = {}
    ...
    while cursor < len(queue) and checked < max(1, int(max_pages)):
        url, depth, force = queue[cursor]
        cursor += 1
        if url in seen_urls:
            continue
        seen_urls.add(url)
        try:
            page_html = _fetch_text(url, force=force, ttl=ttl)
            checked += 1
            ...
            if "HTTP 404" not in compact and "HTTP 410" not in compact:
                incomplete = True
            continue

        fingerprint = hashlib.sha1(page_html.encode("utf-8", "ignore")).hexdigest()
        branch = _listing_branch_key(url)
        fingerprints = branch_fingerprints.setdefault(branch, set())
        repeated_page = fingerprint in fingerprints
        fingerprints.add(fingerprint)

        page_rows = _rows_from_index_page(page_html, url)
        page_leagues = _league_prediction_urls_from_page(page_html)
        league_urls.extend(page_leagues)
        branch_seen = branch_keys.setdefault(branch, set())
        new_on_branch = 0
        for row in page_rows:
            key = str(row.get("prediction_key") or "")
            if not key:
                continue
            if key not in branch_seen:
                branch_seen.add(key)
                new_on_branch += 1
            rows_by_key[key] = row

        # A synthetic ?page=N route that repeats page one must terminate.
        can_continue = bool(depth == 0 or new_on_branch or page_leagues) and not repeated_page
        if can_continue and paginate(url) and depth + 1 < MAX_PAGES_PER_SCAN:
            for next_url in _pagination_urls_from_page(page_html, url):
                if next_url not in queued and next_url not in seen_urls:
                    queued.add(next_url)
                    queue.append((next_url, depth + 1, False))

    if cursor < len(queue) and checked >= max(1, int(max_pages)):
        incomplete = True
        warnings.append(
            f"Scores24: достигнут лимит полного обхода ({int(max_pages)} страниц); "
            "неполный снимок не будет опубликован."
        )
    return (
        list(rows_by_key.values()),
        list(dict.fromkeys(league_urls)),
        checked,
        warnings,
        rate_limited,
        incomplete,
...
def _discover_category_rows(*, force: bool = False) -> tuple[list[dict[str, Any]], int, int, list[str], bool, bool]:
    """Build one complete Scores24 snapshot and return it without publishing.

    The old implementation committed after every category and league page,
    making the UI counter rise by a few picks at a time. This implementation
    stages the full scan in memory. A rate-limited partial snapshot is never
    published by the caller.
    """
    warnings: list[str] = []
    category_urls = _next_category_batch()
    high_yield_paths = {
        f"/{LANG}/predictions/soccer",
        f"/{LANG}/predictions/soccer/today",
        f"/{LANG}/predictions/soccer/tomorrow",
    }

    category_budget = min(
        MAX_TOTAL_DISCOVERY_PAGES,
        max(len(category_urls), len(category_urls) + MAX_PAGES_PER_SCAN * 3),
    )
    (
        category_rows,
        current_leagues,
        ...
        category_urls,
        force_first=force,
        ttl=CATEGORY_REFRESH_SECONDS,
        max_pages=category_budget,
        paginate=lambda url: urllib.parse.urlsplit(url).path.rstrip("/") in high_yield_paths,
    )
    warnings.extend(category_warnings)
    _remember_league_urls(current_leagues)

    checked_leagues = 0
    league_rows: list[dict[str, Any]] = []
    if not rate_limited:
        current_batch = list(dict.fromkeys(current_leagues))[:MAX_CURRENT_LEAGUE_PAGES_PER_SCAN]
        # Current category pages already expose the leagues that contain today's
        # predictions. Scanning old rotating league URLs here only burns the
        # request budget and can trigger 429 before the current set completes.
        # Use historical URLs only as a fallback when the category pages expose
        # no league links at all.
        league_urls = current_batch or _next_league_batch()
        current_branches = {_listing_branch_key(item) for item in league_urls}
        remaining_budget = max(1, MAX_TOTAL_DISCOVERY_PAGES - checked_categories)
        (
            league_rows,
            nested_leagues,
            checked_leagues,
            ...
            league_urls,
            force_first=False,
            ttl=LEAGUE_REFRESH_SECONDS,
            max_pages=remaining_budget,
            paginate=lambda url: _listing_branch_key(url) in current_branches,
        )
        warnings.extend(league_warnings)
        scan_incomplete = scan_incomplete or league_incomplete
        _remember_league_urls(nested_leagues)

    dedup: dict[str, dict[str, Any]] = {}
    for row in category_rows + league_rows:
        key = str(row.get("prediction_key") or "")
        if key:
            dedup[key] = row
    return (
...
def feed(date_text: str, *, force: bool = False) -> dict[str, Any]:
    ...
    # Scores24 discovery runs every five minutes. The complete scan is staged
    # in memory and published once, so the UI counter never rises page by page.
    # API-Football/IGScore enrichment remains a later background step.
    discovery_due = _claim_hourly_discovery(force=force)
    full_scan_complete: bool | None = None
    if discovery_due:
        (
            discovered,
            checked_category_pages,
            checked_league_pages,
            discover_warnings,
            rate_limited,
            scan_incomplete,
        ) = _discover_category_rows(force=force)
        warnings.extend(discover_warnings)
        discovered = [row for row in discovered if (_row_date(row) or "") >= HISTORY_START_DATE]
        full_scan_complete = not rate_limited and not scan_incomplete
        if full_scan_complete:
            # One grouped publish after all Scores24 category, league and
            # pagination pages have been collected.
            raw_recovery = _store_discovered_rows(discovered, discover_warnings)
            ...
            recovery["dates"] = max(int(recovery.get("dates") or 0), int(raw_recovery.get("dates") or 0))
            recovery["rows"] = max(int(recovery.get("rows") or 0), int(raw_recovery.get("rows") or 0))
        else:
            # Never publish an incomplete snapshot after a 429. Keep the last
            # complete database list and retry on the next worker cycle.
            warnings.append("Неполный обход Scores24 не опубликован; сохранён последний полный список.")
            discovered = []
    ...
    payload["scan_status"] = {
        ...
        "atomic_full_scan": ATOMIC_FULL_SCAN,
        "full_scan_complete": full_scan_complete,
        "scan_incomplete": scan_incomplete if discovery_due else None,
        "max_pages_per_listing": MAX_PAGES_PER_SCAN,
        "max_total_discovery_pages": MAX_TOTAL_DISCOVERY_PAGES,
        ...
        try:
            ...
            for offset in offsets:
                ...
                try:
                    ...
                    result = feed(date_text, force=bool(cycle == 0 and offset == 0))
