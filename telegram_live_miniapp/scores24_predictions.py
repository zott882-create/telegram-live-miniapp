...
_VOID_STATUSES = {"PST", "CANC"}
_TERMINAL_STATUSES = _FINISHED_STATUSES | _CANCELLED_STATUSES | {"PST"}
...
def _storage_save(date_text: str, payload: dict[str, Any], *, is_final: bool = False) -> None:
    ...
    try:
        ...
        # Any saved prediction-day change can alter league ROI (especially a
        # newly settled match). Drop the ranking cache immediately so the next
        # UI read sees the fresh PostgreSQL result instead of waiting 15 min.
        roi_cache = globals().get("_LEAGUE_ROI_CACHE")
...
def _scores24_status_from_match_data(match_data: dict[str, Any]) -> dict[str, Any]:
    ...
    cancelled = any(token in status_token for token in ("cancel", "отмен"))
    abandoned = any(token in status_token for token in ("abandon", "прерван"))
    walkover = any(token in status_token for token in ("walkover", "walk over"))
    awarded = any(token in status_token for token in ("awarded", "awarded win"))
    postponed = any(token in status_token for token in ("postpon", "перенес"))
    suspended = any(token in status_token for token in ("suspend", "приостанов"))
    ...
    if cancelled:
        ...
        short, long_name = "ABD", status_name or "Матч прерван"
    elif walkover:
        short, long_name = "WO", status_name or "Технический результат"
    elif awarded:
        short, long_name = "AWD", status_name or "Присуждён результат"
    elif postponed:
        short, long_name = "PST", status_name or "Матч перенесён"
    elif suspended:
        short, long_name = "SUSP", status_name or "Матч приостановлен"
    elif finished:
    ...
    return {
        "finished": finished,
        "live": live,
        "cancelled": bool(cancelled or abandoned or walkover or awarded),
        "postponed": bool(postponed or suspended),
        "status_short": short,
...
def _parse_match_status_page(page_html: str, source_url: str) -> dict[str, Any] | None:
    """Parse one Scores24 match status from either hydration format.

    Scores24 has alternated between React Query hydration (``matchShow``) and
    URQL cache hydration.  The old final checker only understood the former,
    which meant some completed matches could remain in the active predictions
    list indefinitely even though their page already contained the final score.
    """
    ...
    if not isinstance(match_data, dict) or not match_data.get("slug"):
        target_slug = str(source_url or "").split("?", 1)[0].rstrip("/").rsplit("/", 1)[-1].strip()
        ...
        def walk(value: Any) -> None:
            ...
            looks_like_match = bool(
                slug
                and value.get("matchDate")
                and (
                    isinstance(value.get("teams"), list)
                    or isinstance(value.get("status"), dict)
                    or "isFinished" in value
                    or "isLive" in value
                )
            )
            if looks_like_match:
        ...
        match_data = candidates[0] if candidates else {}
...
def _apply_match_status(row: dict[str, Any], status: dict[str, Any]) -> dict[str, Any]:
    ...
    elif str(status.get("status_short") or "").upper() in _VOID_STATUSES:
        status_short = str(status.get("status_short") or "").upper()
        updated["prediction_match_state"] = "CANCELLED"
        updated["settlement"] = "push"
        updated["won"] = None
        updated["result_code"] = "Возврат: матч перенесён" if status_short == "PST" else "Возврат: матч отменён"
        updated["result_pending"] = False
        updated["next_check_at"] = 0
        updated["final_checked_at"] = int(time.time())
    ...
    *,
    ...
    if status_short in _LIVE_STATUSES:
        ...
        if status_short == "PST":
            ...
            updated["settlement"] = "push"
            updated["won"] = None
            updated["result_code"] = "Возврат: матч перенесён"
            updated["result_pending"] = False
        else:
            updated["prediction_match_state"] = "POSTPONED"
...
def _recalculate(date_text: str, rows: list[dict[str, Any]], warnings: list[str] | None = None) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if status in _VOID_STATUSES:
            row["settlement"] = "push"
            row["won"] = None
            row["result_code"] = "Возврат: матч перенесён" if status == "PST" else "Возврат: матч отменён"
            row["result_pending"] = False
            row["prediction_match_state"] = "CANCELLED"
            row["next_check_at"] = 0
            row["final_checked_at"] = int(row.get("final_checked_at") or time.time())
    ...
    with _storage_merge_lock:
        ...
        for key, latest_row in latest_rows.items():
            if key not in rows:
                rows[key] = latest_row
                continue
            outgoing = rows.get(key) or {}
            latest_state = str(latest_row.get("prediction_match_state") or "").upper()
            outgoing_state = str(outgoing.get("prediction_match_state") or "").upper()
            latest_status = _status_short_from_row(latest_row)
            outgoing_status = _status_short_from_row(outgoing)
            # A confirmed terminal row saved by the direct/collector bridge is
            # newer authority than an enrichment pass that still carries 2H.
            if (
                latest_status in _TERMINAL_STATUSES
                and outgoing_status not in _TERMINAL_STATUSES
            ) or (
                latest_state in {"FINAL_PENDING", "FINALIZED", "FINISHED", "FT", "AET", "PEN"}
                and outgoing_state == "LIVE"
            ):
                rows[key] = latest_row
...
def league_roi_ranking(limit: int = 5) -> list[dict[str, Any]]:
    ...
    for raw in groups.values():
        ...
        profit = returned - stake
        group["stake_total"] = round(stake, 2)
        group["returned_total"] = round(returned, 2)
...
def settle_saved_match_from_final_detail(
    match_id: str,
    match: dict[str, Any],
    stats: dict[str, Any] | None = None,
    *,
    source: str = "IGScore direct final",
) -> dict[str, Any]:
    """Immediately project one confirmed final IGScore detail into PostgreSQL.

    The normal Scores24 worker still collects the rich final snapshot/events.
    This bridge only removes the user-visible lag between an exact FT card and
    the prediction feed.  It is intentionally called only for a confirmed
    provider final, never from minute=90 alone.
    """
    wanted = str(match_id or "").strip()
    if not wanted or not isinstance(match, dict):
        return {"ok": False, "updated": 0, "settled": 0}

    minute_text = str(match.get("minute_text") or match.get("period") or "").strip().upper()
    confirmed_final = bool(match.get("finished")) or minute_text in {"FT", "AET", "PEN"}
    if not confirmed_final:
        return {"ok": False, "updated": 0, "settled": 0}
    status_short = minute_text if minute_text in {"FT", "AET", "PEN"} else "FT"
    ...
    home_score = _score_value(match.get("score_home"))
    away_score = _score_value(match.get("score_away"))
    if home_score is None or away_score is None:
        raw_score = str(match.get("score") or "").replace(":", "-")
    ...
    # Convert the collector's compact nested stats into the format already used
    # by the existing corners/cards settlement helper.
    ...
    if isinstance(stats, dict) and stats:
        ...
        for key, label in labels.items():
            ...
            if not isinstance(pair, dict):
                continue
            if pair.get("home") is not None:
                home_stats.append({"type": label, "value": pair.get("home")})
            if pair.get("away") is not None:
                away_stats.append({"type": label, "value": pair.get("away")})
        if home_stats:
            api_stats.append({"statistics": home_stats})
        if away_stats:
            api_stats.append({"statistics": away_stats})
    ...
    with _storage_merge_lock:
        for record in records:
            ...
            if not _valid_date(date_text):
                continue
            current = _storage_get(date_text)
            if not isinstance(current, dict):
                continue
            rows = [dict(row) for row in (current.get("feed") or []) if isinstance(row, dict)]
            changed = False
            for idx, row in enumerate(rows):
                ...
                if status_short == "FT":
                    score["fulltime"] = {"home": home_score, "away": away_score}
                    score["regular_time"] = {"home": home_score, "away": away_score}
                    updated["regulation_home_score"] = home_score
                    updated["regulation_away_score"] = away_score
                fixture_item["score"] = score
                updated["fixture_item"] = fixture_item
                updated["last_status_short"] = status_short
                updated["prediction_match_state"] = "FINAL_PENDING"
                updated["result_score_source"] = source
                updated["direct_final_detail_saved_at"] = now_ts
                updated["live_updated_at"] = now_ts
                # Force the normal worker to fetch final events/statistics on its
                # very next cycle instead of waiting on an old LIVE next_check.
                updated["next_check_at"] = 0

                settlement = won = result_code = None
                if status_short == "FT":
                    settlement, won, result_code = _settle_row(updated, home_score, away_score)
                    if not settlement and api_stats:
                        settlement, won, result_code = _settle_stat_market(updated, api_stats)
                if settlement:
                    updated["settlement"] = settlement
                    updated["won"] = won
                    updated["result_code"] = result_code or f"{home_score}:{away_score}"
                    ...
                    # The match is still finished even when a corners/cards
                    # market is waiting for its final statistic.
                    updated["result_code"] = updated.get("result_code") or f"{home_score}:{away_score}"
                    updated["result_pending"] = True
            ...
            if changed:
                payload = _recalculate(date_text, rows, list(current.get("warnings") or []))
                if isinstance(current.get("scan_status"), dict):
                    payload["scan_status"] = dict(current.get("scan_status") or {})
                _storage_save(date_text, payload, is_final=False)

    return {"ok": True, "updated": updated_total, "settled": settled_total}



def confirm_saved_match_final_from_scores24(match_id: str) -> dict[str, Any]:
    """Confirm one late IGScore-linked prediction directly from its Scores24 page.

    Minute 90/90+ alone is never treated as final.  This helper performs one
    direct Scores24 page refresh and only settles when Scores24 itself reports
    the match as finished.  It is used by the collector bridge for prediction
    matches that have reached the end of regulation but have not yet emitted an
    exact IGScore FT flag.
    """
    wanted = str(match_id or "").strip()
    if not wanted:
        return {"ok": False, "checked": 0, "updated": 0, "settled": 0, "finished": False}

    targets: list[tuple[str, str, str]] = []
    for record in _storage_records_full():
        date_text = str(record.get("date") or "")
        if not _valid_date(date_text):
            continue
        payload = record.get("feed") if isinstance(record.get("feed"), dict) else record
        for row in (payload.get("feed") or []) if isinstance(payload, dict) else []:
            if not isinstance(row, dict):
                continue
            row_mid = str(
                row.get("igscore_match_id")
            ...
            if source_url:
                targets.append((date_text, source_url, source_slug))

    if not targets:
        return {"ok": False, "checked": 0, "updated": 0, "settled": 0, "finished": False}

    # Normally one Scores24 prediction match has one source URL.  Deduplicate in
    # case a legacy row was copied between dates during an older migration.
    unique_urls: list[str] = []
    for _date_text, source_url, _source_slug in targets:
        if source_url not in unique_urls:
            unique_urls.append(source_url)

    checked = 0
    final_status: dict[str, Any] | None = None
    for source_url in unique_urls[:2]:
        try:
            page_html = _fetch_text(source_url, force=True, ttl=30)
            checked += 1
            status = _parse_match_status_page(page_html, source_url)
            ...
            print(f"[scores24] late final confirm failed match={wanted}: {_compact_error(exc)}")

    if not final_status:
        return {"ok": True, "checked": checked, "updated": 0, "settled": 0, "finished": False}

    updated_total = 0
    settled_total = 0
    touched_dates: set[str] = set()
    with _storage_merge_lock:
        for date_text, _source_url, source_slug in targets:
            if date_text in touched_dates:
                continue
            current = _storage_get(date_text)
            if not isinstance(current, dict):
                continue
            ...
            for idx, row in enumerate(rows):
                ...
                same_slug = bool(source_slug) and str(row.get("source_slug") or "").strip() == source_slug
                if row_mid != wanted and not same_slug:
                    continue
                before_settlement = str(row.get("settlement") or "")
                updated = _apply_match_status(row, final_status)
                updated["late_final_confirmed_at"] = int(time.time())
                updated["late_final_confirmed_by"] = "Scores24 direct"
                rows[idx] = updated
                changed = True
                updated_total += 1
                if not before_settlement and str(updated.get("settlement") or "") in {"won", "lost", "push"}:
                    settled_total += 1
            if changed:
                rebuilt = _recalculate(date_text, rows, list(current.get("warnings") or []))
                is_past = date_text < _today()
                all_finished = _rows_fully_finalized(rows)
                _storage_save(
                    date_text,
    ...
    return {
        ...
        "finished": True,
        "status": str(final_status.get("status_short") or "FT"),
    }


_OVERDUE_FINAL_LAST_CHECK: dict[str, float] = {}
_OVERDUE_FINAL_CHECK_INTERVAL = 70.0


def _refresh_overdue_current_predictions_from_scores24(max_matches: int = 8) -> dict[str, int]:
    """Directly confirm overdue active predictions on their Scores24 pages.

    This path is deliberately independent of IGScore matching and of the slow
    collector cycle.  It never declares FT from a displayed 90' alone: the
    Scores24 match page itself must report a terminal state.
    """
    now_ts = int(time.time())
    today = datetime.now(_tz()).date()
    ...
    for record in _storage_records_full():
        ...
        try:
            ...
            continue
        if row_date < today - timedelta(days=1) or row_date > today + timedelta(days=1):
            continue
        for row in record.get("feed") or []:
            if not isinstance(row, dict):
                continue
            if str(row.get("settlement") or "").lower() in {"won", "lost", "push"}:
                continue
            status_short = str(((((row.get("fixture_item") or {}).get("fixture") or {}).get("status") or {}).get("short") or "")).upper()
            if status_short in (_FINISHED_STATUSES | _CANCELLED_STATUSES | _POSTPONED_STATUSES):
                continue
            source_url = str(row.get("source_url") or "").strip()
            ...
            try:
                ...
                kickoff = 0
            try:
                elapsed = int((((fixture.get("status") or {}).get("elapsed")) or 0))
            except Exception:
                elapsed = 0
            state = str(row.get("prediction_match_state") or "").upper()
            age = now_ts - kickoff if kickoff > 0 else 0
            # Start checking near the end of regulation. Wall-clock age includes
            # half-time, so 95 minutes after kickoff is roughly minute 80+.
            due = elapsed >= 88 or (state in {"LIVE", "FINAL_PENDING"} and age >= 95 * 60) or age >= 105 * 60
            if not due:
                continue
            last = float(_OVERDUE_FINAL_LAST_CHECK.get(source_url) or 0.0)
            if time.time() - last < _OVERDUE_FINAL_CHECK_INTERVAL:
                continue
            seen.add(source_url)
            candidates.append((age, source_url, str(row.get("source_slug") or "").strip()))

    candidates.sort(key=lambda item: item[0], reverse=True)
    checked = finished_count = updated_total = settled_total = 0

    for _age, source_url, source_slug in candidates[:max(1, int(max_matches))]:
        _OVERDUE_FINAL_LAST_CHECK[source_url] = time.time()
        try:
            page_html = _fetch_text(source_url, force=True, ttl=30)
            checked += 1
            status = _parse_match_status_page(page_html, source_url)
            ...
            print(f"[scores24] overdue final check failed url={source_url}: {_compact_error(exc)}")
            continue
        if not isinstance(status, dict):
            continue
        if not (status.get("finished") or status.get("cancelled") or status.get("postponed")):
            continue
        finished_count += 1
        touched_dates: set[str] = set()
        with _storage_merge_lock:
            for record in _storage_records_full():
                date_text = str(record.get("date") or "")
                if not _valid_date(date_text) or date_text in touched_dates:
                    continue
                current = _storage_get(date_text)
                if not isinstance(current, dict):
                    continue
                ...
                changed = False
                for idx, row in enumerate(rows):
                    same_url = str(row.get("source_url") or "").strip() == source_url
                    same_slug = bool(source_slug) and str(row.get("source_slug") or "").strip() == source_slug
                    if not same_url and not same_slug:
                        continue
                    before = str(row.get("settlement") or "").lower()
                    updated = _apply_match_status(row, status)
                    updated["overdue_final_checked_at"] = int(time.time())
                    updated["overdue_final_source"] = "Scores24 direct status worker"
                    rows[idx] = updated
                    changed = True
                    updated_total += 1
                    after = str(updated.get("settlement") or "").lower()
                    if before not in {"won", "lost", "push"} and after in {"won", "lost", "push"}:
                        settled_total += 1
                if changed:
                    rebuilt = _recalculate(date_text, rows, list(current.get("warnings") or []))
                    is_past = date_text < _today()
                    ...
                    touched_dates.add(date_text)
        print(
            f"[scores24] overdue final confirmed slug={source_slug or '-'} "
            f"status={status.get('status_short', 'FT')} score={status.get('home_score')}:{status.get('away_score')}"
        )
...
def _worker() -> None:
    ...
    while not _stop.is_set():
        ...
        try:
            ...
            _health_update(heartbeat_at=int(time.time()), current_action="scores24-overdue-final")
            overdue_refresh = _refresh_overdue_current_predictions_from_scores24(8)
            if int(overdue_refresh.get("checked") or 0):
                print(
                    f"[scores24] overdue final scan checked={overdue_refresh.get('checked', 0)} "
                    f"finished={overdue_refresh.get('finished', 0)} "
                    f"updated={overdue_refresh.get('updated', 0)} settled={overdue_refresh.get('settled', 0)}"
                )
