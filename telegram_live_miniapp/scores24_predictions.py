...
REQUEST_GAP_SECONDS = max(0.10, min(2.0, float(os.environ.get("SCORES24_REQUEST_GAP_SECONDS", "0.35"))))
...
def _discover_category_rows(*, force: bool = False) -> tuple[list[dict[str, Any]], int, int, list[str], bool]:
    ...
    for category_url in _next_category_batch():
        try:
            ...
            page_rows = _rows_from_index_page(page_html, category_url)
            discovered.extend(page_rows)
            # Do not wait for all market and league pages or API-Football matching.
            # Save each parsed category immediately so the Mini App can show the
            # complete Scores24 list while the slower enrichment continues.
            if page_rows:
                _store_discovered_rows(page_rows, warnings)
    ...
    if not rate_limited:
        ...
        for league_url in league_urls:
            try:
                ...
                checked_leagues += 1
                league_rows = _rows_from_index_page(page_html, league_url)
                discovered.extend(league_rows)
                if league_rows:
                    _store_discovered_rows(league_rows, warnings)
            except Scores24RateLimited as exc:
                warnings.append(str(exc))
                rate_limited = True
                break
...
def feed(date_text: str, *, force: bool = False) -> dict[str, Any]:
    ...
    # Scores24 discovery runs every five minutes. Raw predictions are persisted
    # immediately; API-Football/IGScore enrichment continues in the background.
    ...
    if discovery_due:
        ...
        # Publish raw Scores24 predictions first. Team IDs, logos, LIVE state and
        # final scores are attached afterwards and must never block the counter.
