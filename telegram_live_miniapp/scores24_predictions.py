...
def _row_date(row: dict[str, Any]) -> str:
    return str((((row.get("fixture_item") or {}).get("fixture") or {}).get("date") or ""))[:10]
...
def feed(date_text: str, *, force: bool = False) -> dict[str, Any]:
    ...
    # A single category scan is distributed across all dates it contains.
    # This allows the bot to recover recent completed days (for example 01-04)
    # without requesting the same Scores24 pages once per selected date.
    discovered, checked_category_pages, discover_warnings, rate_limited = _discover_category_rows(force=force)
    warnings.extend(discover_warnings)
    recovery = _store_discovered_rows(discovered, discover_warnings)
...
def _worker() -> None:
    ...
    while not _stop.is_set():
        ...
        try:
            ...
            history_refresh = _refresh_unsettled_history(MAX_RESULT_PAGES_PER_SCAN)
            if history_refresh.get("checked_pages"):
                print(
                    f"[scores24] history results checked={history_refresh.get('checked_pages')} "
