...
SCORES24_BUILD_ID = "PREDICTION_POSTGRES_ARCHIVE_BACKUP_V24"
...
def league_roi_ranking(limit: int = 5) -> list[dict[str, Any]]:
    ...
    if cached and now - float(_LEAGUE_ROI_CACHE.get("ts") or 0) < _LEAGUE_ROI_CACHE_TTL:
        return [dict(x) for x in cached[:max(1, min(int(limit or 5), 20))]]
    ...
    _LEAGUE_ROI_CACHE["rows"] = [dict(x) for x in ranking[:20]]
    return [dict(x) for x in ranking[:max(1, min(int(limit or 5), 20))]]
...
def feed_for_client(date_text: str, *, include_all: bool = False) -> dict[str, Any]:
    ...
    result["league_roi_top"] = league_roi_ranking(5)
    return result
