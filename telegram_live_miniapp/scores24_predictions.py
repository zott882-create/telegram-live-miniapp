...
def _discover_category_rows(*, force: bool = False, full_categories: bool = False) -> tuple[list[dict[str, Any]], int, int, list[str], bool]:
    ...
    checked_leagues = 0
    rate_limited = False
    category_urls = _category_urls() if full_categories else _next_category_batch()
    for category_url in category_urls:
        try:
            page_html = _fetch_text(category_url, force=bool(force), ttl=CATEGORY_REFRESH_SECONDS)
...
def _recalculate(date_text: str, rows: list[dict[str, Any]], warnings: list[str] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "request_policy": "scores24-raw-discovery-3min_api-card-background-5min_igscore-live-5min-v5",
        "feed": rows,
        "errors": {},
...
def _discover_and_store_raw(*, force: bool = False) -> dict[str, Any]:
    ...
    Every market category is scanned in the same cycle.  Rows are immediately
    stored with the Scores24 team names, start time and editorial pick. Missing
    club/league logos are allowed. API-Football and IGScore enrichment runs in
    the separate five-minute lifecycle worker afterwards.
    """
...
def _discovery_worker() -> None:
    ...
    while not _stop.is_set():
        ...
        try:
            ...
            print(
                f"[scores24] raw discovery categories={result.get('checked_category_pages', 0)} "
