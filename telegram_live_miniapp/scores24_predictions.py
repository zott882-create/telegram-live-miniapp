...
def _graphql_league_catalog(*, force: bool = False) -> list[dict[str, Any]]:
    variables = {
        "first": 6,
        "isOpen": False,
        "langSlug": LANG,
        "sportSlug": "soccer",
        "timezoneOffset": _timezone_offset_minutes(),
        "withMatches": False,
    }
...
def _graphql_discover_rows(*, force: bool = False) -> tuple[list[dict[str, Any]], int, int, list[str], bool]:
    """Load every current football prediction via Scores24's public GraphQL API.

    The result is atomic: an incomplete scan is never published. Existing
    database rows stay untouched until all listed leagues have been fetched.
    """
    global _graphql_last_leagues, _graphql_last_predictions
    warnings: list[str] = []
    try:
        catalog = _graphql_league_catalog(force=force)
    except Scores24RateLimited as exc:
        return [], 0, 0, [str(exc)], True
    except Exception as exc:
        return [], 0, 0, [_compact_error(exc)], False
    ...
    for start in range(0, len(catalog), GRAPHQL_LEAGUE_BATCH_SIZE):
        batch = catalog[start:start + GRAPHQL_LEAGUE_BATCH_SIZE]
        slugs = [str(entry.get("slug") or "") for entry in batch]
        expected = {str(entry.get("slug") or ""): int(entry.get("count") or 0) for entry in batch}
        first = max([1] + list(expected.values()))
        try:
            payload = _graphql_open_payload(slugs, first=first, force=force)
        except Scores24RateLimited as exc:
        ...
        returned_slugs: set[str] = set()
        loaded_counts: dict[str, int] = {}
        for result in results if isinstance(results, list) else []:
            if not isinstance(result, dict):
                continue
            league_slug = str(((result.get("league") or {}).get("slug") or "")).strip()
            if league_slug:
                returned_slugs.add(league_slug)
            checked_leagues += 1
            rows, has_next, cursor = _graphql_rows_from_league_result(result)
            all_rows.extend(rows)
            loaded_counts[league_slug] = loaded_counts.get(league_slug, 0) + len(rows)

            # Normally first=count returns the complete league. Keep real cursor
            # pagination for unusually large leagues or a server-side page cap.
            page = 1
            while has_next and cursor and page < GRAPHQL_MAX_PAGES and league_slug:
                try:
                    follow = _graphql_open_payload(
                        [league_slug],
                        first=max(1, expected.get(league_slug, first)),
                        after=cursor,
                        force=force,
                    )
                except Scores24RateLimited as exc:
                    ...
                    complete = False
                    break
                follow_results = ((follow.get("data") or {}).get("LeaguesPrediction") or [])
                follow_result = next((x for x in follow_results if isinstance(x, dict)), None)
                if not follow_result:
                    complete = False
                    warnings.append(f"Scores24 GraphQL не вернул продолжение лиги {league_slug}")
                    break
                more_rows, has_next, cursor = _graphql_rows_from_league_result(follow_result)
                all_rows.extend(more_rows)
                loaded_counts[league_slug] = loaded_counts.get(league_slug, 0) + len(more_rows)
                page += 1
            if has_next and page >= GRAPHQL_MAX_PAGES:
                complete = False
                warnings.append(f"Scores24 GraphQL достиг лимита страниц для {league_slug}")
            if rate_limited:
                break
        if rate_limited:
            break

        # Some server versions may open only one league from a batch or cap the
        # item count. Retry omitted/incomplete leagues individually.
        retry_entries = [
            entry for entry in batch
            if str(entry.get("slug") or "") not in returned_slugs
            or loaded_counts.get(str(entry.get("slug") or ""), 0) < int(entry.get("count") or 0)
        ]
        for entry in retry_entries:
            slug = str(entry.get("slug") or "")
            expected_count = max(1, int(entry.get("count") or 1))
            try:
                single = _graphql_open_payload([slug], first=expected_count, force=force)
            except Scores24RateLimited as exc:
                warnings.append(str(exc))
                rate_limited = True
                complete = False
                ...
                complete = False
                continue
            single_results = ((single.get("data") or {}).get("LeaguesPrediction") or [])
            result = next((x for x in single_results if isinstance(x, dict)), None)
            if not result:
                complete = False
                warnings.append(f"Scores24 GraphQL не открыл лигу {slug}")
                continue
            checked_leagues += 1
            rows, has_next, cursor = _graphql_rows_from_league_result(result)
            all_rows.extend(rows)
            ...
            while has_next and cursor and page < GRAPHQL_MAX_PAGES:
                try:
                    follow = _graphql_open_payload([slug], first=expected_count, after=cursor, force=force)
                except Scores24RateLimited as exc:
                    warnings.append(str(exc))
                    rate_limited = True
                    complete = False
                ...
                more_rows, has_next, cursor = _graphql_rows_from_league_result(follow_result)
                all_rows.extend(more_rows)
                single_count += len(more_rows)
                page += 1
            if single_count < int(entry.get("count") or 0):
                complete = False
                warnings.append(
                    f"Scores24 GraphQL: лига {slug} — получено {single_count} из {int(entry.get('count') or 0)} прогнозов"
                )
        if rate_limited:
            break
    ...
    if len(final_rows) < expected_total:
        complete = False
        warnings.append(f"Scores24 GraphQL: получено {len(final_rows)} из {expected_total} прогнозов")

    if not complete:
        # Atomic publication: keep the previous full database snapshot.
...
def _recalculate(date_text: str, rows: list[dict[str, Any]], warnings: list[str] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "request_policy": "scores24-graphql-all-leagues-3min_api-card-background-5min_igscore-live-5min-v6",
        "feed": rows,
        "errors": {},
    ...
    while not _stop.is_set():
        ...
        try:
            ...
            print(
                ...
                f"leagues={result.get('checked_league_pages', 0)} "
...
def health(*, database_health: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "prediction_source": "Scores24 GraphQL (LeaguesPredictionHidden + LeaguesPredictionOpen)",
