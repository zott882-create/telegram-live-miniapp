...
MAX_API_HISTORY_DATES_PER_SCAN = max(1, min(12, int(os.environ.get("SCORES24_API_HISTORY_DATES_PER_SCAN", "4"))))
...
def _storage_records_full() -> list[dict[str, Any]]:
    """Return all saved day payloads, including feeds, ordered by date."""
    _storage_init()
    rows: list[Any] = []
    try:
        if _storage_mode() == "postgres":
            with _pg_conn_factory() as conn:  # type: ignore[misc]
                with conn.cursor() as cur:
                    cur.execute(
                        "SELECT stat_date,payload_json,is_final,updated_at FROM scores24_prediction_daily_history ORDER BY stat_date"
                    )
...
def _row_date(row: dict[str, Any]) -> str:
    # The API-Football fixture date may differ by one day because Scores24 and
    # the provider use different source time zones. Date tabs must follow the
    # day encoded in the Scores24 prediction URL, otherwise old picks are moved
    # into today's tab and 01-04 August appear empty.
    return _row_prediction_date(row)


def _discover_category_rows(*, force: bool = False) -> tuple[list[dict[str, Any]], int, list[str], bool]:
    ...
    for category_url in _next_category_batch():
        try:
            page_html = _fetch_text(category_url, force=bool(force and checked == 0), ttl=CATEGORY_REFRESH_SECONDS)
            checked += 1
    ...
    for row in discovered:
        ...
        if key:
            dedup[key] = row
    return list(dedup.values()), checked, warnings, rate_limited
...
def _store_discovered_rows(rows: list[dict[str, Any]], warnings: list[str] | None = None) -> dict[str, int]:
    ...
    oldest = today - timedelta(days=62)
...
def _migrate_saved_prediction_dates() -> dict[str, int]:
    """Move previously saved rows to the Scores24 slug day.

    Older builds grouped rows by API-Football's fixture date. On timezone/day
    mismatches that put an Aug 4 Scores24 prediction into Aug 5. The migration
    is idempotent and keeps all settled results and coefficients.
    """
...
def feed(date_text: str, *, force: bool = False) -> dict[str, Any]:
    ...
    # Scores24 supplies only the editorial pick and published coefficient. One
    # rotating category scan is shared across every date found in that response.
    discovered, checked_category_pages, discover_warnings, rate_limited = _discover_category_rows(force=force)
...
def start_worker() -> None:
    ...
    with _lock:
        ...
        _started = True
    try:
        _migrate_saved_prediction_dates()
    except Exception as exc:
        print(f"[scores24] date migration failed: {_compact_error(exc)}")
...
def health() -> dict[str, Any]:
    return {
        ...
        "date_source": "Scores24 prediction slug",
