...
#   * exactly 15 selected men's top domestic divisions, including Russia.
# The GraphQL catalogue is filtered before follow-up league requests, so
# lower divisions, friendlies and unrelated countries no longer enter the feed.
_SELECTED_DOMESTIC_TOP_DIVISION_SLUGS = {
...
_SELECTED_DOMESTIC_TOP_DIVISION_ALIASES = _SELECTED_DOMESTIC_TOP_DIVISION_SLUGS | {
...
# not include youth, national-team or non-European confederation tournaments.
...
# Direct-query slugs cover UEFA competitions that Scores24 can omit from the
# Hidden catalogue when its web UI considers those league sections "open".
# Unknown aliases are harmless: GraphQL simply returns no league for them.
_SELECTED_QUERY_LEAGUE_SLUGS = _SELECTED_DOMESTIC_TOP_DIVISION_ALIASES | {
    ...
    "uefa-super-cup",
    "international-clubs-uefa-super-cup",
    "uefa-women-s-champions-league",
    "international-clubs-uefa-women-s-champions-league",
    "uefa-womens-champions-league",
    "uefa-women-s-europa-cup",
    "international-clubs-uefa-women-s-europa-cup",
    "uefa-womens-europa-cup",
}

_SELECTED_DOMESTIC_LABEL_RULES = {
    "англия": {"премьер лига"}, "england": {"premier league"},
    "испания": {"ла лига", "примера дивизион"}, "spain": {"laliga", "la liga", "primera division"},
    "италия": {"серия а"}, "italy": {"serie a"},
    "германия": {"бундеслига"}, "germany": {"bundesliga"},
    "франция": {"лига 1"}, "france": {"ligue 1"},
    "нидерланды": {"эредивизи"}, "netherlands": {"eredivisie"},
    "португалия": {"примейра лига"}, "portugal": {"primeira liga"},
    "бельгия": {"высшая лига", "про лига"}, "belgium": {"first division a", "pro league"},
    "турция": {"суперлига"}, "turkey": {"super lig"},
    "россия": {"премьер лига"}, "russia": {"premier league"},
    "чехия": {"первая лига"}, "czech republic": {"1 liga", "first league"},
    "греция": {"суперлига"}, "greece": {"super league"},
    "норвегия": {"элитсерия"}, "norway": {"eliteserien"},
    "австрия": {"бундеслига"}, "austria": {"bundesliga"},
    "дания": {"суперлига"}, "denmark": {"superligaen"},
}
...
def _is_selected_prediction_league(league: dict[str, Any] | None) -> bool:
    # Load every Scores24 football league. Publication is controlled globally
    # by the administrator at prediction-card level.
    return True
...
def _prune_unselected_active_predictions() -> dict[str, int]:
...
def _graphql_discover_rows(*, force: bool = False) -> tuple[list[dict[str, Any]], int, int, list[str], bool]:
    """Load every current football prediction directly from Scores24 GraphQL.

    First request: complete league catalogue with embedded matches. Only leagues
    whose embedded items are incomplete are opened separately. The result is
    atomic, so a partial/blocked scan never replaces the previous database feed.
    """
    ...
    if len(final_rows) < expected_total:
        complete = False
        warnings.append(
            f"Scores24 GraphQL: получено {len(final_rows)} из {expected_total} прогнозов; "
            f"embedded={max(0, best_embedded)} offset={chosen_offset}"
        )

    if not complete:
        return [], 1, checked_leagues, list(dict.fromkeys(warnings)), rate_limited

    _graphql_last_predictions = len(final_rows)
    return final_rows, 1, checked_leagues, [], False
...
def _recalculate(date_text: str, rows: list[dict[str, Any]], warnings: list[str] | None = None) -> dict[str, Any]:
    ...
    selected_rows = [row for row in rows if bool(row.get("global_selected"))]
    ...
    return {
        "date": date_text,
        "source": "Scores24",
        "source_mode": "Scores24 raw picks every 3 min + API-Football card in background + IGScore LIVE statistics",
        "market_policy": "All Scores24 markets; only administrator-selected cards are public",
        "league_policy": "All Scores24 football leagues; global manual publication",
        ...
        "request_policy": "scores24-graphql-all-leagues-global-manual-selection-v9",
        "feed": rows,
        "errors": {},
...
def _discover_and_store_raw(*, force: bool = False) -> dict[str, Any]:
    ...
    pruned = _prune_unselected_active_predictions()
    return {
...
def _discovery_worker() -> None:
    ...
    while not _stop.is_set():
        ...
        try:
            ...
            print(
                f"[scores24] graphql discovery catalog_requests={result.get('checked_category_pages', 0)} "
                f"catalog_leagues={result.get('catalog_total', 0)} selected_leagues={result.get('checked_league_pages', 0)} "
...
def set_global_selection(date_text: str, prediction_key: str, selected: bool) -> dict[str, Any]:
    ...
    with _storage_merge_lock:
        ...
        found = False
        for row in rows:
            if str(row.get("prediction_key") or "") == str(prediction_key):
                row["global_selected"] = bool(selected)
                row["global_selected_at"] = int(time.time()) if selected else None
                found = True
                break
        if not found:
        ...
        rebuilt = _recalculate(date_text, rows, list(payload.get("warnings") or []))
        is_past = date_text < _today()
        _storage_save(date_text, rebuilt, is_final=bool(payload.get("is_final") and is_past))
        return {"ok": True, "date": date_text, "prediction_key": prediction_key, "selected": bool(selected), "selected_total": int(rebuilt.get("selected_predictions_total") or 0)}
...
def feed_for_client(date_text: str, *, include_all: bool = False) -> dict[str, Any]:
    ...
    result["selected_predictions_total"] = sum(1 for row in all_rows if row.get("global_selected"))
    result["selection_admin"] = bool(include_all)
    if not include_all:
        result["feed"] = [row for row in all_rows if row.get("global_selected")]
        result["feed_total"] = len(result["feed"])
    else:
...
def health(*, database_health: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "request_policy": "scores24-graphql-embedded-feed-plus-open-fallback-3min-v7",
