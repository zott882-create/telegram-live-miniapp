...
def league_roi_ranking(limit: int = 5) -> list[dict[str, Any]]:
    ...
    for payload in _storage_records_full():
        for row in payload.get("feed") or []:
            ...
            if not group.get("logo") and (league.get("logo") or league.get("flag")):
                group["logo"] = str(league.get("logo") or league.get("flag") or "")
    ...
    return ranking[:max(1, min(int(limit or 5), 20))]
