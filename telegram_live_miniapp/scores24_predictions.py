...
def _apply_api_fixture(
    ...
    *,
    ...
    # Keep the last stored statistics snapshot when this refresh only updates
    # score/status. A live/final statistics request replaces it below.
    if statistics is None and isinstance(previous_fixture_item, dict):
        ...
        if previous_fixture_item.get("statistics_updated_at"):
            fixture_item["statistics_updated_at"] = previous_fixture_item.get("statistics_updated_at")
        if previous_fixture_item.get("statistics_snapshot_state"):
            fixture_item["statistics_snapshot_state"] = previous_fixture_item.get("statistics_snapshot_state")
    ...
    if status_short in _LIVE_STATUSES:
        ...
        # Prematch metadata and the initial statistics snapshot are saved once.
        # The row is then served from the database until kickoff.
    ...
    # Re-attach metadata after adding statistics to the copied API payload.
    updated["fixture_item"] = fixture_item
...
def _group_refresh_reason(group: list[dict[str, Any]], now_ts: int) -> str:
    """Return why a prediction fixture needs API-Football now.

    Policy:
    - new match: save one complete prematch snapshot immediately;
    - prematch: never poll again before kickoff;
    - live: refresh score/statistics no more than once every five minutes;
    - finished: save one final snapshot and freeze permanently.
    """
    if not group:
        return "skip"
    sample = group[0]
    ...
    last_refresh = max(
        int(sample.get("api_football_last_fixture_refresh_at") or 0),
        int(sample.get("api_football_last_live_refresh_at") or 0),
    )

    if not fixture_id:
        return "discover"
    if status in _FINISHED_STATUSES:
        if all(bool(row.get("api_football_final_snapshot_saved")) for row in group):
            return "skip"
        return "final"
    if status in _LIVE_STATUSES:
        return "live" if now_ts - last_refresh >= LIVE_REFRESH_SECONDS else "skip"

    initial_saved = all(bool(row.get("api_football_initial_snapshot_saved")) for row in group)
    if not initial_saved:
        return "prematch"
    # Do not poll a future prematch. Starting shortly before kickoff, check at
    # the same five-minute cadence so the transition to LIVE is detected.
    if kickoff and now_ts < kickoff - 30:
        return "skip"
    return "kickoff" if now_ts - last_refresh >= LIVE_REFRESH_SECONDS else "skip"
    ...
    *,
    ...
    for key, group in grouped.items():
        ...
        if refresh_reason == "skip":
            ...
            continue
        try:
            # Manual UI refresh still obeys the one-prematch/5-minute-live
            # policy. ``force`` only helps the provider cache once a refresh is
            # actually due.
            api_item = _api_fixture_resolver(sample, bool(force and refresh_reason in {"discover", "live", "kickoff", "final"}))
        except Exception as exc:
        ...
        needs_stats = (
            ...
            or (status_short not in _LIVE_STATUSES | _FINISHED_STATUSES and not previous_initial_stats)
        )
        stats: list[dict[str, Any]] | None = None
        if needs_stats and fixture_id and _api_statistics_loader is not None:
            try:
                # Prematch is fetched once through the normal cache. LIVE and
                # final snapshots must be fresh when their five-minute/final
                # refresh is due.
                stats = _api_statistics_loader(
                    fixture_id,
