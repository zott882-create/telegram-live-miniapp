...
# Match Control Index: лёгкая control/momentum-модель под live-статистику IGScore.
# Идея: не выдавать это за expected goals, а честно показывать контроль матча
# через давление, опасность и территориальный tilt.
_CONTROL_PRESSURE_WEIGHT = 0.55
_CONTROL_DANGER_WEIGHT = 0.30
_CONTROL_FIELD_TILT_WEIGHT = 0.15
_CONTROL_DANGEROUS_WEIGHT = 0.03
_CONTROL_SHOT_WEIGHT = 0.15
_CONTROL_ON_TARGET_WEIGHT = 0.35
_CONTROL_CORNER_WEIGHT = 0.08
_CONTROL_GOAL_WEIGHT = 0.70


def _control_danger_score(dangerous: int, shots: int, on_target: int, corners: int, goals: int = 0) -> float:
    return round(
        (dangerous * _CONTROL_DANGEROUS_WEIGHT)
        + (shots * _CONTROL_SHOT_WEIGHT)
        + (on_target * _CONTROL_ON_TARGET_WEIGHT)
        + (corners * _CONTROL_CORNER_WEIGHT)
        + (goals * _CONTROL_GOAL_WEIGHT),
        2,
    )


def _control_share(home_value: float, away_value: float) -> tuple[float, float]:
    total = float(home_value or 0.0) + float(away_value or 0.0)
    if total <= 0:
        return 0.5, 0.5
    home_share = max(0.0, min(1.0, float(home_value or 0.0) / total))
    return home_share, 1.0 - home_share


def _control_side_payload(latest: dict[str, Any], oldest: dict[str, Any], side: str) -> dict[str, Any]:
    suffix = "_home" if side == "home" else "_away"
    attacks = _pressure_delta(latest, oldest, "attacks" + suffix)
    dangerous = _pressure_delta(latest, oldest, "dangerous" + suffix)
    shots = _pressure_delta(latest, oldest, "shots" + suffix)
    ...
    corners = _pressure_delta(latest, oldest, "corners" + suffix)
    goals = _pressure_delta(latest, oldest, "__chart_score" + suffix)
    pressure_score = _pressure_weighted_score(attacks, dangerous, shots, on_target, corners, goals)
    danger_score = _control_danger_score(dangerous, shots, on_target, corners, goals)
    field_tilt_score = max(0, dangerous)
    return {
        "score": 50.0,
        "percent": 50,
        "pressure_score": pressure_score,
        "danger_score": danger_score,
        "field_tilt_score": field_tilt_score,
        "attacks_delta": attacks,
        "dangerous_delta": dangerous,
        "shots_delta": shots,
        "on_target_delta": on_target,
...
def _control_pair_payload(latest: dict[str, Any], oldest: dict[str, Any], label: str, span_minutes: int = 0) -> dict[str, Any]:
    home = _control_side_payload(latest, oldest, "home")
    away = _control_side_payload(latest, oldest, "away")

    pressure_home_share, _ = _control_share(float(home.get("pressure_score") or 0.0), float(away.get("pressure_score") or 0.0))
    danger_home_share, _ = _control_share(float(home.get("danger_score") or 0.0), float(away.get("danger_score") or 0.0))
    tilt_home_share, _ = _control_share(float(home.get("field_tilt_score") or 0.0), float(away.get("field_tilt_score") or 0.0))

    activity_total = (
        float(home.get("pressure_score") or 0.0)
        + float(away.get("pressure_score") or 0.0)
        + float(home.get("danger_score") or 0.0)
        + float(away.get("danger_score") or 0.0)
        + float(home.get("field_tilt_score") or 0.0)
        + float(away.get("field_tilt_score") or 0.0)
    )

    home_control_share = (
        pressure_home_share * _CONTROL_PRESSURE_WEIGHT
        + danger_home_share * _CONTROL_DANGER_WEIGHT
        + tilt_home_share * _CONTROL_FIELD_TILT_WEIGHT
    )
    home_control_percent = round(max(0.0, min(1.0, home_control_share)) * 100, 1)
    away_control_percent = round(100.0 - home_control_percent, 1)

    home["score"] = home_control_percent
    away["score"] = away_control_percent
    home["percent"] = int(round(home_control_percent))
    away["percent"] = max(0, 100 - int(home["percent"]))

    diff = abs(home_control_percent - away_control_percent)
    if activity_total <= 0.5:
        leader = "none"
    elif diff <= 8.0:
        leader = "balanced"
    else:
        leader = "home" if home_control_percent > away_control_percent else "away"

    components = {
        "pressure_home_percent": int(round(pressure_home_share * 100)),
        "pressure_away_percent": max(0, 100 - int(round(pressure_home_share * 100))),
        "danger_home_percent": int(round(danger_home_share * 100)),
        "danger_away_percent": max(0, 100 - int(round(danger_home_share * 100))),
        "field_tilt_home_percent": int(round(tilt_home_share * 100)),
        "field_tilt_away_percent": max(0, 100 - int(round(tilt_home_share * 100))),
    }

    if activity_total >= 80:
        confidence = "high"
    elif activity_total >= 25:
        confidence = "medium"
    else:
        confidence = "low"

    return {
        "available": activity_total > 0.5,
        "label": label,
        "span_minutes": int(max(0, span_minutes or 0)),
        "leader": leader,
        "total": round(activity_total, 2),
...
def _control_row_parts(row: Any) -> tuple[int, dict[str, Any], int]:
    try:
        ts = int(row[0])
    except Exception:
        ts = 0
...
def _control_index_from_parsed_rows(parsed: list[Any], window_minutes: int = 20) -> dict[str, Any]:
    if not isinstance(parsed, list) or len(parsed) < 2:
        return {"available": False, "reason": "collecting"}

    rows = [_control_row_parts(r) for r in parsed]
    ...
    full = _control_pair_payload(latest_data, first_data, "full", full_span)
    ...
    recent = _control_pair_payload(latest_data, base_data, f"last_{window_minutes}", recent_span)

    basis = recent if bool(recent.get("available")) and float(recent.get("total") or 0.0) >= 10.0 else full
    return {
        "available": bool(full.get("available") or recent.get("available")),
        "kind": "match_control_index",
        "formula": "pressure*0.55 + danger*0.30 + field_tilt*0.15",
...
def _pressure_chart_from_history_impl(mid: str, now: int, match_meta: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    game_flow = _pressure_game_flow_from_bars(bars_full, match_meta or {}, quality_filtered)
    control_index = _control_index_from_parsed_rows(parsed, max(5, int(window_sec / 60)))
...
def _pressure_flow_range_label(start_slot: int, end_slot: int) -> str:
    start_slot = max(0, min(89, int(start_slot)))
    end_slot = max(0, min(89, int(end_slot)))
    if end_slot < 45:
        return f"{start_slot + 1}–{end_slot + 1}'"
    return f"{start_slot + 1}–{end_slot + 1}'"


def _pressure_flow_current_slot(match_meta: dict[str, Any], bars: list[dict[str, Any]]) -> int:
    phase = _pressure_chart_phase(match_meta or {})
    if phase == "HT":
        return 44
    slot = _pressure_chart_slot_from_minute(match_meta or {}, phase)
    if slot is not None:
        return max(0, min(89, int(slot)))
    used = [int(b.get("slot") or 0) for b in bars if not bool(b.get("empty"))]
    return max(used) if used else 0


def _pressure_game_flow_from_bars(bars: list[dict[str, Any]], match_meta: dict[str, Any] | None = None, quality_filtered: int = 0) -> dict[str, Any]:
    """Собирает текстовую историю матча по 15-минутным отрезкам."""
    match_meta = match_meta or {}
    if not isinstance(bars, list) or not bars:
        return {"available": False, "reason": "no_bars", "segments": []}

    home_name = str(match_meta.get("home") or "Хозяева")
    away_name = str(match_meta.get("away") or "Гости")
    current_slot = _pressure_flow_current_slot(match_meta, bars)
    windows = ((0, 14), (15, 29), (30, 44), (45, 59), (60, 74), (75, 89))
    segments: list[dict[str, Any]] = []

    for start_slot, end_slot in windows:
        if start_slot > current_slot:
            continue
        segment_bars = [b for b in bars if start_slot <= int(b.get("slot") or 0) <= end_slot]
        sampled = [b for b in segment_bars if not bool(b.get("empty"))]
        if not segment_bars:
            continue

        h_score = round(sum(float(b.get("h") or 0.0) for b in segment_bars), 1)
        a_score = round(sum(float(b.get("a") or 0.0) for b in segment_bars), 1)
        total = round(h_score + a_score, 1)
        if total > 0:
            home_pct = int(round((h_score / total) * 100))
            away_pct = max(0, 100 - home_pct)
        else:
            home_pct = away_pct = 50

        if not sampled or total <= 0.2:
            leader = "none"
            intensity = "low"
            title = "Данных мало"
            note = "Сервер ещё собирает историю этого отрезка."
        else:
            diff = abs(h_score - a_score)
            if diff <= max(4.0, total * 0.14):
                leader = "balanced"
                title = "Равный отрезок"
            elif h_score > a_score:
                leader = "home"
                title = f"Давление: {home_name}"
            else:
                leader = "away"
                title = f"Давление: {away_name}"

            if total >= 70:
                intensity = "high"
                note = "Высокий темп и много опасных действий."
            elif total >= 30:
                intensity = "medium"
                note = "Заметное давление, но без экстремального всплеска."
            else:
                intensity = "low"
                note = "Спокойный отрезок или мало событий."

        segments.append({
            "range": _pressure_flow_range_label(start_slot, min(end_slot, current_slot)),
            "start_slot": start_slot,
            "end_slot": min(end_slot, current_slot),
            "leader": leader,
            "intensity": intensity,
            "title": title,
            "note": note,
    ...
    if not segments:
        return {"available": False, "reason": "collecting", "segments": [], "quality_filtered": int(quality_filtered or 0)}

    scored = [s0 for s0 in segments if s0.get("leader") in {"home", "away", "balanced"}]
    if scored:
        home_total = sum(float(s0.get("home_score") or 0.0) for s0 in scored)
        away_total = sum(float(s0.get("away_score") or 0.0) for s0 in scored)
        if abs(home_total - away_total) <= max(5.0, (home_total + away_total) * 0.12):
            overall = "balanced"
            summary = "Матч идёт примерно равными волнами."
        elif home_total > away_total:
            overall = "home"
            summary = f"По истории давления чаще активнее {home_name}."
        else:
            overall = "away"
            summary = f"По истории давления чаще активнее {away_name}."
    else:
        overall = "none"
        summary = "История матча ещё собирается."
    ...
    return {
        ...
        "leader": overall,
        "segments": segments,
        "quality_filtered": int(quality_filtered or 0),
        "window_schema": "15-minute-game-flow",
    }
...
def main() -> None:
    ...
    print("=" * 72)
    print("Telegram Live Matches Mini App — v9.96 match control index patch")
