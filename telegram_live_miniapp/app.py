...
def fetch_match_odds_bundle_cached(match_id: str) -> dict[str, Any]:
    """Fetch/cache live odds plus best-effort prematch odds for a detail card.

    The known stable IGScore endpoint is /match/odds/last. Live odds are parsed
    from oddsData. If the same response contains opening/prematch fields, they
    are returned as prematch; otherwise prematch is an empty dict and the UI
    renders dashes without breaking the card.
    """
    mid = str(match_id or "").strip()
    if not mid:
    ...
    with _cache_lock:
        ...
        if cached and now - float(cached.get("saved_at") or 0) < ODDS_CACHE_SECONDS:
            return {"live": dict(cached.get("live") or {}), "prematch": dict(cached.get("prematch") or {})}
    try:
        ...
        if ODDS_DEBUG_LOG:
            ...
            print(f"[odds] prematch parsed for {mid}: {prematch}")
    except Exception as exc:
...
def main() -> None:
    ...
    print("=" * 72)
    print("Telegram Live Matches Mini App — v9.99 live + prematch odds patch")
