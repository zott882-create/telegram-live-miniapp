...
def match_odds_last(match_id: str) -> dict[str, Any]:
    ...
    Response contains a list of odds items, each with:
      oddsType: "eu" | "asia" | "bs"
      oddsData: [val0, val1, val2]
    Where:
      eu    → [П1, X, П2]
      asia  → [homeOdds, handicap, awayOdds]
      bs    → [over, totalLine, under]
    """
    return post_json("/v1/football/match/odds/last", {"matchId": str(match_id), **_base_payload()}, timeout=10.0)
...
def parse_odds_response(resp: dict[str, Any]) -> dict[str, Any]:
    """Parse the /v1/football/match/odds/last response into a structured dict.

    Returns:
    {
        "eu":    {"1": float|None, "X": float|None, "2": float|None},
        "asia":  {"home": float|None, "handicap": float|None, "away": float|None},
        "bs":    {"over": float|None, "line": float|None, "under": float|None},
    }
    Missing markets are absent from the dict.
    """
    result = resp.get("result") if isinstance(resp, dict) else resp
    # The result may be a list of items or a dict with a list inside.
    items: list[Any] = []
    if isinstance(result, list):
        items = result
    elif isinstance(result, dict):
        for key in ("odds", "oddsData", "data", "list", "items"):
            val = result.get(key)
            if isinstance(val, list):
                items = val
                break
        if not items:
            # Flat single-item shape: {"oddsType": "eu", "oddsData": [...]}
            if result.get("oddsType"):
                items = [result]
    ...
    for item in items:
        if not isinstance(item, dict):
            continue
        odds_type = str(item.get("oddsType") or item.get("type") or "").lower()
        raw_data = item.get("oddsData") or item.get("odds") or item.get("data") or []
...
def fetch_match_odds_cached(match_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        parsed = parse_odds_response(resp)
    except Exception as exc:
