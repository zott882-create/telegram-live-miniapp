...
def sportscore_detail_payload(match_id_value: str) -> dict[str, Any]:
    ...
    slug = str(match_id_value or "").strip()
    if slug.startswith("ss:"):
        slug = slug[3:]
    return sportscore_provider.detail(slug)
...
def avg_payload_for_match(match_id_value: str) -> dict[str, Any]:
    ...
    if mid.startswith("ss:"):
        if not _sportscore_ready():
            return {"ok": False, "error": "sportscore_provider_unavailable"}
        return sportscore_provider.avg_for_slug(mid[3:])
...
def main() -> None:
    ...
    print("=" * 72)
    print("Telegram Football Mini App — v10.5 prematch hotfix")
