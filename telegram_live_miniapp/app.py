...
PREMATCH_ENABLED = os.environ.get("PREMATCH_ENABLED", "1").strip().lower() not in {"0", "false", "no", "off"}
...
_opening_odds_snapshots: dict[str, dict[str, Any]] = {}
...
def _odds_movement_payload(prematch: dict[str, Any] | None, live: dict[str, Any] | None) -> dict[str, Any]:
    ...
    for market in ("1", "x", "2", "over", "under"):
        ...
        out[market] = {
            "prematch": round(before, 4), "live": round(now, 4),
        ...
        for mid, sport, odds_json in prepared:
            if phase == "prematch":
                conn.execute("""
                    ...
                    VALUES(?,?,?,?)
                    ON CONFLICT(match_id) DO UPDATE SET sport=excluded.sport,
                    prematch_json=excluded.prematch_json,prematch_updated_at=excluded.prematch_updated_at
                ...
                conn.execute("""
                    ...
                    VALUES(?,?,?,?)
                    ON CONFLICT(match_id) DO UPDATE SET sport=excluded.sport,
                    live_json=excluded.live_json,live_updated_at=excluded.live_updated_at
                """, (mid, sport, odds_json, now_i))
        conn.commit()
    finally:
        conn.close()
...
def _odds_snapshots_for_ids(ids: list[str]) -> dict[str, dict[str, Any]]:
    ...
    try:
        for start in range(0, len(clean_ids), 400):
            ...
            for row in rows:
                try: pre = _clean_odds_payload(json.loads(row["prematch_json"] or "{}"))
                except Exception: pre = {}
                try: live = _clean_odds_payload(json.loads(row["live_json"] or "{}"))
                except Exception: live = {}
                out[str(row["match_id"])] = {
                    "prematch_odds": pre, "live_odds": live,
...
def _capture_and_decorate_odds_rows(rows: list[dict[str, Any]], phase: str) -> None:
    ...
    for row in rows:
        ...
        if snap.get("prematch_odds"): row["prematch_odds"] = snap["prematch_odds"]
        if snap.get("live_odds"): row["live_odds"] = snap["live_odds"]
        if snap.get("odds_movement"): row["odds_movement"] = snap["odds_movement"]
...
def _decorate_detail_odds(payload: dict[str, Any], match_id_value: str) -> dict[str, Any]:
    ...
    if is_live and not live: live = current
    elif not is_live and not pre: pre = current
    if pre: _upsert_odds_rows([{"id": mid, "sport": sport, "odds": pre}], "prematch")
    if live: _upsert_odds_rows([{"id": mid, "sport": sport, "odds": live}], "live")
    ...
    if is_live and live: payload["odds"] = live
    elif pre: payload["odds"] = pre
...
def _odds_market_value(odds: Any, market: str) -> float | None:
    if not isinstance(odds, dict):
        return None
    eu = odds.get("eu") if isinstance(odds.get("eu"), dict) else {}
    bs = odds.get("bs") if isinstance(odds.get("bs"), dict) else {}
    mapping = {
        "1": eu.get("1"), "x": eu.get("X"), "2": eu.get("2"),
        "over": bs.get("over"), "under": bs.get("under"),
    }
    try:
        value = float(mapping.get(str(market or "").lower()))
        return value if value > 1.0 else None
    except (TypeError, ValueError):
        return None


def _opening_odds_index(sport: str) -> dict[str, dict[str, Any]]:
    ...
    with _cache_lock:
        cache = _sport_prematch_caches.get(sport)
        payload = cache.get("payload") if isinstance(cache, dict) else None
        if isinstance(payload, dict):
            payloads.append(payload)
    out: dict[str, dict[str, Any]] = {}
    with _cache_lock:
        for key, value in _opening_odds_snapshots.items():
            if key.startswith(f"{sport}:") and isinstance(value, dict) and isinstance(value.get("odds"), dict):
                out[key.split(":", 1)[1]] = dict(value.get("odds") or {})
...
def _decorate_live_with_opening_odds(rows: list[dict[str, Any]], sport: str) -> None:
    index = _opening_odds_index(sport)
    if not index:
        return
    for row in rows:
        opening = index.get(str(row.get("id") or "").strip())
        if opening:
            row["prematch_odds"] = opening
            row["opening_odds"] = opening
...
def fetch_live_sportscore(force: bool = False, sport: str = "football") -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        item["sport"] = sport
        out.append(item)
    _decorate_live_with_opening_odds(out, sport)
...
def _remember_opening_odds(rows: list[dict[str, Any]], sport: str = "football") -> None:
    now = time.time()
    with _cache_lock:
        for row in rows or []:
            if not isinstance(row, dict):
            ...
            odds = row.get("odds") if isinstance(row.get("odds"), dict) else {}
            if mid and odds:
                _opening_odds_snapshots[f"{sport}:{mid}"] = {"saved_at": now, "odds": dict(odds)}
        cutoff = now - 172800
        stale = [k for k, v in _opening_odds_snapshots.items() if float(v.get("saved_at") or 0) < cutoff]
        for key in stale:
            _opening_odds_snapshots.pop(key, None)


def _prematch_payload_from_rows(rows: list[dict[str, Any]], error: str = "") -> dict[str, Any]:
    detected_sport = str(next((r.get("sport") for r in rows if isinstance(r, dict) and r.get("sport")), "football") or "football")
    _remember_opening_odds(rows, detected_sport)
    ...
    if sport != "football":
        ...
        if force or not cached or now - saved_at >= PREMATCH_CACHE_SECONDS:
            try:
                ...
                _remember_opening_odds(rows, sport)
                payload = _prematch_payload_from_rows(rows)
                payload["sport"] = sport
                _capture_and_decorate_odds_rows(flatten_payload(payload), "prematch")
                with _cache_lock:
...
def load_live_payload(force: bool = False, sport: str = "football") -> dict[str, Any]:
    ...
    with _live_fetch_lock:
        ...
        try:
            if sport != "football":
                try:
                    load_prematch_payload(force=False, sport=sport)
                except Exception:
                    pass
...
def sanitize_notify_filter(f: dict[str, Any] | None) -> dict[str, Any]:
    ...
    return {
        ...
        "odds_market": str(src.get("odds_market") or "").lower()[:12],
        "odds_change_min": max(0.0, min(20.0, _safe_float(src.get("odds_change_min"), 0.0))),
        "odds_direction": str(src.get("odds_direction") or "any").lower()[:12],
        "odds_open_min": max(0.0, min(100.0, _safe_float(src.get("odds_open_min"), 0.0))),
        "odds_live_min": max(0.0, min(100.0, _safe_float(src.get("odds_live_min"), 0.0))),
...
def match_passes_filter(m: dict[str, Any], f: dict[str, Any]) -> bool:
    ...
    if _safe_int(f.get("pressure_min")):
        ...
        if max_pct < _safe_int(f.get("pressure_min"), 60):
            return False
    odds_market = str(f.get("odds_market") or "").lower()
    odds_change_min = _safe_float(f.get("odds_change_min"), 0.0)
    odds_open_min = _safe_float(f.get("odds_open_min"), 0.0)
    odds_live_min = _safe_float(f.get("odds_live_min"), 0.0)
    if odds_market and (odds_change_min > 0 or odds_open_min > 0 or odds_live_min > 0):
        opening = _odds_market_value(m.get("prematch_odds") or m.get("opening_odds"), odds_market)
        current = _odds_market_value(m.get("odds"), odds_market)
        if opening is None or current is None:
            return False
        if odds_open_min > 0 and opening < odds_open_min:
            return False
        if odds_live_min > 0 and current < odds_live_min:
            return False
        delta = current - opening
        direction = str(f.get("odds_direction") or "any").lower()
        if direction == "rise" and delta < odds_change_min:
            return False
        if direction == "drop" and (-delta) < odds_change_min:
            return False
        if direction not in {"rise", "drop"} and abs(delta) < odds_change_min:
            return False
...
def _notify_filter_analysis(m: dict[str, Any], f: dict[str, Any], label: str = "Фильтр") -> dict[str, Any]:
    ...
    if odds_market:
        ...
        if opening is not None and current is not None:
            delta = current - opening
            market_label = {"1":"П1","x":"Х","2":"П2","over":"ТБ","under":"ТМ"}.get(odds_market, odds_market)
            add(f"{market_label}: {opening:.2f} → {current:.2f} ({delta:+.2f})")
            rating += min(8.0, abs(delta) * 5.0)
    ...
    def do_GET(self) -> None:
        try:
            ...
            if path == "/api/live":
                ...
                return json_response(self, load_live_payload(force=force, sport=sport))

            if path == "/api/prematch":
                if not _rate_limit_ok(self, "prematch"):
                    return json_response(self, {"ok": False, "error": "rate limit"}, status=429)
                params = urllib.parse.parse_qs(url.query)
                force = str((params.get("force") or ["0"])[0]).lower() in {"1", "true", "yes"}
                date_filter = str((params.get("date") or [""])[0]).strip()
                sport = str((params.get("sport") or ["football"])[0]).strip().lower()
                return json_response(self, load_prematch_payload(force=force, date_filter=date_filter, sport=sport))
            ...
            if path == "/api/competition":
                if not _rate_limit_ok(self, "match"):
                    return json_response(self, {"ok": False, "error": "rate limit"}, status=429)
                params = urllib.parse.parse_qs(url.query)
                competition_path_value = str((params.get("path") or [""])[0]).strip()
                season_value = str((params.get("season") or [""])[0]).strip()
                round_value = str((params.get("round") or [""])[0]).strip()
                force = str((params.get("force") or ["0"])[0]).lower() in {"1", "true", "yes"}
                if not competition_path_value:
                    return json_response(self, {"ok": False, "error": "missing competition path"}, status=400)
                return json_response(self, sportscore_competition_payload(competition_path_value, season_value, round_value, force=force))
...
def main() -> None:
    ...
    print("=" * 72)
    print("Telegram Multi-Sport Mini App — v10.17 SportScore")
