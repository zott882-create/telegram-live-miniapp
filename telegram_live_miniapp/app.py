...
def _notification_text_for_match(m: dict[str, Any]) -> str:
    ...
    reasons = _alert_reasons_list(m.get("alert_reasons") or m.get("alert_reason") or [])
    extra = ""
    if rating:
        extra += f"\nСила: <b>{rating}/100</b>"
    if reasons:
        extra += "\nПричина: " + html.escape(" · ".join(reasons[:4]))
...
def _cleanup_notify_dedupe(sub: dict[str, Any], now: int | float | None = None) -> bool:
    """Keep de-dupe state bounded without re-sending active/recent matches."""
    now_f = float(now or time.time())
