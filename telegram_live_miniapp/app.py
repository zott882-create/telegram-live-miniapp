...
def collector_detail_for_match(match_id_value: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        return {"match": match, "stats": stats_nested, "events": events, "odds": odds, "pressure": pressure}
    except Exception:
        return None
    finally:
        try:
...
def detail_payload(match_id_value: str) -> dict[str, Any]:
    ...
    return {"ok": True, "match": match, "stats": stats, "avg": avg, "events": events, "odds": odds, "pressure": pressure}
...
def sanitize_notify_filter(f: dict[str, Any] | None) -> dict[str, Any]:
    enabled = bool((f or {}).get("enabled"))
    return fixed_goal_wait_filter(enabled=enabled)
