...
# v9.97: последний хороший /api/live снимок. Нужен, чтобы при падении
# провайдера/пустом ответе не показывать пользователю пустую ленту.
LIVE_LAST_GOOD_SNAPSHOT_FILE = DATA_DIR / "live_last_good_payload.json"
LIVE_LAST_GOOD_ENABLED = os.environ.get("LIVE_LAST_GOOD_ENABLED", "1").strip().lower() not in {"0", "false", "no", "off"}
LIVE_LAST_GOOD_MAX_AGE_SECONDS = int(os.environ.get("LIVE_LAST_GOOD_MAX_AGE_SECONDS", "1800"))
LIVE_LAST_GOOD_MIN_MATCHES = max(1, int(os.environ.get("LIVE_LAST_GOOD_MIN_MATCHES", "1")))
...
def build_payload(matches: list[dict[str, Any]], source: str, error: str | None = None) -> dict[str, Any]:
    ...
    # v9.97: сохраняем только полноценный live-снимок. Пустые ответы, demo и
    # восстановленные snapshot'ы не должны перезаписывать последний хороший файл.
    _save_live_last_good_payload(payload)
    return payload


def _live_payload_total(payload: dict[str, Any] | None) -> int:
    ...
    if total >= 0:
        return total
    try:
        return len(flatten_payload(payload))
    except Exception:
        return 0


def _is_good_live_payload(payload: dict[str, Any] | None) -> bool:
    if not LIVE_LAST_GOOD_ENABLED or not isinstance(payload, dict):
        return False
    if payload.get("ok") is False:
        return False
    source = str(payload.get("source") or "").lower()
    if source in {"demo", "last_good_snapshot"}:
        return False
    if payload.get("snapshot_cache"):
        return False
    return _live_payload_total(payload) >= LIVE_LAST_GOOD_MIN_MATCHES


def _save_live_last_good_payload(payload: dict[str, Any] | None) -> None:
    if not _is_good_live_payload(payload):
        return
    try:
        DATA_DIR.mkdir(exist_ok=True)
        saved_at = now_ts()
        ...
        tmp = LIVE_LAST_GOOD_SNAPSHOT_FILE.with_suffix(".tmp")
        tmp.write_text(json.dumps(data, ensure_ascii=False, separators=(",", ":")), "utf-8")
        os.replace(str(tmp), str(LIVE_LAST_GOOD_SNAPSHOT_FILE))
    except Exception as exc:
        print(f"[live-snapshot] save failed: {exc}")


def _load_live_last_good_payload(reason: str = "") -> dict[str, Any] | None:
    if not LIVE_LAST_GOOD_ENABLED or DATA_MODE == "demo":
        return None
    try:
        if not LIVE_LAST_GOOD_SNAPSHOT_FILE.exists():
            return None
        payload = json.loads(LIVE_LAST_GOOD_SNAPSHOT_FILE.read_text("utf-8") or "{}")
        meta = dict(payload.get("snapshot_cache") or {})
        saved_at = _safe_int(meta.get("saved_at"), _safe_int(payload.get("updated_at"), 0))
        age = max(0, now_ts() - saved_at) if saved_at else LIVE_LAST_GOOD_MAX_AGE_SECONDS + 1
        if LIVE_LAST_GOOD_MAX_AGE_SECONDS > 0 and age > LIVE_LAST_GOOD_MAX_AGE_SECONDS:
            return None
        if _live_payload_total(payload) < LIVE_LAST_GOOD_MIN_MATCHES:
            return None
        original_source = str(meta.get("source") or payload.get("source") or "unknown")
        payload["source"] = "last_good_snapshot"
        payload["snapshot_cache"] = {
            "restored": True,
            "saved_at": saved_at,
            "age_seconds": age,
            "source": original_source,
            "reason": str(reason or "provider_unavailable")[:180],
        }
        payload["error"] = str(reason or payload.get("error") or "Показан последний хороший снимок.")[:240]
        return payload
    except Exception as exc:
        print(f"[live-snapshot] load failed: {exc}")
        return None


def _fallback_live_payload_if_bad(payload: dict[str, Any], reason: str) -> dict[str, Any]:
    if _is_good_live_payload(payload):
        return payload
    cached = _load_live_last_good_payload(reason)
...
_XG_LITE_DANGEROUS_WEIGHT = 0.005
_XG_LITE_SHOT_WEIGHT = 0.06
_XG_LITE_ON_TARGET_BONUS = 0.14
_XG_LITE_CORNER_WEIGHT = 0.025
...
def _xg_lite_from_parsed_rows(parsed: list[Any], window_minutes: int = 20) -> dict[str, Any]:
    ...
    return {
        ...
        "formula": "dangerous_attacks*0.005 + shots*0.06 + shots_on_target*0.14 + corners*0.025",
        "goal_is_counted": False,
    ...
    with _live_fetch_lock:
        ...
        if mode in {"auto", "igscore"} and COLLECTOR_ENABLED and LIVE_FROM_DB_ONLY:
            try:
                ...
                payload = _fallback_live_payload_if_bad(payload, "collector_db пустой или временно недоступен")
                if isinstance(payload, dict):
                    payload.setdefault("collector", _collector_state_copy())
                with _cache_lock:
                ...
                if mode == "igscore":
                    ...
                    payload["collector"] = _collector_state_copy()
                    payload = _fallback_live_payload_if_bad(payload, "; ".join(errors))
                    if isinstance(payload, dict):
                        payload.setdefault("collector", _collector_state_copy())
                    with _cache_lock:
                        _live_cache.update({"saved_at": now, "payload": payload, "raw": {}})
        ...
        if mode in {"auto", "igscore"}:
            try:
                ...
                payload = _fallback_live_payload_if_bad(payload, "IGScore вернул пустой live-снимок")
                if str((payload or {}).get("source") or "") == "last_good_snapshot":
                    raw = {}
                with _cache_lock:
                ...
                if mode == "igscore":
                    payload = _load_live_last_good_payload("; ".join(errors)) or build_payload(DEMO_MATCHES, "demo", "; ".join(errors))
                    with _cache_lock:
                        _live_cache.update({"saved_at": now, "payload": payload, "raw": {}})
                    return payload
        ...
        if mode in {"auto", "sqlite"}:
            try:
                ...
                payload = _fallback_live_payload_if_bad(payload, "sqlite live-снимок пустой")
                with _cache_lock:
                    _live_cache.update({"saved_at": now, "payload": payload, "raw": {}})
                return payload
            except Exception as exc:
...
def main() -> None:
    ...
    print("=" * 72)
    print("Telegram Live Matches Mini App — v9.96 xg-lite strict patch")
