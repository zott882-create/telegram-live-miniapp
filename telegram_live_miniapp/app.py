    ...
    try:
        if NOTIFY_STORAGE == "postgres":
            ...
            with _notify_sqlite_conn() as conn:
                ...
                conn.commit()
        _telegram_send_stats["persisted"] = _safe_int(_telegram_send_stats.get("persisted"), 0) + 1
        return True
    except Exception as exc:
...
def _update_telegram_job_failure(item: dict[str, Any], error: str = "") -> None:
    ...
    if attempts >= TELEGRAM_JOB_MAX_ATTEMPTS:
        ...
        _telegram_send_stats["dropped"] = _safe_int(_telegram_send_stats.get("dropped"), 0) + 1
        ...
        try:
            ...
            for item in due:
                if _queue_put_telegram_item(item):
                    loaded += 1
            if loaded:
                _telegram_send_stats["loaded"] = _safe_int(_telegram_send_stats.get("loaded"), 0) + loaded
