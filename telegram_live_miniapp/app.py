...
def _init_live_cache_db() -> None:
    ...
    try:
        ...
        # v10.1: frozen odds snapshots. Prematch/opening odds must not turn into
        # dashes after kickoff, so the first usable prematch line is stored and
        # kept for the whole match. If the provider does not send a dedicated
        # prematch field, the first available odds row is frozen as fallback.
        conn.execute(
            """
...
def _read_saved_odds_snapshot(match_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            row = conn.execute(
                "SELECT live_json, prematch_json FROM match_odds_snapshots WHERE match_id = ? LIMIT 1",
                (mid,),
            ).fetchone()
            if not row:
                return {"live": {}, "prematch": {}}
...
def _store_odds_snapshot(match_id: str, live: dict[str, Any] | None = None, prematch: dict[str, Any] | None = None) -> dict[str, Any]:
    """Persist odds and freeze prematch values.

    Rule: prematch_json is write-once while it has usable values. This prevents
    IGScore's live odds endpoint from replacing prematch numbers with blanks or
    dashes after kickoff. When no dedicated prematch/opening field is available,
    the first usable live odds seen by the bot is stored as the fallback
    prematch snapshot, then kept unchanged.
    """
    mid = str(match_id or "").strip()
    if not mid:
        return {"live": {}, "prematch": {}}
    ...
    final_live = incoming_live if _odds_dict_has_values(incoming_live) else saved_live
    if _odds_dict_has_values(saved_pre):
        final_pre = saved_pre
    elif _odds_dict_has_values(incoming_pre):
        final_pre = incoming_pre
    elif _odds_dict_has_values(incoming_live):
        ...
        try:
            ...
            conn.execute(
                ...
                INSERT OR REPLACE INTO match_odds_snapshots(
                    match_id, live_json, prematch_json, first_seen_at, updated_at
                ) VALUES(?,?,?,?,?)
                """,
                (
                    mid,
...
def fetch_match_odds_bundle_cached(match_id: str) -> dict[str, Any]:
    ...
    provider responses after kickoff. If IGScore does not expose a separate
    prematch/opening field, the first usable odds seen by the bot is frozen as
    the prematch snapshot so the UI keeps numbers instead of dashes.
    """
    mid = str(match_id or "").strip()
    if not mid:
        return {"live": {}, "prematch": {}}
