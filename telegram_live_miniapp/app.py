...
def _save_last_good_match_detail(match_id_value: str, payload: dict[str, Any]) -> None:
    ...
    if not mid or not isinstance(payload, dict) or not payload.get("ok") or not isinstance(payload.get("match"), dict):
        return
    stored = {"saved_at": now_ts(), "payload": payload}
    _redis_set_json(f"lastgood:match:{mid}", stored, LAST_GOOD_MATCH_TTL_SECONDS)
    if LAST_GOOD_FILE_FALLBACK:
        try:
            LAST_GOOD_MATCH_DIR.mkdir(parents=True, exist_ok=True)
            path = LAST_GOOD_MATCH_DIR / f"{_safe_cache_file_name(mid)}.json"
...
def _load_last_good_match_detail(match_id_value: str, reason: str = "") -> dict[str, Any] | None:
    ...
    if not mid:
        return None
    stored = _redis_get_json(f"lastgood:match:{mid}")
    if not stored and LAST_GOOD_FILE_FALLBACK:
        try:
            path = LAST_GOOD_MATCH_DIR / f"{_safe_cache_file_name(mid)}.json"
            if path.exists():
...
def _pressure_chart_from_history_impl(mid: str, now: int, match_meta: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    latest_capture = int(parsed[-1][0]) if parsed else now
    for i in range(1, len(parsed)):
        ts_prev, prev_stats_raw = parsed[i - 1]
        ts_cur, cur_stats_raw = parsed[i]
        prev_stats = _pressure_chart_with_inferred_timing(prev_stats_raw, ts_prev, latest_capture, match_meta)
        cur_stats = _pressure_chart_with_inferred_timing(cur_stats_raw, ts_cur, latest_capture, match_meta)
        ...
        if cur_slot is None:
            # Last-resort compatibility fallback when neither the stored row
            # nor the current match carries clock information.
...
def _shared_match_cache_name(match_id_value: str) -> str:
    ...
    return f"current:v1034:match:{digest}"
...
def main() -> None:
    ...
    print("=" * 72)
    print("Telegram Football Live Mini App — v10.34 Pressure Graph Fix")
