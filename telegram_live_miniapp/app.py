...
def _notification_text_for_match(m: dict[str, Any]) -> str:
    ...
    return f"{prefix}: <b>{home}</b> — <b>{away}</b> · {score} · {minute}"
...
def _merge_notify_snapshot(live_match: dict[str, Any] | None, snap: dict[str, Any] | None, match_id: str = "") -> dict[str, Any] | None:
    ...
    if not out:
        ...
        for key in (
            "alert_kind", "alert_title", "alert_subtitle", "score", "home_logo", "away_logo",
            "league_logo", "country_code", "period", "finished",
        ):
...
def handle_notify_match_remove(body: dict[str, Any]) -> dict[str, Any]:
    ...
    with _notify_lock:
        ...
        _save_notify_matches()
    return {"ok": True, "chat_id": chat_id, "match_id": match_id, "removed": before - len(items), "total": len(items)}
                ...
                for chat_id_str, sub in list(_notify_subs.items()):
                    ...
                    if cfg.get("enabled"):
                        for m in matches:
                            ...
                            if BOT_TOKEN:
                                ok = send_telegram_message(chat_id_str, _notification_text_for_match(notify_match), link="")
                            else:
                                ok = True
                            if ok:
                                seen[mid] = now
                    ...
                    for mid, alert in list(goal_alerts.items()):
                        ...
                        if current_total > last_total:
                            ...
                            if BOT_TOKEN:
                                ...
                                ok = True
                            if ok:
                                seen[_notify_seen_key(str(mid), "goal")] = now
                                changed = True
...
def handle_notify(body: dict[str, Any]) -> dict[str, Any]:
    ...
    if bool(body.get("auto_goal")) and kind not in {"goal", "goal_wait"}:
        ...
        kind = str(m.get("alert_kind") or "filter").strip().lower()

    now = time.time()
    with _notify_lock:
        sub = _notify_subs.setdefault(chat_id, {"chat_id": chat_id, "filter": {}, "seen": {}, "goal_alerts": {}})
        seen = sub.setdefault("seen", {})
        dedupe_key = _notify_seen_key(match_id, kind)
        prev = float(seen.get(dedupe_key) or 0)
        duplicate = (now - prev) < NOTIFY_COOLDOWN_PER_MATCH
        _store_notify_match(chat_id, m, ts=int(now))
        if kind == "goal":
            if _mark_goal_alert_current(sub, m, now=int(now)):
                pass
