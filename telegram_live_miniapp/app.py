...
def _admin_set_webhook_if_possible() -> bool:
    """Prefer webhook for Telegram commands so Render can wake on /admin."""
    if not (BOT_TOKEN and ADMIN_PANEL_ENABLED and ADMIN_WEBHOOK_ENABLED and ADMIN_IDS):
        return False
    url = _telegram_webhook_url()
    if not url:
        print("[admin] webhook skipped: PUBLIC_BASE_URL/RENDER_EXTERNAL_URL is empty; fallback to polling")
        return False
    payload: dict[str, Any] = {
        "url": url,
    ...
    if result.get("ok"):
        ...
        return True
    print(f"[admin] setWebhook failed: {result.get('description') or result}; fallback to polling")
    return False
...
def main() -> None:
    ...
    print("=" * 72)
    print("Telegram Live Matches Mini App — v10.2 admin webhook fix + frozen prematch odds")
