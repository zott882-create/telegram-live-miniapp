...
def minute_from_match(match: dict[str, Any], server_time: int | None = None) -> tuple[int, str, str]:
    ...
    # Some sources keep an old live timer/kickoff timestamp for stale matches.
    # Never show impossible values like 314' or 341' in the UI; cap live match
    # minutes to a realistic maximum while keeping the match visible.
    if second > 0:
        minute = 45 + max(0, (now - second) // 60)
        shown = int(min(max(minute, 46), 130))
    ...
    if raw:
        mv = int(min(max(_safe_int(match.get("minute_value") or match.get("minute"), 0), 0), 130))
        txt = f"{mv}’" if mv > 0 and re.search(r"\d", raw) else raw
        return mv, txt, "2T" if mv > 45 else "1T"

    mv = int(min(max(_safe_int(match.get("minute") or match.get("minute_value"), 0), 0), 130))
