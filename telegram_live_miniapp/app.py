...
# v10.47: persistent SportScore snapshot database. The UI reads these JSON
# snapshots immediately; source refreshes happen in the background every two
# minutes instead of blocking each user request.
SPORTSCORE_SNAPSHOT_DB_ENABLED = os.environ.get("SPORTSCORE_SNAPSHOT_DB_ENABLED", "1").strip().lower() not in {"0", "false", "no", "off"}
SPORTSCORE_SNAPSHOT_DB = DATA_DIR / "sportscore_snapshots.sqlite3"
SPORTSCORE_SNAPSHOT_STALE_TTL_SECONDS = max(600, int(os.environ.get("SPORTSCORE_SNAPSHOT_STALE_TTL_SECONDS", "21600")))
SPORTSCORE_STATS_REFRESH_WORKERS = max(1, min(6, int(os.environ.get("SPORTSCORE_STATS_REFRESH_WORKERS", "3"))))
SPORTSCORE_STATS_REFRESH_QUEUE_MAX = max(50, int(os.environ.get("SPORTSCORE_STATS_REFRESH_QUEUE_MAX", "500")))
SPORTSCORE_STATS_REQUEST_DELAY = max(0.0, float(os.environ.get("SPORTSCORE_STATS_REQUEST_DELAY", "0.20")))
...
LIVE_BACKGROUND_REFRESH_SECONDS = max(60.0, float(os.environ.get("LIVE_BACKGROUND_REFRESH_SECONDS", "120")))
...
MATCH_DETAIL_FRESH_SECONDS = max(60.0, float(os.environ.get("MATCH_DETAIL_FRESH_SECONDS", "120")))
...
STAT_CACHE_SECONDS = max(60.0, float(os.environ.get("STAT_CACHE_SECONDS", "120")))
EVENT_CACHE_SECONDS = max(60.0, float(os.environ.get("EVENT_CACHE_SECONDS", "120")))
...
COLLECTOR_INTERVAL = float(os.environ.get("COLLECTOR_INTERVAL", "120"))
...
_sportscore_snapshot_db_lock = threading.RLock()
_sportscore_snapshot_db_ready = False
_sportscore_stats_refresh_queue: queue.Queue[str] = queue.Queue(maxsize=SPORTSCORE_STATS_REFRESH_QUEUE_MAX)
_sportscore_stats_refresh_pending: set[str] = set()
_sportscore_stats_refresh_lock = threading.Lock()
_sportscore_stats_workers_started = False
...
def _init_sportscore_snapshot_db() -> None:
    """Create the tiny JSON-snapshot database used by the fast live UI."""
    global _sportscore_snapshot_db_ready
    if not SPORTSCORE_SNAPSHOT_DB_ENABLED or _sportscore_snapshot_db_ready:
        return
    with _sportscore_snapshot_db_lock:
        if _sportscore_snapshot_db_ready:
            return
        DATA_DIR.mkdir(parents=True, exist_ok=True)
        conn = sqlite3.connect(str(SPORTSCORE_SNAPSHOT_DB), timeout=20)
        try:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_snapshots_saved_at ON snapshots(saved_at)")
            conn.commit()
            _sportscore_snapshot_db_ready = True
        finally:
            conn.close()


def _sportscore_snapshot_save(cache_key: str, payload: dict[str, Any], saved_at: int | None = None) -> None:
    if not SPORTSCORE_SNAPSHOT_DB_ENABLED or not cache_key or not isinstance(payload, dict):
        return
    try:
        _init_sportscore_snapshot_db()
        ...
        with _sportscore_snapshot_db_lock:
            ...
            try:
                conn.execute("PRAGMA busy_timeout=20000")
                conn.execute(
                    "INSERT INTO snapshots(cache_key,saved_at,payload_json) VALUES(?,?,?) "
                    "ON CONFLICT(cache_key) DO UPDATE SET saved_at=excluded.saved_at,payload_json=excluded.payload_json",
                    (str(cache_key), int(saved_at or now_ts()), body),
                )
                conn.commit()
            finally:
                conn.close()
    except Exception as exc:
        print(f"[snapshot-db] save failed key={cache_key}: {type(exc).__name__}: {exc}")


def _sportscore_snapshot_load(cache_key: str, max_age: int | None = None) -> dict[str, Any] | None:
    if not SPORTSCORE_SNAPSHOT_DB_ENABLED or not cache_key:
        return None
    try:
        _init_sportscore_snapshot_db()
        conn = sqlite3.connect(str(SPORTSCORE_SNAPSHOT_DB), timeout=20)
        ...
        try:
            conn.execute("PRAGMA busy_timeout=20000")
            row = conn.execute(
                "SELECT saved_at,payload_json FROM snapshots WHERE cache_key=? LIMIT 1",
                (str(cache_key),),
            ).fetchone()
        finally:
            conn.close()
        if not row:
            return None
        saved_at = _safe_int(row["saved_at"], 0)
        age = max(0, now_ts() - saved_at)
        if max_age is not None and age > max(0, int(max_age)):
            return None
        payload = json.loads(str(row["payload_json"] or "{}"))
        if not isinstance(payload, dict):
            return None
        return {"saved_at": saved_at, "age": age, "payload": payload}
    except Exception as exc:
        print(f"[snapshot-db] load failed key={cache_key}: {type(exc).__name__}: {exc}")
        return None


def _cleanup_sportscore_snapshot_db() -> int:
    if not SPORTSCORE_SNAPSHOT_DB_ENABLED:
        return 0
    try:
        _init_sportscore_snapshot_db()
        cutoff = now_ts() - max(600, SPORTSCORE_SNAPSHOT_STALE_TTL_SECONDS)
        with _sportscore_snapshot_db_lock:
            conn = sqlite3.connect(str(SPORTSCORE_SNAPSHOT_DB), timeout=20)
            try:
                cur = conn.execute("DELETE FROM snapshots WHERE saved_at < ?", (cutoff,))
                conn.commit()
...
def _snapshot_match_key(match_id_value: str) -> str:
    return "match:" + str(match_id_value or "").strip()


def _queue_sportscore_match_refresh(match_id_value: str) -> bool:
    """Queue one non-blocking detail/stat refresh; duplicate ids collapse."""
    mid = str(match_id_value or "").strip()
    if not mid:
        return False
    with _sportscore_stats_refresh_lock:
        if mid in _sportscore_stats_refresh_pending:
            return False
        _sportscore_stats_refresh_pending.add(mid)
    try:
        _sportscore_stats_refresh_queue.put_nowait(mid)
        return True
    except queue.Full:
        with _sportscore_stats_refresh_lock:
            _sportscore_stats_refresh_pending.discard(mid)
        return False


def _sportscore_stats_refresh_worker() -> None:
    while True:
        mid = _sportscore_stats_refresh_queue.get()
        try:
            if SPORTSCORE_STATS_REQUEST_DELAY > 0:
                time.sleep(SPORTSCORE_STATS_REQUEST_DELAY)
            _refresh_match_detail_sync(mid, force=True)
            with _cache_lock:
                _stats_cache.pop(mid, None)
                _events_cache.pop(mid, None)
        except Exception as exc:
            print(f"[stats-db] refresh failed match={mid}: {type(exc).__name__}: {exc}")
        finally:
            with _sportscore_stats_refresh_lock:
                _sportscore_stats_refresh_pending.discard(mid)
            _sportscore_stats_refresh_queue.task_done()


def start_sportscore_stats_workers() -> None:
    global _sportscore_stats_workers_started
    if _sportscore_stats_workers_started or not SPORTSCORE_SNAPSHOT_DB_ENABLED:
        return
    _sportscore_stats_workers_started = True
    for index in range(SPORTSCORE_STATS_REFRESH_WORKERS):
        threading.Thread(
            target=_sportscore_stats_refresh_worker,
            name=f"sportscore-stats-db-{index + 1}",
            daemon=True,
        ).start()
    print(f"[stats-db] workers={SPORTSCORE_STATS_REFRESH_WORKERS} refresh={int(MATCH_DETAIL_FRESH_SECONDS)}s db={SPORTSCORE_SNAPSHOT_DB}")
...
def sportscore_detail_payload(match_id_value: str, force: bool = False) -> dict[str, Any]:
    if not _sportscore_ready():
    ...
    if not slug:
        ...
        if sport != "football" and raw.startswith(sport + ":"):
            raw = raw.split(":", 1)[1]
        slug = raw
    payload = sportscore_provider.detail(slug, force=force, expected_match=expected, sport=sport)
...
def _build_live_payload_from_source(force: bool, sport: str) -> dict[str, Any]:
    try:
        ...
        _sportscore_snapshot_save(f"live:{sport}", payload)
        if sport == "football":
...
def load_live_payload(force: bool = False, sport: str = "football") -> dict[str, Any]:
    ...
    if not force:
        db_snapshot = _sportscore_snapshot_load(f"live:{sport}", SPORTSCORE_SNAPSHOT_STALE_TTL_SECONDS)
        if db_snapshot and isinstance(db_snapshot.get("payload"), dict):
            saved_at = _safe_int(db_snapshot.get("saved_at"), now_ts())
            payload = db_snapshot["payload"]
            age = max(0, now_ts() - saved_at)
            with _cache_lock:
                cache.update({"saved_at": float(saved_at), "payload": payload, "raw": {}})
            if age >= LIVE_CACHE_SECONDS:
            ...
            return _decorate_live_cache_metadata(
                ...
                saved_at=saved_at,
                layer="sqlite",
                stale=age >= LIVE_CACHE_SECONDS,
                refreshing=age >= LIVE_CACHE_SECONDS,
            )
...
def _nested_stats_from_detail_payload(detail: dict[str, Any] | None) -> dict[str, dict[str, int]]:
...
def match_stats_cached(match_id_value: str, match_meta: dict[str, Any] | None = None) -> dict[str, dict[str, int]]:
    """Return statistics from memory/SQLite immediately and refresh in background.

    This function is used by feed filters. It must never make the main page wait
    for dozens of SportScore detail requests. A cache miss is queued and the next
    2-minute feed cycle receives the populated database snapshot.
    """
    mid = str(match_id_value or "").strip()
    if not mid:
        return {}
    ...
    if cached and now - float(cached.get("saved_at") or 0) < STAT_CACHE_SECONDS:
        return cached.get("stats") or {}

    stats: dict[str, dict[str, int]] = {}
    snapshot = _sportscore_snapshot_load(_snapshot_match_key(mid), SPORTSCORE_SNAPSHOT_STALE_TTL_SECONDS)
    if snapshot and isinstance(snapshot.get("payload"), dict):
        stats = _nested_stats_from_detail_payload(snapshot.get("payload"))
        if _safe_int(snapshot.get("age"), 0) >= MATCH_DETAIL_FRESH_SECONDS:
            _queue_sportscore_match_refresh(mid)
    else:
        # No cached stats yet: queue one shared refresh and return immediately.
        _queue_sportscore_match_refresh(mid)

    if stats:
        try:
            if not _pressure_chart_is_half_time(match_meta or {}):
                _update_match_stats(mid, flat_stats(stats), dict(match_meta or {}))
        except Exception:
            pass
    _stats_cache[mid] = {"saved_at": now, "stats": stats}
    return stats
...
def enrich_live_matches_for_filters(matches: list[dict[str, Any]], filters: list[dict[str, Any]] | None = None) -> None:
    """Attach SportScore statistics only to matches needed by active filters."""
    ...
    for m in matches:
        ...
        if any(_coarse_filter_passes(m, f) for f in configs):
            ...
            candidates.append(m)
    if candidates:
        enrich_public_matches_with_stats(candidates)
    ...
    with local_lock:
        ...
        if redis_token == "":
            ...
            if fallback:
                return fallback
        try:
            # This function reaches the provider only after the outer two-minute
            # snapshot has expired. Bypass the provider's own TTL as well, or two
            # cache layers can keep returning the same old statistics.
            payload = detail_payload(mid, force=True)
            saved_at = now_ts()
            if payload.get("ok"):
                _sportscore_snapshot_save(_snapshot_match_key(mid), payload, saved_at=saved_at)
...
def load_match_detail_cached(match_id_value: str, force: bool = False) -> dict[str, Any]:
    ...
    if not force:
        db_snapshot = _sportscore_snapshot_load(_snapshot_match_key(mid), SPORTSCORE_SNAPSHOT_STALE_TTL_SECONDS)
        if db_snapshot and isinstance(db_snapshot.get("payload"), dict):
            saved_at = _safe_int(db_snapshot.get("saved_at"), now_i)
            payload = db_snapshot["payload"]
            age = max(0, now_i - saved_at)
            with _match_detail_fast_lock:
                _match_detail_fast_cache[mid] = {"saved_at": saved_at, "payload": payload}
            if age >= MATCH_DETAIL_FRESH_SECONDS:
                _queue_sportscore_match_refresh(mid)
            return _decorate_match_cache_metadata(
                payload,
                saved_at=saved_at,
                layer="sqlite",
                stale=age >= MATCH_DETAIL_FRESH_SECONDS,
                refreshing=age >= MATCH_DETAIL_FRESH_SECONDS,
            )
...
def detail_payload(match_id_value: str, force: bool = False) -> dict[str, Any]:
    ...
    return sportscore_detail_payload(mid, force=force)
...
class MiniAppHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        try:
            ...
            immutable_asset = (
                ".v1048." in file_path.name
                or ".v1047." in file_path.name
                or ".v1046." in file_path.name
...
def main() -> None:
    ...
    LAST_GOOD_MATCH_DIR.mkdir(parents=True, exist_ok=True)
    _init_sportscore_snapshot_db()
    _cleanup_sportscore_snapshot_db()
    _init_logo_db()
    _init_notify_storage()
    _init_admin_storage()
    _load_admin_state()
    start_collector_worker()
    start_sportscore_stats_workers()
    ...
    print("=" * 72)
    print("Telegram Football Live Mini App — v10.48 Two-Minute Stats Refresh Fix")
    print(f"Local:    http://127.0.0.1:{PORT}")
    print(f"Host:     {HOST}:{PORT}")
    print(f"Mode:     {DATA_MODE}")
    print(f"Notify:   {'enabled (' + str(len(_notify_subs)) + ' subs)' if BOT_TOKEN else 'disabled (no BOT_TOKEN)'}")
    print(f"Storage:  notify={NOTIFY_STORAGE} live_cache=memory+sqlite+redis-swr redis={'on' if REDIS_URL else 'off'}")
