...
def resolve_competition_logo(match: dict[str, Any]) -> str:
    ...
    if isinstance(comp, dict):
        candidates.extend([
            comp.get("logo"), comp.get("logoUrl"), comp.get("logo_url"),
            comp.get("competitionLogo"), comp.get("competition_logo"),
        ])
    candidates.extend([
        match.get("competitionLogo"), match.get("competition_logo"),
        match.get("league_logo"), match.get("leagueLogo"),
        match.get("tournamentLogo"), match.get("tournament_logo"),
    ])
...
def avg_total_for_team(team_id_value: str) -> dict[str, Any]:
    """Average match total for a team over recent matches, with 1H/2H split when available."""
...
def demo_detail(match: dict[str, Any]) -> dict[str, Any]:
    presets = {
        "demo-liv-ars": {
            ...
            "avg": {"home": {"avg": 3.1, "total_avg": 3.1, "first_half_avg": 1.2, "second_half_avg": 1.9, "count": 10, "zero_zero": 1}, "away": {"avg": 2.8, "total_avg": 2.8, "first_half_avg": 1.0, "second_half_avg": 1.8, "count": 10, "zero_zero": 2}},
        },
        "demo-mci-che": {
            ...
            "red_cards": {"home": 0, "away": 0},
            "avg": {"home": {"avg": 3.2, "total_avg": 3.2, "first_half_avg": 1.4, "second_half_avg": 1.8, "count": 10, "zero_zero": 0}, "away": {"avg": 2.4, "total_avg": 2.4, "first_half_avg": 0.9, "second_half_avg": 1.5, "count": 10, "zero_zero": 2}},
        },
    }
                    ...
                    for m in matches:
                        ...
                        if s:
                            text += (
                                ...
                                f"  ·  Атаки: {_safe_int(s.get('attacks_home'))}–{_safe_int(s.get('attacks_away'))}"
                                f"\nУгловые: {_safe_int(s.get('corners_home'))}–{_safe_int(s.get('corners_away'))}"
                                f"  ·  Карточки: {_safe_int(s.get('yellow_cards_home')) + _safe_int(s.get('red_cards_home'))}–{_safe_int(s.get('yellow_cards_away')) + _safe_int(s.get('red_cards_away'))}"
                            )
