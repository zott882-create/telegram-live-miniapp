...
# ============================================================================
#  Share cards: Telegram link preview + PNG ticket image
# ============================================================================

def html_response(handler: BaseHTTPRequestHandler, body: str, status: int = 200, cache: str = "no-store") -> None:
...
def _public_base_url(handler: BaseHTTPRequestHandler | None = None) -> str:
    ...
    if proto not in {"http", "https"}:
        proto = "https" if ("onrender.com" in host or "telegram" in host) else "http"
    return f"{proto}://{host}".rstrip("/")


def _share_detail(match_id_value: str) -> dict[str, Any]:
    mid = str(match_id_value or "").strip()
    if not mid:
        return {"ok": False, "error": "missing id"}
    try:
        payload = detail_payload(mid)
        if isinstance(payload, dict) and payload.get("ok") and isinstance(payload.get("match"), dict):
            return payload
    except Exception as exc:
        print(f"[share] detail error for {mid}: {exc}")
    return {"ok": False, "error": "match_not_found", "match_id": mid}


def _share_match(match_id_value: str) -> dict[str, Any]:
    payload = _share_detail(match_id_value)
    m = dict(payload.get("match") or {}) if isinstance(payload, dict) else {}
    if not m:
        m = {"id": str(match_id_value or ""), "home": "Team A", "away": "Team B", "score_home": 0, "score_away": 0, "minute": 0, "minute_text": "LIVE", "league": "Футбол Live", "country": ""}
    m.setdefault("id", str(match_id_value or m.get("id") or ""))
    return {"payload": payload, "match": m}


def _share_minute_text(match: dict[str, Any], payload: dict[str, Any] | None = None) -> str:
    if payload and payload.get("finished"):
        return "FT"
    mt = str(match.get("minute_text") or "").strip()
    if mt:
        return mt
    minute = _safe_int(match.get("minute"), 0)
    return f"{minute}’" if minute > 0 else "LIVE"


def _share_title(match: dict[str, Any]) -> str:
    home = str(match.get("home") or "Team A").strip() or "Team A"
    away = str(match.get("away") or "Team B").strip() or "Team B"
    return f"{home} — {away}"


def _share_description(match: dict[str, Any], payload: dict[str, Any] | None = None) -> str:
    minute = _share_minute_text(match, payload)
    score = f"{_safe_int(match.get('score_home'), 0)}:{_safe_int(match.get('score_away'), 0)}"
    league = str(match.get("league") or "Футбол Live").strip() or "Футбол Live"
    return f"{minute} • {score} • {league} • LIVE TICKET"


def _share_card_url(handler: BaseHTTPRequestHandler, match_id_value: str) -> str:
    base = _public_base_url(handler)
    mid = urllib.parse.quote(str(match_id_value or ""))
    version = int(time.time() // 60)
    return f"{base}/share/card.png?id={mid}&v={version}" if base else f"/share/card.png?id={mid}&v={version}"


def _share_match_url(handler: BaseHTTPRequestHandler, match_id_value: str) -> str:
    base = _public_base_url(handler)
    mid = urllib.parse.quote(str(match_id_value or ""))
    return f"{base}/share/match?id={mid}" if base else f"/share/match?id={mid}"


def share_match_page(handler: BaseHTTPRequestHandler, match_id_value: str) -> None:
    data = _share_match(match_id_value)
    payload = data["payload"]
    match = data["match"]
    title = _share_title(match)
    desc = _share_description(match, payload)
    card_url = _share_card_url(handler, match.get("id") or match_id_value)
    page_url = _share_match_url(handler, match.get("id") or match_id_value)
    score = f"{_safe_int(match.get('score_home'), 0)}:{_safe_int(match.get('score_away'), 0)}"
    minute = _share_minute_text(match, payload)
    league = str(match.get("league") or "Футбол Live").strip() or "Футбол Live"
    country = str(match.get("country") or "").strip()
    open_url = _public_base_url(handler) or "/"
    h = html.escape
    body = f'''<!doctype html>
<html lang="ru">
<head>
...
<title>{h(title)} · {h(score)}</title>
<meta name="description" content="{h(desc)}">
<meta property="og:type" content="website">
<meta property="og:url" content="{h(page_url)}">
<meta property="og:title" content="{h(title)}">
<meta property="og:description" content="{h(desc)}">
<meta property="og:image" content="{h(card_url)}">
<meta property="og:image:width" content="1200">
<meta property="og:image:height" content="630">
<meta name="twitter:card" content="summary_large_image">
<meta name="twitter:title" content="{h(title)}">
<meta name="twitter:description" content="{h(desc)}">
<meta name="twitter:image" content="{h(card_url)}">
<style>
  body {{ margin:0; min-height:100vh; display:grid; place-items:center; background:#050a07; color:#f3f7f4; font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif; }}
  .wrap {{ width:min(92vw,760px); padding:28px; }}
  .card {{ border:1px solid rgba(74,222,99,.35); border-radius:28px; padding:28px; background:radial-gradient(circle at 80% 0%, rgba(74,222,99,.18), transparent 38%), linear-gradient(180deg,#101d17,#09110d); box-shadow:0 0 50px rgba(74,222,99,.13); }}
  .label {{ color:#4ade63; letter-spacing:.14em; font-weight:800; font-size:13px; text-transform:uppercase; }}
  h1 {{ font-size:clamp(30px,6vw,54px); margin:18px 0 14px; line-height:1.05; }}
  .score {{ display:flex; gap:16px; align-items:center; font-size:32px; font-weight:900; margin:18px 0; }}
  .minute {{ color:#4ade63; }}
  .muted {{ color:#9ca3af; line-height:1.5; }}
  a {{ margin-top:22px; display:inline-flex; align-items:center; justify-content:center; min-height:48px; padding:0 22px; border-radius:999px; border:1px solid rgba(74,222,99,.45); color:#4ade63; text-decoration:none; font-weight:800; }}
</style>
</head>
<body>
  <main class="wrap">
    <section class="card">
      <div class="label">● LIVE TICKET</div>
      <h1>{h(title)}</h1>
      <div class="score"><span class="minute">{h(minute)}</span><span>•</span><span>{h(score)}</span></div>
      <div class="muted">{h((country + " · ") if country else "")}{h(league)}</div>
      <a href="{h(open_url)}">Открыть Футбол Live</a>
    </section>
  </main>
</body>
</html>'''
    html_response(handler, body, status=200, cache="public, max-age=60")
...
def _ellipsize(draw: Any, text: str, font: Any, max_width: int) -> str:
    ...
    while lo < hi:
        mid = (lo + hi) // 2
        candidate = text[:mid].rstrip() + tail
        if _text_width(draw, candidate, font) <= max_width:
            lo = mid + 1
        else:
            hi = mid
    return (text[:max(1, lo - 1)].rstrip() + tail) if text else ""


def share_card_png_bytes(match_id_value: str) -> bytes:
    ...
    W, H = 1200, 630
    img = Image.new("RGBA", (W, H), (5, 10, 7, 255))
    ...
    gd.ellipse((710, -130, 1330, 360), fill=(74, 222, 99, 42))
    gd.ellipse((-180, 320, 300, 820), fill=(74, 222, 99, 20))
    gd.ellipse((420, 100, 880, 500), fill=(74, 222, 99, 18))
    glow = glow.filter(ImageFilter.GaussianBlur(58))
    img = Image.alpha_composite(img, glow)
    draw = ImageDraw.Draw(img)

    font_logo = _share_font(34, True)
    font_sub = _share_font(24, False)
    draw.text((78, 60), "Футбол Live", font=font_logo, fill=(245, 247, 246, 255))
    draw.text((78, 103), "мини-приложение", font=font_sub, fill=(150, 160, 155, 255))

    x1, y1, x2, y2 = 62, 168, 1138, 544
    ticket = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    td = ImageDraw.Draw(ticket)
    td.rounded_rectangle((x1, y1, x2, y2), radius=38, fill=(12, 22, 18, 236), outline=(74, 222, 99, 90), width=2)
    for cx in (x1, x2):
        td.ellipse((cx - 38, 318, cx + 38, 394), fill=(5, 10, 7, 255), outline=(74, 222, 99, 90), width=2)
    for side_x in (x1 + 12, x2 - 12):
        for yy in range(y1 + 38, y2 - 26, 28):
            td.ellipse((side_x - 6, yy - 6, side_x + 6, yy + 6), fill=(74, 222, 99, 80))
    img = Image.alpha_composite(img, ticket)
    draw = ImageDraw.Draw(img)

    font_label = _share_font(27, True)
    font_teams = _share_font(54, True)
    font_score = _share_font(58, True)
    font_minute = _share_font(48, True)
    font_small = _share_font(25, False)
    green = (74, 222, 99, 255)
    white = (245, 247, 246, 255)
    muted = (154, 164, 159, 255)

    draw.ellipse((126, 218, 140, 232), fill=green)
    draw.text((158, 208), "LIVE TICKET", font=font_label, fill=green)

    title = _ellipsize(draw, _share_title(match), font_teams, 880)
    draw.text((126, 295), title, font=font_teams, fill=white)

    minute = _share_minute_text(match, payload)
    score = f"{_safe_int(match.get('score_home'), 0)}:{_safe_int(match.get('score_away'), 0)}"
    draw.text((126, 382), minute, font=font_minute, fill=green)
    minute_w = _text_width(draw, minute, font_minute)
    bx = 126 + minute_w + 33
    draw.ellipse((bx, 408, bx + 12, 420), fill=green)
    draw.text((bx + 38, 382), score, font=font_score, fill=white)

    league = str(match.get("league") or "Футбол Live").strip() or "Футбол Live"
    country = str(match.get("country") or "").strip()
    bottom = (country + " · " if country else "") + league
    bottom = _ellipsize(draw, bottom, font_small, 840)
    draw.text((126, 476), bottom, font=font_small, fill=muted)

    deco = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    dd = ImageDraw.Draw(deco)
    dd.rounded_rectangle((808, 208, 1058, 333), radius=16, fill=(74, 222, 99, 26), outline=(74, 222, 99, 110), width=2)
    dd.ellipse((786, 250, 836, 300), fill=(5, 10, 7, 110))
    dd.ellipse((1030, 250, 1080, 300), fill=(5, 10, 7, 110))
    deco = deco.rotate(-11, center=(930, 270), resample=Image.Resampling.BICUBIC)
...
def handle_notify_matches_clear(body: dict[str, Any]) -> dict[str, Any]:
    ...
    if not chat_id and isinstance(body_ids_raw, list) and len(_notify_subs) == 1:
        chat_id = next(iter(_notify_subs.keys()), "")
    if not chat_id:
...
def handle_notify_match_remove(body: dict[str, Any]) -> dict[str, Any]:
    ...
    chat_id = _chat_id_for_notify_cleanup(body)
    if not chat_id and len(_notify_subs) == 1:
        chat_id = next(iter(_notify_subs.keys()), "")
    if not chat_id:
        return {"ok": False, "error": "init_data verification failed"}
    ...
    def do_GET(self) -> None:
        try:
            ...
            if path == "/api/admin/status":
                ...
                return json_response(self, {"ok": True, "stats": _admin_stats_snapshot(), "collector": _collector_state_copy(), "notify": dict(_notify_worker_state)})
            ...
            if path == "/share/match" or path.startswith("/share/match/"):
                params = urllib.parse.parse_qs(url.query)
                mid = str((params.get("id") or [""])[0]).strip()
                if not mid and path.startswith("/share/match/"):
                    mid = urllib.parse.unquote(path.split("/share/match/", 1)[1]).strip("/")
                if not mid:
                    return text_response(self, "Missing match id", status=400)
                return share_match_page(self, mid)
