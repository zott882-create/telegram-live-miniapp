...
def fetch_live_sportscore(force: bool = False) -> list[dict[str, Any]]:
    ...
    rows = sportscore_provider.list_live(force=force)
    out: list[dict[str, Any]] = []
    for row in rows:
        item = dict(row)
...
def load_prematch_payload(force: bool = False, date_filter: str = "") -> dict[str, Any]:
    """Return prematch immediately; slow SportScore work runs in background."""
...
def load_live_payload(force: bool = False) -> dict[str, Any]:
    """Load LIVE only from SportScore with one shared server-side cache."""
    now = time.time()
    with _cache_lock:
    ...
    with _live_fetch_lock:
        ...
        with _cache_lock:
            if not force and _live_cache.get("payload") and now - float(_live_cache.get("saved_at") or 0) < LIVE_CACHE_SECONDS:
                return _live_cache["payload"]
        try:
            matches = fetch_live_sportscore(force=force)
            payload = build_payload(matches, "sportscore")
            payload["provider"] = "sportscore"
            payload["cache_seconds"] = int(LIVE_CACHE_SECONDS)
            _save_last_good_live_payload(payload)
        except Exception as exc:
            reason = f"sportscore: {type(exc).__name__}: {exc}"
            payload = _load_last_good_live_payload(reason) or build_payload([], "sportscore", reason)
            payload["provider"] = "sportscore"
    ...
    def do_GET(self) -> None:
        try:
            ...
            if path == "/api/prematch":
                ...
                return json_response(self, load_prematch_payload(force=force, date_filter=date_filter))
...
def main() -> None:
    ...
    print("=" * 72)
    print("Telegram Football Mini App — v10.14 SportScore Only Hotfix")
