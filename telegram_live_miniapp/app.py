...
def _prematch_row_by_id(match_id_value: str) -> dict[str, Any] | None:
    ...
    rows = flatten_payload(base) if isinstance(base, dict) else []
    for row in rows:
    ...
    # Compatibility with links created before unique data-match-id support.
...
def sportscore_detail_payload(match_id_value: str) -> dict[str, Any]:
    ...
    if not slug:
        slug = mid[3:] if mid.startswith("ss:") else mid
    return sportscore_provider.detail(slug, expected_match=expected)
...
def handle_goal_wait_subscribe(body: dict[str, Any]) -> dict[str, Any]:
    ...
    init_data = str(body.get("init_data") or "")
    enabled = bool(body.get("enabled"))
    auto_goal_enabled = bool(body.get("auto_goal_enabled"))
    user = verify_init_data(init_data)
    if not user:
        if not BOT_TOKEN:
    ...
    with _notify_lock:
        sub = _notify_subs.setdefault(chat_id, {"chat_id": chat_id, "filter": {}, "seen": {}, "goal_alerts": {}})
        sub["goal_wait"] = {"enabled": enabled, "auto_goal_enabled": auto_goal_enabled, "updated_at": int(time.time())}
        sub.setdefault("goal_wait_seen", {})
    ...
    return {"ok": True, "chat_id": chat_id, "enabled": enabled, "auto_goal_enabled": auto_goal_enabled}
...
def handle_subscribe(body: dict[str, Any]) -> dict[str, Any]:
    ...
    with _notify_lock:
        ...
        if "goal_wait_enabled" in body:
            gw = existing.get("goal_wait") if isinstance(existing.get("goal_wait"), dict) else {}
            gw = dict(gw or {})
            gw["enabled"] = bool(body.get("goal_wait_enabled"))
            if "goal_wait_auto_goal_enabled" in body:
                gw["auto_goal_enabled"] = bool(body.get("goal_wait_auto_goal_enabled"))
            else:
                gw.setdefault("auto_goal_enabled", False)
            gw["updated_at"] = int(time.time())
            existing["goal_wait"] = gw
            existing.setdefault("goal_wait_seen", {})
            if not bool(gw.get("auto_goal_enabled")):
