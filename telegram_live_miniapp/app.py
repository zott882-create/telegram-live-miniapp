...
PUBLIC_BASE_URL = (
    ...
    or os.environ.get("RAILWAY_SERVICE_DOMAIN")
    or ""
).strip().rstrip("/")
...
# For this custom build the owner Telegram id is prefilled, so /admin works
# without setting ADMIN_IDS manually. Extra admins can still be added on Render.
...
def _parse_admin_ids_env(raw: str, defaults: set[int]) -> set[int]:
    ...
    В этой сборке ID владельца 851766591 уже добавлен по умолчанию.
    Дополнительных админов можно указать в ADMIN_IDS через запятую.
    Если нужен свой запасной список, можно задать DEFAULT_OWNER_IDS в env.
    """
    result = set(defaults)
...
_DEFAULT_OWNER_IDS = _parse_int_set(os.environ.get("DEFAULT_OWNER_IDS", "851766591"))
...
# v10.2: Telegram webhook for admin commands. Polling does not wake a sleeping
# Render service, so webhook is preferred whenever PUBLIC_BASE_URL/RENDER_EXTERNAL_URL exists.
ADMIN_WEBHOOK_ENABLED = os.environ.get("ADMIN_WEBHOOK_ENABLED", "1").strip().lower() not in {"0", "false", "no", "off"}
TELEGRAM_WEBHOOK_PATH = os.environ.get("TELEGRAM_WEBHOOK_PATH", "/telegram-webhook").strip() or "/telegram-webhook"
...
ODDS_FREEZE_FIRST_LIVE_AS_PREMATCH = os.environ.get("ODDS_FREEZE_FIRST_LIVE_AS_PREMATCH", "0").strip().lower() in {"1", "true", "yes", "on"}
ODDS_CLEAR_LIVE_CLONE_PREMATCH = os.environ.get("ODDS_CLEAR_LIVE_CLONE_PREMATCH", "1").strip().lower() not in {"0", "false", "no", "off"}
...
# v9.98: saved notification cards are snapshots. Refresh their score on read so
# a match found at 0-0 does not stay 0-0 after goals or after full time.
NOTIFY_MATCHES_SCORE_REFRESH_ENABLED = os.environ.get("NOTIFY_MATCHES_SCORE_REFRESH_ENABLED", "1").strip().lower() not in {"0", "false", "no", "off"}
NOTIFY_MATCHES_SCORE_REFRESH_TTL_SECONDS = max(5, int(os.environ.get("NOTIFY_MATCHES_SCORE_REFRESH_TTL_SECONDS", "30")))
NOTIFY_MATCHES_SCORE_REFRESH_MAX_PER_READ = max(0, int(os.environ.get("NOTIFY_MATCHES_SCORE_REFRESH_MAX_PER_READ", "12")))
...
LIVE_FEED_STUCK_AFTER_MINUTES = float(os.environ.get("LIVE_FEED_STUCK_AFTER_MINUTES", "4"))
LIVE_FEED_STUCK_AFTER_SECONDS = int(max(1.0, LIVE_FEED_STUCK_AFTER_MINUTES) * 60)
# Verify suspicious late-live rows against /match/info. The competition/list
# endpoint can keep already finished matches as status=4 for 20+ minutes;
# direct match-info usually flips to a terminal status earlier.
LIVE_FEED_FINISHED_VERIFY_ENABLED = os.environ.get("LIVE_FEED_FINISHED_VERIFY_ENABLED", "1").strip().lower() not in {"0", "false", "no", "off"}
LIVE_FEED_FINISHED_VERIFY_MINUTE = int(os.environ.get("LIVE_FEED_FINISHED_VERIFY_MINUTE", str(LIVE_FEED_STUCK_MINUTE)))
LIVE_FEED_FINISHED_VERIFY_MAX_PER_SCAN = max(0, int(os.environ.get("LIVE_FEED_FINISHED_VERIFY_MAX_PER_SCAN", "40")))
...
def cleanup_runtime_caches(force: bool = False) -> int:
    ...
    with _cache_lock:
        ...
        removed += _cleanup_cache_dict(_odds_bundle_cache, max(300.0, ODDS_CACHE_SECONDS * 20), now_f)
        removed += _cleanup_cache_dict(_odds_presence_cache, max(300.0, ODDS_FEED_CACHE_SECONDS * 20), now_f)
        removed += _cleanup_cache_dict(_odds_movement_cache, max(300.0, ODDS_JUMP_CACHE_SECONDS * 3), now_f)
    if removed:
...
_admin_webhook_state: dict[str, Any] = {"last_set_at": 0, "last_set_ok": False, "last_set_description": "", "last_url": ""}
...
def _live_text_values(match: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(match, dict):
        return values
    for key in (
        "matchStatusName", "matchStatusText", "matchStatusDesc", "matchStatusDescription",
        "matchStatusShort", "shortStatus", "statusName", "statusText", "statusDesc",
        "statusDescription", "statusType", "status", "state", "stateName",
        "gameState", "stage", "phase", "period", "minute_raw", "minute_source",
        "time", "matchTime", "timer",
    ):
        value = match.get(key)
...
def _verify_late_live_finished(item: dict[str, Any], verify_used: int) -> tuple[dict[str, Any], int]:
    """Hide a suspicious 90+ live row if direct match-info says it is finished.

    competition/list is the feed used for the main page, but IGScore can leave a
    finished game there as status=4 for a long time. A cheap direct /match/info
    check for late-live candidates prevents finished games from hanging on the
    home screen for 20-30 minutes. The per-scan cap protects the provider/API.
    """
    if not LIVE_FEED_FINISHED_VERIFY_ENABLED:
        return item, verify_used
    if verify_used >= LIVE_FEED_FINISHED_VERIFY_MAX_PER_SCAN:
        return item, verify_used
    ...
    if not mid:
        return item, verify_used
    status = _safe_int(item.get("status") or item.get("status_id"), 0)
    if status == 3 or status not in LIVE_STATUSES:
        return item, verify_used
    current_timer = _safe_int(item.get("raw_minute") or item.get("minute"), 0)
    if current_timer < max(1, LIVE_FEED_FINISHED_VERIFY_MINUTE):
        return item, verify_used
    ...
    if not fresh:
        return item, verify_used
    fresh_status = _safe_int(fresh.get("status") or fresh.get("status_id"), 0)
    if not _is_finished_status(fresh_status) and not fresh.get("finished"):
        return item, verify_used

    merged = _merge_fresh_match(item, fresh)
    merged["_hidden_from_live"] = True
    merged["_hidden_reason"] = "direct_info_finished"
    return merged, verify_used
...
def _init_live_cache_db() -> None:
    ...
    try:
        ...
        # v10.5: frozen odds snapshots. Prematch/opening odds must not turn into
        # dashes after kickoff, so the first usable provider prematch/opening
        # line is stored and kept for the whole match. Do not copy live odds into
        # prematch by default, otherwise finished matches show identical rows.
        conn.execute(
            """
        ...
        odds_cols = {r[1] for r in conn.execute("PRAGMA table_info(match_odds_snapshots)").fetchall()}
        if "prematch_source" not in odds_cols:
            conn.execute("ALTER TABLE match_odds_snapshots ADD COLUMN prematch_source TEXT NOT NULL DEFAULT ''")
...
def _annotate_live_visibility(matches: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for m in matches:
        ...
        if prev:
            ...
            prev_hidden_reason = str(prev.get("hidden_reason") or "previously_hidden")
            # Once a late-live row was proven stale/finished, keep it out of the
            # main feed while the provider keeps re-sending it. The old code
            # wrote hidden_from_live_at to SQLite, but ignored it on the next scan,
            # so a 90-95' finished match could disappear and then reappear.
            if prev_hidden_at > 0 and status != 3 and current_timer >= min(stuck_minute, feed_max_minute):
                reasons.extend([x for x in prev_hidden_reason.split(",") if x])
...
def collector_detail_for_match(match_id_value: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        odds_bundle = fetch_match_odds_bundle_cached(mid) if mid else {"live": {}, "prematch": {}}
        odds = dict(odds_bundle.get("live") or {})
        prematch_odds = dict(odds_bundle.get("prematch") or {})
        pressure = pressure_from_stats_history(mid) if mid else {}
        pressure_chart = pressure_chart_from_history(mid, match) if mid else {}
        return {"match": match, "stats": stats_nested, "events": events, "odds": odds, "prematch_odds": prematch_odds, "pressure": pressure, "pressure_chart": pressure_chart}
    except Exception:
        return None
    finally:
        try:
...
def collector_update_once() -> dict[str, Any]:
    ...
    try:
        ...
        for match in iter_competition_matches(response):
            ...
            item, late_finish_verify_used = _verify_late_live_finished(item, late_finish_verify_used)
            if item.get("_hidden_from_live"):
                pass
            elif _has_finished_live_text(match):
...
def demo_detail(match: dict[str, Any]) -> dict[str, Any]:
    ...
    demo_live_odds = {"eu": {"1": 2.10, "X": 2.87, "2": 4.50}, "bs": {"over": 2.10, "line": 1.75, "under": 1.70}}
    demo_prematch_odds = {"eu": {"1": 1.95, "X": 3.15, "2": 4.10}, "bs": {"over": 1.88, "line": 2.50, "under": 1.92}}
    return {
        "match": match,
        "stats": {k: v for k, v in p.items() if k != "avg"},
        "avg": p["avg"],
        "events": demo_events,
        "odds": demo_live_odds,
        "prematch_odds": demo_prematch_odds,
    }
...
# v9.99: one shared raw-response parse for the detail screen.  It keeps live
# odds and possible prematch/opening odds together so /api/match does not hit
# IGScore twice for the same card.
_odds_bundle_cache: dict[str, dict[str, Any]] = {}
ODDS_CACHE_SECONDS = 30

# IGScore live odds always use oddsData. Some provider responses also include
# the original/prematch line in sibling fields; we parse those best-effort and
# the frontend shows dashes if the provider does not send them.
PREMATCH_ODDS_DATA_KEYS = (
    "preOddsData", "prematchOddsData", "initialOddsData", "initOddsData",
    "openingOddsData", "openOddsData", "firstOddsData", "originOddsData",
    "originalOddsData", "startOddsData", "oddsOpeningData", "oddsDataPre",
)
...
# v10.7: odds jump alerts. When a live coefficient moves sharply (for example
# team odds 2.00 -> 5.00), the snapshot updater records the movement in memory
# so the notification worker can send a Telegram alert without another API call.
ODDS_JUMP_CACHE_SECONDS = int(os.environ.get("ODDS_JUMP_CACHE_SECONDS", "300"))
ODDS_JUMP_MAX_MOVEMENTS_PER_MATCH = max(1, int(os.environ.get("ODDS_JUMP_MAX_MOVEMENTS_PER_MATCH", "12")))
_odds_movement_cache: dict[str, dict[str, Any]] = {}
...
def _odds_raw_data_from_item(item: dict[str, Any], data_keys: tuple[str, ...]) -> list[Any]:
    """Return the first odds data list present on a provider odds item."""
...
def parse_odds_response(resp: dict[str, Any], match_id: str = "", data_keys: tuple[str, ...] | None = None) -> dict[str, Any]:
    """Parse result.matchRecentOdds[matchId] list from /match/odds/last."""
    result = resp.get("result") if isinstance(resp, dict) else {}
    if not isinstance(result, dict):
        return {}

    data_keys = data_keys or ("oddsData",)
    match_recent = (
        result.get("matchRecentOdds")
        or result.get("matchOdds")
        or result.get("matchOddsMap")
        or result.get("odds")
        or {}
    )
    items: list[Any] = []
    if isinstance(match_recent, dict):
    ...
    elif isinstance(match_recent, list):
        items = match_recent
    ...
    for item in items:
        ...
        raw_data = _odds_raw_data_from_item(item, data_keys)
...
def _odds_dict_has_values(odds: Any) -> bool:
    """True when a parsed odds dict contains at least one usable value."""
    if not isinstance(odds, dict):
        return False
    eu = odds.get("eu") if isinstance(odds.get("eu"), dict) else {}
    bs = odds.get("bs") if isinstance(odds.get("bs"), dict) else {}
    asia = odds.get("asia") if isinstance(odds.get("asia"), dict) else {}
    values = (
        eu.get("1"), eu.get("X"), eu.get("2"),
        bs.get("over"), bs.get("under"),
        asia.get("home"), asia.get("away"),
    )
    return any(_safe_odds(v) is not None for v in values)


def _loads_odds_json(raw: Any) -> dict[str, Any]:
    try:
...
def _odds_dict_same(a: Any, b: Any) -> bool:
    """Compare parsed odds dicts by the numeric market values the UI renders."""
    if not _odds_dict_has_values(a) or not _odds_dict_has_values(b):
        return False

    def norm(odds: Any) -> tuple[Any, ...]:
        ...
        def n(v: Any) -> Any:
            parsed = _safe_odds(v)
            return None if parsed is None else round(float(parsed), 4)

        def line(v: Any) -> Any:
            try:
                return None if v is None or v == "" else round(float(str(v).replace(",", ".")), 4)
            except Exception:
                return None

        return (
            n(eu.get("1")), n(eu.get("X")), n(eu.get("2")),
            n(bs.get("over")), line(bs.get("line")), n(bs.get("under")),
            n(asia.get("home")), line(asia.get("handicap")), n(asia.get("away")),
        )
...
def _fmt_odds_value(value: Any) -> str:
    parsed = _safe_odds(value)
    if parsed is None:
        return "—"
    return f"{parsed:.2f}"


def _odds_value_map(odds: Any) -> dict[str, dict[str, Any]]:
    """Flatten parsed odds into comparable market keys."""
    odds = odds if isinstance(odds, dict) else {}
    out: dict[str, dict[str, Any]] = {}

    eu = odds.get("eu") if isinstance(odds.get("eu"), dict) else {}
    for key, label in (("1", "П1"), ("X", "Ничья"), ("2", "П2")):
        val = _safe_odds(eu.get(key))
        if val is not None:
            out[f"eu:{key}"] = {"market": "eu", "key": key, "label": label, "value": val}
    ...
    try:
        if line not in (None, ""):
            line_label = f" {float(str(line).replace(',', '.')):g}"
    except Exception:
        line_label = ""
    for key, label in (("over", f"ТБ{line_label}"), ("under", f"ТМ{line_label}")):
        val = _safe_odds(bs.get(key))
        if val is not None:
            out[f"bs:{key}"] = {"market": "bs", "key": key, "label": label, "value": val}

    asia = odds.get("asia") if isinstance(odds.get("asia"), dict) else {}
    hcp = asia.get("handicap")
    hcp_label = ""
    try:
        if hcp not in (None, ""):
            hcp_label = f" {float(str(hcp).replace(',', '.')):g}"
    except Exception:
        hcp_label = ""
    for key, label in (("home", f"Ф1{hcp_label}"), ("away", f"Ф2{hcp_label}")):
        val = _safe_odds(asia.get(key))
        if val is not None:
            out[f"asia:{key}"] = {"market": "asia", "key": key, "label": label, "value": val}

    return out


def _detect_odds_movements(old_odds: Any, new_odds: Any) -> list[dict[str, Any]]:
    """Return rising odds movements between two parsed live snapshots."""
    old_map = _odds_value_map(old_odds)
    new_map = _odds_value_map(new_odds)
    movements: list[dict[str, Any]] = []
    for full_key, old_item in old_map.items():
        new_item = new_map.get(full_key)
        if not new_item:
            continue
        old_v = _safe_odds(old_item.get("value"))
        new_v = _safe_odds(new_item.get("value"))
        if old_v is None or new_v is None:
            continue
        # Alerts are about drift up: 2.00 -> 5.00. Tiny provider rounding is ignored.
        if new_v <= old_v + 0.009:
            continue
        market = str(new_item.get("market") or "")
        key = str(new_item.get("key") or "")
        label = str(new_item.get("label") or full_key)
        movements.append({
            "market": market,
            "key": key,
            "full_key": full_key,
            "label": label,
            "old": round(float(old_v), 2),
            "new": round(float(new_v), 2),
            "delta": round(float(new_v) - float(old_v), 2),
            "ratio": round(float(new_v) / max(0.01, float(old_v)), 3),
            "signature": f"{market}:{key}:{old_v:.2f}>{new_v:.2f}",
        })
    movements.sort(key=lambda x: (float(x.get("delta") or 0), float(x.get("new") or 0)), reverse=True)
    return movements


def _remember_odds_movements(match_id: str, old_live: Any, new_live: Any, now_i: int | None = None) -> None:
    mid = str(match_id or "").strip()
    if not mid:
        return
    movements = _detect_odds_movements(old_live, new_live)
    if not movements:
        return
    now_i = int(now_i or now_ts())
    existing = _odds_movement_cache.get(mid)
    old_items = existing.get("movements") if isinstance(existing, dict) and isinstance(existing.get("movements"), list) else []
    seen = {str(x.get("signature") or "") for x in old_items if isinstance(x, dict)}
    merged = []
    for mv in movements:
        mv = dict(mv)
        mv["ts"] = now_i
        if str(mv.get("signature") or "") not in seen:
            merged.append(mv)
            seen.add(str(mv.get("signature") or ""))
    for mv in old_items:
        if isinstance(mv, dict):
            merged.append(mv)
    _odds_movement_cache[mid] = {"saved_at": now_i, "movements": merged[:ODDS_JUMP_MAX_MOVEMENTS_PER_MATCH]}


def _recent_odds_movements(match_id: str, now_i: int | None = None) -> list[dict[str, Any]]:
    mid = str(match_id or "").strip()
    if not mid:
        return []
    now_i = int(now_i or now_ts())
    item = _odds_movement_cache.get(mid)
    if not isinstance(item, dict):
        return []
    saved_at = _safe_int(item.get("saved_at"), 0)
    if saved_at and now_i - saved_at > max(1, ODDS_JUMP_CACHE_SECONDS):
        return []
    out = []
    for mv in item.get("movements") or []:
        if not isinstance(mv, dict):
            continue
        ts = _safe_int(mv.get("ts"), saved_at)
        if ts and now_i - ts > max(1, ODDS_JUMP_CACHE_SECONDS):
            continue
        out.append(mv)
    return out


def _read_saved_odds_snapshot(match_id: str) -> dict[str, Any]:
    """Read persistent live/prematch odds from the local live cache DB."""
    mid = str(match_id or "").strip()
    if not mid:
        return {"live": {}, "prematch": {}, "prematch_source": ""}
    try:
        _init_live_cache_db()
        conn = sqlite3.connect(str(LIVE_CACHE_DB), timeout=30)
        conn.row_factory = sqlite3.Row
        try:
            row = conn.execute(
                """
                SELECT live_json, prematch_json, COALESCE(prematch_source, '') AS prematch_source
            ...
            if not row:
                return {"live": {}, "prematch": {}, "prematch_source": ""}
            return {
                "live": _loads_odds_json(row["live_json"]),
                "prematch": _loads_odds_json(row["prematch_json"]),
                "prematch_source": str(row["prematch_source"] or ""),
            }
        finally:
            conn.close()
    except Exception:
...
def _store_odds_snapshot(
    ...
    prematch: dict[str, Any] | None = None,
    *,
    allow_live_as_prematch_fallback: bool | None = None,
) -> dict[str, Any]:
    """Persist live odds and freeze true prematch/opening odds.

    Prematch is intentionally NOT copied from the live odds after kickoff by
    default. Older versions used the first live row as a fallback, which made
    the UI show identical LIVE and PREMATCH odds for already-running or
    finished games. Now prematch is frozen only when the provider exposes a
    dedicated prematch/opening field. The old fallback remains behind
    ODDS_FREEZE_FIRST_LIVE_AS_PREMATCH=1 for installations that explicitly want
    it.
    """
    mid = str(match_id or "").strip()
    if not mid:
    ...
    if allow_live_as_prematch_fallback is None:
        allow_live_as_prematch_fallback = bool(ODDS_FREEZE_FIRST_LIVE_AS_PREMATCH)

    incoming_live = live if isinstance(live, dict) else {}
    incoming_pre = prematch if isinstance(prematch, dict) else {}
    saved = _read_saved_odds_snapshot(mid)
    saved_live = saved.get("live") if isinstance(saved.get("live"), dict) else {}
    saved_pre = saved.get("prematch") if isinstance(saved.get("prematch"), dict) else {}
    saved_pre_source = str(saved.get("prematch_source") or "")

    has_incoming_live = _odds_dict_has_values(incoming_live)
    has_incoming_pre = _odds_dict_has_values(incoming_pre)
    has_saved_pre = _odds_dict_has_values(saved_pre)

    final_live = incoming_live if has_incoming_live else saved_live
    final_pre: dict[str, Any] = {}
    final_pre_source = saved_pre_source

    # Clean up legacy rows created by the old first-live fallback. They have no
    # source marker and are byte-for-byte the same line as the current live odds.
    legacy_live_clone = (
        ODDS_CLEAR_LIVE_CLONE_PREMATCH
        and has_saved_pre
        and not saved_pre_source
        and not has_incoming_pre
        and has_incoming_live
        and _odds_dict_same(saved_pre, incoming_live)
    )

    if has_saved_pre and not legacy_live_clone:
        final_pre = saved_pre
        final_pre_source = saved_pre_source or "legacy"
    elif has_incoming_pre:
        final_pre = incoming_pre
        final_pre_source = "provider"
    elif allow_live_as_prematch_fallback and has_incoming_live:
        final_pre = incoming_live
        final_pre_source = "first_live"
    else:
        final_pre = {}
        final_pre_source = ""

    now_i = now_ts()
    if has_incoming_live and _odds_dict_has_values(saved_live) and not _odds_dict_same(saved_live, incoming_live):
        _remember_odds_movements(mid, saved_live, incoming_live, now_i=now_i)

    if not _odds_dict_has_values(final_live) and not _odds_dict_has_values(final_pre):
        return {"live": final_live or {}, "prematch": final_pre or {}}
    ...
    try:
        ...
        try:
            row = conn.execute(
                "SELECT first_seen_at FROM match_odds_snapshots WHERE match_id = ? LIMIT 1",
                (mid,),
            ).fetchone()
            if row:
                first_seen = _safe_int(row[0], now_i) or now_i
            conn.execute(
                """
                INSERT OR REPLACE INTO match_odds_snapshots(
                    match_id, live_json, prematch_json, prematch_source, first_seen_at, updated_at
                ) VALUES(?,?,?,?,?,?)
                """,
                (
                    mid,
                    json.dumps(final_live or {}, ensure_ascii=False, separators=(",", ":")),
                    json.dumps(final_pre or {}, ensure_ascii=False, separators=(",", ":")),
                    final_pre_source,
                    first_seen,
                    now_i,
                ),
        ...
        if ODDS_DEBUG_LOG:
            print(f"[odds] snapshot save failed {mid}: {exc}")
    return {"live": final_live or {}, "prematch": final_pre or {}}
    ...
    for start in range(0, len(to_fetch), ODDS_FEED_BATCH_SIZE):
        ...
        with _cache_lock:
            for mid in batch:
                ...
                odds_resp = {"result": {"matchRecentOdds": {mid: items}}}
                parsed = parse_odds_response(odds_resp, match_id=mid) if items else {}
                prematch = parse_prematch_odds_response(odds_resp, match_id=mid) if items else {}
                has = bool(parsed) or _odds_items_have_values(items)
                saved_bundle = _store_odds_snapshot(mid, parsed, prematch) if (parsed or prematch) else {"live": parsed, "prematch": prematch}
                _odds_presence_cache[mid] = {"saved_at": now, "has_odds": has}
                if parsed:
                    _odds_cache[mid] = {"saved_at": now, "odds": parsed}
                    _odds_bundle_cache[mid] = {
                        "saved_at": now,
                        "live": dict(saved_bundle.get("live") or parsed or {}),
                        "prematch": dict(saved_bundle.get("prematch") or prematch or {}),
                    }
                out[mid] = has
...
def fetch_match_odds_cached(match_id: str) -> dict[str, Any]:
    ...
    bundle = fetch_match_odds_bundle_cached(match_id)
    return dict(bundle.get("live") or {})


def parse_prematch_odds_response(resp: dict[str, Any], match_id: str = "") -> dict[str, Any]:
    """Parse prematch/opening odds when IGScore includes them with live odds."""
    return parse_odds_response(resp, match_id=match_id, data_keys=PREMATCH_ODDS_DATA_KEYS)


def fetch_match_odds_bundle_cached(match_id: str) -> dict[str, Any]:
    """Fetch/cache live odds and frozen prematch odds for a detail card.

    Prematch values are persisted in SQLite and are not overwritten by empty
    provider responses after kickoff. They are frozen only from a dedicated
    prematch/opening field so LIVE and PREMATCH do not become identical after
    ...
    now = time.time()
    with _cache_lock:
        cached = _odds_bundle_cache.get(mid)
        if cached and now - float(cached.get("saved_at") or 0) < ODDS_CACHE_SECONDS:
            live_cached = dict(cached.get("live") or {})
            pre_cached = dict(cached.get("prematch") or {})
            if _odds_dict_has_values(pre_cached):
                return {"live": live_cached, "prematch": pre_cached}
            saved_cached = _read_saved_odds_snapshot(mid)
            pre_saved = dict(saved_cached.get("prematch") or {})
            if _odds_dict_has_values(pre_saved):
                return {"live": live_cached or dict(saved_cached.get("live") or {}), "prematch": pre_saved}
            if _odds_dict_has_values(live_cached):
                frozen = _store_odds_snapshot(mid, live_cached, pre_cached)
                return {"live": live_cached, "prematch": dict(frozen.get("prematch") or {})}
            return {"live": live_cached, "prematch": pre_cached}

    try:
        resp = match_odds_last(mid)
        live = parse_odds_response(resp, match_id=mid)
        prematch = parse_prematch_odds_response(resp, match_id=mid)
        saved_bundle = _store_odds_snapshot(mid, live, prematch)
        live = live if _odds_dict_has_values(live) else dict(saved_bundle.get("live") or {})
        prematch = prematch if _odds_dict_has_values(prematch) else dict(saved_bundle.get("prematch") or {})
        if ODDS_DEBUG_LOG:
            raw_preview = str(resp)[:400]
            print(f"[odds] raw for {mid}: {raw_preview}")
            print(f"[odds] live parsed for {mid}: {live}")
            print(f"[odds] prematch/frozen parsed for {mid}: {prematch}")
    except Exception as exc:
        print(f"[odds] fetch failed {mid}: {exc}")
        saved_bundle = _read_saved_odds_snapshot(mid)
        live = dict(saved_bundle.get("live") or {})
        prematch = dict(saved_bundle.get("prematch") or {})

    with _cache_lock:
        _odds_cache[mid] = {"saved_at": now, "odds": live}
        _odds_bundle_cache[mid] = {"saved_at": now, "live": live, "prematch": prematch}
    return {"live": live, "prematch": prematch}
...
def detail_payload(match_id_value: str) -> dict[str, Any]:
    ...
    if not match:
        ...
        if fresh_info:
            ...
            try:
                ...
                stats = {}
            saved_odds = _read_saved_odds_snapshot(str(match_id_value))
            return {
                "ok": True,
                ...
                "odds": dict(saved_odds.get("live") or {}),
                "prematch_odds": dict(saved_odds.get("prematch") or {}),
    ...
    # v9.99: fetch live odds + best-effort prematch/opening odds from one
    # cached /v1/football/match/odds/last response.
    odds: dict[str, Any] = {}
    prematch_odds: dict[str, Any] = {}
    if mid:
        try:
            odds_bundle = fetch_match_odds_bundle_cached(mid)
            odds = dict(odds_bundle.get("live") or {})
            prematch_odds = dict(odds_bundle.get("prematch") or {})
        except Exception:
            odds = {}
    ...
    return {"ok": True, "match": match, "stats": stats, "avg": avg, "events": events, "odds": odds, "prematch_odds": prematch_odds, "pressure": pressure, "pressure_chart": pressure_chart}
...
def _score_tuple_from_public_match(m: dict[str, Any] | None) -> tuple[int, int, str]:
    """Return canonical score fields from a public/live/notify match object."""
    m = m if isinstance(m, dict) else {}
    h = _safe_int(m.get("score_home"), 0)
    a = _safe_int(m.get("score_away"), 0)
    score_text = str(m.get("score") or "").strip()
    parsed = re.search(r"(\d+)\s*[-:]\s*(\d+)", score_text)
    if parsed and (h + a) == 0:
        h, a = int(parsed.group(1)), int(parsed.group(2))
    return h, a, f"{h}-{a}"


def _notify_replace_score_text(value: Any, score_text: str, generic: bool = False) -> Any:
    """Replace only the score token inside alert reasons/titles, keep the rest."""
    if isinstance(value, list):
        return [_notify_replace_score_text(x, score_text, generic=generic) for x in value]
    text = str(value or "")
    if not text:
        return value
    # Russian UI reasons: "счёт 0-0" / "счет 0-0".  Do NOT blindly replace
    # any N-N in reasons, otherwise minute ranges like "45-65" get corrupted.
    updated, count = re.subn(r"(?i)(\bсч[её]т\s*)\d+\s*[-:]\s*\d+", rf"\g<1>{score_text}", text)
    if count:
        return updated
    if generic:
        # Titles/subtitles can contain "Team A 0-0 Team B".
        return re.sub(r"(?<!\d)\d+\s*[-:]\s*\d+(?!\d)", score_text, text, count=1)
    return text


def _merge_score_into_notify_record(item: dict[str, Any], fresh: dict[str, Any] | None, source: str = "") -> tuple[dict[str, Any], bool]:
    """Overlay fresh score/minute/status fields onto a saved notification card."""
    if not isinstance(item, dict) or not isinstance(fresh, dict):
        return item, False
    out = dict(item)
    old_h, old_a, old_score = _score_tuple_from_public_match(out)
    new_h, new_a, new_score = _score_tuple_from_public_match(fresh)
    old_total = old_h + old_a
    new_total = new_h + new_a

    changed = False
    # Football goals should not decrease.  This prevents bad provider snapshots
    # or stale 0-0 cached responses from overwriting a better saved score.
    if new_score != old_score and (new_total > old_total or old_total == 0):
        out["score_home"] = new_h
        out["score_away"] = new_a
        out["score"] = new_score
        for key in ("alert_reason", "alert_reasons"):
            if key in out:
                out[key] = _notify_replace_score_text(out.get(key), new_score, generic=False)
        for key in ("alert_title", "title"):
            if key in out:
                out[key] = _notify_replace_score_text(out.get(key), new_score, generic=True)
        changed = True

    # Keep the visual card fresh too, but do not erase alert-specific metadata.
    for key in (
        "minute", "minute_text", "period", "finished", "status", "status_id",
        "home", "away", "home_logo", "away_logo", "country", "country_code", "league", "league_logo", "link", "stats",
    ):
        value = fresh.get(key)
        if value not in (None, "") and out.get(key) != value:
            out[key] = value
            changed = True
    ...
    out["score_refreshed_at"] = now_i
    if out != item:
        changed = True
    return out, changed


def _refresh_notify_records_for_read(items: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], bool]:
    """Refresh saved notification records before /api/notify/matches returns them.

    Root cause fixed here: notify cards are intentionally persistent snapshots.
    If a card was first found at 0-0, it used to remain 0-0 forever.  On read we
    merge the current live row, then the collector DB row, and only for a small
    capped number of stale cards call direct match/info for final score.
    """
    if not items:
        return items, False
    ...
    try:
        live_map = {str(m.get("id") or ""): m for m in flatten_payload(load_live_payload(force=False)) if str(m.get("id") or "")}
    except Exception:
        live_map = {}
    ...
    for raw in items:
        ...
        fresh = live_map.get(mid)
        source = "live"
        if not fresh:
            try:
                detail = collector_detail_for_match(mid)
                fresh = detail.get("match") if isinstance(detail, dict) and isinstance(detail.get("match"), dict) else None
                source = "collector_db" if fresh else ""
            except Exception:
                fresh = None
                source = ""

        if fresh:
            item, changed = _merge_score_into_notify_record(item, fresh, source=source)
            changed_any = changed_any or changed

        if NOTIFY_MATCHES_SCORE_REFRESH_ENABLED and remote_used < NOTIFY_MATCHES_SCORE_REFRESH_MAX_PER_READ:
            last_refreshed = _safe_int(item.get("score_refreshed_at"), 0)
            old_h, old_a, _old_score = _score_tuple_from_public_match(item)
            # Remote lookup is capped and throttled.  Prioritise the exact bug:
            # cards that still show 0-0, plus cards that have left the live map.
            needs_remote = (now_i - last_refreshed >= NOTIFY_MATCHES_SCORE_REFRESH_TTL_SECONDS) and (
                (old_h + old_a) == 0 or mid not in live_map or bool(item.get("finished"))
            )
            if needs_remote:
                remote_used += 1
                direct = fresh_match_info_public(mid, force=True)
                if direct:
                    item, changed = _merge_score_into_notify_record(item, direct, source="match_info")
                    changed_any = changed_any or changed
...
def _notification_text_for_match(m: dict[str, Any]) -> str:
    ...
    if kind == "odds_jump":
        return _odds_jump_notification_text_for_match(m)
...
def _odds_jump_notification_text_for_match(m: dict[str, Any]) -> str:
    home = html.escape(str(m.get("home") or "Home"))
    away = html.escape(str(m.get("away") or "Away"))
    score = html.escape(str(m.get("score") or f"{_safe_int(m.get('score_home'), 0)}-{_safe_int(m.get('score_away'), 0)}"))
    minute = html.escape(str(m.get("minute_text") or (str(m.get("minute")) + "'" if m.get("minute") else "LIVE")))
    mv = m.get("odds_movement") if isinstance(m.get("odds_movement"), dict) else {}
    label = html.escape(str(mv.get("label") or "КФ"))
    old_v = html.escape(_fmt_odds_value(mv.get("old")))
    new_v = html.escape(_fmt_odds_value(mv.get("new")))
    return f"📈 КФ вырос: <b>{home}</b> — <b>{away}</b> · {score} · {minute}\n{label}: <b>{old_v}</b> → <b>{new_v}</b>"
...
def _merge_notify_snapshot(live_match: dict[str, Any] | None, snap: dict[str, Any] | None, match_id: str = "") -> dict[str, Any] | None:
    ...
    v9.98: a browser/localStorage snapshot can be stale (often 0-0).  When a
    live row is available, never let that stale snapshot overwrite the current
    score.  We only accept the snapshot score if it has more goals than the live
    row, which protects against race conditions without regressing to 0-0.
    """
    ...
    if not out:
        ...
        for key in (
            ...
            "home_logo", "away_logo", "league_logo", "country_code", "period", "finished",
        ):
            if key in snap and snap.get(key) not in (None, ""):
                out[key] = snap.get(key)

        snap_h = _safe_int(snap.get("score_home"), 0)
        snap_a = _safe_int(snap.get("score_away"), 0)
        out_h = _safe_int(out.get("score_home"), 0)
        out_a = _safe_int(out.get("score_away"), 0)
        if (snap_h + snap_a) > (out_h + out_a):
            out["score_home"] = snap_h
            out["score_away"] = snap_a
            out["score"] = str(snap.get("score") or f"{snap_h}-{snap_a}")

        for key in ("minute", "minute_text", "stats"):
            if key in snap and snap.get(key) not in (None, ""):
                out[key] = snap.get(key)
    if match_id:
...
def _notify_kind(kind: str | None) -> str:
    ...
    if kind_s in {"goal", "goal_wait", "filter", "odds_jump"}:
        return kind_s
    return "filter"
...
def _notify_delivery_key(match_id: str, kind: str, m: dict[str, Any] | None = None) -> str:
    ...
    if kind_s == "odds_jump":
        mv = (m or {}).get("odds_movement") if isinstance(m, dict) else None
        sig = str((mv or {}).get("signature") or "").strip() if isinstance(mv, dict) else ""
        safe_sig = re.sub(r"[^A-Za-z0-9_.:>+-]+", "_", sig)[:80]
        return f"odds_jump:{mid}:{safe_sig}" if safe_sig else f"odds_jump:{mid}"
    return f"filter:{mid}"
...
def _notify_key_match_id(key: str | None) -> str:
    ...
    if not key_s:
        return ""
    parts = key_s.split(":")
    if parts[0] in {"filter", "goal_wait", "goal", "odds_jump"} and len(parts) >= 2:
        return parts[1]
    return key_s
...
def handle_notify_matches(init_data: str) -> dict[str, Any]:
    ...
    refreshed_items, refreshed = _refresh_notify_records_for_read(items)
    if refreshed:
        with _notify_lock:
            _notify_matches[chat_id] = refreshed_items[:NOTIFY_MATCHES_MAX_PER_USER]
...
def _admin_send(chat_id: str | int, text: str, reply_markup: dict[str, Any] | None = None) -> dict[str, Any]:
    payload: dict[str, Any] = {
    ...
    return _tg_api("sendMessage", payload, timeout=15)
...
def _admin_stats_snapshot() -> dict[str, Any]:
    ...
    for uid, sub in subs.items():
        if now_i - _safe_int(sub.get("updated_at"), 0) <= 86400:
            active_24h += 1
        has_filter = bool((sub.get("filter") or {}).get("enabled"))
        has_goal_wait = bool((sub.get("goal_wait") or {}).get("enabled"))
        if has_filter:
            filter_on += 1
        if has_goal_wait:
            goal_wait_on += 1
        if has_filter or has_goal_wait:
            notify_any_on += 1
    ...
    online = _online_snapshot(include_users=True)
    online_users = online.get("users") if isinstance(online.get("users"), list) else []
    online_total = _safe_int(online.get("online"), 0)
    online_tg_ids = {
        str(u.get("id"))
        for u in online_users
        if isinstance(u, dict) and str(u.get("kind") or "") == "telegram" and str(u.get("id") or "")
    }
    online_known_ids = online_tg_ids & user_ids
    online_registered = len(online_known_ids)
    offline_known = max(0, len(user_ids - online_tg_ids))
    online_guests = max(0, online_total - len(online_tg_ids))
    return {
        "users_total": len(user_ids),
        "active_24h": active_24h,
        "allowed": allowed,
        "blocked": blocked,
        "notify_profiles": len(subs),
        "notify_any_on": notify_any_on,
        "filter_on": filter_on,
...
def _admin_main_text() -> str:
    return (
        ...
        "Твой ID уже добавлен: <code>851766591</code>.\n\n"
        "Выбери раздел кнопками ниже.\n"
        "Команды: /admin, /stats, /online, /notify, /users, /system"
    )
...
def _admin_stats_text() -> str:
    ...
    return (
        ...
        f"Онлайн Telegram-пользователей: <b>{snap['online_registered']}</b>\n"
        f"Оффлайн известных: <b>{snap['offline']}</b>\n"
        f"Гостей/анонимных онлайн: <b>{snap['online_guests']}</b>\n"
        f"Добавлены вручную: <b>{snap['allowed']}</b>\n"
        f"Заблокированы: <b>{snap['blocked']}</b>\n\n"
        f"<b>Уведомления</b>\n"
        f"Профилей уведомлений: <b>{snap['notify_profiles']}</b>\n"
        f"Поставили фильтр уведомлений: <b>{snap['filter_on']}</b>\n"
        f"Любые уведомления включены: <b>{snap['notify_any_on']}</b>\n"
...
def _admin_system_text() -> str:
    ...
    return (
        ...
        f"Admin webhook: <b>{'ON' if ADMIN_PANEL_ENABLED and ADMIN_WEBHOOK_ENABLED and _telegram_webhook_url() else 'OFF'}</b>\n"
        ...
        f"Online/offline: <b>{_safe_int(snap.get('online'), 0)} / {_safe_int(snap.get('offline'), 0)}</b>\n"
        f"Online limit: <b>{_safe_int(snap.get('online'), 0)}/{_safe_int(snap.get('online_limit'), ONLINE_USER_LIMIT_DEFAULT)}</b>"
    )
...
def _admin_online_text() -> str:
    ...
    known_ids = _all_known_user_ids()
    online_tg_ids = {
        str(u.get("id"))
        for u in (snap.get("users") or [])
        if isinstance(u, dict) and str(u.get("kind") or "") == "telegram" and str(u.get("id") or "")
    }
    registered_online = len(online_tg_ids & known_ids)
    offline_known = max(0, len(known_ids - online_tg_ids))
    online_guests = max(0, online - len(online_tg_ids))
    rows = []
    ...
    return (
        "<b>👤 Онлайн / оффлайн Mini App</b>\n\n"
        f"Сейчас онлайн: <b>{online}</b> из <b>{limit}</b>\n"
        f"Онлайн Telegram-пользователей: <b>{registered_online}</b>\n"
        f"Оффлайн известных: <b>{offline_known}</b>\n"
        f"Гостей/анонимных онлайн: <b>{online_guests}</b>\n"
...
def _handle_admin_message(message: dict[str, Any]) -> None:
    ...
    if cmd in {"/id", "/whoami"}:
        _admin_send(chat_id, f"Твой Telegram ID: <code>{html.escape(user_id)}</code>")
        return
    if _is_admin_id(user_id):
        ...
        if cmd in {"/admin", "/start"}:
            ...
            _admin_send(chat_id, _admin_online_text(), reply_markup=_admin_keyboard("online"))
        elif cmd == "/notify":
            _admin_send(chat_id, _admin_notify_text(), reply_markup=_admin_keyboard("notify"))
        elif cmd == "/users":
...
def _admin_set_webhook_if_possible() -> bool:
    """Prefer webhook for Telegram commands so Render/Railway can wake on /admin."""
    if not (BOT_TOKEN and ADMIN_PANEL_ENABLED and ADMIN_WEBHOOK_ENABLED and ADMIN_IDS):
        _admin_webhook_state.update({
            "last_set_at": int(time.time()),
            "last_set_ok": False,
            "last_set_description": "missing BOT_TOKEN/ADMIN_PANEL_ENABLED/ADMIN_WEBHOOK_ENABLED/ADMIN_IDS",
            "last_url": _telegram_webhook_url(),
        })
        return False
    url = _telegram_webhook_url()
    if not url:
        msg = "PUBLIC_BASE_URL/RENDER_EXTERNAL_URL/RAILWAY_PUBLIC_DOMAIN is empty; fallback to polling"
        _admin_webhook_state.update({"last_set_at": int(time.time()), "last_set_ok": False, "last_set_description": msg, "last_url": ""})
        print(f"[admin] webhook skipped: {msg}")
        return False
    payload: dict[str, Any] = {
        "url": url,
    ...
    desc = str(result.get("description") or result.get("error_code") or ("ok" if ok else result))[:500]
    _admin_webhook_state.update({
        "last_set_at": int(time.time()),
        "last_set_ok": ok,
        "last_set_description": desc,
        "last_url": url,
    })
    if ok:
        print(f"[admin] webhook enabled: {url}")
        return True
    print(f"[admin] setWebhook failed: {desc}; fallback to polling")
    return False


def _admin_telegram_status_payload(force_set: bool = False, test_send_to: str = "") -> dict[str, Any]:
    """Small browser diagnostics for Telegram admin panel. Does not expose BOT_TOKEN."""
    if force_set:
        try:
            _admin_set_webhook_if_possible()
        except Exception as exc:
            _admin_webhook_state.update({
                "last_set_at": int(time.time()),
                "last_set_ok": False,
                "last_set_description": f"exception: {type(exc).__name__}: {str(exc)[:240]}",
                "last_url": _telegram_webhook_url(),
            })
    get_me: dict[str, Any] = {}
    webhook_info: dict[str, Any] = {}
    if BOT_TOKEN:
        get_me = _tg_api("getMe", {}, timeout=10)
        webhook_info = _tg_api("getWebhookInfo", {}, timeout=10)
    test_result: dict[str, Any] | None = None
    if test_send_to:
        chat_id = str(test_send_to).strip()
        if _is_admin_id(chat_id):
            test_result = _admin_send(chat_id, "✅ Тест админ-панели: бот может отправлять сообщения. Теперь попробуй /admin")
        else:
            test_result = {"ok": False, "description": "test_send_to must be one of ADMIN_IDS"}
    return {
        "ok": True,
        "version": "v10.4-admin-diagnostics",
        "bot_token_configured": bool(BOT_TOKEN),
        "bot_username": BOT_USERNAME,
        "admin_panel_enabled": bool(ADMIN_PANEL_ENABLED),
        "admin_polling_enabled": bool(ADMIN_POLLING_ENABLED),
        "admin_webhook_enabled": bool(ADMIN_WEBHOOK_ENABLED),
        "admin_ids": sorted(ADMIN_IDS),
        "owner_851766591_is_admin": _is_admin_id("851766591"),
        "public_base_url": PUBLIC_BASE_URL,
        "computed_webhook_url": _telegram_webhook_url(),
        "webhook_path": TELEGRAM_WEBHOOK_PATH,
        "last_set_webhook": dict(_admin_webhook_state),
        "telegram_get_me": get_me,
        "telegram_get_webhook_info": webhook_info,
        "test_send": test_result,
        "hint": "Open /api/admin/telegram-status?set=1 to force setWebhook, or add &test=851766591 to test sendMessage.",
    }


def _handle_telegram_update(update: dict[str, Any]) -> None:
    """Handle one Telegram update from webhook or getUpdates."""
...
def start_admin_panel_worker() -> None:
    ...
    if not (BOT_TOKEN and ADMIN_PANEL_ENABLED and ADMIN_IDS):
        ...
        return
    # Webhook is better for Render/Railway: Telegram POST wakes the service.
    # Polling is kept as fallback for VPS/local deployments without public URL.
    if ADMIN_WEBHOOK_ENABLED and _admin_set_webhook_if_possible():
        _admin_polling_started = True
        # Re-set once more after HTTP server has started accepting requests.
        def _delayed_webhook_refresh() -> None:
            try:
                time.sleep(5)
                _admin_set_webhook_if_possible()
            except Exception as exc:
                print(f"[admin] delayed webhook refresh failed: {exc}")
        threading.Thread(target=_delayed_webhook_refresh, daemon=True, name="admin-webhook-refresh").start()
        return
    if not ADMIN_POLLING_ENABLED:
        print("[admin] polling disabled and webhook not configured")
        return
    _admin_polling_started = True
...
def sanitize_notify_filter(f: dict[str, Any] | None) -> dict[str, Any]:
    ...
    return {
        ...
        # v10.7: odds jump trigger. Fill both values to enable, e.g. old <= 2.00 and new >= 5.00.
        "odds_jump_enabled": bool(src.get("odds_jump_enabled")),
        "odds_jump_from_max": round(max(0.0, min(100.0, _safe_float(src.get("odds_jump_from_max"), 0.0))), 2),
        "odds_jump_to_min": round(max(0.0, min(100.0, _safe_float(src.get("odds_jump_to_min"), 0.0))), 2),
        "odds_jump_min_delta": round(max(0.0, min(100.0, _safe_float(src.get("odds_jump_min_delta"), 0.0))), 2),
        "odds_jump_market": str(src.get("odds_jump_market") or "team")[:16],
...
def _odds_jump_filter_enabled(f: dict[str, Any]) -> bool:
    if not isinstance(f, dict):
        return False
    from_max = _safe_float(f.get("odds_jump_from_max"), 0.0)
    to_min = _safe_float(f.get("odds_jump_to_min"), 0.0)
    return bool(f.get("odds_jump_enabled") or (from_max > 0 and to_min > 0))


def _odds_jump_scope_passes(m: dict[str, Any], f: dict[str, Any]) -> bool:
    """Scope for odds alerts: minute/country only, not heavy stat thresholds.

    The normal filter still works as before. Odds jump is a separate trigger so
    a pure market move (2.00 -> 5.00) is not blocked by shots/corners settings.
    """
    minute = _safe_int(m.get("minute"), 0)
    mn = _safe_int(f.get("minute_min"), 0)
    mx = _safe_int(f.get("minute_max"), 130)
...
def _odds_jump_movement_passes(mv: dict[str, Any], f: dict[str, Any]) -> bool:
    if not isinstance(mv, dict):
        return False
    old_v = _safe_odds(mv.get("old"))
    new_v = _safe_odds(mv.get("new"))
    if old_v is None or new_v is None or new_v <= old_v:
        return False
    from_max = _safe_float(f.get("odds_jump_from_max"), 0.0)
    to_min = _safe_float(f.get("odds_jump_to_min"), 0.0)
    min_delta = _safe_float(f.get("odds_jump_min_delta"), 0.0)
    if from_max > 0 and old_v > from_max:
        return False
    if to_min > 0 and new_v < to_min:
        return False
    if min_delta > 0 and (new_v - old_v) < min_delta:
        return False
    market_mode = str(f.get("odds_jump_market") or "team").strip().lower()
    market = str(mv.get("market") or "")
    key = str(mv.get("key") or "")
    if market_mode in {"team", "teams", "1x2"}:
        return market == "eu" and key in {"1", "2"}
    if market_mode in {"all", "any", "любой"}:
        return True
    if market_mode in {"total", "totals", "bs"}:
        return market == "bs"
    if market_mode in {"handicap", "asia"}:
        return market == "asia"
    return market == "eu" and key in {"1", "2"}


def _best_odds_jump_for_match(match_id: str, f: dict[str, Any], now: int | float | None = None) -> dict[str, Any] | None:
    movements = _recent_odds_movements(match_id, now_i=int(now or now_ts()))
    passed = [mv for mv in movements if _odds_jump_movement_passes(mv, f)]
    if not passed:
        return None
    passed.sort(key=lambda x: (float(x.get("delta") or 0), float(x.get("new") or 0)), reverse=True)
    return dict(passed[0])


def _build_odds_jump_notify_match(m: dict[str, Any], mv: dict[str, Any]) -> dict[str, Any]:
    out = dict(m)
    label = str(mv.get("label") or "КФ")
    old_s = _fmt_odds_value(mv.get("old"))
    new_s = _fmt_odds_value(mv.get("new"))
    out["alert_kind"] = "odds_jump"
    out["odds_movement"] = mv
    out["alert_title"] = f"КФ вырос: {m.get('home') or ''} {m.get('score_home') or 0}-{m.get('score_away') or 0} {m.get('away') or ''}"
    out["alert_subtitle"] = f"{m.get('minute_text') or ''} · {m.get('country') or ''} · {m.get('league') or ''}"
    out["alert_score"] = 0
    out["alert_reasons"] = [f"{label} {old_s} → {new_s}"]
    out["alert_reason"] = f"{label} {old_s} → {new_s}"
    return out


def _process_odds_jump_alerts_for_sub(chat_id_str: str, sub: dict[str, Any], matches: list[dict[str, Any]], now: int | float | None = None) -> bool:
    cfg = sanitize_notify_filter(sub.get("filter") or {})
    if not cfg.get("enabled") or not _odds_jump_filter_enabled(cfg):
        return False
    now_f = float(now or time.time())
    changed = False
    for m in matches or []:
        if not _odds_jump_scope_passes(m, cfg):
            continue
        mid = str(m.get("id") or "").strip()
        if not mid:
            continue
        if _notify_match_dismissed(sub, mid):
            continue
        mv = _best_odds_jump_for_match(mid, cfg, now=now_f)
        if not mv:
            continue
        odds_match = _build_odds_jump_notify_match(m, mv)
        delivery_key = _notify_delivery_key(mid, "odds_jump", odds_match)
        if _notify_already_delivered(sub, delivery_key, set()):
            continue
        _store_notify_match(chat_id_str, odds_match, ts=int(now_f))
        ok, queued = _send_or_queue_telegram(
            chat_id_str,
            sub,
            delivery_key,
            _notification_text_for_match(odds_match),
        ...
        try:
            ...
            with _notify_lock:
                ...
                for chat_id_str, sub in list(_notify_subs.items()):
                    ...
                    if _process_odds_jump_alerts_for_sub(chat_id_str, sub, matches, now=now):
                        changed = True
...
def _scan_notify_subscription_once(chat_id_str: str | int, include_goal_wait: bool = True) -> None:
    ...
    try:
        ...
        with _notify_lock:
            ...
            if _process_odds_jump_alerts_for_sub(chat_key, sub, matches, now=now):
                changed = True
...
class MiniAppHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        try:
            ...
            if path in {"/api/admin/telegram-status", "/api/admin/webhook-status", "/telegram-status"}:
                params = urllib.parse.parse_qs(url.query)
                force_set = str((params.get("set") or params.get("force") or ["0"])[0]).lower() in {"1", "true", "yes", "on"}
                test_send_to = str((params.get("test") or params.get("send") or [""])[0]).strip()
                return json_response(self, _admin_telegram_status_payload(force_set=force_set, test_send_to=test_send_to))

            if path in {TELEGRAM_WEBHOOK_PATH, "/telegram-webhook", "/webhook", "/bot-webhook"}:
                return json_response(self, {"ok": True, "message": "Telegram webhook endpoint is alive. Telegram must POST JSON updates here.", "path": path})
    ...
    def do_POST(self) -> None:
        try:
            ...
            if path in {TELEGRAM_WEBHOOK_PATH, "/telegram-webhook", "/webhook", "/bot-webhook"}:
                if TELEGRAM_WEBHOOK_SECRET_TOKEN:
                    got_secret = self.headers.get("X-Telegram-Bot-Api-Secret-Token") or ""
                    if got_secret != TELEGRAM_WEBHOOK_SECRET_TOKEN:
                        return json_response(self, {"ok": False, "error": "bad webhook secret"}, status=403)
                _handle_telegram_update(body if isinstance(body, dict) else {})
...
def main() -> None:
    ...
    print("=" * 72)
    print("Telegram Live Matches Mini App — v10.4 admin diagnostics + Railway webhook fix")
    ...
    print(f"Admins:   {sorted(ADMIN_IDS) if ADMIN_IDS else '[]'} (panel={'on' if ADMIN_PANEL_ENABLED else 'off'}, polling={'on' if ADMIN_POLLING_ENABLED else 'off'}, webhook={'on' if ADMIN_WEBHOOK_ENABLED else 'off'})")
    print(f"Webhook:  {_telegram_webhook_url() or 'not configured'}")
