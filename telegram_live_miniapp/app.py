...
def collector_detail_for_match(match_id_value: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        pressure_chart = pressure_chart_from_history(mid) if mid else {}
...
def pressure_chart_from_history(match_id_value: str) -> dict[str, Any]:
    ...
    now = now_ts()
    # v9.21: serve from cache when fresh enough. Cache key is the match id —
    # bars and timestamps don't depend on the requester.
    if PRESSURE_CHART_CACHE_SECONDS > 0:
        with _cache_lock:
            cached = _pressure_chart_cache.get(mid)
    ...
    payload = _pressure_chart_from_history_impl(mid, now)

    if PRESSURE_CHART_CACHE_SECONDS > 0:
        with _cache_lock:
            _pressure_chart_cache[mid] = {"saved_at": now, "payload": payload}
...
def _pressure_chart_from_history_impl(mid: str, now: int) -> dict[str, Any]:
    _init_live_cache_db()
    ...
    for i in range(1, len(parsed)):
        ...
        # Keep only what the chart needs — t (epoch sec) plus the two scores.
        # Float is fine because the indicator formula uses floats too and the
        # SVG just needs ratios for bar heights.
        bar = {"t": ts_cur, "h": float(home["score"] or 0.0), "a": float(away["score"] or 0.0)}
...
def match_stats_cached(match_id_value: str) -> dict[str, dict[str, int]]:
    ...
    if stats:
        try:
            _update_match_stats(mid, flat_stats(stats))
        except Exception:
            pass
...
def detail_payload(match_id_value: str) -> dict[str, Any]:
    ...
    if not match:
        ...
        if fresh_info:
            stats = {}
            try:
                stats = match_stats_cached(str(match_id_value))
            except Exception:
                stats = {}
            return {
                "ok": True,
                ...
                "pressure_chart": pressure_chart_from_history(str(match_id_value)),
                "finished": True,
            }
