...
def _init_live_cache_db() -> None:
    ...
    try:
        ...
        if pressure_version != "v1040-original":
            conn.execute("DELETE FROM match_stats_history")
            conn.execute(
                "INSERT INTO app_meta(meta_key, meta_value) VALUES (?, ?) "
                "ON CONFLICT(meta_key) DO UPDATE SET meta_value=excluded.meta_value",
                ("pressure_graph_version", "v1040-original"),
            )
...
def pressure_from_stats_history(match_id_value: str) -> dict[str, Any]:
    ...
    try:
        ...
        latest_row = rows[-1]
        latest_ts, latest, _latest_minute = _row_payload(latest_row)
        oldest_row = conn.execute(
            ...
            LIMIT 1
            """,
            (mid, cutoff, int(latest_ts)),
        ).fetchone()
        if not oldest_row:
            oldest_row = rows[0]

        oldest_ts, oldest, _oldest_minute = _row_payload(oldest_row)
        if int(oldest_ts) >= int(latest_ts):
            return {
                "available": False,
                "reason": "collecting",
        ...
        return {
            ...
            "sample_count": conn.execute(
                ...
                (mid, int(oldest_ts), int(latest_ts)),
            ).fetchone()[0],
            "leader": leader,
            "home": home,
            "away": away,
...
def _pressure_chart_with_inferred_timing(
    stats: dict[str, Any],
    captured_at: int,
    latest_capture: int,
    match_meta: dict[str, Any] | None,
) -> dict[str, Any]:
    """Add approximate chart timing to legacy rows that lack football minute.

    Historical rows from older versions contain cumulative stats but no
    ``__chart_minute``. Using their array index packed them at kickoff. We infer
    the minute from the current live clock and the snapshot age, while leaving
    modern rows untouched.
    """
    payload = dict(stats or {})
    existing_phase = _pressure_chart_phase(payload)
    if _pressure_chart_slot_from_minute(payload, existing_phase) is not None:
        return payload

    current_minute = _safe_int((match_meta or {}).get("minute"), 0)
    if current_minute <= 0 or latest_capture < int(captured_at):
        return payload

    elapsed_minutes = max(0, int(round((latest_capture - int(captured_at)) / 60.0)))
    inferred_minute = max(1, current_minute - elapsed_minutes)
    current_phase = _pressure_chart_phase(match_meta or {})
    if current_phase == "2T" or current_minute > 45:
        inferred_phase = "2T" if inferred_minute > 45 else "1T"
    else:
        inferred_phase = "1T"
        inferred_minute = min(45, inferred_minute)

    payload["__chart_minute"] = inferred_minute
    payload["__chart_period"] = inferred_phase
    payload["__chart_minute_text"] = f"{inferred_minute}'"
    return payload
...
def _pressure_chart_from_history_impl(mid: str, now: int, match_meta: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    latest_capture = int(rows[-1]["captured_at"])
    for r in rows:
        ...
        try:
            ...
            data = {}
        # Older rows did not store football-minute metadata. Infer it from the
        # current live clock so their bars do not pile up at kickoff.
        data = _pressure_chart_with_inferred_timing(data, captured_at, latest_capture, match_meta)
        parsed.append((captured_at, data))
...
def main() -> None:
    ...
    print("=" * 72)
    print("Telegram Football Live Mini App — v10.42 Instant Main Page")
