...
# v9.76: чистые helper-модули вынесены из этого файла. Дальнейшее разбиение —
# см. helpers/__init__.py. Реэкспорт сохраняет совместимость со старыми именами.
...
from helpers.countries import (
    is_generic_region as _is_generic_region,
    looks_like_league_name as _looks_like_league_name,
    country_name_from_text as _country_name_from_text,
)
from helpers.stats import (
    STAT_TYPE_NAMES,
    flat_stats,
    stat_pairs_from_response,
    pressure_power_from_stats as _pressure_power_from_stats,
    stat_total_from_match_stats as _stat_total_from_match_stats,
    stat_side_from_match_stats as _stat_side_from_match_stats,
)
...
ADMIN_IDS = _parse_int_set(os.environ.get("ADMIN_IDS", "851766591"))
...
# v9.76: STAT_TYPE_NAMES, RUS_COUNTRY_TO_CODE, COUNTRY_CODE_MAP, CONTINENT_NAMES,
# CONTINENT_FLAG_EMOJI, COUNTRY_NAME_ALIASES, LEAGUE_POWER_RULES, COUNTRY_POWER_ORDER
# и связанные функции вынесены в helpers/countries.py и helpers/stats.py.
# Импорт см. в шапке файла.
...
_telegram_send_stats: dict[str, Any] = {"ok": 0, "fail": 0, "last_ok": 0, "last_fail": 0, "last_error": ""}
...
# v9.76: функции _is_generic_region / _country_name_from_text / _looks_like_league_name
# вынесены в helpers.countries; импорт см. в шапке файла.
...
# v9.76: country_code() вынесен в helpers/countries.py
...
def _cleanup_finished_matches() -> int:
    ...
    try:
        ...
        if stale_ids:
            conn.executemany("DELETE FROM live_matches WHERE match_id = ?", [(mid,) for mid in stale_ids])
        conn.commit()
        return len(ids) + len(stale_ids)
    finally:
        conn.close()
...
# v9.76: stat_pairs_from_response() и flat_stats() вынесены в helpers/stats.py.
...
# v9.76: debug-print raw odds responses засирал stdout при 300 матчах × 30 сек.
# По умолчанию выключено. Включай DEBUG_ODDS=1 только при разборе бага.
DEBUG_ODDS = os.environ.get("DEBUG_ODDS", "0").strip().lower() in {"1", "true", "yes", "on"}
...
def _save_notify_table(table: str, values: dict[str, Any]) -> None:
    """v9.76: УБРАН антипаттерн `DELETE FROM ... + INSERT всех подряд`.

    Старый код на каждое микро-изменение (один пользователь поставил галочку)
    удалял ВСЕ строки таблицы и заново вставлял всех 100/500/1000 подписчиков.
    Это в проде убивало Postgres free tier за полчаса.

    Теперь только UPSERT по тем chat_id, что переданы. Если нужно удалить
    подписку — есть `_delete_notify_row`. Точечный путь — `_upsert_notify_row`.
    """
    table = _notify_table_name(table)
    if NOTIFY_STORAGE not in {"sqlite", "postgres"}:
        return
...
def _upsert_notify_row(table: str, chat_id: str | int, payload: Any) -> None:
    """v9.76: точечный UPSERT одной строки. Используй для горячих путей."""
    chat_key = str(chat_id or "").strip()
    if not chat_key:
        return
    _save_notify_table(table, {chat_key: payload})


def _delete_notify_row(table: str, chat_id: str | int) -> None:
    """v9.76: удалить одну строку по chat_id."""
    table = _notify_table_name(table)
    chat_key = str(chat_id or "").strip()
    ...
    try:
        ...
        print(f"[notify] {NOTIFY_STORAGE} delete failed for {table} {chat_key}: {exc}")
...
def _admin_delete_user(user_id: str | int, admin_id: str | int = "") -> bool:
    ...
    with _notify_lock:
        ...
        # v9.76: точечный DELETE одной строки, а не перезапись всей таблицы.
...
def _save_notify_matches() -> None:
    """v9.76: использует dirty-set вместо перезаписи всего словаря."""
    try:
        if NOTIFY_STORAGE in {"sqlite", "postgres"}:
            dirty = _drain_notify_dirty("matches")
            if dirty is None:
                _save_notify_table("notify_matches", _notify_matches)
                return
            if not dirty:
                return
            payload = {cid: _notify_matches.get(cid) for cid in dirty if cid in _notify_matches}
            removed = [cid for cid in dirty if cid not in _notify_matches]
            if payload:
                _save_notify_table("notify_matches", payload)
            for cid in removed:
                _delete_notify_row("notify_matches", cid)
            return
...
def _flush_pending_telegram_for_sub(chat_id: str | int, sub: dict[str, Any], now: int | float | None = None) -> bool:
    ...
    for raw in list(queue):
        ...
        raw["tries"] = tries + 1
        raw["last_try"] = now_i
        if send_telegram_message(chat_id, text_msg, link=link):
...
def handle_notify_matches_clear(body: dict[str, Any]) -> dict[str, Any]:
    ...
    # v9.76: убран небезопасный fallback "если в системе один подписчик —
    # авторизуй неподписанный запрос на него". Это давало возможность
    # анонимно стирать уведомления единственного пользователя без init_data.
    if not chat_id:
...
def handle_notify_match_remove(body: dict[str, Any]) -> dict[str, Any]:
    ...
    chat_id = _chat_id_for_notify_cleanup(body)
    # v9.76: убран небезопасный fallback "len(_notify_subs) == 1".
    if not chat_id:
        return {"ok": False, "error": "init_data verification failed"}
...
def _save_notify_subs() -> None:
    """v9.76: НЕ пиши через эту функцию в горячих путях.

    Старый код вызывал её на каждое микро-изменение, и она перезаписывала
    ВЕСЬ словарь _notify_subs. Под Postgres это убивало free tier.
    Теперь:
      - в горячих путях используй `_save_one_notify_sub(chat_id)`;
      - эта функция сохраняет только отмеченные dirty подписки,
        иначе всю карту целиком (для редких ручных вызовов и совместимости).
    """
    try:
        if NOTIFY_STORAGE in {"sqlite", "postgres"}:
            dirty = _drain_notify_dirty("subs")
            if dirty is None:
                # full save (только при первичной миграции / редких ручных вызовах)
                _save_notify_table("notify_subs", _notify_subs)
                return
            if not dirty:
                return
            payload = {cid: _notify_subs.get(cid) for cid in dirty if cid in _notify_subs}
            removed = [cid for cid in dirty if cid not in _notify_subs]
            if payload:
                _save_notify_table("notify_subs", payload)
            for cid in removed:
                _delete_notify_row("notify_subs", cid)
            return
...
# v9.76: dirty-set механика — отмечаем chat_id, который реально изменился.
# Если _notify_subs_dirty is None, значит "сохранить всё" (форс).
_notify_subs_dirty: set[str] | None = set()
_notify_matches_dirty: set[str] | None = set()


def _mark_notify_dirty(kind: str, chat_id: str | int) -> None:
    """Пометить одну подписку/одного юзера матчей как изменённую."""
    chat_key = str(chat_id or "").strip()
    if not chat_key:
        return
    global _notify_subs_dirty, _notify_matches_dirty
    if kind == "subs":
        if _notify_subs_dirty is not None:
            _notify_subs_dirty.add(chat_key)
    elif kind == "matches":
        if _notify_matches_dirty is not None:
            _notify_matches_dirty.add(chat_key)


def _force_full_save(kind: str) -> None:
    """Сбросить dirty-set в "сохранить всё" (None). Используется крайне редко."""
    global _notify_subs_dirty, _notify_matches_dirty
    if kind == "subs":
        _notify_subs_dirty = None
    elif kind == "matches":
        _notify_matches_dirty = None


def _drain_notify_dirty(kind: str) -> set[str] | None:
    """Вернуть и очистить dirty-set. None означает "сохранить весь словарь"."""
    global _notify_subs_dirty, _notify_matches_dirty
    if kind == "subs":
        out = _notify_subs_dirty
        _notify_subs_dirty = set()
        return out
    if kind == "matches":
        out = _notify_matches_dirty
        _notify_matches_dirty = set()
        return out
    return set()


def _save_one_notify_sub(chat_id: str | int) -> None:
    """v9.76: точечно сохранить одну подписку. Заменяет _save_notify_subs() в горячих путях."""
    chat_key = str(chat_id or "").strip()
    if not chat_key:
        return
    ...
    try:
        if NOTIFY_STORAGE in {"sqlite", "postgres"}:
            if sub is None:
                ...
                _upsert_notify_row("notify_subs", chat_key, sub)
            # выкидываем из dirty, чтобы не дублировать запись
            if _notify_subs_dirty is not None:
                _notify_subs_dirty.discard(chat_key)
            return
        # JSON-фолбэк: пишем весь файл (для совместимости)
        NOTIFY_DB.parent.mkdir(parents=True, exist_ok=True)
        NOTIFY_DB.write_text(json.dumps(_notify_subs, ensure_ascii=False), encoding="utf-8")
    except Exception as exc:
...
def _save_one_notify_matches(chat_id: str | int) -> None:
    """v9.76: точечно сохранить найденные матчи одного пользователя."""
    chat_key = str(chat_id or "").strip()
    if not chat_key:
        return
    ...
    try:
        if NOTIFY_STORAGE in {"sqlite", "postgres"}:
            if items is None:
                ...
                _upsert_notify_row("notify_matches", chat_key, items[:NOTIFY_MATCHES_MAX_PER_USER])
            if _notify_matches_dirty is not None:
                _notify_matches_dirty.discard(chat_key)
            return
        NOTIFY_MATCHES_DB.parent.mkdir(parents=True, exist_ok=True)
        NOTIFY_MATCHES_DB.write_text(json.dumps(_notify_matches, ensure_ascii=False), encoding="utf-8")
    except Exception as exc:
    ...
    for attempt in range(max(1, NOTIFY_SEND_RETRIES)):
        try:
            ...
            with urllib.request.urlopen(req, timeout=12) as resp:
                resp.read()
            _telegram_send_stats["ok"] = _safe_int(_telegram_send_stats.get("ok"), 0) + 1
            ...
            if exc.code in {400, 403}:
                ...
                _telegram_send_stats["fail"] = _safe_int(_telegram_send_stats.get("fail"), 0) + 1
...
def _admin_notify_text() -> str:
    ...
    return (
        ...
        f"Очередь Telegram: <b>{snap['pending']}</b>\n"
...
def _admin_system_text() -> str:
    ...
    return (
        ...
        f"ADMIN_IDS: <code>{html.escape(','.join(str(x) for x in sorted(ADMIN_IDS)) or 'нет')}</code>\n"
...
# v9.76: _stat_total_from_match_stats, _stat_side_from_match_stats и
# _pressure_power_from_stats вынесены в helpers/stats.py.
...
def _collect_pending_retry_tasks(chat_id_str: str | int, sub: dict[str, Any], now: float) -> list[dict[str, Any]]:
    """v9.76: собрать задачи на retry из pending_telegram без сетевых вызовов.

    Старый `_flush_pending_telegram_for_sub` слал Telegram **под локом**, что
    превращало notify_worker_loop в долгий блокирующий монолит. Теперь worker
    под локом только собирает таски, отметив `last_try`/`tries`, а отправка
    делается вне лока в фазе 2 цикла.
    """
    chat_key = str(chat_id_str or "")
    if not BOT_TOKEN or chat_key == "local":
        return []
    queue = sub.get("pending_telegram")
    if not isinstance(queue, list) or not queue:
        return []
    ...
    if mutated:
        sub["pending_telegram"] = keep[:NOTIFY_PENDING_MAX_PER_USER]
        _mark_notify_dirty("subs", chat_key)
    return out
...
def notify_worker_loop() -> None:
    ...
    v9.76: переписан на 2 фазы. Раньше Telegram-отправка шла прямо внутри
    `with _notify_lock`, и поток мог держать лок до 12 сек × N подписок —
    при 100 подписчиках это полностью клинило сервер до 20 минут.

    Теперь:
      1. Под локом — собираем outbox + retry, помечаем delivered/store_match.
      2. Без лока — шлём в Telegram с лёгкой паузой 50мс (≤ 20 msg/s).
      3. Под локом — для неудачных откатываем delivered и кладём в pending.
    """
    while True:
        try:
            ...
            # === ФАЗА 1: под локом, без сетевых вызовов ===
            with _notify_lock:
                match_by_id = {str(m.get("id") or ""): m for m in matches if str(m.get("id") or "")}

                # 1a. собрать retry-таски (раньше это делал _flush_pending_telegram_for_sub под локом)
                for chat_id_str, sub in _notify_subs.items():
                    outbox.extend(_collect_pending_retry_tasks(chat_id_str, sub, now))

                # 1b. найти новые матчи и положить в outbox
                for chat_id_str, sub in list(_notify_subs.items()):
                    ...
                    if cfg.get("enabled"):
                        for m in matches:
                            ...
                            _mark_notify_dirty("matches", chat_id_str)
                            # пометить delivered СРАЗУ — это гарантирует дедупликацию даже если
                            # в фазе 2 произойдёт сбой/рестарт; если send не пройдёт, фаза 3
                            # откатит delivered и положит в pending_telegram.
                            _mark_notify_delivered(sub, delivery_key, now, legacy_keys)
                            _mark_notify_dirty("subs", chat_id_str)
                            outbox.append({
                                "chat_id": chat_id_str,
                                "key": delivery_key,
                                "text": _notification_text_for_match(notify_match),
                    ...
                    if bool(goal_wait.get("enabled")):
                        ...
                        for m in matches:
                            ...
                            outbox.append({
                                "chat_id": chat_id_str,
                                "key": delivery_key,
                                "text": _notification_text_for_match(goal_wait_match),
                                "link": notification_match_link(mid),
                                "legacy_keys": legacy_keys,
                                "is_retry": False,
                            })
                    ...
                    for mid, alert in list(goal_alerts.items()):
                        ...
                        if current_total > last_total:
                            ...
                            if not _notify_already_delivered(sub, delivery_key, set()):
                                ...
                                _mark_notify_dirty("matches", chat_id_str)
                                _mark_notify_delivered(sub, delivery_key, now, set())
                                _mark_notify_dirty("subs", chat_id_str)
                                outbox.append({
                                    "chat_id": chat_id_str,
                                    "key": delivery_key,
                                    "text": _goal_notification_text_for_match(m),
                                    "link": notification_match_link(mid),
                                    "legacy_keys": set(),
                                    "is_retry": False,
                                })
                ...
                # cleanup устаревших ключей dedupe (всё ещё под локом, без I/O)
                for chat_id_str, sub in _notify_subs.items():
                    if _cleanup_notify_dedupe(sub, now):
                        _mark_notify_dirty("subs", chat_id_str)

                # сохранить изменения dirty-подписок (под локом — это БД-запись, но
                # без сетевых Telegram-вызовов, поэтому быстрая)
                _save_notify_subs()
                _save_notify_matches()

            # === ФАЗА 2: без лока — шлём в Telegram ===
            failed: list[dict[str, Any]] = []
            sent_ok = 0
            send_rate_delay = max(0.0, float(os.environ.get("NOTIFY_SEND_RATE_DELAY", "0.05")))
            for task in outbox:
                chat_key = str(task.get("chat_id") or "")
                if not BOT_TOKEN or chat_key == "local" or not chat_key:
                    continue
                ok = send_telegram_message(chat_key, task["text"], link=task.get("link") or "")
                if ok:
                    sent_ok += 1
                else:
                    failed.append(task)
                if send_rate_delay > 0:
                    time.sleep(send_rate_delay)

            # === ФАЗА 3: под локом — обработать неудачные ===
            if failed:
                with _notify_lock:
                    now_after = time.time()
                    for task in failed:
                        ...
                        if not isinstance(sub, dict):
                            continue
                        key = task.get("key") or ""
                        # для новых уведомлений: откатить delivered, чтобы попало в pending_telegram
                        if not task.get("is_retry"):
                            delivered = sub.get("delivered")
                            if isinstance(delivered, dict):
                            ...
                            _queue_telegram_message(
                                ...
                                now=now_after,
                                legacy_keys=task.get("legacy_keys") or set(),
                            )
                            _mark_notify_dirty("subs", chat_key)
                        # retry-таски остаются в pending_telegram, мы уже инкрементнули tries в фазе 1
                    _save_notify_subs()
    ...
    def do_GET(self) -> None:
        try:
            ...
            if path == "/api/admin/status":
                ...
                return json_response(self, {"ok": True, "stats": _admin_stats_snapshot(), "collector": _collector_state_copy(), "notify": dict(_notify_worker_state), "online": _online_snapshot(include_users=True)})
