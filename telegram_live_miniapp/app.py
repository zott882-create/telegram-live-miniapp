...
# ============================================================
def calculate_value_bet_payload(body: dict[str, Any] | None) -> dict[str, Any]:
    """Calculate Expected Value, fair odds and fractional Kelly sizing.

    Input accepts decimal odds (1.80, 2.10), user's estimated probability in %,
    and optional bankroll. This endpoint only calculates numbers; it never places
    bets and does not provide a guaranteed prediction.
    """
    body = body or {}
    odds = _safe_float(body.get("odds"), 0.0)
    probability_percent = _safe_float(body.get("probability_percent"), 0.0)
    bankroll_raw = body.get("bankroll")
    bankroll = _safe_float(bankroll_raw, 0.0) if bankroll_raw not in {None, ""} else 0.0

    if odds <= 1.0 or odds > 1000:
        return {"ok": False, "error": "Коэффициент должен быть больше 1.00"}
    if probability_percent <= 0.0 or probability_percent >= 100.0:
        return {"ok": False, "error": "Вероятность должна быть от 0 до 100%"}
    if bankroll < 0:
        return {"ok": False, "error": "Банк не может быть отрицательным"}

    p = probability_percent / 100.0
    implied_probability = 1.0 / odds
    fair_odds = 1.0 / p
    ev = (p * odds) - 1.0
    kelly = max(0.0, ev / (odds - 1.0)) if ev > 0 else 0.0

    quarter_kelly = kelly * 0.25
    half_kelly = kelly * 0.5
    full_kelly = kelly

    def money(frac: float) -> float:
        return round(bankroll * frac, 2) if bankroll > 0 else 0.0

    if ev <= 0:
        verdict = "no_value"
        verdict_text = "Нет value"
    elif ev < 0.03:
        verdict = "weak_value"
        verdict_text = "Слабое value"
    elif ev < 0.08:
        verdict = "good_value"
        verdict_text = "Хорошее value"
    else:
        verdict = "strong_value"
        verdict_text = "Сильное value — проверь риск"

    return {
        "ok": True,
        "odds": round(odds, 3),
        "probability_percent": round(probability_percent, 2),
        "bookmaker_probability_percent": round(implied_probability * 100.0, 2),
        "fair_odds": round(fair_odds, 3),
        "ev_percent": round(ev * 100.0, 2),
        "kelly_percent": round(full_kelly * 100.0, 2),
        "half_kelly_percent": round(half_kelly * 100.0, 2),
        "quarter_kelly_percent": round(quarter_kelly * 100.0, 2),
        "half_kelly_stake": money(half_kelly),
        "quarter_kelly_stake": money(quarter_kelly),
        "verdict": verdict,
        "verdict_text": verdict_text,
        "disclaimer": "Это математический расчёт, а не гарантия выигрыша. Проверяй коэффициент перед ставкой и соблюдай лимит банка.",
    }
...
class MiniAppHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        try:
            ...
            if path == "/api/value-bet":
                if not _rate_limit_ok(self, "value-bet"):
                    return json_response(self, {"ok": False, "error": "rate limit"}, status=429)
                payload = calculate_value_bet_payload(body)
