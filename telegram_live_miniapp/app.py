...
except Exception as _sportscore_import_error:
...
_prematch_fetch_lock = threading.Lock()
...
def build_payload(matches: list[dict[str, Any]], source: str, error: str | None = None) -> dict[str, Any]:
    ...
    # Prematch rows already carry SportScore odds. Sending hundreds of ``ss:``
    # IDs to the IGScore live-odds presence endpoint made /api/prematch take
    # minutes and could never enrich those foreign IDs correctly.
...
def fetch_live_sportscore(force: bool = False) -> list[dict[str, Any]]:
    ...
    if not _sportscore_ready():
        raise RuntimeError(f"SportScore provider unavailable: {_sportscore_import_error}")
...
def _strict_prematch_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Keep only genuinely upcoming fixtures.

    This is a second line of defence after the provider parser. It also cleans
    stale cache entries as time passes, so a match disappears from PREMATCH as
    soon as it starts instead of remaining there until the next full refresh.
    """
    ...
    for raw in rows:
        ...
        period = str(item.get("period") or "").strip().upper()
        if item.get("is_live") or item.get("finished"):
            continue
        if status in {"live", "finished", "ended", "completed", "cancelled", "postponed", "abandoned"}:
            continue
        if period in {"LIVE", "FT", "HT", "AET", "PEN"}:
            continue
...
def _build_prematch_base(force: bool = False) -> dict[str, Any]:
    try:
        matches = _strict_prematch_rows(sportscore_provider.list_upcoming(force=force))
        for item in matches:
            item["country_code"] = item.get("country_code") or country_code(str(item.get("country") or ""))
            item["source"] = "sportscore_prematch"
        base = build_payload(matches, "sportscore_prematch")
        base["provider"] = "sportscore"
        base["attribution"] = {"text": "Powered by SportScore", "url": "https://sportscore.com/"}
        try:
            base["source_stats"] = sportscore_provider.upcoming_diagnostics()
        except Exception:
            base["source_stats"] = {}
        return base
    except Exception as exc:
        base = build_payload([], "sportscore_prematch", f"{type(exc).__name__}: {exc}")
        base["ok"] = False
        base["provider"] = "sportscore"
        base["attribution"] = {"text": "Powered by SportScore", "url": "https://sportscore.com/"}
        return base
...
def _sanitize_prematch_base(base: dict[str, Any]) -> dict[str, Any]:
    rows = _strict_prematch_rows(flatten_payload(base))
    clean = build_payload(rows, "sportscore_prematch", str(base.get("error") or ""))
    clean["provider"] = "sportscore"
    clean["attribution"] = base.get("attribution") or {"text": "Powered by SportScore", "url": "https://sportscore.com/"}
    clean["source_stats"] = base.get("source_stats") or {}
    if base.get("ok") is False and not rows:
        clean["ok"] = False
    return clean


def load_prematch_payload(force: bool = False, date_filter: str = "") -> dict[str, Any]:
    """Upcoming football fixtures from SportScore, grouped like /api/live."""
    if not PREMATCH_ENABLED:
        return build_payload([], "sportscore_prematch", "prematch disabled")
    if not _sportscore_ready():
        return build_payload([], "sportscore_prematch", f"provider unavailable: {_sportscore_import_error}")

    now_f = time.time()
    with _cache_lock:
        cached = _prematch_cache.get("payload")
        fresh = bool(cached and now_f - float(_prematch_cache.get("saved_at") or 0) < PREMATCH_CACHE_SECONDS)
        base = cached if (not force and fresh) else None

    if base is None:
        # Single-flight: a burst of users must not start several full calendar
        # scans at once.
        with _prematch_fetch_lock:
            now_f = time.time()
            with _cache_lock:
                cached = _prematch_cache.get("payload")
                fresh = bool(cached and now_f - float(_prematch_cache.get("saved_at") or 0) < PREMATCH_CACHE_SECONDS)
                base = cached if (not force and fresh) else None
            if base is None:
                base = _build_prematch_base(force=force)
                with _cache_lock:
                    _prematch_cache.update({"saved_at": time.time(), "payload": base})

    base = _sanitize_prematch_base(base)
    if not date_filter or date_filter == "all":
        return base

    wanted = str(date_filter).strip()
    filtered = []
    for item in flatten_payload(base):
    ...
    payload = build_payload(filtered, "sportscore_prematch")
    payload["provider"] = "sportscore"
    payload["attribution"] = base.get("attribution") or {"text": "Powered by SportScore", "url": "https://sportscore.com/"}
    payload["source_stats"] = base.get("source_stats") or {}
    return payload
...
def main() -> None:
    ...
    print("=" * 72)
    print("Telegram Football Mini App — v10.4 fast SportScore prematch")
