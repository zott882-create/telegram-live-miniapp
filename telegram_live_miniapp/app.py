...
def load_live_payload(force: bool = False, sport: str = "football") -> dict[str, Any]:
    """Load LIVE from SportScore for football, basketball or tennis."""
    sport = str(sport or "football").strip().lower()
    if sport not in {"football", "basketball", "tennis"}:
        sport = "football"
    ...
    now = time.time()
    with _cache_lock:
        if not force and cache.get("payload") and now - float(cache.get("saved_at") or 0) < LIVE_CACHE_SECONDS:
    ...
    with _live_fetch_lock:
        ...
        with _cache_lock:
            cache.update({"saved_at": now, "payload": payload, "raw": {}})
        return payload
...
def main() -> None:
    ...
    print("=" * 72)
    print("Telegram Football Live Mini App — v10.32 Welcome Onboarding")
    print(f"Local:    http://127.0.0.1:{PORT}")
    print(f"Host:     {HOST}:{PORT}")
    print(f"Mode:     {DATA_MODE}")
    print(f"Notify:   {'enabled (' + str(len(_notify_subs)) + ' subs)' if BOT_TOKEN else 'disabled (no BOT_TOKEN)'}")
    print(f"Storage:  notify={NOTIFY_STORAGE} live_cache=memory+last-good redis={'on' if REDIS_URL else 'off'}")
