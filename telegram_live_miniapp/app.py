                ...
                for chat_id_str, sub in list(_notify_subs.items()):
                    ...
                    if cfg.get("enabled"):
                        for m in matches:
                            ...
                            if ok:
                                if _ensure_goal_alert_for_match(sub, notify_match, now=int(now)):
                                    changed = True
                                changed = True
                            elif queued:
                                # Keep auto-goal watch even while Telegram delivery is pending.
                                if _ensure_goal_alert_for_match(sub, notify_match, now=int(now)):
                                    changed = True
                                changed = True
        ...
        with _notify_lock:
            ...
            if cfg.get("enabled"):
                for m in matches:
                    ...
                    if ok or queued:
                        _ensure_goal_alert_for_match(sub, notify_match, now=int(now))
                        changed = True
...
def handle_notify(body: dict[str, Any]) -> dict[str, Any]:
    ...
    Filter/Ждём гол notifications automatically subscribe the match for the next goal.
    ...
    with _notify_lock:
        ...
        elif bool(body.get("auto_goal")) or kind in {"filter", "goal_wait"}:
