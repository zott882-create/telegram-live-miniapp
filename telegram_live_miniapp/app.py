...
def handle_notify(body: dict[str, Any]) -> dict[str, Any]:
    ...
    if not m:
        # Frontend sends the live snapshot too. Store it if /api/live rotated between
        # detecting the alert and this POST; notification history should not depend
        # on the 24h live-cache/detail retention.
        snap = body.get("match") if isinstance(body.get("match"), dict) else {}
        if snap and match_id:
            m = dict(snap)
