...
def _pressure_chart_from_history_impl(mid: str, now: int, match_meta: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for i in range(1, len(parsed)):
        ...
        if minute_slot is not None:
            slot = minute_slot
            if phase == "1T":
                half1_slots = max(half1_slots, min(45, slot + 1))
            elif phase == "2T":
                half2_slots = max(half2_slots, min(45, slot - 44))
        elif phase == "2T":
            ...
            if half1_slots < 45 and half2_slots == 0:
                slot = half1_slots
                half1_slots += 1
            else:
                slot = 45 + half2_slots
        ...
        bar = {"t": ts_cur, "slot": int(slot), "phase": phase or ("2T" if slot >= 45 else "1T"), "h": float(home["score"] or 0.0), "a": float(away["score"] or 0.0)}
