...
PREMATCH_CACHE_SECONDS = max(15, int(os.environ.get("PREMATCH_CACHE_SECONDS", "60")))
...
def load_prematch_payload(force: bool = False, date_filter: str = "") -> dict[str, Any]:
    ...
    with _cache_lock:
        ...
        if not force and cached and now_f - float(_prematch_cache.get("saved_at") or 0) < PREMATCH_CACHE_SECONDS:
            base = cached
        else:
            base = None
    if base is None:
        try:
            matches = sportscore_provider.list_upcoming(force=force)
            for item in matches:
                item["country_code"] = item.get("country_code") or country_code(str(item.get("country") or ""))
            ...
            base["ok"] = False
        with _cache_lock:
            _prematch_cache.update({"saved_at": now_f, "payload": base})
    ...
    try:
        payload["source_stats"] = sportscore_provider.upcoming_diagnostics()
    except Exception:
...
def main() -> None:
    ...
    print("=" * 72)
    print("Telegram Football Mini App — v10 SportScore prematch")
