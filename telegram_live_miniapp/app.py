...
def _sportscore_team_average(team_slug: str, team_name: str = "") -> dict[str, Any]:
    slug = str(team_slug or "").strip()
    if not slug or not _sportscore_ready():
        return _avg_empty()
    try:
        payload = sportscore_provider.team_detail(slug, str(team_name or "").strip())
        summary = payload.get("summary") if isinstance(payload, dict) else None
        return dict(summary or _avg_empty())
    except Exception:
        return _avg_empty()
...
def avg_payload_for_match(match_id_value: str) -> dict[str, Any]:
    ...
    expected = _prematch_row_by_id(mid)
    if expected:
        home = _sportscore_team_average(str(expected.get("home_id") or ""), str(expected.get("home") or ""))
        away = _sportscore_team_average(str(expected.get("away_id") or ""), str(expected.get("away") or ""))
        return {"ok": True, "match_id": mid, "avg": {"home": home, "away": away}}
    slug = mid[3:]
...
def avg_bulk_payload_for_matches(match_ids: list[str]) -> dict[str, Any]:
    ...
    for mid in seen:
        ...
        for side in ("home", "away"):
            ...
            if tid:
                needed[tid] = str(row.get(side) or "")

    team_avg = {tid: _sportscore_team_average(tid, name) for tid, name in needed.items()}
    items: dict[str, dict[str, Any]] = {}
    for mid, row in rows.items():
        h = team_avg.get(str(row.get("home_id") or ""), {}).get("total_avg")
        a = team_avg.get(str(row.get("away_id") or ""), {}).get("total_avg")
