...
# Live remains on the proven IGScore collector by default. Set LIVE_PROVIDER=sportscore
# to switch the LIVE tab to SportScore; PREMATCH always uses SportScore.
LIVE_PROVIDER = os.environ.get("LIVE_PROVIDER", "igscore").strip().lower()
if LIVE_PROVIDER not in {"igscore", "sportscore", "auto"}:
...
def build_payload(matches: list[dict[str, Any]], source: str, error: str | None = None) -> dict[str, Any]:
    ...
    if source not in {"demo", "sportscore_prematch"}:
        try:
...
def load_live_payload(force: bool = False) -> dict[str, Any]:
    ...
    with _live_fetch_lock:
        ...
        # Optional SportScore LIVE mode. Default remains IGScore so existing
        # statistics filters and pressure history do not regress.
        if LIVE_PROVIDER in {"sportscore", "auto"}:
            try:
                ss_matches = fetch_live_sportscore(force=force)
                # In auto mode an empty list is allowed to fall through to IGScore.
                if ss_matches or LIVE_PROVIDER == "sportscore":
                    payload = build_payload(ss_matches, "sportscore")
                    payload["provider"] = "sportscore"
                ...
                errors.append(f"sportscore: {type(exc).__name__}: {exc}")
                if LIVE_PROVIDER == "sportscore":
                    payload = _load_last_good_live_payload("; ".join(errors)) or build_payload([], "sportscore", "; ".join(errors))
                    with _cache_lock:
                        _live_cache.update({"saved_at": now, "payload": payload, "raw": {}})
                    return payload
        ...
        if mode in {"auto", "igscore"} and COLLECTOR_ENABLED and LIVE_FROM_DB_ONLY:
            try:
                ...
                collector_state = _collector_state_copy()
                if matches:
                    payload = build_payload(matches, "collector_db")
                    payload["collector"] = collector_state
                    _save_last_good_live_payload(payload)
                    with _cache_lock:
                        _live_cache.update({"saved_at": now, "payload": payload, "raw": {}})
                    return payload

                # Immediately after a deploy the SQLite collector cache is empty
                # until the first scan finishes. Do not show a false "0 matches"
                # screen: use last-good data or make one direct IGScore request.
                collector_ready = bool(collector_state.get("last_finish")) and not collector_state.get("last_error")
                if collector_ready:
                    payload = build_payload([], "collector_db")
                    payload["collector"] = collector_state
                    with _cache_lock:
                        _live_cache.update({"saved_at": now, "payload": payload, "raw": {}})
                    return payload
                reason = str(collector_state.get("last_error") or "collector warming up")
                errors.append(f"collector_db: {reason}")
                fallback = _load_last_good_live_payload("; ".join(errors))
                if fallback:
                    fallback["collector"] = collector_state
                    with _cache_lock:
                        _live_cache.update({"saved_at": now, "payload": fallback, "raw": {}})
                    return fallback
                # No last-good snapshot: fall through to the existing direct
                # IGScore loader below. The live single-flight lock prevents a
                # thundering herd during cold start.
            except Exception as exc:
                errors.append(f"collector_db: {type(exc).__name__}: {exc}")
                fallback = _load_last_good_live_payload("; ".join(errors))
                if fallback:
                    fallback["collector"] = _collector_state_copy()
                    with _cache_lock:
                        _live_cache.update({"saved_at": now, "payload": fallback, "raw": {}})
                    return fallback
                # Fall through to one direct IGScore request.
...
def avg_payload_for_match(match_id_value: str) -> dict[str, Any]:
    ...
    if mid.startswith("ss:"):
        if not _sportscore_ready():
            return {"ok": False, "error": "sportscore_provider_unavailable"}
        expected = _prematch_row_by_id(mid)
        slug = str((expected or {}).get("slug") or mid[3:]).strip()
        return sportscore_provider.avg_for_slug(slug, expected_match=expected)
...
def detail_payload(match_id_value: str) -> dict[str, Any]:
    if str(match_id_value or "").startswith("ss:"):
        return sportscore_detail_payload(str(match_id_value))
    live_payload = load_live_payload(force=False)
...
def main() -> None:
    ...
    print("=" * 72)
    print("Telegram Football Mini App — v10.10 tournament centre")
