...
COUNTRY_NAME_ALIASES = {
    ...
    "tanzanian": "Tanzania", "sudanese": "Sudan", "zambian": "Zambia", "zimbabwean": "Zimbabwe",
    ...
    "kiwi": "New Zealand", "canadian": "Canada", "american": "USA", "usa": "USA", "us": "USA",
...
def match_events_cached(match_id_value: str, match: dict[str, Any] | None = None, stats: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    # Merge — API events (with player names) come first when they exist; DB
    # events fill the gap when API has nothing.
    if api_events:
        return api_events
    return db_events
...
def _extract_odds(info: dict[str, Any]) -> dict[str, Any]:
    """Try to pull 1X2 (live + prematch) odds from a match_info response.

    IGScore wraps odds in different shapes depending on the match.
    We try a few known paths and return whatever we find.
    """
    result = info.get("result") if isinstance(info, dict) else info
    if not isinstance(result, dict):
        return {}

    def _scan(obj: Any, depth: int = 0) -> dict[str, Any]:
        if depth > 6 or not isinstance(obj, dict):
            return {}
        # Direct 1X2 keys
        for win1_key in ("win1", "homeOdds", "home_odds", "1", "h", "odd1", "sp1"):
            if obj.get(win1_key) is not None:
                o1 = _safe_odds(obj.get(win1_key))
                ox = _safe_odds(obj.get("draw") or obj.get("x") or obj.get("odd_x") or obj.get("drawOdds") or obj.get("draw_odds"))
                o2 = _safe_odds(obj.get("win2") or obj.get("awayOdds") or obj.get("away_odds") or obj.get("2") or obj.get("a") or obj.get("odd2") or obj.get("sp2"))
                if o1 and o2:
                    return {"1": o1, "X": ox, "2": o2}
        # Recurse
        for key, val in obj.items():
            k = str(key).lower()
            if any(x in k for x in ("odds", "bet", "1x2", "market", "sp", "live", "prematch")):
                found = _scan(val, depth + 1)
                if found:
                    return found
        return {}

    # Try live odds first, then prematch
    for section_key in ("liveOdds", "live_odds", "odds", "prematchOdds", "prematch_odds", "markets"):
        section = result.get(section_key)
        if section:
            found = _scan(section)
            if found:
                found["source"] = "live" if "live" in str(section_key).lower() else "prematch"
                return found

    # Brute force scan the entire result
    found = _scan(result)
    if found:
...
def detail_payload(match_id_value: str) -> dict[str, Any]:
    ...
    # v8c: try to extract odds from match_info
    odds: dict[str, Any] = {}
    if mid:
        try:
            odds = _extract_odds(match_info(mid))
        except Exception:
            odds = {}

    return {"ok": True, "match": match, "stats": stats, "avg": avg, "events": events, "odds": odds}
