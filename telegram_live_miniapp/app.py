...
ODDS_FREEZE_FIRST_LIVE_AS_PREMATCH = os.environ.get("ODDS_FREEZE_FIRST_LIVE_AS_PREMATCH", "1").strip().lower() in {"1", "true", "yes", "on"}
...
def _store_odds_snapshot(
    ...
    *,
    ...
    Prematch is frozen once and kept static. If the provider sends a dedicated
    prematch/opening line, that value wins. If IGScore does not expose opening
    odds, the first live odds row seen by the bot is stored as the frozen
    prematch fallback. Later live odds updates never overwrite final_pre, so
    the PREMATCH row stays fixed while the LIVE row keeps moving.
    """
    mid = str(match_id or "").strip()
    if not mid:
...
def fetch_match_odds_bundle_cached(match_id: str) -> dict[str, Any]:
    ...
    provider responses after kickoff. Dedicated provider prematch/opening odds
    are preferred. When those are missing, the first live odds row seen by the
    bot is frozen as a static prematch fallback so the PREMATCH row does not
    disappear.
    """
    mid = str(match_id or "").strip()
    if not mid:
