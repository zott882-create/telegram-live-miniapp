...
def _prematch_payload_from_rows(rows: list[dict[str, Any]], error: str = "") -> dict[str, Any]:
    ...
    for item in clean:
        item["country_code"] = item.get("country_code") or country_code(str(item.get("country") or ""))
    payload = build_payload(clean, "sportscore_prematch", error)
    payload["provider"] = "sportscore"
    payload["attribution"] = {"text": "Powered by SportScore", "url": "https://sportscore.com/"}
    try:
...
def main() -> None:
    ...
    print("=" * 72)
    print("Telegram Football Mini App — v10.6 full prematch ID fix")
