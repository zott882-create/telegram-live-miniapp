...
function leagueHeaderIcon(g) {
  // League logos from the feed can be random/unclear. Headers use country flags.
...
function renderLiveMatchStandings(d) {
  ...
  const statChip = (label, value) => `<div class="live-standings-stat"><span>${esc(label)}</span><b>${esc(value)}</b></div>`;
  return `<section class="live-match-standings"><div class="live-standings-title"><span>STANDINGS</span><h3>Турнирная таблица</h3></div>${rows.length ? `<div class="live-standings-cards">${rows.map(row => `<div class="live-standings-card ${row?.is_home ? "current-home" : row?.is_away ? "current-away" : ""}"><div class="live-standings-top"><div class="live-standings-rank">${safeInt(row?.position)}</div><div class="live-standings-team-wrap">${teamCell(row)}</div><div class="live-standings-points"><span>О</span><b>${safeInt(row?.points)}</b></div></div><div class="live-standings-stats">${statChip('И', safeInt(row?.played))}${statChip('В', safeInt(row?.wins))}${statChip('Н', safeInt(row?.draws))}${statChip('П', safeInt(row?.losses))}${statChip('РМ', row?.goal_difference ?? '—')}</div></div>`).join("")}</div>` : `<div class="live-standings-empty">Таблица этой лиги пока не передана SportScore.</div>`}</section>`;
}
...
function pressureChartSvg(bars, opts) {
  ...
  if (mode === "full") {
    ...
    for (const b of src) {
      ...
      // SportScore's match minute can be one refresh behind the stats history.
      // A sampled zero-delta minute is still real graph progress, so use the
      // explicit `empty` flag rather than only non-zero pressure values.
      if (b && b.empty === false) latestSampleIndex = Math.max(latestSampleIndex, slot);
      else if (Math.abs(momentumValue(b)) > 0) latestSampleIndex = Math.max(latestSampleIndex, slot);
    }
  } else {
  ...
  // Synchronise the red cursor with whichever source is freshest: the live
  // match clock or the latest stored pressure sample. This removes the old
  // one-minute lag when the detailed stats arrive before the list minute.
  const clockIndex = clamp(activeCount - 1, 0, TOTAL_BINS - 1);
  const syncedMarkerIndex = mode === "full"
    ? Math.max(clockIndex, latestSampleIndex)
    : TOTAL_BINS - 1;
  const syncedActiveCount = mode === "full"
    ? clamp(syncedMarkerIndex + 1, 1, TOTAL_BINS)
    : activeCount;

  // If the stats history is ahead, expose the same additional minute slots as
  // neutral marks instead of drawing the cursor through an apparently future area.
  for (let i = activeCount; i < syncedActiveCount; i++) {
    if (values[i] === null) values[i] = 0;
  }

  const elapsedW = (syncedActiveCount * slotW).toFixed(2);
  ...
  // Put the cursor through the centre of the latest visible minute bar, not
  // on the previous slot boundary. This also fixes the visual off-by-one lag.
  const markerX = centerXForIndex(clamp(syncedMarkerIndex, 0, TOTAL_BINS - 1));
  const marker = `<g>
...
function buildPressureChartFallbackBars(detail, mode = "full") {
  ...
  // There is no real per-minute history yet. Draw a short provisional trace
  // ending at the CURRENT match minute, never from minute one. V10.33 placed
  // fallback bars in slots 0..N, so a match at 57' looked as if all pressure
  // happened at kickoff while the red cursor stayed at 57'.
  const count = Math.max(1, Math.min(8, slotMax + 1));
  const startSlot = Math.max(0, slotMax - count + 1);
  const now = Math.floor(Date.now() / 1000);
  const bars = [];
  for (let i = 0; i < count; i++) {
    const ratio = count <= 1 ? 1 : i / (count - 1);
    const slot = mode === "full" ? (startSlot + i) : i;
    const scale = statTotalValue ? (0.32 + ratio * 0.68) / count : 0.18;
...
function renderMatchEvents(events, m) {
  ...
  for (const ev of sorted) {
    ...
    // If the feed gives a player, show the player; otherwise keep an invisible
...
renderDetailDetailsContent = function renderDetailDetailsContentNoGoalWait(d, m, s) {
  const oddsHtml = renderOddsSection(d.odds || {}, d.prematch_odds || {}, d.odds_movement || {}, !!m.is_live) || `<div class="detail-tab-empty">КФ пока нет.</div>`;
  return `${oddsHtml}${renderPressureChartFull(d)}${renderMatchEvents(d.events || [], m)}`;
};
...
// ============================================================================
const FAST_LIVE_CACHE_KEY = "live-zot-v1033-live-snapshot";
const FAST_LIVE_CACHE_MAX_AGE_MS = 5 * 60 * 1000;
const FAST_DETAIL_CACHE_PREFIX = "live-zot-v1034-detail:";
const FAST_DETAIL_INDEX_KEY = "live-zot-v1034-detail-index";
