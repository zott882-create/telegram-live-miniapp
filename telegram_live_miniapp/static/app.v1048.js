...
const LIVE_POLL_INTERVAL_MS = Math.max(120000, Number(window.MINIAPP_LIVE_POLL_INTERVAL_MS || 120000));
const DETAIL_POLL_INTERVAL_MS = Math.max(120000, Number(window.MINIAPP_DETAIL_POLL_INTERVAL_MS || 120000));
...
let _detailFreshRetryTimer = null;

function cancelDetailFreshRetry() {
  if (_detailFreshRetryTimer) {
    clearTimeout(_detailFreshRetryTimer);
    _detailFreshRetryTimer = null;
  }
}

function scheduleDetailFreshRetry(id, attempt = 0) {
  id = normalizeMatchId(id);
  if (!id || attempt >= 4) return;
  cancelDetailFreshRetry();
  const delay = 3500 + attempt * 1500;
  _detailFreshRetryTimer = setTimeout(async () => {
    _detailFreshRetryTimer = null;
    if (state.view !== "detail" || String(state.selectedMatch || "") !== String(id) || state.sheet) return;
    try {
      const res = await fetch(`/api/match?id=${encodeURIComponent(id)}`, { cache: "no-store" });
      let payload = await res.json();
      if (state.view !== "detail" || String(state.selectedMatch || "") !== String(id)) return;
      if (payload?.ok) {
        ...
        if (payload.refreshing) scheduleDetailFreshRetry(id, attempt + 1);
      }
    } catch (_) {
      scheduleDetailFreshRetry(id, attempt + 1);
    }
  }, delay);
}
...
async function loadDetail(id, opts = {}) {
  ...
  try {
    ...
    if (payload && payload.ok) {
      ...
      state.detail = payload;
      saveFastDetailCache(id, payload);
      if (payload.refreshing) scheduleDetailFreshRetry(id, 0);
      else cancelDetailFreshRetry();
...
function lineupPlayerRole(player) {
  const role = String(player?.role || player?.lineup_role || player?.type || player?.status || "").toLowerCase();
  return /sub|bench|reserve|запас/.test(role) ? "substitute" : "starter";
}

function lineupFullName(player) {
  const p = player || {};
  return String(p.name || p.player_name || p.full_name || p.playerName || p.fullName || "Игрок")
    .replace(/\s+/g, " ")
    .trim() || "Игрок";
}

function lineupSurname(player) {
  const raw = lineupFullName(player);
  const parts = raw.split(" ").filter(Boolean);
  return parts.length > 1 ? parts[parts.length - 1] : raw;
}

function firstLineupArray(...values) {
...
function lineupSidePlayers(lineups, side) {
  const l = lineups && typeof lineups === "object" ? lineups : {};
  const own = l[side] && typeof l[side] === "object" && !Array.isArray(l[side]) ? l[side] : {};
  const starting = l.starting && typeof l.starting === "object" ? l.starting : {};
  const starters = l.starters && typeof l.starters === "object" ? l.starters : {};
  const teams = l.teams && typeof l.teams === "object" ? l.teams : {};
  const team = teams[side] && typeof teams[side] === "object" ? teams[side] : {};
  return firstLineupArray(
    l[side], l[`${side}_players`], l[`${side}Players`],
    own.players, own.lineup, own.starting, own.starters,
    starting[side], starters[side],
    team.players, team.lineup, team.starting, team.starters
  );
}

function startingLineupPlayers(players) {
  const rows = Array.isArray(players) ? players.filter(Boolean) : [];
  const starters = rows.filter(p => lineupPlayerRole(p) !== "substitute");
  return (starters.length ? starters : rows).slice(0, 11);
}

function renderLiveLineupPlayer(player) {
  const p = player || {};
  const number = safeInt(p.number ?? p.shirt_number ?? p.jersey_number, 0);
  const surname = lineupSurname(p);
  const fullName = lineupFullName(p);
  return `<div class="live-lineup-player compact" title="${esc(fullName)}">
    <span class="live-lineup-number">${number || "—"}</span>
    <span class="live-lineup-name surname-only"><b>${esc(surname)}</b></span>
  </div>`;
}

function renderLiveLineupSide(side, teamName, teamLogo, formation, players, coach) {
  const starters = startingLineupPlayers(players);
  ...
  return `<div class="live-lineup-side ${side}">
    ...
    ${starters.length ? `<div class="live-lineup-subhead">ОСНОВНОЙ СОСТАВ · ${starters.length}</div><div class="live-lineup-list">${starters.map(renderLiveLineupPlayer).join("")}</div>` : `<div class="live-lineup-empty">Состав пока недоступен</div>`}
...
function renderLiveLineups(detail, match) {
  const d = detail || {};
  const m = match || {};
  const l = d.lineups && typeof d.lineups === "object" ? d.lineups : {};
  const home = lineupSidePlayers(l, "home");
  const away = lineupSidePlayers(l, "away");
  ...
  return `<section class="live-lineups-card">
    ...
    <div class="live-lineups-grid">
      ${renderLiveLineupSide("home", m.home, m.home_logo, l.home_formation || l.homeFormation, home, l.home_coach || l.homeCoach)}
      ${renderLiveLineupSide("away", m.away, m.away_logo, l.away_formation || l.awayFormation, away, l.away_coach || l.awayCoach)}
    </div>
  </section>`;
}
...
function standingsRowsFrom(value, depth = 0) {
  if (depth > 4 || value == null) return [];
  if (Array.isArray(value)) {
    const direct = value.filter(row => row && typeof row === "object" && !Array.isArray(row) && (row.team || row.team_name || row.name || row.position != null));
    if (direct.length) return direct;
    for (const item of value) {
      const nested = standingsRowsFrom(item, depth + 1);
      if (nested.length) return nested;
    }
    return [];
  }
  if (typeof value !== "object") return [];
  for (const key of ["rows", "table", "standings", "ranking", "rankings", "data", "groups", "items"]) {
    const nested = standingsRowsFrom(value[key], depth + 1);
    if (nested.length) return nested;
  }
  return [];
}
...
function renderPrematchStandings(d) {
  const rows = standingsRowsFrom(d?.standings || d?.table || d?.ranking || []);
  return `<div class="prematch-info-card standings-card"><div class="prematch-card-title">Турнирная таблица</div>
    <div class="prematch-table-wrap"><table class="prematch-table"><thead><tr><th>#</th><th>Команда</th><th>И</th><th>В</th><th>Н</th><th>П</th><th>РМ</th><th>О</th><th>Форма</th></tr></thead><tbody>
      ${rows.map(r => `<tr class="${r.is_home ? "current-home" : r.is_away ? "current-away" : ""}"><td>${safeInt(r.position ?? r.rank)}</td><td class="team-cell">${(r.logo || r.team_logo) ? `<img src="${esc(r.logo || r.team_logo)}" alt=""/>` : ""}<span>${esc(r.team || r.team_name || r.name || "")}</span></td><td>${safeInt(r.played ?? r.matches)}</td><td>${safeInt(r.wins ?? r.win)}</td><td>${safeInt(r.draws ?? r.draw)}</td><td>${safeInt(r.losses ?? r.loss)}</td><td>${esc(r.goal_difference ?? r.goalDifference ?? r.diff ?? "")}</td><td><b>${safeInt(r.points ?? r.pts)}</b></td><td><div class="prematch-form nowrap">${formPills(r.form)}</div></td></tr>`).join("")}
    </tbody></table></div>${rows.length ? "" : `<div class="prematch-empty-text">Таблица пока недоступна у источника.</div>`}</div>${renderSportScoreAttribution()}`;
}

function renderLineupSide(title, formation, players, coach) {
  const list = startingLineupPlayers(players);
  return `<div class="lineup-side"><div class="lineup-side-head"><b>${esc(title)}</b><span>${esc(formation || "Схема —")}</span></div>
    <div class="lineup-player-list-v10">${list.map(p => `<div title="${esc(lineupFullName(p))}"><span class="shirt">${safeInt(p.number ?? p.shirt_number ?? p.jersey_number) || "—"}</span><b>${esc(lineupSurname(p))}</b></div>`).join("") || `<span class="prematch-empty-text">Нет данных</span>`}</div>
...
function renderPrematchLineups(d, m) {
  ...
  const home = lineupSidePlayers(l, "home");
  const away = lineupSidePlayers(l, "away");
  if (!home.length && !away.length) return `<div class="prematch-info-card lineups-wait"><div class="lineups-ball">⚽</div><div class="prematch-card-title">${esc(l.message || "Составы ещё не объявлены")}</div><p>${esc(l.submessage || "Обычно составы появляются примерно за час до начала матча.")}</p></div>${renderSportScoreAttribution()}`;
  return `<div class="prematch-info-card"><div class="prematch-card-title">${l.confirmed ? "Подтверждённые составы" : "Предварительные составы"}</div><div class="lineups-grid-v10">${renderLineupSide(m.home,l.home_formation || l.homeFormation,home,l.home_coach || l.homeCoach)}${renderLineupSide(m.away,l.away_formation || l.awayFormation,away,l.away_coach || l.awayCoach)}</div></div>${renderSportScoreAttribution()}`;
}
...
async function refreshDetailSilently() {
  ...
  try {
    ...
    if (detailResult.status === "fulfilled" && detailResult.value?.ok) {
      ...
      renderSoftPreserveScroll();
      if (payload.refreshing) scheduleDetailFreshRetry(selectedId, 0);
      else cancelDetailFreshRetry();
    }
  } catch (_) {}
}
...
function refreshLiveForFilters(attempt = 0) {
  ...
  return Promise.resolve(loadLive(false)).then(() => true).catch(() => false);
}
