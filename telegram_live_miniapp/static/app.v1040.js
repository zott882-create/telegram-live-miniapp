...
function matchHasDetailedStats(match) {
  ...
  return meaningful.some(key => Object.prototype.hasOwnProperty.call(stats, key));
}
...
function renderLiveLineupPlayer(player) {
  ...
  return `<div class="live-lineup-player">
    ...
    <span class="live-lineup-avatar ${photo ? "" : "empty"}">${photo ? `<img src="${esc(photo)}" alt="" loading="lazy" onerror="this.parentElement.classList.add('empty')"/>` : "👤"}</span>
    <span class="live-lineup-name"><b>${esc(p.name || "Игрок")}</b>${position ? `<small>${esc(position)}</small>` : ""}</span>
    ${Number.isFinite(rating) && rating > 0 ? `<em>${rating.toFixed(1)}</em>` : ""}
  </div>`;
}
