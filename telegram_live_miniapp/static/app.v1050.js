...
const LIVE_POLL_INTERVAL_MS = Math.max(60000, Number(window.MINIAPP_LIVE_POLL_INTERVAL_MS || 90000));
...
function notifySnapshotScorePair(value) {
  const v = value || {};
  let home = Number(v.score_home);
  let away = Number(v.score_away);
  let valid = Number.isFinite(home) && Number.isFinite(away);
  if (!valid) {
    const m = String(v.score || "").match(/(\d+)\s*[-:]\s*(\d+)/);
    if (m) {
      home = Number(m[1]);
      away = Number(m[2]);
      valid = true;
  ...
  return { valid, home: valid ? Math.max(0, Math.trunc(home)) : 0, away: valid ? Math.max(0, Math.trunc(away)) : 0 };
}
function mergeNotifyCardSnapshots(base, incoming) {
  const oldCard = base && typeof base === "object" ? base : {};
  const newCard = incoming && typeof incoming === "object" ? incoming : {};
  const out = { ...oldCard, ...newCard };
  const oldScore = notifySnapshotScorePair(oldCard);
  const newScore = notifySnapshotScorePair(newCard);
  let chosen = newScore.valid ? newScore : oldScore;
  if (oldScore.valid && newScore.valid) {
    const oldTotal = oldScore.home + oldScore.away;
    const newTotal = newScore.home + newScore.away;
    const oldMinute = safeInt(oldCard.minute, 0);
    const newMinute = safeInt(newCard.minute, 0);
    // A later empty/reset snapshot must not erase an already observed goal.
    if (newTotal < oldTotal) chosen = oldScore;
    else if (newTotal === oldTotal && newMinute < oldMinute) chosen = oldScore;
  }
  if (chosen.valid) {
    out.score_home = chosen.home;
    out.score_away = chosen.away;
    out.score = `${chosen.home}-${chosen.away}`;
  }
  out.minute = Math.max(safeInt(oldCard.minute, 0), safeInt(newCard.minute, 0));
  if (safeInt(oldCard.minute, 0) > safeInt(newCard.minute, 0) && oldCard.minute_text) out.minute_text = oldCard.minute_text;
  out.found_at = safeInt(oldCard.found_at, 0) || safeInt(newCard.found_at, 0);
  out.cached_at = Math.max(Number(oldCard.cached_at || 0), Number(newCard.cached_at || 0), Date.now());
  return out;
}
...
function saveNotifyCache(id, match) {
  ...
  const key = String(id);
  store[key] = { ...mergeNotifyCardSnapshots(store[key], match), cached_at: Date.now(), id: key };
...
function saveNotifyCacheFromAlerts(items) {
  ...
  for (const item of items) {
    ...
    if (!id) continue;
    store[id] = {
      ...mergeNotifyCardSnapshots(store[id], card),
      found_at: safeInt(store[id]?.found_at || item?.found_at || card?.found_at, Math.floor(Date.now() / 1000)),
      cached_at: Date.now(),
      id,
    };
    changed = true;
...
async function refreshNotifySnapshotsFromBackend(items) {
  if (state.notifyRefreshRunning) return;
  const liveById = currentLiveById();
  const now = Date.now();
  const targets = [];
  for (const card of items || []) {
    const id = String(card?.id || card?.match_id || "");
    if (!id || liveById.has(id) || !id.startsWith("ss:")) continue;
    if (now - Number(state.notifyRefreshAt[id] || 0) < 120000) continue;
    state.notifyRefreshAt[id] = now;
    targets.push(id);
    if (targets.length >= 6) break;
  }
  if (!targets.length) return;
  state.notifyRefreshRunning = true;
  let changed = false;
  try {
    for (const id of targets) {
      try {
        const res = await fetch(`/api/match?id=${encodeURIComponent(id)}&force=1`, { cache: "no-store" });
        const payload = await res.json();
        if (!payload?.ok || !payload?.match) continue;
        const oldCached = getNotifyCachedMatch(id) || {};
        const fresh = mergeNotifyCardSnapshots(oldCached, {
          ...payload.match,
          id,
          finished: Boolean(payload.finished || payload.match.finished || oldCached.finished),
          is_live: false,
        });
        const before = notifySnapshotScorePair(oldCached);
        const after = notifySnapshotScorePair(fresh);
        if (before.home !== after.home || before.away !== after.away || safeInt(oldCached.minute,0) !== safeInt(fresh.minute,0)) changed = true;
        saveNotifyCache(id, fresh);
        state.notifyMatches = (state.notifyMatches || []).map(item => String(item?.id || item?.match_id || "") === id ? mergeNotifyCardSnapshots(item, fresh) : item);
        fetch("/api/notify/match-sync", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ init_data: tg?.initData || "", match_id: id, match: fresh }),
        }).catch(() => {});
      } catch (_) {}
    }
  } finally {
    state.notifyRefreshRunning = false;
  }
  if (changed && state.view === "notify") render();
}
...
function resolveMatchCardData(candidate, liveById = null) {
  ...
  if (liveMatch) {
    ...
    const merged = mergeNotifyCardSnapshots(notified || candidate, liveMatch);
    return { ...merged, ...meta, id, is_live: true, finished: false };
  }
  const cached = getFavCachedMatch(id);
  if (cached) return { ...mergeNotifyCardSnapshots(candidate, cached), ...meta, id, is_live: false, finished: Boolean(cached.finished || candidate?.finished) };
  const notified = getNotifyCachedMatch(id);
  if (notified) {
    const merged = mergeNotifyCardSnapshots(candidate, notified);
    return { ...merged, ...meta, id, is_live: false, finished: Boolean(merged.finished || candidate?.finished) };
  }
  return { ...(candidate || {}), id };
}
...
function renderMatchFeedInner() {
  const hasMatches = fastPayloadHasMatches(state.live);
  if (state.loading && !hasMatches) {
    return `<div class="loading fast-warming"><div class="spinner"></div><b>Подключаем матчи…</b><span>Главная уже работает. Список появится автоматически.</span></div>`;
  }
  const visible = filteredMatches();
  const groups = groupByLeague(visible);
  if (groups.length === 0 && state.live?.refreshing) {
    return `<div class="loading fast-warming"><div class="spinner"></div><b>Обновляем список матчей</b><span>Сервер просыпается, повторная проверка идёт автоматически.</span></div>`;
  }
...
function renderNotify() {
  ...
  if (!isFilter && shown.length) {
    refreshNotifySnapshotsFromBackend(resolveMatchCardList(shown.map(renderAlert)));
  }
}
...
function renderLiveLineupPlayer(player) {
  ...
  return `<div class="live-lineup-player compact ${number ? "" : "no-number"}">
    ${number ? `<span class="live-lineup-number">${number}</span>` : ""}
    <span class="live-lineup-name surname-only"><b>${esc(displayName)}</b></span>
  </div>`;
}

function renderLiveLineupSide(side, teamName, teamLogo, formation, players, coach) {
  const safeStarters = getStartingLineupPlayers(players);
  const logo = String(teamLogo || "").trim();
  const section = list => list.length ? `<div class="live-lineup-subhead">СТАРТОВЫЙ СОСТАВ · 11 ИГРОКОВ</div><div class="live-lineup-list">${list.map(renderLiveLineupPlayer).join("")}</div>` : "";
...
function renderLineupSide(title, formation, players, coach) {
  const list = getStartingLineupPlayers(players);
  return `<div class="lineup-side"><div class="lineup-side-head"><b>${esc(title)}</b><span>${esc(formation || "Схема —")}</span></div>
    <div class="lineup-player-list-v10">${list.map(p => {
      const number = safeInt(p.number, 0);
      return `<div class="${number ? "" : "no-number"}">${number ? `<span class="shirt">${number}</span>` : ""}<b>${esc(lineupShortName(p.name || p.player_name || p.full_name || ""))}</b></div>`;
    }).join("") || `<span class="prematch-empty-text">Нет данных</span>`}</div>
...
const FAST_LIVE_CACHE_MAX_AGE_MS = 2 * 60 * 60 * 1000;
...
async function fetchJsonWithTimeout(url, timeoutMs) {
  ...
  const timer = controller ? setTimeout(() => controller.abort(), Math.max(500, Number(timeoutMs) || 2500)) : null;
  try {
...
async function loadFastLiveSnapshot() {
  try {
    const { res, payload } = await fetchJsonWithTimeout(`/api/live/fast?sport=football`, 1400);
    if (!res.ok || !payload || payload.ok === false) return false;
    if (fastPayloadHasMatches(payload)) {
...
function scheduleFastSnapshotRetry(delayMs = 4000) {
  clearTimeout(_fastLiveRefreshRetryTimer);
  const delay = Math.max(3000, Math.min(12000, Number(delayMs) || 4000));
  _fastLiveRefreshRetryTimer = setTimeout(async () => {
    if (document.hidden || state.view !== "matches" || _loadLiveInFlight) {
      scheduleFastSnapshotRetry(Math.min(12000, delay + 1500));
      return;
    }
    const ready = await loadFastLiveSnapshot();
    if (!ready) scheduleFastSnapshotRetry(Math.min(12000, delay + 1500));
  }, delay);
}
...
loadLive = async function loadLiveFastStart(force = false) {
  ...
  try {
    ...
    const timeoutMs = force ? 15000 : 9000;
    const result = await fetchJsonWithTimeout(`/api/live?${query}`, timeoutMs);
    ...
    if (payload.refreshing) {
      // Never retry the heavy endpoint every 3-4 seconds. An aborted browser
      // request keeps running on the server and used to fill the Waitress queue.
      scheduleFastSnapshotRetry(Math.max(5000, Number(payload.retry_after_seconds || 5) * 1000));
    }
  } catch (err) {
    ...
    // Use the non-blocking snapshot endpoint while the server/provider recovers.
    scheduleFastSnapshotRetry(5000);
  } finally {
    state.loading = false;
    _liveSportInFlight.delete(requestSport);
    _loadLiveInFlight = _liveSportInFlight.size > 0;
...
bootApp = async function bootAppFastStart() {
  ...
  if (!restored) {
    state.live = { ok: true, total: 0, countries: [], sport: "football", loading: true, refreshing: true, cold_start: true };
    state.liveBySport.football = state.live;
    state.loading = false;
    render();
  }
  ...
  // Start only the non-blocking endpoint during boot. The old build fired
  // /api/live at the same time and retried it after 3 seconds, producing a
  // thundering herd when SportScore was slow.
  const gatePromise = checkOnlineGate(true);
  const fastReady = await loadFastLiveSnapshot();
  ...
  if (!fastReady && !fastPayloadHasMatches(state.live)) scheduleFastSnapshotRetry(3500);
  // One normal refresh is enough after the screen is already visible. It can
  // read Redis metadata and then regular 90-second polling takes over.
  setTimeout(() => {
    if (!document.hidden && state.view === "matches" && !_loadLiveInFlight) loadLive(false);
  }, fastReady || restored ? 10000 : 18000);
