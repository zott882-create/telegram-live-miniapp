...
function filteredMatches() {
  ...
  if (state.country !== 'all') {
    matches = matches.filter(m => (m.country || 'Без страны') === state.country);
  }
  if (state.league !== 'all') {
    matches = matches.filter(m => (m.league || 'Без лиги') === state.league);
  }
...
function renderClubBadge(name, logo = '') {
  if (logo) {
    return `<span class="club-badge is-image"><img src="${escapeHtml(logo)}" alt="${escapeHtml(name)}"></span>`;
  }
...
function renderList() {
  ...
  if (activeChips) {
    html += `<div class="active-filters">${activeChips}<button class="clear-link" onclick="clearAdvancedFilters()">Сбросить</button></div>`;
  }
...
function renderMatchRow(m) {
  ...
  return `
    <div class="match-row" onclick="loadDetail('${escapeHtml(m.id)}')">
      ...
      <div class="teams">
        <div class="team-line with-badge">
          ${renderClubBadge(m.home, m.home_logo)}
          <span class="team-name">${escapeHtml(m.home)}</span>
        </div>
        <div class="team-line with-badge">
          ${renderClubBadge(m.away, m.away_logo)}
  ...
  app.innerHTML = `
    <div class="detail">
      ...
      <div class="avg-wrap">
        ...
        <div class="avg-cards">
          <div class="avg-card">
            <div class="name with-badge mini">${renderClubBadge(m.home, m.home_logo)} <span>${escapeHtml(m.home)}</span></div>
            <div class="num">${formatAvg(avg.home?.avg)}</div>
            <div class="meta">0:0 → ${Number(avg.home?.zero_zero || 0)}/${Number(avg.home?.count || 10)}</div>
          </div>
          <div class="avg-card">
            <div class="name with-badge mini">${renderClubBadge(m.away, m.away_logo)} <span>${escapeHtml(m.away)}</span></div>
...
function renderChoiceChip(active, label, onClick) {
  return `<button class="sheet-chip${active ? ' active' : ''}" onclick='${escapeHtml(onClick)}'>${label}</button>`;
}
