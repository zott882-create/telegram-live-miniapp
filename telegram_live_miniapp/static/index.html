...
<title>Футбол · LIVE и ПРЕМАТЧ</title>
<link rel="stylesheet" href="/styles.css?v=10.7-team-links" />
...
const LAST_DETAIL_KEY = "miniapp-v10-last-detail";
...
const LIVE_POLL_INTERVAL_MS = Math.max(10000, Number(window.MINIAPP_LIVE_POLL_INTERVAL_MS || 30000));
const DETAIL_POLL_INTERVAL_MS = Math.max(10000, Number(window.MINIAPP_DETAIL_POLL_INTERVAL_MS || 30000));
...
function syncNotifyFilterToServer(cfg, opts = {}) {
  ...
  const body = JSON.stringify({
    ...
    goal_wait_auto_goal_enabled: getGoalWaitAutoGoalEnabled(),
  });
...
const state = {
  ...
  feedMode: (() => { try { return localStorage.getItem("miniapp-v10-feed-mode") === "prematch" ? "prematch" : "live"; } catch (_) { return "live"; } })(),
  prematchDate: "all",
...
const COUNTRY_FLAG_CODE_MAP = {
  ...
  // IGScore sometimes groups these under EUROPE / AMERICAS / OTHERS and leaves country_code empty.
...
async function loadTeamStats(matchId, side = "home") {
  const mid = String(matchId || "").trim();
  if (!mid) return;
  const seq = ++_loadTeamSeq;
  state.teamSide = side === "away" ? "away" : "home";
  state.teamTab = "stats";
  state.teamData = null;
  state.teamError = "";
  ...
  render();
  try {
    const res = await fetch(`/api/team?match_id=${encodeURIComponent(mid)}&side=${encodeURIComponent(state.teamSide)}`, { cache: "no-store" });
    const payload = await res.json();
...
function retryTeamStats() {
  const mid = String(state.detail?.match?.id || state.selectedMatch || state.teamData?.context?.match_id || "").trim();
  if (mid) loadTeamStats(mid, state.teamSide);
}
...
function setTeamTab(tab) {
  state.teamTab = tab === "matches" ? "matches" : "stats";
  render();
}
...
function renderTeamHeroLogo(team) {
  ...
  return `<div class="team-page-logo ${logo ? "" : "logo-error"}">${logo ? `<img src="${esc(logo)}" alt="${esc(name)}" loading="lazy" onerror="this.parentElement.classList.add('logo-error')" />` : ""}<span class="logo-fallback">${esc(initials(name))}</span></div>`;
}
...
function renderTeamStatCard(label, value, sub = "") {
  return `<div class="team-stat-card"><span>${esc(label)}</span><b>${esc(value)}</b>${sub ? `<em>${esc(sub)}</em>` : ""}</div>`;
}

function renderTeamSplit(title, s) {
  const count = safeInt(s?.count, 0);
  return `<div class="team-split-card"><div class="team-split-title">${esc(title)} <span>${count} матч.</span></div><div class="team-split-line"><b>${safeInt(s?.wins)}В</b><b>${safeInt(s?.draws)}Н</b><b>${safeInt(s?.losses)}П</b></div><div class="team-split-line muted"><span>Забито ${teamAverage(s?.scored_avg)}</span><span>Пропущено ${teamAverage(s?.conceded_avg)}</span></div></div>`;
}
...
function renderInternalTeamMatch(row, upcoming = false) {
  ...
  return `<div class="team-match-row"><div class="team-match-meta"><span>${esc(dateTime || "—")}</span><em>${esc(row?.league || "")}</em></div><div class="team-match-main"><span class="team-match-name ${row?.side === "home" ? "selected" : ""}">${esc(row?.home || "")}</span><strong>${esc(score)}</strong><span class="team-match-name ${row?.side === "away" ? "selected" : ""}">${esc(row?.away || "")}</span>${!upcoming && result ? `<i class="team-result-pill ${result}">${esc(String(row.result).toUpperCase())}</i>` : ""}</div></div>`;
}

function renderTeamStatsContent(data) {
  const s = data?.summary || {};
  return `<div class="team-section"><div class="team-section-title">Форма — последние ${safeInt(s.count,0)} матчей</div><div class="team-form-row">${renderTeamResultPills(s.form)}</div><div class="team-wdl"><div><b>${safeInt(s.wins)}</b><span>Победы</span></div><div><b>${safeInt(s.draws)}</b><span>Ничьи</span></div><div><b>${safeInt(s.losses)}</b><span>Поражения</span></div></div></div>
  <div class="team-stat-grid">${renderTeamStatCard("Забито за матч", teamAverage(s.scored_avg))}${renderTeamStatCard("Пропущено", teamAverage(s.conceded_avg))}${renderTeamStatCard("Средний тотал", teamAverage(s.total_avg))}${renderTeamStatCard("Обе забьют", teamPercent(s.btts_percent), `${safeInt(s.btts)} из ${safeInt(s.count)}`)}${renderTeamStatCard("ТБ 1.5", teamPercent(s.over_1_5_percent))}${renderTeamStatCard("ТБ 2.5", teamPercent(s.over_2_5_percent))}${renderTeamStatCard("ТБ 3.5", teamPercent(s.over_3_5_percent))}${renderTeamStatCard("Сухие матчи", teamPercent(s.clean_sheets_percent), `${safeInt(s.clean_sheets)} из ${safeInt(s.count)}`)}</div>
  <div class="team-section"><div class="team-section-title">Дома и в гостях</div><div class="team-split-grid">${renderTeamSplit("Дома", data?.splits?.home || {})}${renderTeamSplit("В гостях", data?.splits?.away || {})}</div></div>`;
}

function renderTeamMatchesContent(data) {
  const recent = Array.isArray(data?.recent) ? data.recent : [];
  const upcoming = Array.isArray(data?.upcoming) ? data.upcoming : [];
  return `<div class="team-section"><div class="team-section-title">Последние матчи</div><div class="team-match-list">${recent.length ? recent.map(r => renderInternalTeamMatch(r, false)).join("") : `<div class="prematch-empty-text">Последние матчи пока недоступны.</div>`}</div></div><div class="team-section"><div class="team-section-title">Ближайшие матчи</div><div class="team-match-list">${upcoming.length ? upcoming.map(r => renderInternalTeamMatch(r, true)).join("") : `<div class="prematch-empty-text">Ближайшие матчи не найдены.</div>`}</div></div>`;
}
...
function renderTeamView() {
  ...
  let content = "";
  if (state.teamLoading) content = `<div class="loading"><div class="spinner"></div>Загружаем статистику команды…</div>`;
  else if (state.teamError) content = `<div class="empty"><div class="empty-icon">!</div><div>Не удалось загрузить статистику</div><div class="empty-sub">${esc(state.teamError)}</div><button class="retry-btn" onclick="retryTeamStats()">Повторить</button></div>`;
  else content = tab === "matches" ? renderTeamMatchesContent(data) : renderTeamStatsContent(data);
  app.innerHTML = `<div class="detail-screen team-screen"><div class="detail-topbar"><button class="back-btn" onclick="closeTeamStats()">‹</button><div class="detail-breadcrumb">Статистика команды</div><span class="team-top-spacer"></span></div><div class="team-hero">${renderTeamHeroLogo(team)}<div class="team-hero-copy"><h2>${esc(title)}</h2><div><span class="team-hero-flag">${leagueHeaderIcon({country_code:team.country_code,country:team.country,league:team.league})}</span><span>${esc([team.country,team.league].filter(Boolean).join(" · ") || "Командная статистика")}</span></div></div></div><div class="detail-tabs team-tabs"><button class="tab-btn ${tab === "stats" ? "active" : ""}" onclick="setTeamTab('stats')">Статистика</button><button class="tab-btn ${tab === "matches" ? "active" : ""}" onclick="setTeamTab('matches')">Матчи</button></div><div class="team-content">${content}</div></div>`;
}
...
function groupByLeague(matches) {
  ...
  for (const m of matches) {
    ...
    const key = c + "|||" + l;
    if (!map.has(key)) {
      map.set(key, { country: c, country_code: m.country_code || inferCountryCodeFromText(c, l) || "", league: l, league_logo: m.league_logo || "", has_odds: false, odds_count: 0, signal_count: 0, signal_max: 0, matches: [] });
    }
    const g = map.get(key);
    g.matches.push(m);
    if (matchHasOdds(m)) {
...
function notifyFilterSignal(m, cfg = null, label = "Фильтр") {
  ...
  if (possMin > 0) {
    const poss = statMaxSide(s, "possession", 50);
...
// ============================================================
function render() {
  app.classList.toggle("detail-mode", state.view === "detail" || state.view === "team");
...
function pressureChartSvg(bars, opts) {
  ...
  if (mode === "full") {
    ...
    const visible = src.length > TOTAL_BINS ? src.slice(src.length - TOTAL_BINS) : src;
    visible.forEach((b, i) => {
...
function renderMatchEvents(events, m) {
  ...
  for (const ev of sorted) {
    ...
    const icon = kind === "goal"
      ? `<span class="event-row-icon goal"><span class="event-goal-dot"></span></span>`
...
function computeMatchPulse(match, stats, avgData) {
  ...
  const possHome = safeInt(s?.possession?.home, 50);
  const possAway = safeInt(s?.possession?.away, 50);
  const possessionSwing = Math.max(Math.abs(possHome - 50), Math.abs(possAway - 50));
...
function renderFeedModeTabs() {
  return `<div class="feed-mode-tabs" role="tablist" aria-label="Режим матчей">
    <button class="feed-mode-tab ${state.feedMode === "live" ? "active live" : ""}" onclick="setFeedMode('live')"><span></span>LIVE</button>
    <button class="feed-mode-tab ${state.feedMode === "prematch" ? "active prematch" : ""}" onclick="setFeedMode('prematch')">🕒 ПРЕМАТЧ</button>
  </div>`;
}
...
function renderPrematchLeagueGroup(g) {
  return `<div class="league-group prematch-group" data-league-key="${esc(leagueGroupKey(g))}">
    <div class="league-header"><div class="league-flag">${leagueHeaderIcon(g)}</div><div class="league-text"><div class="league-country">${esc(g.country)}</div><div class="league-name">${esc(g.league)}</div></div><div class="league-count">${g.matches.length}</div></div>
    ${g.matches.sort((a,b) => safeInt(a.kickoff_at,0)-safeInt(b.kickoff_at,0)).map(renderPrematchRow).join("")}
  </div>`;
}

function renderSportScoreAttribution() {
  // Provider links are intentionally hidden: all navigation stays inside the bot.
  return "";
}
...
function renderPrematchOverview(d, m) {
  ...
  return `${prematchOddsCard(d.odds || {})}
    <div class="prematch-info-card"><div class="prematch-card-title">Информация о матче</div>
      <div class="prematch-info-lines"><div><span>Начало</span><b>${formatPrematchKickoff(m, true)}</b></div><div><span>Турнир</span><b>${esc(m.league || "—")}</b></div><div><span>Стадион</span><b>${esc([v.name,v.city].filter(Boolean).join(", ") || "—")}</b></div>${v.capacity ? `<div><span>Вместимость</span><b>${safeInt(v.capacity).toLocaleString("ru-RU")}</b></div>` : ""}</div>
    </div>
...
function renderLineupSide(title, formation, players, coach) {
  ...
  return `<div class="lineup-side"><div class="lineup-side-head"><b>${esc(title)}</b><span>${esc(formation || "Схема —")}</span></div>
    <div class="lineup-player-list-v10">${list.map(p => `<div><span class="shirt">${safeInt(p.number) || "·"}</span><b>${esc(p.name || "")}</b>${p.rating != null ? `<em>${esc(p.rating)}</em>` : ""}</div>`).join("") || `<span class="prematch-empty-text">Нет данных</span>`}</div>
...
function renderPrematchDetail() {
  ...
  app.innerHTML = `<div class="detail-screen prematch-detail-screen">
    <div class="detail-topbar"><button class="back-btn" onclick="goBack()">‹</button><div class="detail-breadcrumb">${leagueHeaderIcon({country_code:m.country_code,country:m.country,league:m.league})} ${esc(m.country || "")} · ${esc(m.league || "")}</div><button class="star-btn ${fav ? "active" : ""}" onclick="toggleFavWithGoalPrompt('${esc(m.id)}')">${fav ? "★" : "☆"}</button></div>
    ${renderDetailSwipeHud()}
...
window.toggleGoalWaitAutoGoalEnabled = toggleGoalWaitAutoGoalEnabled;
