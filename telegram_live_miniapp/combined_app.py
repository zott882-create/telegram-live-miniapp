...
PREDICTION_SCAN_STATE_VERSION = "five-minute-multi-market-v4-fixed-total-25"
...
PREDICTION_ODDS_POLICY = "lowest-bookmaker-outcome150to350-plus-fixed-ou25-model50-150to300-v7"
...
def _prediction_row_allowed(row: dict[str, Any] | None) -> bool:
    ...
    if market == 'total':
        ...
        return (
            parts is not None
            and abs(float(parts[1]) - PREDICTION_TOTAL_LINE) < 1e-9
...
def _prediction_evaluate_fixture(
    ...
    *,
    ...
    forecasts: the main 1X2 outcome and the fixed 2.5-goal total.
    This adds fixed ТБ/ТМ 2.5 without doubling request usage.
    """
    now_ts = int(time.time())
    fixture = item.get('fixture') or {}
...
def _prediction_recalculate_payload(result: dict[str, Any], rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    payload.update({
        ...
        'total_line': PREDICTION_TOTAL_LINE,
    })
    return payload
...
def _prediction_recalculate_saved_month_filtered() -> None:
    ...
    predictions/odds check and adds only the model-selected ТБ/ТМ 2.5 when its lowest
