...
PREDICTION_LIVE_REFRESH_SECONDS = max(300, int(os.environ.get("PREDICTION_LIVE_REFRESH_SECONDS", "3600")))
...
def _prediction_feed_for_date(date_text: str) -> dict[str, Any]:
    ...
    def evaluate(item: dict[str, Any]) -> dict[str, Any] | None:
        ...
        probability = _prediction_percent(percent.get('home' if side == 'home' else 'away' if side == 'away' else 'draw'))
        # Only predictions with model probability of at least 50% are included.
        # Do not request odds for weaker predictions, which reduces API usage.
        if probability < PREDICTION_MIN_MODEL_PERCENT:
            return None
        odds_data = _api_call('odds', {'fixture': fixture_id}, odds_ttl, allow_demo=False)
        odd = _prediction_lowest_1x2_odd(odds_data, teams, pick_code)
    ...
    if fixtures:
        with ThreadPoolExecutor(max_workers=min(8, max(1, len(fixtures)))) as pool:
            for result in pool.map(evaluate, fixtures):
                if result:
                    rows.append(result)

    # Every row has an available odd because evaluate() rejects missing odds.
    feed_rows = list(rows)
    ...
    # Daily hit rate and ROI only include completed matches that also have an odd.
...
def _prediction_refresh_today_scores() -> None:
    """Refresh today's scores and recalculate stats without reloading predictions/odds."""
    ...
    if not payload or not (payload.get('feed') or []):
        return
    fixtures_data = _api_call(
        'fixtures',
        {'date': today, 'timezone': APP_TIMEZONE},
        min(10 * 60, max(60, PREDICTION_LIVE_REFRESH_SECONDS // 2)),
        allow_demo=False,
    )
    updated, _ = _prediction_update_archived_payload(payload, fixtures_data.get('response') or [])
    updated['date'] = today
    updated['archived'] = False
    updated['updated_at'] = int(time.time())
    _prediction_storage_save(today, updated, keep_feed=True, is_final=False)
    key = _cache_key('prediction-feed-result-v8-prob50-minodd150', {'date': today})
    _cache_set(key, updated, _prediction_feed_cache_ttl(today))
    print(
        f"[prediction-history] live refresh date={today} "
        f"finished={updated.get('finished_total', 0)} checked={updated.get('predicted_total', 0)} "
...
def _prediction_finalize_old_days() -> None:
    ...
    for record in _prediction_storage_records(with_feed_only=True):
        ...
        try:
            ...
            _prediction_storage_save(date_text, updated, keep_feed=not final_now, is_final=final_now)
            if final_now:
...
def _prediction_recalculate_saved_month_filtered() -> None:
    ...
    for record in pending:
        ...
        if not date_text:
            continue
        cache_key = _cache_key('prediction-feed-result-v8-prob50-minodd150', {'date': date_text})
        try:
            with _lock_for(cache_key):
                ...
                keep_feed = date_text == today
                rebuilt['archived'] = not keep_feed
...
def _prediction_feed_cached(date_text: str, *, force: bool = False) -> dict[str, Any]:
    """Build once, share with everyone, persist daily summaries forever.

    Current-day match cards are retained and merged for the whole day. After
    all matches are settled, only the compact daily summary remains.
    """
    today = _prediction_today()
    record = _prediction_storage_get(date_text)

    # Historical dates never call the expensive per-match endpoints again once
    # a summary has been stored. Their individual match cards are intentionally
    # removed after settlement.
    if date_text < today and record and isinstance(record.get('summary'), dict) and record['summary'].get('odds_policy') == PREDICTION_ODDS_POLICY:
        result = dict(record['summary'])
        result.update({'date': date_text, 'feed': [], 'response': [], 'archived': True, '_persistent_store': 'history'})
        return result

    key = _cache_key('prediction-feed-result-v8-prob50-minodd150', {'date': date_text})
    if not force:
        cached = _cache_get(key)
    ...
    # A container restart should not trigger hundreds of API calls again during
    # the same day. Reuse the persistent full-day payload while it is fresh.
    if date_text == today and not force and record and isinstance(record.get('feed'), dict) and record['feed'].get('odds_policy') == PREDICTION_ODDS_POLICY:
        ...
        if age < _prediction_feed_cache_ttl(date_text):
            ...
            result.update({'archived': False, '_persistent_store': 'today', '_shared_cache': 'persistent-hit'})
            _cache_set(key, result, _prediction_feed_cache_ttl(date_text) - age)
            return result
    ...
    with lock:
        ...
        # A stored historical summary is authoritative even when the refresh
        # button is pressed; refreshing must not spend the API quota again.
        record = _prediction_storage_get(date_text)
        if date_text < today and record and isinstance(record.get('summary'), dict) and record['summary'].get('odds_policy') == PREDICTION_ODDS_POLICY:
            result = dict(record['summary'])
            result.update({'date': date_text, 'feed': [], 'response': [], 'archived': True, '_persistent_store': 'history'})
        ...
        if date_text == today:
            ...
            _prediction_storage_save(date_text, result, keep_feed=True, is_final=False)
        elif date_text < today:
            result['archived'] = True
            _prediction_storage_save(date_text, result, keep_feed=False, is_final=True)
            result['feed'] = []
            result['response'] = []
