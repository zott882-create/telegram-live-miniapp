...
PREDICTION_SCAN_STATE_VERSION = "five-minute-multi-market-v3-total-probability"
...
PREDICTION_ODDS_POLICY = "lowest-bookmaker-outcome150to350-plus-total-model50-150to300-v6"
...
def _prediction_total_code(item: dict[str, Any] | None) -> str:
    predictions = (item or {}).get('predictions') or {}
    return _prediction_total_market_code(predictions.get('under_over'))
...
def _prediction_row_allowed(row: dict[str, Any] | None) -> bool:
    ...
    if market == 'total':
        try:
            ...
            probability = 0.0
        return (
            _prediction_total_parts(row.get('pick_code')) is not None
            and probability >= PREDICTION_TOTAL_MIN_MODEL_PERCENT
            and PREDICTION_TOTAL_MIN_ODD <= odd <= PREDICTION_TOTAL_MAX_ODD
        )
    try:
        probability = float(row.get('probability') or 0.0)
    except Exception:
...
def _prediction_evaluate_fixture(
    ...
    *,
    ...
    """Evaluate both supported markets for one fixture.

    One ``predictions`` call and one ``odds`` call can produce up to two saved
    forecasts: the main 1X2 outcome and API-Football's under/over selection.
    This adds totals without doubling request usage.
    """
    now_ts = int(time.time())
    fixture = item.get('fixture') or {}
    ...
    if not pred_item:
        base_state.update({
            ...
            'markets': {
                '1x2': {'status': 'no_prediction', 'next_check': now_ts + PREDICTION_NO_DATA_RECHECK_SECONDS},
                'total': {'status': 'no_prediction', 'next_check': now_ts + PREDICTION_NO_DATA_RECHECK_SECONDS},
            },
        })
        return [], base_state
    ...
    outcome_ready = pick_code in {'1', 'X', '2'} and probability >= PREDICTION_MIN_MODEL_PERCENT
    if pick_code not in {'1', 'X', '2'}:
        market_states['1x2'] = {'status': 'unsupported_pick', 'next_check': now_ts + PREDICTION_WEAK_MODEL_RECHECK_SECONDS}
    elif probability < PREDICTION_MIN_MODEL_PERCENT:
        market_states['1x2'] = {
            'status': 'weak_model',
    ...
    total_code = _prediction_total_code(pred_item)
    total_parts = _prediction_total_parts(total_code)
    total_probability = _prediction_total_probability(pred_item, total_code) if total_parts else None
    total_ready = bool(
        total_parts
        and total_probability is not None
        and total_probability >= PREDICTION_TOTAL_MIN_MODEL_PERCENT
    )
    if not total_parts:
        market_states['total'] = {
            'status': 'no_total_prediction',
            'next_check': now_ts + PREDICTION_NO_DATA_RECHECK_SECONDS,
        }
    elif total_probability is None:
        market_states['total'] = {
            'status': 'no_model_probability',
            'pick_code': total_code,
            'next_check': now_ts + PREDICTION_WEAK_MODEL_RECHECK_SECONDS,
        }
    elif total_probability < PREDICTION_TOTAL_MIN_MODEL_PERCENT:
        market_states['total'] = {
            'status': 'weak_model',
    ...
    if not outcome_ready and not total_ready:
        pending = [int(value.get('next_check') or 0) for value in market_states.values() if int(value.get('next_check') or 0) > 0]
        base_state.update({
            'status': 'no_eligible_market',
            'next_check': min(pending) if pending else 0,
    ...
    try:
        ...
        for market_name, state in market_states.items():
            if state.get('status') == 'awaiting_odd':
                state['status'] = 'odds_api_error'
                state['next_check'] = now_ts + 15 * 60
        base_state.update({
    ...
    if outcome_ready:
        ...
        state = market_states['1x2']
        if odd is None:
            state.update({'status': 'no_odd', 'next_check': now_ts + PREDICTION_ODDS_RECHECK_SECONDS})
        elif float(odd) < PREDICTION_MIN_ODD:
            state.update({'status': 'below_odd', 'odd': round(float(odd), 3), 'next_check': now_ts + PREDICTION_ODDS_RECHECK_SECONDS})
        elif float(odd) > PREDICTION_MAX_ODD:
            state.update({'status': 'above_odd', 'odd': round(float(odd), 3), 'next_check': now_ts + PREDICTION_ODDS_RECHECK_SECONDS})
        else:
            if side == 'home':
            ...
            rows.append(_prediction_apply_result(row, item))
            state.update({'status': 'included', 'odd': round(float(odd), 3), 'next_check': 0})

    if total_ready:
        total_odd = _prediction_lowest_total_odd(odds_data, total_code)
        state = market_states['total']
        if total_odd is None:
            state.update({'status': 'no_odd', 'next_check': now_ts + PREDICTION_ODDS_RECHECK_SECONDS})
        elif float(total_odd) < PREDICTION_TOTAL_MIN_ODD:
            state.update({'status': 'below_odd', 'odd': round(float(total_odd), 3), 'next_check': now_ts + PREDICTION_ODDS_RECHECK_SECONDS})
        elif float(total_odd) > PREDICTION_TOTAL_MAX_ODD:
            state.update({'status': 'above_odd', 'odd': round(float(total_odd), 3), 'next_check': now_ts + PREDICTION_ODDS_RECHECK_SECONDS})
        else:
            row = {
                **common,
                'market': 'total',
                'pick_code': total_code,
                'pick_label': _prediction_total_label(total_code),
                'probability': round(float(total_probability or 0.0), 2),
                'odd': float(total_odd),
                'prediction_key': f'{fixture_id}:total:{total_code}',
            }
            rows.append(_prediction_apply_result(row, item))
            state.update({'status': 'included', 'odd': round(float(total_odd), 3), 'next_check': 0})

    pending = [int(value.get('next_check') or 0) for value in market_states.values() if int(value.get('next_check') or 0) > 0]
    included_markets = [name for name, value in market_states.items() if value.get('status') == 'included']
    complete = len(included_markets) == 2
    base_state.update({
        'status': 'included_all' if complete else ('included_partial' if included_markets else 'not_included'),
        'next_check': 0 if complete else (min(pending) if pending else 0),
        'markets': market_states,
        'included_markets': included_markets,
        'complete': complete,
    })
    return rows, base_state
...
def _prediction_recalculate_payload(result: dict[str, Any], rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    def sort_key(row: dict[str, Any]) -> tuple[int, str, int, str]:
        ...
        market_pri = 0 if str(row.get('market') or '1x2') == '1x2' else 1
        return (pri, dt, market_pri, str(row.get('pick_code') or ''))
    ...
    market_counts = {
        '1x2': sum(1 for row in feed_rows if str(row.get('market') or '1x2') == '1x2'),
        'total': sum(1 for row in feed_rows if str(row.get('market') or '') == 'total'),
    }
    payload.update({
        ...
        'total_min_odd': PREDICTION_TOTAL_MIN_ODD,
        'total_max_odd': PREDICTION_TOTAL_MAX_ODD,
    })
    return payload
...
def _prediction_refresh_today_scores() -> None:
    """Five-minute incremental refresh of today's forecasts and results.

    A single fixture-list call updates every score. For a fixture that needs a
    forecast check, one predictions call and one odds call can add both the 1X2
    pick and the total pick. Accepted rows keep their original pre-match values;
    only match status and settlement are updated afterwards.
    """
    ...
    markets_by_fixture: dict[int, set[str]] = {}
    for row in updated.get('feed') or []:
        ...
        markets_by_fixture.setdefault(fixture_id, set()).add(str(row.get('market') or '1x2').lower())
    ...
    if payload.get('_prediction_scan_state_version') == PREDICTION_SCAN_STATE_VERSION and isinstance(raw_scan_state, dict):
        ...
        # A policy/version change deliberately rechecks missing markets once.
        scan_state = {}

    for fixture_id, markets in markets_by_fixture.items():
        if {'1x2', 'total'}.issubset(markets):
            scan_state[str(fixture_id)] = {
                'checked_at': int((scan_state.get(str(fixture_id)) or {}).get('checked_at') or now_ts),
                'next_check': 0,
                'status': 'included_all',
                'included_markets': ['1x2', 'total'],
                'complete': True,
            }
        elif str(fixture_id) not in scan_state:
            scan_state[str(fixture_id)] = {
                'checked_at': now_ts,
                'next_check': now_ts,
    ...
    for item in fixtures:
        ...
        if not fixture_id:
            continue
        existing_markets = markets_by_fixture.get(fixture_id, set())
        if {'1x2', 'total'}.issubset(existing_markets):
            continue
        key = str(fixture_id)
        status = str((fixture.get('status') or {}).get('short') or '')
        entry = scan_state.get(key) if isinstance(scan_state.get(key), dict) else None

        # Never create a new forecast after kickoff. Existing accepted forecasts
        # continue to be settled from the score updates above.
        if status not in {'NS', 'TBD'}:
            scan_state[key] = {
                'checked_at': now_ts,
                'next_check': 0,
                'status': 'started_without_all_markets',
                'included_markets': sorted(existing_markets),
                'complete': False,
            }
            continue
        ...
        due = not entry or next_check == 0 or next_check <= now_ts
        if not due:
            continue
    ...
    def evaluate_candidate(candidate: tuple[int, str, dict[str, Any], dict[str, Any] | None]) -> tuple[int, list[dict[str, Any]], dict[str, Any]]:
        ...
        # Keep retries economical for leagues where predictions or odds are slow.
    ...
    if candidates:
        with ThreadPoolExecutor(max_workers=min(8, max(1, len(candidates)))) as pool:
            for fixture_id, result_rows, state in pool.map(evaluate_candidate, candidates):
                if not fixture_id:
                    continue
                existing_markets = markets_by_fixture.setdefault(fixture_id, set())
                for row in result_rows:
                    ...
                    if key not in existing_rows:
                        ...
                        added += 1
                included = set(state.get('included_markets') or []) | existing_markets
                state['included_markets'] = sorted(included)
                if {'1x2', 'total'}.issubset(included):
                    state['complete'] = True
                    state['status'] = 'included_all'
                    state['next_check'] = 0
    ...
    key = _cache_key('prediction-feed-result-v10-totals', {'date': today})
    _cache_set(key, updated, _prediction_feed_cache_ttl(today))
    print(
        f"[prediction-history] 5m refresh date={today} fixtures={len(fixtures)} "
...
def _prediction_recalculate_saved_month_filtered() -> None:
    """Upgrade saved current-month rows to the outcome + total policy.

    Existing 1X2 forecasts are preserved exactly as they were recorded. For a
    saved match without a total row, the migration performs at most one shared
    predictions/odds check and adds the API-Football total only when its lowest
    bookmaker odd is between 1.50 and 3.00. This is intentionally limited to
    fixtures already present in history so an upgrade does not rescan thousands
    of unrelated past matches.
    """
    if API_FOOTBALL_DEMO or not API_FOOTBALL_KEY:
        return
    today = _prediction_today()
    month_prefix = today[:7] + '-'
    records = [r for r in _prediction_storage_records() if str(r.get('date') or '').startswith(month_prefix)]
    records.sort(key=lambda r: str(r.get('date') or ''))
    pending = [r for r in records if (r.get('summary') or {}).get('odds_policy') != PREDICTION_ODDS_POLICY]
    if not pending:
        return
    print(f"[prediction-history] outcome + totals migration dates={len(pending)}")
    for record in pending:
        date_text = str(record.get('date') or '')
        if not date_text:
            continue
        cache_key = _cache_key('prediction-feed-result-v10-totals', {'date': date_text})
        try:
            with _lock_for(cache_key):
                latest = _prediction_storage_get(date_text)
                if latest and (latest.get('summary') or {}).get('odds_policy') == PREDICTION_ODDS_POLICY:
                    continue
                existing_feed = latest.get('feed') if latest and isinstance(latest.get('feed'), dict) else None
                if existing_feed and (existing_feed.get('feed') or []):
                    existing_rows = [dict(row) for row in (existing_feed.get('feed') or [])]
                    total_fixture_ids = {
                        int(row.get('fixture_id') or 0)
                        for row in existing_rows
                        if str(row.get('market') or '1x2').lower() == 'total'
                    }
                    fixture_items: dict[int, dict[str, Any]] = {}
                    for row in existing_rows:
                        try:
                            fixture_id = int(row.get('fixture_id') or 0)
                        except Exception:
                            fixture_id = 0
                        fixture_item = row.get('fixture_item')
                        if fixture_id and fixture_id not in total_fixture_ids and isinstance(fixture_item, dict):
                            fixture_items[fixture_id] = fixture_item

                    def evaluate_total(item: dict[str, Any]) -> list[dict[str, Any]]:
                        rows, _ = _prediction_evaluate_fixture(
                            item,
                            pred_ttl=12 * 3600,
                            odds_ttl=12 * 3600,
                        )
                        return [row for row in rows if str(row.get('market') or '') == 'total']

                    added_totals: list[dict[str, Any]] = []
                    items = list(fixture_items.values())
                    if items:
                        with ThreadPoolExecutor(max_workers=min(6, max(1, len(items)))) as pool:
                            for rows in pool.map(evaluate_total, items):
                                added_totals.extend(rows)
                    rebuilt = _prediction_recalculate_payload(existing_feed, existing_rows + added_totals)
                    rebuilt['date'] = date_text
                    rebuilt['_prediction_scan_state_version'] = PREDICTION_SCAN_STATE_VERSION
                else:
                    rebuilt = _prediction_feed_for_date(date_text)
                rebuilt['odds_policy'] = PREDICTION_ODDS_POLICY
                rebuilt['min_model_percent'] = PREDICTION_MIN_MODEL_PERCENT
                rebuilt['min_odd'] = PREDICTION_MIN_ODD
                rebuilt['max_odd'] = PREDICTION_MAX_ODD
                rebuilt['total_min_odd'] = PREDICTION_TOTAL_MIN_ODD
                rebuilt['total_max_odd'] = PREDICTION_TOTAL_MAX_ODD
                rebuilt['updated_at'] = int(time.time())
                rebuilt['archived'] = date_text < today
                rebuilt['history_retention'] = PREDICTION_HISTORY_RETENTION_POLICY
                _prediction_storage_save(date_text, rebuilt, keep_feed=True, is_final=date_text < today)
                cache_payload = dict(rebuilt)
                cache_payload['archived'] = date_text < today
                cache_payload['history_retention'] = PREDICTION_HISTORY_RETENTION_POLICY
                _cache_set(cache_key, cache_payload, _prediction_feed_cache_ttl(date_text))
                print(
                    f"[prediction-history] recalculated outcome+totals date={date_text} "
                    f"settled={rebuilt.get('predicted_total', 0)} totals={(rebuilt.get('market_counts') or {}).get('total', 0)} "
                    f"roi={rebuilt.get('roi_percent', 0)}%"
                )
        except Exception as exc:
            print(f"[prediction-history] outcome + totals migration failed date={date_text}: {exc}")
        time.sleep(1.0)
...
def _prediction_feed_cached(date_text: str, *, force: bool = False) -> dict[str, Any]:
    ...
    key = _cache_key('prediction-feed-result-v10-totals', {'date': date_text})
...
def start_notification_runtime() -> None:
    ...
    start_prediction_history_worker()
