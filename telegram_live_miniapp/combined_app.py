...
def _prediction_best_1x2_odd(payload: dict[str, Any] | None, teams: dict[str, Any] | None, code: str) -> float | None:
...
def _prediction_audit_for_date(date_text: str) -> dict[str, Any]:
    ...
    def evaluate(item: dict[str, Any]) -> dict[str, Any] | None:
        ...
        odd = _prediction_best_1x2_odd(odds_data, teams, pick_code)
...
def _prediction_refresh_today_scores() -> None:
    ...
    key = _cache_key('prediction-feed-result-v5', {'date': today})
...
def _prediction_feed_cached(date_text: str, *, force: bool = False) -> dict[str, Any]:
    ...
    if date_text < today and record and isinstance(record.get('summary'), dict):
        result = dict(record['summary'])
        result.update({'date': date_text, 'feed': [], 'response': [], 'archived': True, '_persistent_store': 'history'})
        return result

    key = _cache_key('prediction-feed-result-v5', {'date': date_text})
    if not force:
        cached = _cache_get(key)
    ...
    if date_text == today and not force and record and isinstance(record.get('feed'), dict):
