...
def _scores24_resolve_api_fixture(row: dict[str, Any], force: bool = False) -> dict[str, Any] | None:
    ...
    if known_id:
        # Once a link is known, refresh the fixture directly instead of trying
        # to rediscover it by date and translated team names.
        try:
            direct = _api_call(
                "fixtures",
...
def _scores24_wsgi(environ: dict[str, Any], start_response: Callable[..., Any]) -> list[bytes]:
    ...
    try:
        ...
        if path == "/scores24/prediction-feed":
            ...
            force = params.get("refresh", "").strip().lower() in {"1", "true", "yes"}
            payload = (
                scores24_predictions.feed(date_text, force=True)
                if force
                else scores24_predictions.cached_feed(date_text)
            )
