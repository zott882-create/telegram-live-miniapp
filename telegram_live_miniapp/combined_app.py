...
PREDICTION_ODDS_POLICY = "lowest-bookmaker-prob50-minodd150-v3"
...
def _prediction_audit_for_date(date_text: str) -> dict[str, Any]:
    ...
    def evaluate(item: dict[str, Any]) -> dict[str, Any] | None:
        ...
        # A prediction without an available 1X2 odd, or with an odd below 1.50,
        # must not appear or affect statistics.
        if odd is None or float(odd) < PREDICTION_MIN_ODD:
            return None
...
def _prediction_recalculate_payload(result: dict[str, Any], rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    feed_rows = [
        ...
        and float(row.get('odd') or 0.0) >= PREDICTION_MIN_ODD
    ]
    def sort_key(row: dict[str, Any]) -> tuple[int, str]:
        fixture = (row.get('fixture_item') or {}).get('fixture') or {}
        dt = str(fixture.get('date') or '')
...
def _prediction_recalculate_saved_month_filtered() -> None:
    ...
    if not pending:
        return
    print(f"[prediction-history] 50% + КФ 1.50 migration dates={len(pending)}")
    for record in pending:
        ...
        try:
            with _lock_for(cache_key):
                ...
                print(
                    f"[prediction-history] recalculated 50%+ КФ>=1.50 date={date_text} "
                    f"checked={rebuilt.get('predicted_total', 0)} roi={rebuilt.get('roi_percent', 0)}%"
                )
        except Exception as exc:
            print(f"[prediction-history] 50% + КФ 1.50 migration failed date={date_text}: {exc}")
        time.sleep(1.0)
