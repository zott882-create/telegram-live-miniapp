...
PREDICTION_ODDS_POLICY = "lowest-bookmaker-v1"
...
def _prediction_audit_for_date(date_text: str) -> dict[str, Any]:
    ...
    def evaluate(item: dict[str, Any]) -> dict[str, Any] | None:
        ...
        # A prediction without an available 1X2 odd must not appear or affect statistics.
        if odd is None:
            return None
...
def _prediction_refresh_today_scores() -> None:
    ...
    key = _cache_key('prediction-feed-result-v6-lowest', {'date': today})
...
def _prediction_recalculate_saved_month_lowest_odds() -> None:
    """One-time migration of saved current-month statistics to lowest bookmaker odds."""
    if API_FOOTBALL_DEMO or not API_FOOTBALL_KEY:
        return
    today = _prediction_today()
    ...
    if not pending:
        return
    print(f"[prediction-history] lowest-odds migration dates={len(pending)}")
    for record in pending:
        date_text = str(record.get('date') or '')
        if not date_text:
            continue
        cache_key = _cache_key('prediction-feed-result-v6-lowest', {'date': date_text})
        try:
            with _lock_for(cache_key):
                ...
                print(
                    f"[prediction-history] recalculated lowest odds date={date_text} "
                    f"checked={rebuilt.get('predicted_total', 0)} roi={rebuilt.get('roi_percent', 0)}%"
                )
        except Exception as exc:
            print(f"[prediction-history] lowest-odds migration failed date={date_text}: {exc}")
        time.sleep(1.0)


def _prediction_history_worker() -> None:
    _prediction_recalculate_saved_month_lowest_odds()
...
def _prediction_feed_cached(date_text: str, *, force: bool = False) -> dict[str, Any]:
    ...
    key = _cache_key('prediction-feed-result-v6-lowest', {'date': date_text})
    if not force:
        cached = _cache_get(key)
