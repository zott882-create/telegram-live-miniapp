...
def _number_stat(value: Any) -> float:
    if value is None:
        return 0.0
    try:
...
def _fixture_pressure_snapshot(fixture_id: str, elapsed: int | None = None) -> dict[str, Any]:
    """Build a derived pressure/activity point from API-Football fixture statistics.

    API-Football does not expose a native pressure timeline. We derive a live
    balance from available official fields (shots on goal, total/blocked shots,
    corners and possession) and keep only snapshots requested while a match is
    open. This avoids background polling of every fixture.
    """
    if API_FOOTBALL_DEMO or not API_FOOTBALL_KEY:
        stats_payload = {"response": _demo_stats(), "demo": True, "_quota": _quota_snapshot()}
    else:
        stats_payload = _api_call("fixtures/statistics", {"fixture": fixture_id}, 90, allow_demo=False)

    teams = stats_payload.get("response") or []
    if len(teams) < 2:
        return {
            "ok": True,
            "available": False,
            "reason": "Статистика для этого матча пока не поступила",
            "points": [],
            "_quota": _quota_snapshot(),
        }

    def team_values(item: dict[str, Any]) -> dict[str, float]:
        values = {str(x.get("type") or ""): _number_stat(x.get("value")) for x in (item.get("statistics") or [])}
        return {
            "shots_on_goal": values.get("Shots on Goal", 0.0),
            "total_shots": values.get("Total Shots", 0.0),
            "blocked_shots": values.get("Blocked Shots", 0.0),
            "corners": values.get("Corner Kicks", 0.0),
            "possession": values.get("Ball Possession", 0.0),
        }

    home_values = team_values(teams[0])
    away_values = team_values(teams[1])
    usable = any(value > 0 for value in [*home_values.values(), *away_values.values()])
    if not usable:
        return {
            "ok": True,
            "available": False,
            "reason": "API-Football не передал показатели для расчёта давления",
            "points": [],
            "teams": [teams[0].get("team") or {}, teams[1].get("team") or {}],
            "_quota": _quota_snapshot(),
        }

    def score(values: dict[str, float]) -> float:
        # The result is a relative activity index, not a bookmaker prediction.
        return (
            values["shots_on_goal"] * 5.0
            + values["total_shots"] * 1.7
            + values["blocked_shots"] * 1.2
            + values["corners"] * 2.4
            + values["possession"] * 0.08
        )

    home_score = score(home_values)
    away_score = score(away_values)
    total = home_score + away_score
    home_percent = int(round(home_score / total * 100)) if total > 0 else 50
    home_percent = max(5, min(95, home_percent))
    away_percent = 100 - home_percent
    now = int(time.time())
    minute = max(0, int(elapsed or 0))

    history_key = f"{CACHE_PREFIX}:pressure-history:{fixture_id}"
    history = _cache_get(history_key)
    ...
    point = {
        ...
        "home": home_percent,
        "away": away_percent,
        "raw": {"home": home_values, "away": away_values},
    }
    last = history[-1] if history else None
    should_append = not last or int(last.get("minute") or -1) != minute or now - int(last.get("captured_at") or 0) >= 75
    if should_append:
        history.append(point)
        history = history[-24:]
        _cache_set(history_key, history, 6 * 3600)
    elif history:
        history[-1] = point
        _cache_set(history_key, history, 6 * 3600)
    ...
    return {
        ...
        "explanation": "Рассчитано из ударов, ударов в створ, блокированных ударов, угловых и владения мячом.",
        "teams": [teams[0].get("team") or {}, teams[1].get("team") or {}],
        "points": history,
        "current": point,
        "demo": bool(stats_payload.get("demo")),
        "_quota": _quota_snapshot(),
    }
...
def _demo_team_profile(team_id: str, league_id: str = "39", season: int = 2026) -> dict[str, Any]:
    ...
    statistics = {
        ...
        "goals": {
            "for": {"total": {"home": 19, "away": 13, "total": 32}, "average": {"home": "2.4", "away": "1.9", "total": "2.1"}},
            "against": {"total": {"home": 7, "away": 8, "total": 15}, "average": {"home": "0.9", "away": "1.1", "total": "1.0"}},
        },
    ...
    return {"team": item, "last": fixtures[:2], "next": fixtures[1:], "statistics": statistics, "standing": standing, "context": {"league": int(league_id or 39), "season": season}, "demo": True, "_quota": _quota_snapshot()}
...
def _demo_league_view(league_id: str, season: int) -> dict[str, Any]:
    ...
    for i, item in enumerate(teams, start=1):
        standings.append({"rank": i, "team": item["team"], "points": max(1, 18 - i * 2), "goalsDiff": 8 - i, "form": "WWDLW", "all": {"played": 8, "win": 5, "draw": 2, "lose": 1, "goals": {"for": 15, "against": 7}}})
    fixtures = _demo_fixtures(datetime.now().strftime("%Y-%m-%d"))["response"]
    return {"league": league_item, "season": season, "standings": standings, "next": fixtures, "demo": True, "_quota": _quota_snapshot()}
    ...
    try:
        ...
        if path == "/api-football/team-profile":
            ...
            return _json_response(start_response, {
                ...
                "errors": team_data.get("errors") or last_data.get("errors") or next_data.get("errors") or statistics_errors or standings_errors or {},
                "_quota": _quota_snapshot(),
            })
        ...
        if path == "/api-football/league-view":
            ...
            return _json_response(start_response, {
                ...
                "season": season,
                "standings": table,
                "next": next_data.get("response") or [],
        ...
        if path == "/api-football/pressure":
            fixture_id = params.get("fixture", "").strip()
            if not fixture_id.isdigit():
                return _json_response(start_response, {"ok": False, "error": "invalid fixture id"}, 400)
            try:
                elapsed = int(params.get("elapsed", "0") or 0)
            except Exception:
                elapsed = 0
            return _json_response(start_response, _fixture_pressure_snapshot(fixture_id, elapsed))
