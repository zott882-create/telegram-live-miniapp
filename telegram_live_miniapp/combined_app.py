...
PREDICTION_SCAN_STATE_VERSION = "five-minute-incremental-v1"
...
PREDICTION_ODDS_POLICY = "lowest-bookmaker-prob50-odd150to350-v4"
...
def _prediction_market_kind(name: Any) -> str:
    ...
    if 'double chance' in n or 'double result' in n or 'двойной шанс' in n:
        return 'double'
    if 'over under' in n or 'total goals' in n or 'goals over under' in n or 'тотал' in n:
        return 'total'
...
def _prediction_audit_for_date(date_text: str) -> dict[str, Any]:
    ...
    for item in fixtures:
        ...
        if str(status) in {'FT', 'AET', 'PEN'} and _fixture_result_code(item) in {'1', 'X', '2'}:
            finished_items.append(item)

    is_past = date_text < _prediction_today()
    pred_ttl = 12 * 3600 if is_past else 3600
    odds_ttl = 12 * 3600 if is_past else 1800

    def evaluate(item: dict[str, Any]) -> dict[str, Any] | None:
        fixture = item.get('fixture') or {}
        fixture_id = str(fixture.get('id') or '').strip()
        if not fixture_id.isdigit():
            return None
        teams = item.get('teams') or {}
        pred_data = _api_call('predictions', {'fixture': fixture_id}, pred_ttl, allow_demo=False)
        ...
        # Only predictions with model probability of at least 50% are included.
        # This check is intentionally before the odds call to save API requests.
        if probability < PREDICTION_MIN_MODEL_PERCENT:
            return None
        odds_data = _api_call('odds', {'fixture': fixture_id}, odds_ttl, allow_demo=False)
        odd = _prediction_lowest_1x2_odd(odds_data, teams, pick_code)
        # A prediction without an available 1X2 odd, below 1.50 or above 3.50,
        # must not appear or affect statistics.
        if odd is None or not (PREDICTION_MIN_ODD <= float(odd) <= PREDICTION_MAX_ODD):
            return None
        result_code = _fixture_result_code(item)
        won = bool(result_code and pick_code == result_code)
        stake = 1.0
        returned = float(odd) if won else 0.0
        return {
    ...
    if finished_items:
        with ThreadPoolExecutor(max_workers=min(8, max(1, len(finished_items)))) as pool:
            for result in pool.map(evaluate, finished_items):
                if result:
                    rows.append(result)

    rows.sort(key=lambda x: (str(x.get('league') or ''), str(x.get('home') or ''), str(x.get('away') or '')))
    checked = len(rows)
    won = sum(1 for row in rows if row.get('won'))
    lost = max(0, checked - won)
    hit_rate = round((won / checked) * 100, 2) if checked else 0.0
    stake_total = round(sum(float(row.get('stake') or 0.0) for row in rows), 2)
    returned_total = round(sum(float(row.get('returned') or 0.0) for row in rows), 2)
    profit = round(returned_total - stake_total, 2)
    roi = round((profit / stake_total) * 100, 2) if stake_total > 0 else 0.0
    odds_count = checked
    return {
        'date': date_text,
...
def _prediction_evaluate_fixture(
    ...
    *,
    ...
    """Evaluate one fixture and return both a feed row and scanner metadata.

    Scanner metadata is persisted even for fixtures that do not qualify. That
    lets the five-minute worker avoid requesting the same unavailable forecast
    288 times per day while still retrying later when data can reasonably change.
    """
    now_ts = int(time.time())
    fixture = item.get('fixture') or {}
    fixture_id = str(fixture.get('id') or '').strip()
...
def _prediction_feed_for_date(date_text: str) -> dict[str, Any]:
    ...
    def evaluate(item: dict[str, Any]) -> tuple[int, dict[str, Any] | None, dict[str, Any]]:
        fixture_id = int(((item.get('fixture') or {}).get('id')) or 0)
        row, scan = _prediction_evaluate_fixture(item, pred_ttl=pred_ttl, odds_ttl=odds_ttl)
        return fixture_id, row, scan

    rows: list[dict[str, Any]] = []
    scan_state: dict[str, dict[str, Any]] = {}
    if fixtures:
        with ThreadPoolExecutor(max_workers=min(8, max(1, len(fixtures)))) as pool:
            for fixture_id, row, scan in pool.map(evaluate, fixtures):
                if fixture_id:
    ...
    settled_rows = [row for row in rows if row.get('result_code') in {'1', 'X', '2'}]
    checked = len(settled_rows)
    won = sum(1 for row in settled_rows if row.get('won'))
    lost = max(0, checked - won)
    hit_rate = round((won / checked) * 100, 2) if checked else 0.0
    stake_total = round(sum(float(row.get('stake') or 0.0) for row in settled_rows), 2)
    returned_total = round(sum(float(row.get('returned') or 0.0) for row in settled_rows), 2)
    profit = round(returned_total - stake_total, 2)
    roi = round((profit / stake_total) * 100, 2) if stake_total > 0 else 0.0
    return {
        'date': date_text,
        'fixtures_total': len(fixtures),
        'finished_total': sum(1 for item in fixtures if str((((item.get('fixture') or {}).get('status') or {}).get('short')) or '') in {'FT', 'AET', 'PEN'}),
        'predicted_total': checked,
        'won_total': won,
        'lost_total': lost,
        'hit_rate': hit_rate,
...
def _prediction_recalculate_payload(result: dict[str, Any], rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    feed_rows = [
        ...
        if float(row.get('probability') or 0.0) >= PREDICTION_MIN_MODEL_PERCENT
        and row.get('odd') is not None
        and PREDICTION_MIN_ODD <= float(row.get('odd') or 0.0) <= PREDICTION_MAX_ODD
    ]
    def sort_key(row: dict[str, Any]) -> tuple[int, str]:
        fixture = (row.get('fixture_item') or {}).get('fixture') or {}
        dt = str(fixture.get('date') or '')
        ...
        return (pri, dt)
    feed_rows.sort(key=sort_key)
    settled = [row for row in feed_rows if row.get('result_code') in {'1', 'X', '2'} and row.get('odd') is not None]
    checked = len(settled)
    won = sum(1 for row in settled if row.get('won'))
    lost = max(0, checked - won)
...
def _prediction_merge_day_payload(old_payload: dict[str, Any] | None, new_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for row in old_payload.get('feed') or []:
        try:
            merged[int(row.get('fixture_id') or 0)] = dict(row)
        except Exception:
            continue
    for row in new_payload.get('feed') or []:
        ...
        if not fixture_id:
            continue
        previous = merged.get(fixture_id) or {}
        combined = dict(previous)
        combined.update(row)
        merged[fixture_id] = combined
...
def _prediction_update_archived_payload(payload: dict[str, Any], fixtures: list[dict[str, Any]]) -> tuple[dict[str, Any], bool]:
    ...
    for original in payload.get('feed') or []:
        ...
        if current:
            row['fixture_item'] = current
            result_code = _fixture_result_code(current)
            row['result_code'] = result_code or ''
            row['won'] = bool(result_code and str(row.get('pick_code') or '') == result_code)
            row['stake'] = 1.0
            row['returned'] = float(row.get('odd') or 0.0) if row['won'] else 0.0
        if row.get('result_code') not in {'1', 'X', '2'}:
            all_settled = False
        rows.append(row)
    updated = dict(payload)
    updated['finished_total'] = sum(
        1 for item in fixtures or []
        if str((((item.get('fixture') or {}).get('status') or {}).get('short')) or '') in {'FT', 'AET', 'PEN'}
    )
...
def _prediction_refresh_today_scores() -> None:
    ...
    One fixtures request updates all scores at once. Predictions and odds are
    requested only for newly discovered fixtures or for previously unavailable
    entries whose retry time has arrived. Already accepted forecasts keep their
    original pre-match probability and odd forever; only the score/result changes.
    """
    ...
    # A TTL slightly below the five-minute loop guarantees one fresh fixture
    # request per cycle while still deduplicating simultaneous browser requests.
    ...
    for row in updated.get('feed') or []:
        try:
            ...
            fixture_id = 0
        if fixture_id:
            existing_rows[fixture_id] = dict(row)

    raw_scan_state = payload.get('_prediction_scan_state')
    scan_state: dict[str, dict[str, Any]] = dict(raw_scan_state) if isinstance(raw_scan_state, dict) else {}
    for fixture_id, row in existing_rows.items():
        scan_state[str(fixture_id)] = {
            ...
            'next_check': 0,
            'status': 'included',
            'probability': round(float(row.get('probability') or 0.0), 2),
            'pick_code': str(row.get('pick_code') or ''),
            'odd': round(float(row.get('odd') or 0.0), 3),
        }
    ...
    for item in fixtures:
        ...
        try:
            ...
            fixture_id = 0
        if not fixture_id or fixture_id in existing_rows:
            continue
        key = str(fixture_id)
        status = str((fixture.get('status') or {}).get('short') or '')
        entry = scan_state.get(key) if isinstance(scan_state.get(key), dict) else None

        # A new forecast must be recorded before kickoff. This prevents a late
        # provider response from being counted retroactively after play started.
        if status not in {'NS', 'TBD'}:
            if not entry or int(entry.get('next_check') or 0) > 0:
                scan_state[key] = {
                    'checked_at': now_ts,
                    'next_check': 0,
        ...
        due = not entry or int(entry.get('next_check') or 0) <= now_ts
        if not due:
            continue
    ...
    def evaluate_candidate(candidate: tuple[int, str, dict[str, Any], dict[str, Any] | None]) -> tuple[int, dict[str, Any] | None, dict[str, Any]]:
        _, _, item, previous = candidate
        fixture_id = int(((item.get('fixture') or {}).get('id')) or 0)
        is_retry = bool(previous)
        row, state = _prediction_evaluate_fixture(
            item,
        ...
        # Exponential retry backoff: the worker still wakes every five minutes,
        # but unsupported leagues are not hammered every cycle.
        ...
        elif status in {'no_odd', 'below_odd'}:
            delay = min(3 * 3600, PREDICTION_ODDS_RECHECK_SECONDS * (2 ** max(0, attempts - 1)))
            state['next_check'] = int(time.time()) + delay
        elif status in {'api_error', 'odds_api_error'}:
            state['next_check'] = int(time.time()) + min(3600, 15 * 60 * attempts)
        return fixture_id, row, state

    added = 0
    if candidates:
        with ThreadPoolExecutor(max_workers=min(8, max(1, len(candidates)))) as pool:
            for fixture_id, row, state in pool.map(evaluate_candidate, candidates):
                if fixture_id:
                    scan_state[str(fixture_id)] = state
                if row and fixture_id:
                    existing_rows[fixture_id] = row
                    added += 1
    ...
    updated.update({
        ...
        '_prediction_scan_cycle': {
            ...
            'pending_candidates': max(0, len(candidates)),
        },
    })
    _prediction_storage_save(today, updated, keep_feed=True, is_final=False)
    key = _cache_key('prediction-feed-result-v9-full-history', {'date': today})
...
def _prediction_recalculate_saved_month_filtered() -> None:
    """One-time migration of saved current-month statistics to the current prediction policy."""
    if API_FOOTBALL_DEMO or not API_FOOTBALL_KEY:
        return
    today = _prediction_today()
    ...
    if not pending:
        return
    print(f"[prediction-history] 50% + КФ 1.50–3.50 migration dates={len(pending)}")
    for record in pending:
        date_text = str(record.get('date') or '')
        if not date_text:
            continue
        cache_key = _cache_key('prediction-feed-result-v9-full-history', {'date': date_text})
        try:
            with _lock_for(cache_key):
                ...
                if existing_feed and (existing_feed.get('feed') or []):
                    filtered_rows = [
                        row for row in (existing_feed.get('feed') or [])
                        if float(row.get('probability') or 0.0) >= PREDICTION_MIN_MODEL_PERCENT
                        and row.get('odd') is not None
                        and PREDICTION_MIN_ODD <= float(row.get('odd') or 0.0) <= PREDICTION_MAX_ODD
                    ]
                    rebuilt = _prediction_recalculate_payload(existing_feed, filtered_rows)
                ...
                _prediction_storage_save(date_text, rebuilt, keep_feed=keep_feed, is_final=date_text < today)
                ...
                print(
                    f"[prediction-history] recalculated 50%+ КФ 1.50–3.50 date={date_text} "
                    f"checked={rebuilt.get('predicted_total', 0)} roi={rebuilt.get('roi_percent', 0)}%"
                )
        except Exception as exc:
            print(f"[prediction-history] 50% + КФ 1.50–3.50 migration failed date={date_text}: {exc}")
        time.sleep(1.0)
...
def _prediction_feed_cached(date_text: str, *, force: bool = False) -> dict[str, Any]:
    ...
    key = _cache_key('prediction-feed-result-v9-full-history', {'date': date_text})
