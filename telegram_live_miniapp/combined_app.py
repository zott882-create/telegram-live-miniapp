...
def _team_trend_stats(fixtures: list[dict[str, Any]], team_id: str) -> dict[str, int]:
    ...
    for item in fixtures:
        ...
        margin = gf - ga
        out["played"] += 1
        out["unbeaten"] += int(margin >= 0)
        out["wins"] += int(margin > 0)
        out["scored"] += int(gf > 0)
        out["conceded"] += int(ga > 0)
        out["btts"] += int(gf > 0 and ga > 0)
        # +1 includes a push when the team loses by exactly one goal.
        out["cover_plus1"] += int(margin >= -1)
        # +1.5 wins when the team loses by no more than one goal.
        out["cover_plus15"] += int(margin >= -1)
        # -1 wins outright only when the team wins by two or more goals.
...
def _build_trends(home_id: str, away_id: str, home_name: str, away_name: str) -> dict[str, Any]:
    if API_FOOTBALL_DEMO or not API_FOOTBALL_KEY:
        home = {"played": 20, "unbeaten": 19, "scored": 18, "conceded": 11, "btts": 10, "wins": 15, "cover_plus1": 20, "cover_plus15": 20, "win_by2": 8}
        away = {"played": 20, "unbeaten": 14, "scored": 17, "conceded": 16, "btts": 13, "wins": 9, "cover_plus1": 17, "cover_plus15": 17, "win_by2": 4}
        demo = True
    else:
            ...
            "items": [
                {
                    ...
                    "notes": [
                        ...
                        {"team": "away", "text": f"{away_name} побеждает минимум в 2 мяча", "stat": phrase(away["win_by2"], away["played"])},
                    ],
                ...
                },
                {
                    ...
                    "notes": [
                        ...
                        {"team": "home", "text": f"{home_name} побеждает минимум в 2 мяча", "stat": phrase(home["win_by2"], home["played"])},
                    ],
                ...
                },
                {
                    "label": f"{home_name} с форой (-1)",
                    "notes": [
                        {"team": "home", "text": f"{home_name} выигрывает минимум в 2 мяча", "stat": phrase(home["win_by2"], home["played"])},
                    ],
                    "pick": f"{home_name} Фора (-1)",
                },
            ],
...
def _demo_player_profile(player_id: str, season: int = 2026, fixture_id: str = "", team_id: str = "") -> dict[str, Any]:
    pid = int(player_id)
    player = {
        "id": pid,
    ...
    league = {"id": 39, "name": "Premier League", "country": "England", "logo": "https://media.api-sports.io/football/leagues/39.png", "season": season}
    season_stats = {
        "team": team,
        "league": league,
    ...
    return {
        "profile": {"player": player, "statistics": [season_stats]},
...
def _api_football_wsgi(environ: dict[str, Any], start_response: Callable[..., Any]) -> list[bytes]:
    ...
    try:
        ...
        if path == "/api-football/player-profile":
            ...
            if API_FOOTBALL_DEMO or not API_FOOTBALL_KEY:
                return _json_response(start_response, _demo_player_profile(player_id, season, fixture_id, team_id))

            player_data = _api_call("players", {"id": player_id, "season": season}, 12 * 3600, allow_demo=False)
            player_items = player_data.get("response") or []
            profile = player_items[0] if player_items else None
            match_player = None
            match_errors: Any = {}
            if fixture_id.isdigit():
                fixture_players = _api_call("fixtures/players", {"fixture": fixture_id}, 5 * 60, allow_demo=False)
            ...
            return _json_response(start_response, {
                ...
                "match": match_player,
                "season": season,
                "errors": player_data.get("errors") or match_errors or {},
                "_quota": _quota_snapshot(),
            })
