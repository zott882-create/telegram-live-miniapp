...
HTTP_TIMEOUT = max(3.0, min(20.0, float(os.environ.get("SPORTSCORE_HTTP_TIMEOUT", "8"))))
...
PREMATCH_WORKERS = max(1, min(7, int(os.environ.get("SPORTSCORE_PREMATCH_WORKERS", "4"))))
...
def _cached(key: str, ttl: int, loader):
    ...
    try:
        ...
        # A short provider outage should not blank the whole prematch screen.
        # Return the last snapshot; the caller still applies a fresh time/status
        # filter before exposing it to the Mini App.
...
def _nearest_fixture_section(node: Any) -> str:
    """Return scheduled/live/finished from the nearest preceding section title.

    SportScore's main football page contains all three blocks in one document.
    Query parameters are not enough to guarantee that only one block is
    rendered, so section context is a much safer discriminator than row text.
    """
    if node is None or not hasattr(node, "find_all_previous"):
        return ""
    for heading in node.find_all_previous(["h1", "h2", "h3", "h4"], limit=8):
        text = " ".join(heading.get_text(" ", strip=True).lower().split())
        if re.search(r"\b(scheduled|upcoming|fixtures?)\b", text):
            return "scheduled"
        if re.search(r"\b(live|in[- ]?play)\b", text):
            return "live"
        if re.search(r"\b(finished|results?|completed)\b", text):
            return "finished"
    return ""


def _row_status_blob(container: Any, node: Any) -> str:
    parts: list[str] = []
    for el in (node, container):
        attrs = getattr(el, "attrs", {}) or {}
        classes = attrs.get("class") or []
        if isinstance(classes, str):
            classes = [classes]
        parts.extend(str(x) for x in classes)
        for key in (
            "data-status", "data-state", "data-event-status", "data-match-status",
            "aria-label", "title",
        ):
            value = attrs.get(key)
    ...
    if container is not None and hasattr(container, "select"):
        for el in container.select(
            ".status, .match-status, .fixture-status, .football-match-status, "
            "[data-status], [data-state], [data-event-status], [data-match-status]"
        ):
...
def _row_is_not_prematch(container: Any, node: Any) -> bool:
    section = _nearest_fixture_section(node)
    if section in {"live", "finished"}:
        return True
    blob = _row_status_blob(container, node)
    terminal = (
        "finished", "completed", "full-time", "full time", "ended", "cancelled",
        "postponed", "abandoned", "eventcompleted", "eventcancelled",
    )
    live = (
        " live ", "inplay", "in-play", "in_play", "started", "halftime",
        "half-time", "eventinprogress",
    )
    padded = f" {blob} "
    return any(token in padded for token in terminal) or any(token in padded for token in live)
...
def parse_upcoming_html(text: str, default_date: dt.date | None = None) -> list[dict[str, Any]]:
    ...
    nodes: list[Any] = list(soup.select(
        'a[href*="/football/match/"], [data-match-url*="/football/match/"], '
        '[data-url*="/football/match/"], [data-href*="/football/match/"], '
        '[data-fixture-url*="/football/match/"], '
    ...
    for node in nodes:
        ...
        if container is None:
            continue
        if _row_is_not_prematch(container, node):
            continue
        ...
        if hasattr(container, "select"):
            for img in container.select('img[src]'):
                ...
                if src and src not in logos:
                    logos.append(src)
        href = str(
            node.get("href") or node.get("data-match-url") or node.get("data-url")
            or node.get("data-href") or node.get("data-fixture-url")
            or f"/football/match/{slug}/"
        )
        obj: dict[str, Any] = {
            "home": names[0],
            "away": names[1],
...
def list_upcoming(force: bool = False) -> list[dict[str, Any]]:
    """Return only future, scheduled fixtures without blocking on serial pages.

    The SportScore football page can contain live, scheduled and finished
    sections in the same HTML document. Dated pages are fetched in parallel and
    the widget API is used only as a fallback because it is capped at 50 rows
    and intentionally mixes live/recent matches.
    """
    ...
    def fetch_pages(specs: list[tuple[str, dt.date]]) -> int:
        nonlocal source_succeeded
        parsed_total = 0
        workers = min(PREMATCH_WORKERS, max(1, len(specs)))
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers, thread_name_prefix="sportscore-prematch") as pool:
            future_map = {pool.submit(load_page, spec): spec for spec in specs}
            for future in concurrent.futures.as_completed(future_map):
                ...
                try:
                    ...
                    for item in rows:
                        ...
                        # HTML rows are authoritative. API values, when used as
                        # fallback, may enrich only missing fields later.
                        combined[item["slug"]] = {**current, **{k: v for k, v in item.items() if v not in (None, "", {}, [])}}
                except Exception as exc:
                    errors.append(f"{path}: {type(exc).__name__}: {exc}")
        return parsed_total

    parsed_total = fetch_pages(date_paths)

    # Legacy routes help when a dated URL is temporarily changed or empty.
    if parsed_total == 0:
        legacy = [
            ("/football/?filter=upcoming", today),
            ("/football/tomorrow/", today + dt.timedelta(days=1)),
        ]
        parsed_total += fetch_pages(legacy)

    # Official widget API is capped at 50 and mixes live/recent rows. It is a
    # last-resort fallback only, never part of the normal full calendar path.
    api_used = False
    if not combined:
        try:
    ...
    pages.sort(key=lambda p: (p.get("date") or "", p.get("path") or ""))
    _LAST_UPCOMING_DIAGNOSTICS = {
        ...
        "discarded": discarded,
        "api_fallback_used": api_used,
        "parallel_workers": min(PREMATCH_WORKERS, max(1, len(date_paths))),
