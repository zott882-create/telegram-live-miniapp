...
def _participant_slug(item: dict[str, Any], obj: Any, side: str, sport: str, name: str) -> str:
    """Resolve a stable participant slug across SportScore payload versions."""
    side = "away" if side == "away" else "home"
    idx = "2" if side == "away" else "1"
    obj_value = _dig(obj, "slug", "id", "url", "link", "@id", default="")
    if sport == "football":
        slug = _slug_generic(obj_value, "team")
    else:
        slug = _slug_generic(obj_value, "team") or _slug_from_url(obj_value)
    if slug:
        return slug
    candidates = (
        f"{side}_slug", f"{side}Slug", f"{side}_team_slug", f"{side}TeamSlug",
        f"{side}_id", f"{side}Id", f"{side}_team_id", f"{side}TeamId",
        f"participant{idx}_slug", f"participant{idx}Slug", f"participant{idx}_id",
        f"player{idx}_slug", f"player{idx}Slug", f"player{idx}_id",
    )
    value = _dig(item, *candidates, default="")
    slug = _slug_generic(value, "team") or _slug_from_url(value)
    if slug:
        return slug
    # The public team endpoint accepts URL-style slugs. Name fallback keeps
    # live team cards usable when a compact list response omits participant IDs.
