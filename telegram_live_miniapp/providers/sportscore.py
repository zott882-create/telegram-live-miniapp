...
def normalize_match(item: dict[str, Any], *, source: str = "sportscore") -> dict[str, Any]:
    ...
    competition_identity = _competition_identity(_dig(competition_obj, "url", "link", "@id", default=""))
    ...
    # Keep the real SportScore team pages.  These are different from home_id /
    # away_id: the ids are used for API lookups, while the links are safe URLs
    # that the Mini App can open when the user taps a team badge/name.
    ...
    return {
        ...
        "competition_slug": competition_identity.get("slug") or _slug_generic(_dig(competition_obj, "url", "slug", "@id"), "competition"),
        "competition_id": competition_identity.get("id") or "",
        "competition_path": competition_identity.get("path") or "",
        "competition_country_slug": competition_identity.get("country_slug") or "",
...
def _team_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    clean_sheets = sum(1 for r in finished if _to_int(r.get("goals_against"), 0) == 0)
    failed_to_score = sum(1 for r in finished if _to_int(r.get("goals_for"), 0) == 0)
    btts = sum(1 for r in finished if _to_int(r.get("goals_for"), 0) > 0 and _to_int(r.get("goals_against"), 0) > 0)
    over_1_5 = sum(1 for r in finished if _to_int(r.get("goals_for"), 0) + _to_int(r.get("goals_against"), 0) >= 2)
    over_2_5 = sum(1 for r in finished if _to_int(r.get("goals_for"), 0) + _to_int(r.get("goals_against"), 0) >= 3)
    over_3_5 = sum(1 for r in finished if _to_int(r.get("goals_for"), 0) + _to_int(r.get("goals_against"), 0) >= 4)
    return {
        "count": count,
        "wins": wins,
        "draws": draws,
...
def team_detail(team_slug: str, team_name: str = "", force: bool = False) -> dict[str, Any]:
    ...
    def load() -> dict[str, Any]:
        raw_matches = _team_matches(slug, 30)
    ...
    if force:
        return load()
    return _cached("team-detail:" + slug + ":" + _normalized_team_name(team_name), TEAM_CACHE_SECONDS, load)
...
def _expected_match_overlay(actual: dict[str, Any], expected: dict[str, Any]) -> dict[str, Any]:
    out = dict(actual or {})
    # The list row came from data-match-id and is authoritative for identity,
    # teams, kickoff and pre-match odds. Page/API data may enrich it, but cannot
    # replace it with an older meeting that happens to share the same slug.
...
def detail(slug: str, force: bool = False, expected_match: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    def load():
        ...
        try:
            ...
            if not expected:
                raise
            parsed = {
                "match": normalize_match({"slug": slug, "url": f"/football/match/{slug}/"}, source="sportscore_detail_fallback"),
        ...
        try:
            ...
            for key in ("score_home", "score_away", "score", "minute", "minute_text", "period", "status", "is_live", "finished", "kickoff_at", "kickoff_iso"):
        ...
        if expected:
            ...
            if expected_odds:
                parsed["odds"] = expected_odds
                parsed["match"]["odds"] = expected_odds
