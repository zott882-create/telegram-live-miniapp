...
def normalize_match(item: dict[str, Any], *, source: str = "sportscore") -> dict[str, Any]:
    ...
    mid = "ss:" + (provider_match_id or slug)
...
def list_live(force: bool = False) -> list[dict[str, Any]]:
    ...
    try:
        api_rows = [x for x in list_api_matches(force=force) if x.get("is_live") and not x.get("finished")]
    except Exception as exc:
...
def tracker(match_id: str, force: bool = False) -> dict[str, Any]:
    match_id = str(match_id or "").strip()
    if not match_id:
        return {}
    params = {"sport": "football", "id": match_id, "src": SOURCE_TAG}
    if force:
        params["_t"] = int(time.time())
