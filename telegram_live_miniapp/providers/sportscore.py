...
def _generic_sport_stats(raw: Any) -> list[dict[str, Any]]:
    ...
    if isinstance(raw, list):
        for row in raw:
            if not isinstance(row, dict):
                continue
            label = str(_dig(row, "label", "name", "type", "title", default="") or "").strip()
            home = _dig(row, "home", "home_value", "participant1", "value1")
            away = _dig(row, "away", "away_value", "participant2", "value2")
            if label and (home not in (None, "") or away not in (None, "")):
                rows.append({"label": label, "home": home, "away": away})
    elif isinstance(raw, dict):
        for label, value in raw.items():
            if isinstance(value, dict):
                home = _dig(value, "home", "home_value", "participant1", "value1")
                away = _dig(value, "away", "away_value", "participant2", "value2")
                if home not in (None, "") or away not in (None, ""):
                    rows.append({"label": str(label), "home": home, "away": away})
            elif isinstance(value, (list, tuple)) and len(value) >= 2:
                rows.append({"label": str(label), "home": value[0], "away": value[1]})
    return rows[:40]
...
def normalize_match(item: dict[str, Any], *, source: str = "sportscore", sport: str = "football") -> dict[str, Any]:
    ...
    league = str(_dig(item, "competition_name", "league_name", "tournament_name", "competition", "league", "tournament", default="") or _entity_name(competition_obj) or "Без лиги").strip()
    ...
    country = str(_dig(item, "country", "country_name", "location.address.addressCountry", "addressCountry", default="") or "Без страны").strip()
    ...
    return {
        ...
        "home": home, "away": away,
        "home_id": _slug_generic(_dig(home_obj, "url", "slug", "@id"), "team") if sport == "football" else str(_dig(home_obj, "slug", "id", default="") or ""),
        "away_id": _slug_generic(_dig(away_obj, "url", "slug", "@id"), "team") if sport == "football" else str(_dig(away_obj, "slug", "id", default="") or ""),
...
def _extract_matches_envelope(payload: Any) -> list[dict[str, Any]]:
    ...
    if not isinstance(payload, dict):
        return []
    for key in ("data", "matches", "fixtures", "results", "items", "events"):
...
def _generic_sport_detail(slug: str, sport: str, force: bool = False, expected_match: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    def load() -> dict[str, Any]:
        ...
        sport_stats = _generic_sport_stats(raw_stats)
