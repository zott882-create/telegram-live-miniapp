...
def parse_upcoming_html(text: str) -> list[dict[str, Any]]:
    ...
    # Generic DOM fallback. SportScore may expose more fixtures in the rendered
    # list than in the SEO JSON-LD block.
    for link in soup.select('a[href*="/football/match/"]'):
        href = str(link.get("href") or "")
        slug = _slug_from_url(href)
        if not slug or slug in out:
            continue
        row = link.find_parent(class_=lambda c: c and "football-match-table-container" in (c if isinstance(c, list) else str(c)))
        row = row or link.find_parent("li") or link.parent
        if not row:
            continue
        team_links = row.select('a[href*="/football/team/"]')
        names = []
        for a in team_links:
            ...
            if name and name not in names:
                names.append(name)
        if len(names) < 2:
            aria = str(link.get("aria-label") or link.get("title") or "")
        ...
        start = time_el.get("datetime") if time_el else ""
        if not start:
            txt = " ".join(row.get_text(" ", strip=True).split())
            hm = re.search(r"\b([01]?\d|2[0-3]):[0-5]\d\b", txt)
            if hm:
                today = dt.datetime.now(dt.timezone.utc).date()
                start = f"{today.isoformat()}T{hm.group(0)}:00+00:00"
        comp_link = row.select_one('a[href*="/football/competition/"]')
        comp_name = " ".join(comp_link.get_text(" ", strip=True).split()) if comp_link else "Без лиги"
        obj = {
            "home": names[0], "away": names[1], "url": href,
            "startDate": start, "eventStatus": "EventScheduled",
            "competition": {"name": comp_name, "url": comp_link.get("href") if comp_link else ""},
        }
        out[slug] = normalize_match(obj, source="sportscore_html")
...
def list_upcoming(force: bool = False) -> list[dict[str, Any]]:
    ...
    for path in ("/football/?filter=upcoming", "/football/tomorrow/"):
        try:
            text = _html(path, ttl=10 if force else CACHE_SECONDS)
            source_succeeded = True
            for item in parse_upcoming_html(text):
    ...
    return sorted(out, key=lambda x: (int(x.get("kickoff_at") or 2**62), x.get("league") or "", x.get("home") or ""))
