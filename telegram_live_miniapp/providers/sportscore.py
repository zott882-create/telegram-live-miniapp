...
LIVE_CACHE_SECONDS = max(60, int(os.environ.get("SPORTSCORE_LIVE_CACHE_SECONDS", "120")))
LIVE_HTML_CACHE_SECONDS = max(60, int(os.environ.get("SPORTSCORE_LIVE_HTML_CACHE_SECONDS", "120")))
DETAIL_CACHE_SECONDS = max(60, int(os.environ.get("SPORTSCORE_DETAIL_CACHE_SECONDS", "120")))
...
def _flat_stats(raw: Any) -> dict[str, int]:
    """Extract football statistics from changing SportScore payload shapes.

    SportScore currently returns the same statistic in several forms depending
    on league and endpoint: flat keys, row lists, nested groups, camelCase
    objects, and tracker payloads.  The old parser only understood one shallow
    English shape, so corners/cards and even total shots were often shown as 0.
    """
    metrics = (
        "possession", "shots", "on_target", "off_target", "dangerous",
        "attacks", "corners", "yellow_cards", "red_cards",
    )
    out: dict[str, int] = {f"{metric}_{side}": 0 for metric in metrics for side in ("home", "away")}
    seen: set[str] = set()
    if not isinstance(raw, (dict, list, tuple)):
        return _finalize_flat_stats(out)
    ...
    def metric_from_label(value: Any) -> str:
        label = clean(value)
        if not label:
            return ""
        # Specific labels must be checked before generic "attacks"/"shots".
        if any(x in label for x in ("dangerous attack", "danger attack", "опасн атак")):
            return "dangerous"
        if any(x in label for x in (
            "shots on target", "shot on target", "shots on goal", "shot on goal",
            "on target", "on goal", "удар в створ", "удары в створ", "в створ",
        )):
            return "on_target"
        if any(x in label for x in (
            "shots off target", "shot off target", "shots wide", "shot wide",
            "off target", "off goal", "missed shots", "удар мимо", "удары мимо", "мимо",
        )):
            return "off_target"
        if any(x in label for x in (
            "yellow red", "second yellow", "2nd yellow", "straight red",
            "red card", "red cards", "красн", "удален", "удаление",
        )):
            return "red_cards"
        if any(x in label for x in (
            "yellow card", "yellow cards", "bookings", "booking",
            "желт", "жёлт", "предупрежден",
        )):
            return "yellow_cards"
        if any(x in label for x in (
            "corner kicks", "corner kick", "corners", "corner",
            "угловые", "угловой", "угловых",
        )):
            return "corners"
        if any(x in label for x in ("ball possession", "possession", "владение мяч", "владение")):
            return "possession"
        if any(x in label for x in ("total attacks", "attacks", "attack", "все атаки", "атаки", "атака")):
            return "attacks"
        if any(x in label for x in ("total shots", "shot attempts", "shots", "shot", "всего удар", "удары", "удар")):
            return "shots"
        return ""

    def number(value: Any) -> int:
        if isinstance(value, dict):
            for key in ("value", "count", "total", "display", "text", "amount"):
                if key in value:
                    return number(value.get(key))
            return 0
        if isinstance(value, (list, tuple)):
            return number(value[0]) if value else 0
        text_value = str(value or "").replace("%", "").strip()
        match = re.search(r"-?\d+(?:[.,]\d+)?", text_value)
        return _to_int(match.group(0).replace(",", ".") if match else 0)

    home_keys = (
        "home", "home_value", "homeValue", "value_home", "valueHome", "home_count", "homeCount",
        "team1", "team_1", "participant1", "participant_1", "player1", "left", "first", "value1",
    )
    away_keys = (
        "away", "away_value", "awayValue", "value_away", "valueAway", "away_count", "awayCount",
        "team2", "team_2", "participant2", "participant_2", "player2", "right", "second", "value2",
    )
    ...
    def set_side(metric: str, side: str, value: Any) -> None:
        if metric not in metrics or side not in {"home", "away"}:
            return
        key = f"{metric}_{side}"
        parsed = number(value)
        # Football event counters are cumulative. Keep the richest source and
        # never let an empty nested row erase a real value found elsewhere.
        if metric == "possession":
            if parsed > 0:
                out[key] = parsed
                seen.add(key)
        elif parsed > out[key] or key not in seen:
            out[key] = max(0, parsed)
            seen.add(key)

    def set_pair(metric: str, home: Any, away: Any) -> None:
        if not metric:
            return
        if home not in (None, ""):
            set_side(metric, "home", home)
        if away not in (None, ""):
            set_side(metric, "away", away)

    id_map = {
        2: "corners", 3: "yellow_cards", 4: "red_cards",
        21: "on_target", 22: "off_target", 23: "attacks",
        24: "dangerous", 25: "possession",
    }

    def label_from_node(node: dict[str, Any]) -> str:
        for key in (
            "name", "label", "title", "caption", "description", "stat_name", "statName",
            "statistic_name", "statisticName", "display_name", "displayName", "type_name", "typeName",
        ):
            value = node.get(key)
            if isinstance(value, dict):
                value = _dig(value, "name", "label", "title", "displayName", "value", default="")
            if value not in (None, ""):
                return str(value)
        for key in ("type", "statistic", "stat", "category"):
            value = node.get(key)
            if isinstance(value, dict):
                value = _dig(value, "name", "label", "title", "displayName", "value", default="")
    ...
    def row_pair(node: dict[str, Any]) -> tuple[Any, Any]:
        home = first_value(node, home_keys)
        away = first_value(node, away_keys)
        for key in ("values", "value", "result", "score", "data"):
            value = node.get(key)
            if home in (None, "") and away in (None, ""):
                if isinstance(value, (list, tuple)) and len(value) >= 2:
                    home, away = value[0], value[1]
                elif isinstance(value, dict):
                    home = first_value(value, home_keys)
                    away = first_value(value, away_keys)
        return home, away

    def direct_scalar_key(key: Any, value: Any) -> None:
        normalized = clean(key)
        if not normalized or isinstance(value, (dict, list, tuple)):
            return
        side = ""
        tokens = normalized.split()
        if "home" in tokens or "team1" in tokens or "participant1" in tokens or "host" in tokens:
            side = "home"
        elif "away" in tokens or "team2" in tokens or "participant2" in tokens or "guest" in tokens:
            side = "away"
        if not side:
            return
        reduced = " ".join(t for t in tokens if t not in {
            "home", "away", "team1", "team2", "participant1", "participant2", "host", "guest", "value", "count",
        })
        metric = metric_from_label(reduced)
        if metric:
            set_side(metric, side, value)

    def walk(node: Any, parent_label: str = "") -> None:
        if isinstance(node, dict):
            label = label_from_node(node) or parent_label
            metric = metric_from_label(label)
            if not metric:
                for key in ("statistic_type_id", "statisticTypeId", "type_id", "typeId", "stat_id", "statId"):
                    stat_id = number(node.get(key))
                    if stat_id in id_map:
                        metric = id_map[stat_id]
                        break
            home, away = row_pair(node)
            if metric and (home not in (None, "") or away not in (None, "")):
                set_pair(metric, home, away)

            for key, value in node.items():
                # Common shape: {"Corner Kicks": {"home": 2, "away": 3}}
                key_metric = metric_from_label(key)
                if key_metric and isinstance(value, dict):
                    h, a = row_pair(value)
                    set_pair(key_metric, h, a)
                elif key_metric and isinstance(value, (list, tuple)) and len(value) >= 2:
                    set_pair(key_metric, value[0], value[1])
                direct_scalar_key(key, value)
                walk(value, str(key))
        elif isinstance(node, (list, tuple)):
            # Some HTML-to-JSON adapters produce [home, label, away].
            if len(node) >= 3 and not isinstance(node[1], (dict, list, tuple)):
                metric = metric_from_label(node[1])
                if metric:
                    set_pair(metric, node[0], node[-1])
            for value in node:
                walk(value, parent_label)

    walk(raw)
    return _finalize_flat_stats(out)


def _finalize_flat_stats(flat: dict[str, int]) -> dict[str, int]:
    out = dict(flat or {})
    # Several SportScore leagues omit "total shots" while providing on/off
    # target rows. Derive the minimum truthful total instead of displaying 0.
    for side in ("home", "away"):
        derived = _to_int(out.get(f"on_target_{side}"), 0) + _to_int(out.get(f"off_target_{side}"), 0)
        if derived > _to_int(out.get(f"shots_{side}"), 0):
            out[f"shots_{side}"] = derived
    for key in ("shots", "on_target", "off_target", "dangerous", "attacks", "corners", "yellow_cards", "red_cards"):
        out[f"{key}_total"] = _to_int(out.get(f"{key}_home"), 0) + _to_int(out.get(f"{key}_away"), 0)
    return out


def _merge_flat_stats(*sources: Any) -> dict[str, int]:
    merged = _flat_stats({})
    for source in sources:
        flat = source if isinstance(source, dict) and any(str(k).endswith(("_home", "_away")) for k in source) else _flat_stats(source)
        for key, value in (flat or {}).items():
            if key.endswith("_total"):
                continue
            parsed = _to_int(value, 0)
            if key.startswith("possession_"):
                if parsed > 0:
                    merged[key] = parsed
            elif parsed > _to_int(merged.get(key), 0):
                merged[key] = parsed
    return _finalize_flat_stats(merged)


def _event_counter_stats(events: Any) -> dict[str, int]:
    flat = _flat_stats({})
    ...
    for event in events:
        ...
        metric = {"corner": "corners", "yellow": "yellow_cards", "red": "red_cards"}.get(kind)
        if metric and side in {"home", "away"}:
            flat[f"{metric}_{side}"] = _to_int(flat.get(f"{metric}_{side}"), 0) + 1
    return _finalize_flat_stats(flat)
...
def normalize_match(item: dict[str, Any], *, source: str = "sportscore", sport: str = "football") -> dict[str, Any]:
    ...
    stats_available = bool(raw_stats) if isinstance(raw_stats, (dict, list, tuple)) else False
    ...
    return {
        ...
        "stats": stats, "stats_available": stats_available,
        "sport_stats": _generic_sport_stats(raw_stats) if sport != "football" else [],
...
def _merge_lineups(base: dict[str, Any] | None, extra: dict[str, Any] | None) -> dict[str, Any]:
    base = dict(base or {})
    extra = dict(extra or {})
    if not extra:
        return base
    out = dict(base)
    for key in ("announced", "confirmed", "home_formation", "away_formation", "home_coach", "away_coach"):
        if extra.get(key) not in (None, "", False, [], {}):
            out[key] = extra.get(key)
    for side in ("home", "away"):
        old_rows = [x for x in (base.get(side) or []) if isinstance(x, dict)]
        new_rows = [x for x in (extra.get(side) or []) if isinstance(x, dict)]
        merged: list[dict[str, Any]] = []
        seen: set[tuple[str, int, str]] = set()
        for row in old_rows + new_rows:
            key = (
                str(row.get("name") or "").strip().casefold(),
                _to_int(row.get("number"), 0),
        ...
        if merged:
            out[side] = merged
    out["announced"] = bool(out.get("announced") or out.get("home") or out.get("away"))
    return out


def _normalize_lineups_payload(raw: Any) -> dict[str, Any]:
    """Normalize API lineup envelopes used by different SportScore widgets."""
    result: dict[str, Any] = {
        "announced": False,
        "confirmed": False,
        "home_formation": "",
    ...
    seen: dict[str, set[tuple[str, str]]] = {"home": set(), "away": set()}

    def side_from_key(key: str, current: str = "") -> str:
        low = str(key or "").lower().replace("-", "_")
        if any(x in low for x in ("home", "team1", "participant1", "local")):
            return "home"
        if any(x in low for x in ("away", "team2", "participant2", "visitor", "guest")):
            return "away"
        return current

    def role_from_key(key: str, current: str = "") -> str:
        low = str(key or "").lower().replace("-", "_")
        if any(x in low for x in ("substitute", "substitutes", "bench", "reserve")):
            return "substitute"
        if any(x in low for x in ("starting", "starter", "lineup", "formation", "first_team", "xi")):
            return current or "starter"
        return current

    def add_player(node: dict[str, Any], side: str, role: str) -> bool:
        if side not in {"home", "away"}:
            return False
        player_obj = _dig(node, "player", "athlete", "person", default={})
        name = _entity_name(_dig(node, "player_name", "playerName", "name", "full_name", "fullName", default=""))
        if not name and isinstance(player_obj, dict):
            name = _entity_name(player_obj)
        number = _to_int(_dig(node, "number", "shirt_number", "shirtNumber", "jersey", "jerseyNumber", default=0), 0)
        position = str(_dig(node, "position", "pos", "role_name", "roleName", default="") or "").strip()
        rating = _to_float(_dig(node, "rating", "score", "player_rating", default=None))
        photo = str(_dig(node, "photo", "image", "avatar", "player.logo", "player.image", default="") or "")
        explicit_player = bool(player_obj) or any(k in node for k in ("player_name", "playerName", "shirtNumber", "shirt_number", "jerseyNumber"))
        if not name or (not explicit_player and not number and not position and not role):
            return False
        clean_role = str(_dig(node, "lineup_role", "lineupRole", "type", default=role) or role or "starter").lower()
        if any(x in clean_role for x in ("sub", "bench", "reserve")):
            clean_role = "substitute"
        elif any(x in clean_role for x in ("coach", "manager")):
            return False
        else:
            clean_role = "starter"
        key = (name.casefold(), clean_role)
        if key in seen[side]:
            return True
        seen[side].add(key)
        result[side].append({
    ...
    def walk(node: Any, side: str = "", role: str = "") -> None:
        if isinstance(node, list):
            for value in node:
                walk(value, side, role)
            return
        if not isinstance(node, dict):
            return
        local_side = side
        local_role = role
        # Direct side markers inside a row.
        side_value = str(_dig(node, "side", "team_side", "teamSide", default="") or "").lower()
        if side_value in {"home", "away"}:
            local_side = side_value
        elif side_value in {"team1", "participant1", "local"}:
            local_side = "home"
        elif side_value in {"team2", "participant2", "visitor", "guest"}:
            local_side = "away"
        add_player(node, local_side, local_role)
        for key, value in node.items():
            low = str(key).lower()
            child_side = side_from_key(low, local_side)
            child_role = role_from_key(low, local_role)
            if "formation" in low and not isinstance(value, (dict, list)) and child_side in {"home", "away"}:
                result[f"{child_side}_formation"] = str(value or "").strip()
                continue
            if any(x in low for x in ("coach", "manager")) and not isinstance(value, (dict, list)) and child_side in {"home", "away"}:
                result[f"{child_side}_coach"] = str(value or "").strip()
                continue
            walk(value, child_side, child_role)

    walk(raw)
    result["announced"] = bool(result["home"] or result["away"])
    result["confirmed"] = bool(_dig(raw, "confirmed", "isConfirmed", "lineupsConfirmed", default=False))
    return result


def _absence_item(node: Any, default_reason: str = "") -> dict[str, Any] | None:
    if isinstance(node, str):
        name = " ".join(node.split())
        return {"name": name, "reason": default_reason, "photo": ""} if name else None
    if not isinstance(node, dict):
        return None
    player = _dig(node, "player", "athlete", "person", default={})
    name = _entity_name(_dig(node, "player_name", "playerName", "name", "full_name", "fullName", default=""))
    if not name and isinstance(player, dict):
        name = _entity_name(player)
    reason = str(_dig(node, "reason", "status", "injury", "type", "description", "detail", default=default_reason) or default_reason).strip()
    photo = str(_dig(node, "photo", "image", "avatar", "player.logo", "player.image", default="") or "")
    explicit_player = bool(player) or any(k in node for k in ("player_name", "playerName", "full_name", "fullName"))
    if not name or (not explicit_player and not reason):
        return None
    return {"name": name, "reason": reason, "photo": photo}


def _normalize_absences_payload(raw: Any) -> dict[str, list[dict[str, Any]]]:
    result: dict[str, list[dict[str, Any]]] = {"home": [], "away": []}
    seen: dict[str, set[str]] = {"home": set(), "away": set()}

    def walk(node: Any, side: str = "", reason: str = "") -> None:
        if isinstance(node, list):
            for value in node:
                walk(value, side, reason)
            return
        if not isinstance(node, dict):
            if side in result:
                item = _absence_item(node, reason)
                if item and item["name"].casefold() not in seen[side]:
                    seen[side].add(item["name"].casefold())
                    result[side].append(item)
            return
        explicit_side = str(_dig(node, "side", "team_side", "teamSide", default="") or "").lower()
        if explicit_side in {"home", "away"}:
            side = explicit_side
        elif explicit_side in {"team1", "participant1", "local"}:
            side = "home"
        elif explicit_side in {"team2", "participant2", "visitor", "guest"}:
            side = "away"
        item = _absence_item(node, reason)
        if side in result and item:
        ...
        for key, value in node.items():
            ...
            if low in {"reason", "status", "injury", "type", "description", "detail", "photo", "image", "avatar", "name", "player_name", "playername", "full_name", "fullname"}:
                continue
            child_side = side
            if any(x in low for x in ("home", "team1", "participant1", "local")):
                child_side = "home"
            elif any(x in low for x in ("away", "team2", "participant2", "visitor", "guest")):
                child_side = "away"
            child_reason = reason
            if "injur" in low:
                child_reason = "Травма"
            elif any(x in low for x in ("suspend", "suspens", "disqual", "ban")):
                child_reason = "Дисквалификация"
            elif any(x in low for x in ("absence", "missing", "unavailable")):
                child_reason = "Не сыграет"
            walk(value, child_side, child_reason)

    walk(raw)
    return result


def _parse_absences(soup: BeautifulSoup) -> dict[str, list[dict[str, Any]]]:
    result: dict[str, list[dict[str, Any]]] = {"home": [], "away": []}
    roots = soup.select(
        "#injuries, #absences, #missing-players, #suspensions, "
        ".injuries, .absences, .missing-players, .suspensions, "
        "[data-tab='injuries'], [data-tab='absences'], [class*='injur'], [class*='absence']"
    )
    seen: dict[str, set[str]] = {"home": set(), "away": set()}

    def parse_container(container: Any, side: str) -> None:
        if side not in result or container is None:
            return
        for row in container.select("li, .player, .injury-player, .absence-player, .missing-player, [class*='player-row']"):
            name_el = row.select_one(".name, .player-name, [class*='player-name'], a")
            reason_el = row.select_one(".reason, .status, .injury, .description, [class*='reason'], [class*='status']")
            img = row.select_one("img")
            name = " ".join((name_el.get_text(" ", strip=True) if name_el else "").split())
            if not name:
                continue
            key = name.casefold()
            if key in seen[side]:
            ...
            result[side].append({
                "name": name,
                "reason": " ".join((reason_el.get_text(" ", strip=True) if reason_el else "").split()),
                "photo": str(img.get("src") or "") if img else "",
            })

    for root in roots:
        parse_container(root.select_one(".home, .team-home, [data-side='home'], [data-team='home']"), "home")
        parse_container(root.select_one(".away, .team-away, [data-side='away'], [data-team='away']"), "away")
    return result

def _parse_html_stats(soup: BeautifulSoup) -> dict[str, int]:
    roots = soup.select("#stats, #statistics, .fb-stats, .match-stats, .game-stats, [data-tab='stats'], [data-tab-content='stats'], [class*='statistics']")
    rows: list[dict[str, Any]] = []
    for root in roots:
        ...
        for block in root.select(".stat-row, .stat-bar, .v-stat-row, [class*='stat-row'], [class*='stats-row']"):
            label = block.select_one(".label, .stat-bar-label, .v-stat-row__label, [class*='label'], [class*='name']")
            vals = block.select(".value, .stat-bar-value, .v-stat__value, [class*='value']")
    ...
    # Embedded application JSON is often richer than visible markup.
    embedded: list[Any] = []
...
def _event_text(value: Any, limit: int = 80) -> str:
    parts: list[str] = []
...
def _normalize_event_type(value: Any) -> str:
    ...
    # Red must be checked before yellow because feeds often use labels such as
    # "second yellow red card" for a dismissal.
    if any(token in text for token in (
        "second yellow", "yellow red", "red yellow", "straight red",
        "red card", "sent off", "sending off", "dismissal", "красн",
        "удален", "удалён",
    )):
        return "red"
    if any(token in text for token in (
        "yellow card", "yellow cards", "yellow", "booking", "жёлт", "желт",
    )):
        return "yellow"
    if any(token in text for token in (
        "goal", "penalty scored", "own goal", "гол",
    )) and not any(token in text for token in (
        "disallowed", "cancelled", "canceled", "var no goal", "missed penalty",
    )):
        return "goal"
    if "corner" in text or "углов" in text:
        return "corner"

    # Compact provider codes. Use these only when a type-like field is present.
    if isinstance(value, dict):
        codes = {
            str(value.get(key) or "").strip().casefold()
            for key in ("type", "event_type", "eventType", "incidentType", "incident_type", "code", "kind")
        }
        if codes & {"1", "goal", "g"}:
            return "goal"
        if codes & {"3", "yellow", "yellow_card", "yc", "card_yellow"}:
            return "yellow"
        if codes & {"4", "red", "red_card", "rc", "card_red"}:
            return "red"
    return ""


def _event_rows(raw: Any) -> list[dict[str, Any]]:
    """Flatten SportScore timeline/event envelopes without assuming one shape."""
    rows: list[dict[str, Any]] = []
    seen_nodes: set[int] = set()

    def rec(node: Any, depth: int = 0) -> None:
        if depth > 10:
            return
        if isinstance(node, list):
        ...
        # A dictionary is treated as one event only when its own keys look like
        # an incident row. Parent envelopes can contain event text recursively,
        # but must never become fake minute-0 events themselves.
        own_keys = {str(key or "").casefold() for key in node.keys()}
        has_type_key = bool(own_keys & {
            "type", "event_type", "eventtype", "incidenttype", "incident_type",
            "code", "kind", "label", "eventname", "event_name",
        })
        has_minute_key = bool(own_keys & {
            "minute", "minutes", "minute_text", "minutetext", "time", "clock",
            "matchminute", "eventminute", "periodtime",
        })
        has_named_incident = has_minute_key and bool(own_keys & {"name", "title", "description", "detail"})
        if (has_type_key or has_named_incident) and _normalize_event_type(node):
            rows.append(node)

        for key, value in node.items():
            key_text = str(key or "").casefold()
            if isinstance(value, (dict, list)) and (
                any(token in key_text for token in (
                    "event", "incident", "timeline", "moment", "action",
                    "goal", "card", "yellow", "red", "items", "data",
                ))
                or depth < 3
            ):
                rec(value, depth + 1)
...
def _normalize_events(raw: Any, match: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        kind = _normalize_event_type(row)
        if kind not in {"goal", "yellow", "red"}:
            continue
        minute_value = _dig(
            row, "minute_text", "minuteText", "time", "minute", "minutes",
            "matchMinute", "eventMinute", "clock", "periodTime", default=""
        )
        if isinstance(minute_value, dict):
            minute_value = _dig(minute_value, "display", "minute", "time", "value", default="")
        minute_raw = str(minute_value or "").strip()
        minute_match = re.search(r"(\d{1,3})(?:\s*\+\s*(\d{1,2}))?", minute_raw)
        base_minute = _to_int(minute_match.group(1) if minute_match else 0)
        extra_minute = _to_int(minute_match.group(2) if minute_match and minute_match.group(2) else 0)
        minute = base_minute + extra_minute
        minute_text = (
            f"{base_minute}+{extra_minute}'" if extra_minute
            else (f"{base_minute}'" if base_minute else "—")
        )

        team_obj = _dig(row, "team", "participant", "competitor", "club", default={})
        team_id = str(
            _dig(row, "team_id", "teamId", "participant_id", "participantId", "competitorId", default="")
            or _dig(team_obj, "id", "slug", default="") or ""
        )
        team_display = str(
            _dig(row, "team_name", "teamName", "participantName", "competitorName", default="")
            or _entity_name(team_obj) or ""
        ).strip()
        team_name = _normalized_team_name(team_display)
        side = str(_dig(row, "side", "team_side", "homeAway", "home_away", default="") or "").lower()
        if side not in {"home", "away"}:
            is_home = _dig(row, "isHome", "is_home", default=None)
            if isinstance(is_home, bool):
                side = "home" if is_home else "away"
            elif team_id and home_id and team_id == home_id:
                side = "home"
                ...
                side = "neutral"
        if not team_display:
            team_display = str(match.get("home") or "") if side == "home" else str(match.get("away") or "") if side == "away" else ""

        player_obj = _dig(row, "player", "scorer", "athlete", "person", default={})
        player = str(
            _dig(row, "player_name", "playerName", "scorerName", "athleteName", default="")
            or _entity_name(player_obj) or ""
        ).strip()
        score = str(
            _dig(row, "score", "result", "currentScore", "matchScore", default="") or ""
        ).replace(":", "-").strip()
        event = {
            "type": kind,
            "minute": minute,
        ...
        key = (kind, minute, side, player.casefold(), score)
        if key in seen:
            continue
        seen.add(key)
        out.append(event)
    priority = {"goal": 0, "yellow": 1, "red": 2}
    out.sort(key=lambda x: (_to_int(x.get("minute"), 999), priority.get(str(x.get("type")), 9)))
    return out
...
def list_live(force: bool = False, sport: str = "football") -> list[dict[str, Any]]:
    ...
    API and HTML are fetched concurrently so a cold refresh waits for the slower
    source once instead of waiting for both sources one after another.
    """
    sport = str(sport or "football").strip().lower()
    if sport not in {"football", "basketball", "tennis"}:
        sport = "football"
    ...
    def load_api() -> list[dict[str, Any]]:
        return [x for x in list_api_matches(force=force, sport=sport) if x.get("is_live") and not x.get("finished")]

    def load_html() -> list[dict[str, Any]]:
        path = "/football/?filter=live" if sport == "football" else f"/{sport}/?filter=live"
        html_text = str(_cached("live-html:" + path, 10 if force else LIVE_HTML_CACHE_SECONDS, lambda: _request(path)))
        if sport == "football":
            return parse_live_html(html_text)
        return [x for x in parse_generic_sport_html(html_text, sport) if x.get("is_live") and not x.get("finished")]

    with concurrent.futures.ThreadPoolExecutor(max_workers=2, thread_name_prefix="sportscore-live") as pool:
        futures = {pool.submit(load_api): "api", pool.submit(load_html): "html"}
        for future, source_name in list(futures.items()):
            try:
                rows = future.result()
                if source_name == "api":
                    api_rows = rows
...
def _detail_mismatch(actual: dict[str, Any], expected: dict[str, Any]) -> tuple[bool, str]:
    ...
    # SportScore sometimes omits kickoff_at from an otherwise correct detail
    # page. Matching teams are enough in that case; do not erase standings and
    # lineups merely because the timestamp is absent.
...
def _generic_sport_detail(slug: str, sport: str, force: bool = False, expected_match: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    def load() -> dict[str, Any]:
        ...
        if match.get("is_live") and tracker_id:
            try:
                tracker_data = tracker(tracker_id, force=force, sport=sport)
            except Exception:
                tracker_data = {}
        if tracker_data:
...
def detail(slug: str, force: bool = False, expected_match: dict[str, Any] | None = None, sport: str = "football") -> dict[str, Any]:
    ...
    def load():
        ...
        try:
            ...
            api_stats = _merge_flat_stats(
                _dig(api_obj, "stats", "statistics", default={}),
                api_obj,
                api,
            )
            parsed["stats_flat"] = _merge_flat_stats(parsed.get("stats_flat") or {}, api_stats)
            # SportScore responses are not fully uniform: events and lineups can
            # be siblings of data.match instead of children of the match object.
            # Search the full envelope first, then fall back to the normalized match.
            events = _dig(
                api, "events", "timeline.events", "timeline", "data.events", "data.timeline.events",
                "data.match.events", "data.match.timeline.events", default=None,
            ...
            if api_lineups is None:
                api_lineups = _dig(api_obj, "lineups", "data.lineups", "match.lineups", default=None)
            if api_lineups:
                parsed["lineups"] = _merge_lineups(parsed.get("lineups") or {}, _normalize_lineups_payload(api_lineups))
            api_absences = _dig(
                api, "absences", "injuries", "missing_players", "missingPlayers", "suspensions",
                "data.absences", "data.injuries", "data.match.absences", "match.absences", default=None,
            ) if isinstance(api, dict) else None
            if api_absences is None:
                api_absences = _dig(
                    api_obj, "absences", "injuries", "missing_players", "missingPlayers", "suspensions",
                    "data.absences", "data.injuries", "match.absences", default=None,
                )
            if api_absences:
                normalized_absences = _normalize_absences_payload(api_absences)
                current_absences = parsed.get("absences") or {"home": [], "away": []}
                for absence_side in ("home", "away"):
                    combined = list(current_absences.get(absence_side) or [])
                    names = {str(x.get("name") or "").casefold() for x in combined if isinstance(x, dict)}
                    for absence in normalized_absences.get(absence_side) or []:
                        if str(absence.get("name") or "").casefold() not in names:
                            combined.append(absence)
                    current_absences[absence_side] = combined
                parsed["absences"] = current_absences
        ...
        if expected:
            ...
            if mismatch:
                # Dynamic data from another fixture must not leak into this one.
                # A competition table is not fixture-specific, so preserve it.
                # Lineups are preserved only when the team pair itself matches.
                same_pair = _same_team_pair(actual_match, expected)
                parsed["events"] = []
                parsed["stats_flat"] = {}
                if not same_pair:
                    parsed["lineups"] = {"announced": False, "message": "Составы ещё не подтверждены"}
                    parsed["absences"] = {"home": [], "away": []}
                    parsed["standings"] = []
                parsed["venue"] = {}
        ...
        if match.get("is_live") and tracker_id:
            try:
                tracker_data = tracker(tracker_id, force=force)
            except Exception:
                tracker_data = {}
        if tracker_data:
            tracker_stats = _flat_stats(tracker_data)
            tracker_events_raw = _dig(
                tracker_data, "timeline.events", "events", "timeline",
                "data.events", "data.timeline.events", "match.events", default=[]
            )
            tracker_events = _normalize_events(tracker_events_raw, match)
            if tracker_events:
                existing_events = list(parsed.get("events") or [])
                event_keys = {(str(e.get("type")), _to_int(e.get("minute"), 0), str(e.get("side")), str(e.get("player"))) for e in existing_events if isinstance(e, dict)}
                for event in tracker_events:
                    key = (str(event.get("type")), _to_int(event.get("minute"), 0), str(event.get("side")), str(event.get("player")))
                    if key not in event_keys:
                        existing_events.append(event)
                        event_keys.add(key)
                existing_events.sort(key=lambda e: (_to_int(e.get("minute"), 0), str(e.get("type") or "")))
                parsed["events"] = existing_events
            parsed["stats_flat"] = _merge_flat_stats(
                parsed.get("stats_flat") or {},
                tracker_stats,
                _event_counter_stats(parsed.get("events") or []),
            )
        else:
            parsed["stats_flat"] = _merge_flat_stats(
                parsed.get("stats_flat") or {},
        ...
        return {
            ...
            "absences": parsed.get("absences") or {"home": [], "away": []},
    ...
    return _cached("detail:v1046:" + cache_identity + ":" + slug, 10 if force else DETAIL_CACHE_SECONDS, load)
